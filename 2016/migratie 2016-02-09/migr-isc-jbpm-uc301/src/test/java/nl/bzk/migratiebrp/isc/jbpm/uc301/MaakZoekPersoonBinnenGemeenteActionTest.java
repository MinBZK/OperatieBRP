...
public class MaakZoekPersoonBinnenGemeenteActionTest {
    ...
    public void test() {
        ...
        final ZoekPersoonOpActueleGegevensVerzoekBericht verzoek =
                (ZoekPersoonOpActueleGegevensVerzoekBericht) berichtenDao.leesBericht((Long) result.get("zoekPersoonBinnenGemeenteBericht"));
        Assert.assertNotNull(verzoek);
        Assert.assertEquals("1234567891", verzoek.getBsn());
        Assert.assertEquals("000160801109100045678", verzoek.getAanvullendeZoekcriteria());
    }
