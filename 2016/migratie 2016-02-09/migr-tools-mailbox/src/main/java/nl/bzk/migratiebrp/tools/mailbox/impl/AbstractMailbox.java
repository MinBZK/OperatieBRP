...
public abstract class AbstractMailbox implements Mailbox {
     ...
     * 
     * @param entries
     *            map met als key de MsSequenceNr en als value de MailboxEntry
     */
     ...
     *            zoeken vanaf (inclusief) MsSequenceId
     * @param limit
     *            maximum aantal
    ...
    public final FilterResult filterInbox(final String msStatus, final int startAtMsSequenceId, final int limit) {
        ...
        for (final MailboxEntry entry : getEntries().values()) {
            ...
            if (fetchNew && MailboxEntry.STATUS_NEW == entry.getStatus()) {
                ...
                filteredList.add(entry);
            } else if (fetchProcessed && MailboxEntry.STATUS_PROCESSED == entry.getStatus()) {
                filteredList.add(entry);
            }
        }

        LOGGER.debug("[Mailbox {}] filtering: size={}, nextSequenceId={} ", new Object[] {mailboxnr,
                                                                                          filteredList.size(),
                                                                                          nextFilteredMSSequenceId, });
    ...
    public static final class FilterResultImpl implements FilterResult {
         ...
         * @param entries
         *            gevonden entries
         * @param nextMsSequenceId
         *            volgende MsSequenceId om te gebruiken om te zoeken
         */
