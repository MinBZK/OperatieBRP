...
/**
 * Deze actionhandler controleert dat alle uc811 child processen zijn afgehandeld. Dit is de tegenhanger van
 * {@link BepaalVoorkomensAction}; wanneer alle child tokens zijn afgehandeld wordt de default transition genomen om het
 * proces verder af te handelen.
 */
@Component("uc812ControleerGemeentenAction")
public final class ControleerGemeentenAction implements SpringAction, NoSignal {
    ...
    public Map<String, Object> execute(final Map<String, Object> parameters) {
        ...
        if (token.getParent() != null) {
            if (token.getName().startsWith(BepaalVoorkomensAction.TOKEN_PREFIX)) {
                ...
                token.forceUnlock();
                token.lock(BepaalVoorkomensAction.LOCK + "_" + new Date());
            } else {
