...
package nl.bzk.migratiebrp.isc.jbpm.logging;
...
public final class MigratieLoggingService implements LoggingService {
    ...
    public void log(final ProcessLog processLog) {
        ...
        if (persistenceService instanceof DbPersistenceService) {
            final DbPersistenceService dbPersistenceService = (DbPersistenceService) persistenceService;
            if (dbPersistenceService.isTransactionActive()) {
                final ProcessLog filteredLog = filter(processLog);

                if (filteredLog != null) {
                    dbPersistenceService.getSession().save(filteredLog);
                }
            }
        }
    }
