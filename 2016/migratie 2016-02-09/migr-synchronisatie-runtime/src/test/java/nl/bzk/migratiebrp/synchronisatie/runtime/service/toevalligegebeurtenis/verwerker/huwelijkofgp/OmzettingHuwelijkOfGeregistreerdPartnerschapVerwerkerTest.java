...
import nl.bzk.migratiebrp.synchronisatie.runtime.service.toevalligegebeurtenis.controle.huwelijkofgp.OmzettingHuwelijkOfGeregistreerdPartnerschapControle;
...
public class OmzettingHuwelijkOfGeregistreerdPartnerschapVerwerkerTest {
    ...
    @InjectMocks
    private OmzettingHuwelijkOfGeregistreerdPartnerschapControle omzettingHuwelijkOfGeregistreerdPartnerschapControle;
    ...
    @InjectMocks
    private OmzettingHuwelijkOfGeregistreerdPartnerschapVerwerker subject;

    @Test
    public void testGevuldBinnenland() {
        setup(SoortRelatie.GEREGISTREERD_PARTNERSCHAP);
        ...
        Assert.assertNotNull(opdracht.getGBAOmzettingHuwelijkGeregistreerdPartnerschap());
        Assert.assertNotNull(opdracht.getGBAOmzettingHuwelijkGeregistreerdPartnerschap().getValue().getPartijCode());
        Assert.assertEquals(opdracht.getGBAOmzettingHuwelijkGeregistreerdPartnerschap().getValue().getPartijCode().getValue().getValue(), "600");
        Assert.assertNotNull(opdracht.getGBAOmzettingHuwelijkGeregistreerdPartnerschap().getValue().getActies());
        Assert.assertNotNull(opdracht.getGBAOmzettingHuwelijkGeregistreerdPartnerschap().getValue().getBronnen());
    }

    @Test
    public void testGevuldBuitenland() {
        setup(SoortRelatie.HUWELIJK);
    ...
    public void setup(final SoortRelatie soortRelatie) {
        ...
        final Set<Relatie> relaties = Collections.<Relatie>singleton(VerwerkToevalligeGebeurtenisVerzoekHelper.maakRelatie(1, soortRelatie));
        ...
        if (SoortRelatie.GEREGISTREERD_PARTNERSCHAP.equals(soortRelatie)) {
            Mockito.when(soortRelatieConversietabel.converteerNaarBrp(Matchers.any(Lo3SoortVerbintenis.class))).thenReturn(
        ...
        Mockito.when(redenEindeRelatieConversietabel.converteerNaarBrp(Matchers.any(Lo3RedenOntbindingHuwelijkOfGpCode.class))).thenReturn(
            new BrpRedenEindeRelatieCode('Z'));
    ...
    {
        ReflectionTestUtils.setField(omzettingHuwelijkOfGeregistreerdPartnerschapControle, "persoonControle", persoonControle);
        ReflectionTestUtils.setField(omzettingHuwelijkOfGeregistreerdPartnerschapControle, "huwelijkControle", huwelijkControle);
        ReflectionTestUtils.setField(
            omzettingHuwelijkOfGeregistreerdPartnerschapControle,
            "soortVerbintenisOngelijkControle",
            soortVerbintenisOngelijkControle);
    ...
    private VerwerkToevalligeGebeurtenisVerzoekBericht maakVerzoekBinnenland() {
        ...
        VerwerkToevalligeGebeurtenisVerzoekHelper.voegRelatieOmzettingHuwelijkToe(
            relatie,
            SoortRelatie.HUWELIJK,
            BigInteger.valueOf(20150915L),
            "0630",
                "0599");
