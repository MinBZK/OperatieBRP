...
public class MailboxServerProxyImpl implements MailboxClient {
    ...
    public final void logOff() throws SpdProtocolException {
        ...
        try {
            ...
            final int operationCode = Integer.parseInt(new String(buf, SpdConstants.OPCODE_START, SpdConstants.OPCODE_LENGTH));
            if (operationCode != SpdConstants.OPC_LOGOFF_CONFIRMATION) {
                throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_LOGOFF_CONF_OPCODE, new Object[] {operationCode });
            }
     ...
     * @throws SpdProtocolException
     */
    private void checkMesgLength(final byte[] buffer, final int nrBytesRead, final String mesgCode, final Object... params) throws SpdProtocolException {
        final int length = Integer.parseInt(new String(buffer, SpdConstants.LENGTH_START, SpdConstants.LENGTH_LENGTH));
    ...
    public final void logOn(final Mailbox mailbox) throws SpdProtocolException, MailboxServerPasswordException {
        ...
        try {
            ...
            if (operationCode != SpdConstants.OPC_LOGON_CONFIRMATION) {
                throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_LOGON_CONF_OPCODE, new Object[] {operationCode });
            }

            final int logonResult = Integer.parseInt(new String(buf, SpdConstants.LOGON_RESULT_START, SpdConstants.RESULT_LENGTH));
            switch (logonResult) {
                case 0:
                    break;
                ...
                default:
                    throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_LOGON_ERROR, new Object[] {logonResult });
            }
        } catch (final NumberFormatException nfe) {
    ...
    public final void putMessage(final Bericht bericht) throws SpdProtocolException {
        ...
        try {
            ...
            checkMesgLength(buf, nrBytesRead, MessagesCodes.ERRMSG_VOSPG_SPD_PUTMESSAGE_CONF_LENGTH, bericht.getMessageId(), bericht.getRecipient());

            final int operationCode = Integer.parseInt(new String(buf, SpdConstants.OPCODE_START, SpdConstants.OPCODE_LENGTH));

            if (operationCode != SpdConstants.OPC_PUT_MESSAGE_CONFIRMATION) {
                throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_PUTMESSAGE_CONF_OPCODE, new Object[] {operationCode,
                                                                                                                    bericht.getMessageId(),
                                                                                                                    bericht.getRecipient(), });
            }

            final int putMessageResult = Integer.parseInt(new String(buf, SpdConstants.PUTMESSAGE_RESULT_START, SpdConstants.RESULT_LENGTH));

            if (putMessageResult != RESULT_CORRECT) {
                throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_PUTMESSAGE_ERROR, new Object[] {putMessageResult,
                                                                                                              bericht.getMessageId(),
                                                                                                              bericht.getRecipient(), });
            } else {
                final int dispatchSeqNr =
                        Integer.parseInt(new String(buf, SpdConstants.PUTMESSAGE_DISPATCH_SEQNR_START, SpdConstants.PUTMESSAGE_DISPATCH_SEQNR_LEN));
                LOGGER.debug("PutMessageConfirmation.DispatchSeqNr= " + dispatchSeqNr);
                bericht.setDispatchSequenceNumber(dispatchSeqNr);

                final String submissionTime = new String(buf, SpdConstants.PUTMESSAGE_SUBMISSION_TIME_START, SpdConstants.PUTMESSAGE_SUBMISSION_TIME_LEN);
                LOGGER.debug("PutMessageConfirmation.SubmissionTime= " + submissionTime);
                bericht.setTijdstipMailbox(VoaUtil.convertSpdTimeStringToDate(submissionTime));

                // trim() because this field can be padded with spaces.
                final String messageId = new String(buf, SpdConstants.PUTMESSAGE_MESSAGE_ID_START, SpdConstants.PUTMESSAGE_MESSAGE_ID_LEN).trim();
                if (!messageId.equals(bericht.getMessageId())) {
                    throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_PUTMESSAGE_CONF_INCORRECT_MSG_ID, new Object[] {bericht.getMessageId(),
                                                                                                                                  messageId, });
                }
            }
        } catch (final NumberFormatException nfe) {
    ...
    public final Bericht getMessage(final int msSequenceNumber) throws SpdProtocolException {
        ...
        result.setDispatchSequenceNumber(msSequenceNumber);
        if (nrBytesRead < 0) {
        ...
        switch (gmr.getMessageType()) {
            case GetMessageRecord.MT_MESSAGE_CONFIRMATION:
                ...
                throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_GETMESSAGE_ERROR, new Object[] {error, });
            ...
            case GetMessageRecord.MT_GBA_MESSAGE:
                ...
                break;
            default:
                // Er is geen default. Als de verkeerde opcode wordt meegegeven, dan resulteerd dit in een
    ...
    private void handleDeliveryReport(final String deliveryReport, final Bericht delRep) throws SpdProtocolException {
        ...
        try {
            final int length =
                    Integer.parseInt(deliveryReport.substring(SpdConstants.LENGTH_START, SpdConstants.LENGTH_START + SpdConstants.LENGTH_LENGTH));
            final int realLength = deliveryReport.length() - SpdConstants.LENGTH_LENGTH;
            if (length != realLength) {
                throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_DELIVERY_REPORT_LENGTH, new Object[] {length, realLength, }, delRep);
            }

            // ReportDeliveryTime [11]
            final String deliveryTime =
                    deliveryReport.substring(SpdConstants.DELIVERY_REP_DELIVERY_TIME_START, SpdConstants.DELIVERY_REP_DELIVERY_TIME_START
                                                                                            + SpdConstants.DELIVERY_REP_DELIVERY_TIME_LEN);
            delRep.setTijdstipMailbox(VoaUtil.convertSpdTimeStringToDate(deliveryTime));

            // DispatchSequenceNumber [9]
            final int dispatchSeqNr =
                    Integer.parseInt(deliveryReport.substring(
                        SpdConstants.DELIVERY_REP_DISPATCH_SEQNR_START,
                        SpdConstants.DELIVERY_REP_DISPATCH_SEQNR_START + SpdConstants.DELIVERY_REP_DISPATCH_SEQNR_LEN));
            ...
            final int nrOfRecipients =
                    Integer.parseInt(deliveryReport.substring(
                        ...
                        SpdConstants.DELIVERY_REP_NR_OR_RECEPIENTS_START + SpdConstants.DELIVERY_REP_NR_OR_RECEPIENTS_LEN));
            if (nrOfRecipients != 1) {
            ...
            final String recipient =
                    deliveryReport.substring(SpdConstants.DELIVERY_REP_RECIPIENT_ORNAME_START, SpdConstants.DELIVERY_REP_RECIPIENT_ORNAME_START
                                                                                               + SpdConstants.DELIVERY_REP_RECIPIENT_ORNAME_LEN);
            // De recipient waaraan niet geleverd kon worden. Dus de originator voor het 'terug'-bericht.
            delRep.setOriginator(recipient);

            // MessageDeliveryTime or MailboxBlockDTS [11] // is not used
            // NonDeliveryReason [4]
            final String nonDeliveryReason =
                    deliveryReport.substring(SpdConstants.DELIVERY_REP_NON_DEL_REASON_START, SpdConstants.DELIVERY_REP_NON_DEL_REASON_START
    ...
    private void handleStatusReport(final String statusReport, final Bericht statRep) throws SpdProtocolException {
        try {
            // length
            final int length = Integer.parseInt(statusReport.substring(SpdConstants.LENGTH_START, SpdConstants.LENGTH_START + SpdConstants.LENGTH_LENGTH));
            final int realLength = statusReport.length() - SpdConstants.LENGTH_LENGTH;
            if (length != realLength) {
                throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_STATUS_REPORT_LENGTH, new Object[] {length, realLength, }, statRep);
            }

            // MessageID
            final String messageId =
                    statusReport.substring(SpdConstants.STATUS_REPORT_MESSAGE_ID_START, SpdConstants.STATUS_REPORT_MESSAGE_ID_START
                                                                                        + SpdConstants.STATUS_REPORT_MESSAGE_ID_LEN);
            statRep.setCorrelationId(messageId);

            // NotificationType [1]
            final String notificationType =
                    statusReport.substring(SpdConstants.STATUS_REPORT_NOT_TYPE_START, SpdConstants.STATUS_REPORT_NOT_TYPE_START
                                                                                      + SpdConstants.STATUS_REPORT_NOT_TYPE_LEN);
            statRep.setNotificationType(notificationType);
        } catch (final NumberFormatException nfe) {
    ...
    private void handleMessageHeading(final String heading, final Bericht bericht) throws SpdProtocolException {
        try {
            final int length = Integer.parseInt(heading.substring(SpdConstants.LENGTH_START, SpdConstants.LENGTH_START + SpdConstants.LENGTH_LENGTH));
            final int realLength = heading.length() - SpdConstants.LENGTH_LENGTH;
            if (length != realLength) {
                throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_HEADING_LENGTH, new Object[] {length, realLength, }, bericht);
            }

            // MessageID [12]
            final String messageId =
                    heading.substring(SpdConstants.MH_MESSAGE_ID_START, SpdConstants.MH_MESSAGE_ID_START + SpdConstants.MH_MESSAGE_ID_LEN);
            bericht.setMessageId(messageId);

            // CrossReference [12]
            String crossReference =
                    heading.substring(SpdConstants.MH_CROSS_REFERENCE_START, SpdConstants.MH_CROSS_REFERENCE_START + SpdConstants.MH_CROSS_REFERENCE_LEN);
            ...
            bericht.setCorrelationId(crossReference);
    ...
    private void handleGetEnvelope(final String envelope, final Bericht bericht) throws SpdProtocolException {
        try {
            final int length = Integer.parseInt(envelope.substring(SpdConstants.LENGTH_START, SpdConstants.LENGTH_START + SpdConstants.LENGTH_LENGTH));
            final int realLength = envelope.length() - SpdConstants.LENGTH_LENGTH;
            if (length != realLength) {
                throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_ENVELOPE_LENGTH, new Object[] {length, realLength, }, bericht);
            }

            // OriginatORName (Mailboxnumber from the sender) [7]
            final String originator =
                    envelope.substring(SpdConstants.GE_ORIGINATOR_ORNAME_START, SpdConstants.GE_ORIGINATOR_ORNAME_START
                                                                                + SpdConstants.GE_ORIGINATOR_ORNAME_LEN);
            ...
            // DeliveryTime [11]
            final String deliveryTime =
                    envelope.substring(SpdConstants.GE_DELIVERYTIME_START, SpdConstants.GE_DELIVERYTIME_START + SpdConstants.GE_DELIVERYTIME_LEN);
            bericht.setTijdstipMailbox(VoaUtil.convertSpdTimeStringToDate(deliveryTime));
    ...
    private void handleMSEntry(final String entry, final Bericht bericht) throws SpdProtocolException {
        try {
            ...
            final int msSequenceNumber =
                    Integer.parseInt(entry.substring(SpdConstants.MSE_MSSEQUENCE_NUMBER_START, SpdConstants.MSE_MSSEQUENCE_NUMBER_START
                                                                                               + SpdConstants.MSE_MSSEQUENCE_NUMBER_LEN));

            bericht.setDispatchSequenceNumber(msSequenceNumber);
        } catch (final NumberFormatException nfe) {
            throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_NOT_NUMERIC_FIELD, new Object[] {"MSEntry.MSSequenceNumber" }, nfe, bericht);
        }
    }
    ...
    private int handleMessageConfirmation(final String confirmation) throws SpdProtocolException {
        ...
        final int length = Integer.parseInt(confirmation.substring(SpdConstants.LENGTH_START, SpdConstants.LENGTH_LENGTH));
        final int realLength = confirmation.length() - SpdConstants.LENGTH_LENGTH;
        if (length != realLength) {
            throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_GETMESSAGE_CONF_LENGTH, new Object[] {length, realLength, });
        }

        final int error =
                Integer.parseInt(confirmation.substring(
                    SpdConstants.GETMESSAGE_CONFIRMATION_RESULT_START,
     ...
     */
    @Override
    public final void changePassword(final Mailbox mailbox, final String newPassWord) throws SpdProtocolException, MailboxServerPasswordException {
    ...
    private int validateChangePasswordConfirmation(final Message message) throws SpdProtocolException {
        ...
        if (length != nrBytesRead - SpdConstants.LENGTH_LENGTH) {
            throw new SpdProtocolException(
                ...
                new Object[] {length, nrBytesRead - SpdConstants.LENGTH_LENGTH, });
        }

        final int operationCode = Integer.parseInt(new String(buf, SpdConstants.OPCODE_START, SpdConstants.OPCODE_LENGTH));
        if (operationCode != SpdConstants.OPC_CHANGE_PASSWORD_CONFIRMATION) {
            throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_CHANGEPASSWORD_CONF_OPCODE, new Object[] {operationCode });
        }
    ...
    private void validateNewPassword(final String newPassWord) throws MailboxServerPasswordException {
        ...
        if (newPassWord.length() > pwdMaxLength || newPassWord.length() < pwdMinLength) {
            throw new MailboxServerPasswordException(MessagesCodes.ERRMSG_VOSPG_SPD_NEW_PASSWORD_LENGTH, new Object[] {newPassWord.length() });
        }
    }
    ...
    private void checkPasswordResult(final int changePassWordResult) throws MailboxServerPasswordException {
        switch (changePassWordResult) {
            ...
            case SpdConstants.ERR_CODE_CHANGEPASSWORD_OLD_PWD_MISSING:
                throw new MailboxServerPasswordException(MessagesCodes.ERRMSG_VOSPG_SPD_CHANGEPASSWORD_OLD_PWD_MISSING);
            case SpdConstants.ERR_CODE_CHANGEPASSWORD_OLD_PWD_INVALID:
                throw new MailboxServerPasswordException(MessagesCodes.ERRMSG_VOSPG_SPD_CHANGEPASSWORD_OLD_PWD_INVALID);
            case SpdConstants.ERR_CODE_CHANGEPASSWORD_NEW_PWD_MISSING:
                throw new MailboxServerPasswordException(MessagesCodes.ERRMSG_VOSPG_SPD_CHANGEPASSWORD_NEW_PWD_MISSING);
            case SpdConstants.ERR_CODE_CHANGEPASSWORD_NEW_PWD_UNACCEPTABLE:
                throw new MailboxServerPasswordException(MessagesCodes.ERRMSG_VOSPG_SPD_CHANGEPASSWORD_NEW_PWD_UNACCEPTABLE);
            default:
                throw new MailboxServerPasswordException(MessagesCodes.ERRMSG_VOSPG_SPD_CHANGEPASSWORD_ERROR, new Object[] {changePassWordResult });
        }
    }
    ...
    private void handleMessageBody(final String body, final Bericht bericht) throws SpdProtocolException {
        try {
            final int length = Integer.parseInt(body.substring(SpdConstants.LENGTH_START, SpdConstants.LENGTH_START + SpdConstants.LENGTH_LENGTH));
            ...
            if (length != realLength) {
                ...
                throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_BODY_LENGTH, new Object[] {length, realLength, }, bericht);
            }
            ...
            if (body.length() > SpdConstants.MB_BODYSTRING_START) {
                ...
                final String bodyString =
                        body.substring(SpdConstants.MB_BODYSTRING_START, SpdConstants.MB_BODYSTRING_START + length - SpdConstants.OPCODE_LENGTH);
                bericht.setBerichtInhoud(bodyString);

                if (hasNullBytes(bodyString)) {
                    final List<Integer> nullBytePositions = new ArrayList<>();
                    ...
                    throw new SpdProtocolException(
                        MessagesCodes.ERRMSG_VOSPG_SPD_GETMESSAGE_CONTAINS_NULL_BYTES,
                        new Object[] {nullBytePositions },
                        bericht);
                }
            }
            ...
            switch (operationCode) {
                case SpdConstants.OPC_LIST_MESSAGES_CONFIRMATION:
                    final String listError =
                            new String(buf, SpdConstants.LIST_MESSAGES_CONFIRMATION_ERROR_START, SpdConstants.LIST_MESSAGES_CONFIRMATION_ERROR_LEN);

                    if (Integer.parseInt(listError) != SpdConstants.ERR_CODE_LIST_MESSAGES_NO_ENTRIES) {
                        throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_LISTMESSAGES_ERROR, new Object[] {Integer.valueOf(listError) });
                    }
                    break;
                case SpdConstants.OPC_LIST_MESSAGES_RESULT:
                    ...
                    nextMSSequenceNr = Integer.parseInt(new String(buf, SpdConstants.LR_MSSEQUENCE_NUMBER_START, SpdConstants.LR_MSSEQUENCE_NUMBER_LEN));
                    final int numberOfEntries =
                            Integer.parseInt(new String(buf, SpdConstants.MSL_NUMBER_OF_MSENTRIES_START, SpdConstants.MSL_NUMBER_OF_MSENTRIES_LEN));
                    ...
                    for (int i = 0; i < numberOfEntries; i++) {
                        final int berichtSeqNr =
                                Integer.parseInt(new String(
                                    buf,
                                    SpdConstants.MSL_MSSEQUENCE_NUMBER_START + SpdConstants.MSL_ENTRY_LEN * i,
                                    SpdConstants.MSL_MSSEQUENCE_NUMBER_LEN));
                        LOGGER.debug("SequenceNr= " + berichtSeqNr);
                        sequenceNumbers.add(berichtSeqNr);
                    }
                    break;
                default:
                    throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SPD_LISTMESSAGES_CONF_OPCODE, new Object[] {operationCode });
            }
        } catch (final NumberFormatException nfe) {
     ...
     */
    private Message createListMessages(final int listLimit, final String status, final String priority, final int nextSeqNr) {
        ...
        final String listMessages =
                "00028"
                        + SpdConstants.OPC_LIST_MESSAGES
                        + dfLimitNr.format(limitNumber)
                        + StringUtil.fillBefore(status, ' ', SpdConstants.LR_STATUS_LENGTH)
                        + StringUtil.fillBefore(priority, ' ', SpdConstants.LR_PRIORITY_LENGTH)
                        + StringUtil.fillBefore(DF_SEQ_NR.format(nextSeqNr), ' ', SpdConstants.LR_MSSEQUENCE_NUMBER_LEN)
                        + "         ";
    ...
    private Message createChangePasswordRequest(final String oldPasswd, final String newPasswd) {
        final String changePasswordRequest =
                "00019"
                        + "910"
                        + StringUtil.fillBefore(oldPasswd, ' ', SpdConstants.CPWD_MAX_PWD_LENGTH)
    ...
    private Message createPutMessage(final Bericht bericht) throws SpdProtocolException {
        ...
        if (mailboxNr == null || mailboxNr.length() != SpdConstants.GE_RECIPIENT_ORNAME_LEN) {
            throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SEND_RECIPIENT_INVALID, new Object[] {bericht.getId() });
        }

        // test fields
        if (bericht.getMessageId() == null || bericht.getMessageId().isEmpty()) {
            throw new SpdProtocolException(MessagesCodes.ERRMSG_VOSPG_SEND_NO_EREF, new Object[] {bericht.getId(), });
        }

        berichtInhoud = bericht.getBerichtInhoud();
        berichtLength = berichtInhoud.length() + SpdConstants.OPCODE_LENGTH;

        final String messageEnvelope = "00024" + "120" + emtpyOriginatorORName + "2" + SpdConstants.PUTMESSAGE_PRIORITY_DEFAULT + "           " + "1";
    ...
    private String correctNullBytes(final String s, final List<Integer> nullBytePositions) {
        int fromIndex = 0;
        int nullBytePos = 1;
        final StringBuilder bodyBuffer = new StringBuilder(s);
        while (nullBytePos > 0) {
