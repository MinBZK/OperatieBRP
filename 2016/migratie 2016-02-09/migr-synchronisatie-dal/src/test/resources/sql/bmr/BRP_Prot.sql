...
\echo
\echo Controle query:
select schemaname, count(*) from pg_catalog.pg_tables where schemaname in ('prot') group by schemaname order by schemaname;
\echo 'select schemaname, count(*) from pg_catalog.pg_tables where schemaname in (''prot'') group by schemaname order by schemaname;'
