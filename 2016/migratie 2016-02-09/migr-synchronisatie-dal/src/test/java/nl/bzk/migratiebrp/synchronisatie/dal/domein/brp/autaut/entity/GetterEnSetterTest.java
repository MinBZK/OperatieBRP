...
public class GetterEnSetterTest {
    ...
    private static final String ROOT_PACKAGE = "nl.bzk.migratiebrp.synchronisatie.dal.domein.brp.autaut.entity";
    ...
    static {
        ...
        VALUES.put(Protocolleringsniveau.class, Protocolleringsniveau.GEEN_BEPERKINGEN);
        // excludes
        //TODO BRM44
        ...
        EXCLUDES.add(Leveringsautorisatie.class);
        EXCLUDES.add(LeveringsautorisatieHistorie.class);
    }
     ...
     *             indien er fouten optreden.
     */
    private void testModelGetters(final List<Class<?>> classes) throws Exception {
     ...
     *             indien er fouten optreden.
     */
    private void testModelSetters(final List<Class<?>> classes) throws Exception {
        for (final Class<?> clazz : classes) {
            final Object obj;
            final Constructor[] cons = clazz.getConstructors();
            if (cons != null && cons.length > 0 && !Modifier.isAbstract(clazz.getModifiers())) {
     ...
     *             juiste constructor.
     */
    private <T> T creeerEntityInstantie(final Class<T> clazz) throws Exception {
     ...
     * deze lijst van classes wordt geretourneerd. Classes die voorkomen in de {@link #EXCLUDES} lijst worden niet
     * verwerkt.
     *
     * @param packageNaam
     ...
     */
    private static List<Class<?>> getClassesInPackage(final String packageNaam) throws ClassNotFoundException, IOException, URISyntaxException {
     ...
     *             indien een class niet gevonden kan worden.
     */
    private static List<Class<?>> vindClassesInPackageFolderEnSubpackages(final File folder, final String packageNaam) throws ClassNotFoundException {
        final List<Class<?>> classes = new ArrayList<>();
        ...
        if (bestanden != null) {
            for (final File bestand : bestanden) {
                if (bestand.isDirectory()) {
                    ...
                    final Class<?> clazz = Class.forName(bouwClassNaamVoorBestand(bestand, packageNaam));
