...
public class BrpStaatloosIndicatieMapperTest extends BrpAbstractTest {

    private final BrpOnderzoekMapper brpOnderzoekMapper = new BrpOnderzoekMapperImpl(new ArrayList<Onderzoek>());

    @Inject
    private BrpStaatloosIndicatieMapper mapper;

    @Test
    public void testMapInhoud() {
        final PersoonIndicatieHistorie historie =
                new PersoonIndicatieHistorie(new PersoonIndicatie(new Persoon(SoortPersoon.INGESCHREVENE), SoortIndicatie.STAATLOOS), true);

        final BrpStaatloosIndicatieInhoud result = mapper.mapInhoud(historie, brpOnderzoekMapper);
