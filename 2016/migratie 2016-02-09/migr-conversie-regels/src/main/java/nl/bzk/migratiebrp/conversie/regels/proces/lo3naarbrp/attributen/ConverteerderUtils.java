...
public final class ConverteerderUtils {
     ...
     */
    public <T extends BrpGroepInhoud> boolean isLijstMetAlleenLegeInhoud(final List<TussenGroep<T>> lijst) {
        assert lijst != null;
     ...
     *            lo3 adellijketitel of predikaat code
     * @param voornamen
     *            lo3 voornamen
     * @param voorvoegselGeslachtsnaam
     *            lo3 voorvoegsel geslachtsnaam
     * @param geslachtsnaam
     *            lo3 geslachtsnaam
     * @param lo3Historie
     *            lo3 historie
     ...
     * @param lo3Herkomst
     *            lo3 herkomst
     * @return een gevulde {@link nl.bzk.migratiebrp.conversie.model.tussen.TussenGroep} met een
     *         {@link BrpSamengesteldeNaamInhoud}
     */
    public TussenGroep<BrpSamengesteldeNaamInhoud> maakSamengesteldeNaamGroep(
     ...
     *            true als de gegevens afgeleid gaan worden
     * @return een gevulde {@link nl.bzk.migratiebrp.conversie.model.tussen.TussenGroep} met een
     *         {@link BrpSamengesteldeNaamInhoud}
     */
    public TussenGroep<BrpSamengesteldeNaamInhoud> maakSamengesteldeNaamGroep(
        ...
        final boolean indicatieAfgeleid)
    {
        ...
        return new TussenGroep<>(new BrpSamengesteldeNaamInhoud(
            predicaatCode,
            voornamenBrp,
            ...
            indAfgeleid), lo3Historie, lo3Documentatie, lo3Herkomst);
    }
     ...
     * @param lo3Herkomst
     *            lo3 historie
     * @return een {@link nl.bzk.migratiebrp.conversie.model.tussen.TussenGroep} gevuld met een
     *         {@link BrpIdentificatienummersInhoud}
     */
    public TussenGroep<BrpIdentificatienummersInhoud> maakIdentificatieGroep(
        final Lo3Long anummer,
        final Lo3Integer bsn,
        final Lo3Historie lo3Historie,
        final Lo3Documentatie lo3Documentatie,
        final Lo3Herkomst lo3Herkomst)
    {
        final BrpLong administratienummer = converteerder.converteerLong(anummer);
        final BrpInteger burgerservicenummer = converteerder.converteerInteger(bsn);
     ...
     *         {@link BrpNummerverwijzingInhoud}
     */
    public TussenGroep<BrpNummerverwijzingInhoud> maakNummerverwijzingGroep(
        final Lo3Long vorigAnummer,
        final Lo3Long volgendAnummer,
        final Lo3Historie lo3Historie,
        final Lo3Documentatie lo3Documentatie,
        final Lo3Herkomst lo3Herkomst)
    {
        final BrpLong vorigeAdministratienummer = converteerder.converteerLong(vorigAnummer);
        final BrpLong volgendAdministratienummer = converteerder.converteerLong(volgendAnummer);
     ...
     *            lo3 gemeentecode van geboorte
     * @param geboorteLandCode
     *            lo3 landcode van geboorte
     * @param geboortedatum
     *            lo3 geboortedatum
     * @param lo3Historie
     *            lo3 historie
     ...
     *            lo3 herkomst
     * @return een {@link nl.bzk.migratiebrp.conversie.model.tussen.TussenGroep} gevuld met een
     *         {@link BrpGeboorteInhoud}.
     */
    public TussenGroep<BrpGeboorteInhoud> maakGeboorteGroep(
        ...
        final Lo3Herkomst lo3Herkomst)
    {
        ...
        return new TussenGroep<>(new BrpGeboorteInhoud(
            brpGeboortedatum,
            ...
            brpPlaatsLand.getBrpOmschrijvingLocatie()), lo3Historie, lo3Documentatie, lo3Herkomst);
    }
     ...
     *         {@link BrpGeslachtsaanduidingInhoud}
     */
    public TussenGroep<BrpGeslachtsaanduidingInhoud> maakGeslachtsaanduidingInhoud(
    ...
    public BrpDocumentInhoud maakDocumentInhoud(final Lo3Documentatie documentatie) {
        ...
        if (documentatie.isDocument()) {
            ...
            documentInhoud =
                    new BrpDocumentInhoud(
                        ...
                        null,
                        null,
                        converteerder.converteerString(documentatie.getBeschrijvingDocument()),
                        documentPartij);
        }
        ...
        if (documentatie.isAkte()) {
            ...
            documentInhoud = new BrpDocumentInhoud(documentSoort, null, aktenummer, null, documentPartij);
        }
     ...
     *
     * @param lo3Inhoud
     *            inhoud van het huwelijk of geregistreerd partnerschap lo3 categorie
     * @param eerdereLo3Inhoud
     *            (optioneel) De eerdere inhoud van huwelijk of geregistreerd partnerschap waaarvan gegevens worden
     *            overgenomen. Doorgaans zijn dit gegevens over de sluiting die worden overgenomen bij de ontbinding. In
     *            een zeldzaam geval (registratie ontbinding < registratie sluiting) is het echter andersom en worden de
     *            gegevens van de ontbinding overgenomen bij de sluiting (ORANJE-438).
     *
     * @param lo3Documentatie
     *            lo3 documentatie
     * @param lo3Historie
     ...
     */
    public TussenGroep<BrpRelatieInhoud> migreerRelatie(
        ...
        final Lo3Herkomst lo3Herkomst)
    {
        ...
        return new TussenGroep<>(new BrpRelatieInhoud(
            datumAanvang,
            gemeenteCodeAanvang,
            ...
            landCodeEinde,
            omschrijvingLocatieEinde), lo3Historie, lo3Documentatie, lo3Herkomst);
    }
     ...
     *
     * @param adellijkeTitelPredikaatCode
     *            lo3 adellijketitel of predicaat code
     * @return brp predicaat of null als input een adellijke titel is
     */
    public BrpPredicaatCode converteerPredicaatCode(final Lo3AdellijkeTitelPredikaatCode adellijkeTitelPredikaatCode) {
     ...
     */
    public BrpAdellijkeTitelCode converteerAdellijkeTitelCode(final Lo3AdellijkeTitelPredikaatCode adellijkeTitelPredikaatCode) {
