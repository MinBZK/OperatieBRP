...
public class InschrijvingConverteerder {
        ...
        final Lo3Historie inschrijvingFormeleHistorie)
    {
        ...
        final BrpPersoonAfgeleidAdministratiefInhoud afgeleidAdministratiefInhoud = new BrpPersoonAfgeleidAdministratiefInhoud(new BrpBoolean(false));

        final List<TussenGroep<BrpPersoonAfgeleidAdministratiefInhoud>> persoonAfgeleidAdministratiefGroepen = new ArrayList<>();
        persoonAfgeleidAdministratiefGroepen.add(
            new TussenGroep<>(
                afgeleidAdministratiefInhoud,
    ...
    @Definitie({Definities.DEF040, Definities.DEF078, Definities.DEF079 })
        ...
        final Lo3Historie inschrijvingFormeleHistorie)
    {
        ...
        final BrpBijhoudingsaardCode inschrijvingBijhoudingsaardCode;
        final BrpNadereBijhoudingsaardCode inschrijvingNadereBijhoudingsaardCode;
        final Lo3Datum cat07DatumAanvangGeldigheid;
        final boolean isOpgeschort = inschrijvingInhoud.isOpgeschort();

        if (isOpgeschort) {
            final Lo3RedenOpschortingBijhoudingCode lo3RedenOpschorting = inschrijvingInhoud.getRedenOpschortingBijhoudingCode();
            final boolean isGroep13ActueelAanwezig = actueelVerblijfplaatsInhoud.isEmigratie();

            cat07DatumAanvangGeldigheid = inschrijvingInhoud.getDatumOpschortingBijhouding();
            inschrijvingNadereBijhoudingsaardCode = converteerder.converteerLo3RedenOpschortingBijhoudingCode(lo3RedenOpschorting);

            if (!isGroep13ActueelAanwezig && (lo3RedenOpschorting.isFout() || lo3RedenOpschorting.isOverleden())) {
                // DEF079
                inschrijvingBijhoudingsaardCode = BrpBijhoudingsaardCode.INGEZETENE;
            } else {
                // DEF078
                inschrijvingBijhoudingsaardCode = BrpBijhoudingsaardCode.NIET_INGEZETENE;
            }
        } else {
            // DEF040
            inschrijvingBijhoudingsaardCode = BrpBijhoudingsaardCode.INGEZETENE;
            inschrijvingNadereBijhoudingsaardCode = BrpNadereBijhoudingsaardCode.ACTUEEL;
            cat07DatumAanvangGeldigheid = actueelDatumInschrijvingGemeente;
        }
        ...
        Lo3VerblijfplaatsInhoud verblijfplaatsInhoud =
                maakBijhoudingUitVerblijfplaats(
                    ...
                    inschrijvingBijhoudingsaardCode,
                    inschrijvingNadereBijhoudingsaardCode,
                    cat07DatumAanvangGeldigheid,
                    bijhoudingGroepen,
                    isOpgeschort);
        if (isOpgeschort) {
            ...
            final BrpPartijCode bijhoudingsPartij =
                    converteerder.converteerLo3GemeenteCodeNaarBrpPartijCode(verblijfplaatsInhoud.getGemeenteInschrijving());
            final BrpBoolean indicatieOnverwerktDocument = converteerder.converteerIndicatieDocument(verblijfplaatsInhoud.getIndicatieDocument());
        ...
        final boolean isOpgeschort)
    {
        ...
        for (final Lo3Categorie<Lo3VerblijfplaatsInhoud> voorkomen : lo3VerblijfplaatsStapel.getCategorieen()) {
            ...
            final BrpBoolean indicatieOnverwerktDocument = converteerder.converteerIndicatieDocument(inhoud.getIndicatieDocument());
            final Lo3Datum datumInschrijvingGemeente = inhoud.getDatumInschrijving();

            final Pair<BrpBijhoudingsaardCode, BrpNadereBijhoudingsaardCode> bijhoudingsaard =
                    bepaalBijhoudingsaard(
                        inschrijvingBijhoudingsaardCode,
                        inschrijvingNadereBijhoudingsaardCode,
                        cat07DatumAanvangGeldigheid,
                        voorkomen,
            ...
            if (cat07DatumAanvangGeldigheid.compareTo(actueelDatumInschrijvingGemeente) < 0
                && !historie.isOnjuist()
                && datumInschrijvingGemeente.compareTo(cat07DatumAanvangGeldigheid) < 0
                && (verblijfplaatsInhoud == null || datumInschrijvingGemeente.compareTo(verblijfplaatsInhoud.getDatumInschrijving()) > 0))
            {
            ...
            bijhoudingGroepen.add(
                maakBrpBijhoudingGroep(
                    ...
                    bijhoudingsaard.getLeft(),
                    bijhoudingsaard.getRight(),
                    indicatieOnverwerktDocument,
    ...
    private Pair<BrpBijhoudingsaardCode, BrpNadereBijhoudingsaardCode> bepaalBijhoudingsaard(
        ...
        final boolean isOpgeschort)
    {
        Pair<BrpBijhoudingsaardCode, BrpNadereBijhoudingsaardCode> result =
                new ImmutablePair<>(BrpBijhoudingsaardCode.ONBEKEND, BrpNadereBijhoudingsaardCode.ONBEKEND);
        if (isOpgeschort) {
            final Lo3Datum datumInschrijvingGemeente = voorkomen.getInhoud().getDatumInschrijving();
            if (datumInschrijvingGemeente.compareTo(cat07DatumAanvangGeldigheid) > 0) {
                result = new ImmutablePair<>(bijhoudingsaardCode, nadereBijhoudingsaardCode);
            }
        } else {
    ...
    private TussenGroep<BrpBijhoudingInhoud> maakBrpBijhoudingGroep(
        ...
        final BrpBijhoudingsaardCode bijhoudingsaardCode,
        final BrpNadereBijhoudingsaardCode nadereBijhoudingsaardCode,
        final BrpBoolean onverwerktDocumentAanwezig,
        ...
        final Lo3Herkomst lo3Herkomst)
    {
        final BrpBijhoudingInhoud bijhoudingInhoud =
                new BrpBijhoudingInhoud(bijhoudingspartijCode, bijhoudingsaardCode, nadereBijhoudingsaardCode, onverwerktDocumentAanwezig);
