...
public abstract class AbstractLo3HistorieConversieVariantTest {
    ...
    private static final BrpLong ANUMMER = new BrpLong(1000000000L);
    private static final BrpInteger BSN = new BrpInteger(123456789);
    ...
    protected void maakLo3Groep(final int voorkomen, final String onjuist, final int ingang, final int opneming) {
        invoer.add(new TussenGroep<>(new BrpIdentificatienummersInhoud(new BrpLong(ANUMMER.getWaarde() + voorkomen), BSN), Lo3StapelHelper.lo3His(
            onjuist,
            ingang,
        ...
        final Long tsVerval)
    {
        ...
        Assert.assertEquals(new BrpLong(ANUMMER.getWaarde() + lo3Voorkomen).getWaarde(), brpRij.getInhoud().getAdministratienummer().getWaarde());
