...
public class RegressieSyncIT extends IscTestConfiguration {

    @Override
    public File getInputFolder() {
        return new File("./regressie-sync");
    }

    @Override
    public FilenameFilter getThemaFilter() {
        // return new BaseFilter(FilterType.DIRECTORY);
        return new EqualsFilter("AS-blokkering", FilterType.DIRECTORY);
    }

    @Override
    public FilenameFilter getCasusFilter() {
        // return new BaseFilter(FilterType.DIRECTORY);
        return new CompositeFilter(Type.AND, new StartsWithFilter("", FilterType.DIRECTORY), DirectoryFilters.notEndsWith("nok", "NO TEST"));
        // return new EqualsFilter("zoekpersoon", FilterType.DIRECTORY);
    }
}
