...
public final class SeleniumKanaal extends AbstractKanaal {
    ...
    private void initialize(final boolean provideAngular) throws KanaalException {
        if (!initialized) {
            try {
                ...
                final File libDir = new File("selenium-lib");
        ...
        try {
            ...
            if (provideAngular) {
                ...
                final Method executeMethod =
                        executorClass.getMethod("execute", new Class<?>[] {File.class,
                                                                           File.class,
                                                                           String.class,
                                                                           String.class,
                                                                           Boolean.TYPE,
                ...
                final boolean waitForElement = !"false".equalsIgnoreCase(verwachtBericht.getTestBericht().getTestBerichtProperty("selenium.wait"));
                final boolean focusAndBlur = !"false".equalsIgnoreCase(verwachtBericht.getTestBericht().getTestBerichtProperty("selenium.focus"));
                ...
                // --remote-version <browser-version> Desired remote browser version
                // (Remote only)
                // -H,--highlight highlight locator always.
                // -s,--screenshot-dir <dir> override captureEntirePageScreenshot
                // directory.
                // -S,--screenshot-all <dir> take screenshot at all commands to
                // specified directory.
                // --screenshot-on-fail <dir> take screenshot on fail commands to
                // specified directory.
                // --ignore-screenshot-command ignore captureEntirePageScreenshot
                // command.
                // -b,--baseurl <baseURL> override base URL set in selenese.
                // --chromedriver <path> path to 'chromedriver' binary.
                // (implies '--driver chrome')
                // --iedriver <path> path to 'IEDriverServer' binary.
                // (implies '--driver ie')
                // --phantomjs <path> path to 'phantomjs' binary. (implies
                // '--driver phantomjs')
                // --xml-result <dir> output XML JUnit results to specified
                // directory.
                // --html-result <dir> output HTML results to specified
                // directory.
                // -t,--timeout <timeout> set timeout (ms) for waiting.
                // (default: 30000 ms)
                // --set-speed <speed> same as executing setSpeed(ms)
                // command first.
                // --height <height> set initial height. (excluding
                // mobile)
                // --width <width> set initial width. (excluding mobile)
                // -D,--define <key=value or key+=value> define parameters for capabilities.
                // (multiple)
                // --rollup <file> define rollup rule by JavaScript.
                // (multiple)
                ...
                if ("htmlunit".equalsIgnoreCase(verwachtBericht.getTestBericht().getTestBerichtProperty("selenium"))) {
                    ...
                    final File phantomJsBinary = new File(new File(new File("selenium-lib"), "phantomjs"), detectPhantomJs());
                ...
                arguments.add(verwachtBericht.getTestBericht().getOutputFile().getCanonicalPath() + ".selenium");
    ...
    private Bericht maakResultaatBericht(final Bericht verwachtBericht, final boolean succes) {
        ...
        final String directoryName = verwachtBericht.getTestBericht().getOutputFile().getName() + ".selenium";
