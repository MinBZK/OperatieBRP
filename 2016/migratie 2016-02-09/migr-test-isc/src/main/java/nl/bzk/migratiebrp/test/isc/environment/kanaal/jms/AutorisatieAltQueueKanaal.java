 ...
 * Sync queue kanaal.
 */
public final class AutorisatieAltQueueKanaal extends LazyLoadingKanaal {

    /** Kanaal naam. */
    public static final String KANAAL = "autorisatie_alt";

    /**
     * Constructor.
     */
    public AutorisatieAltQueueKanaal() {
        super(new Worker(),
              new Configuration(
                  "classpath:configuratie.xml",
                  "classpath:infra-jms-isc.xml",
                  "classpath:infra-container-autorisatie.xml",
                  "classpath:infra-queues-isc-autorisatie.xml"));
    }
    ...
    public static final class Worker extends AbstractKanaal {
        ...
        @Inject
        @Named("autorisatieVerzoek")
        private Destination uitgaandDestination;

        @Inject
        @Named("autorisatieRegisterService")
        private RegisterHandler registerHandler;
        @Inject
        @Named("autorisatieRegisterListener")
        private DefaultMessageListenerContainer autorisatieRegisterListener;
        ...
        public void voorTestcase(final TestCasusContext testCasus) {
            autorisatieRegisterListener.start();
        }
        ...
        public Bericht verwerkInkomend(final TestCasusContext testCasus, final Bericht verwachtBericht) throws KanaalException {
            try {
                ...
                throw new KanaalException("Geen autorisatieregister ontvangen", e);
            }
        }

        @Override
        public List<Bericht> naTestcase(final TestCasusContext testcasus) {
            autorisatieRegisterListener.stop();
