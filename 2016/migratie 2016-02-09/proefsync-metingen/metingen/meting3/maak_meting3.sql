drop table if exists resultaatMeting3;

create table resultaatMeting3 (
	verwerkingssysteem varchar(6) NOT NULL,
	bericht_id varchar(36) NOT NULL,
	status varchar(60) NOT NULL,
	indicatie_beheerder boolean NOT NULL,
	CONSTRAINT meting3_primary_key PRIMARY KEY (bericht_id, verwerkingssysteem)
);

INSERT INTO resultaatMeting3 (
select 
...
where meting2.bericht_id not in (select meting1.bericht_id from resultaatmeting2 meting1 where meting1.status = conv.statusGbav)
	and meting2.status = 'Ok')
;

select * from resultaatMeting3;
