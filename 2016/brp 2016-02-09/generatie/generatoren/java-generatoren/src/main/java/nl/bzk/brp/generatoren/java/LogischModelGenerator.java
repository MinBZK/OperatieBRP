 ...
 * Tot het logisch java model behoren alle objecttypen en groepen waarvan het vlaggetje inLgm in het BMR op 'Ja' staat.
 */
...
public class LogischModelGenerator extends AbstractJavaGenerator {
    ...
    public void genereer(final GeneratorConfiguratie generatorConfiguratie) {
        ...
        final List<JavaInterface> logischeGroepen = genereerLogischeGroepen();
        voegGeneratedAnnotatiesToe(logischeObjectTypes, generatorConfiguratie);
        voegGeneratedAnnotatiesToe(logischeGroepen, generatorConfiguratie);
        gegenereerdeInterfaces.addAll(writer.genereerEnSchrijfWeg(logischeObjectTypes, this));
        gegenereerdeInterfaces.addAll(writer.genereerEnSchrijfWeg(logischeGroepen, this));
    ...
    private List<JavaInterface> genereerLogischeObjectTypes() {
        ...
        List<JavaInterface> objectInterfaces = new ArrayList<>();
    ...
    private JavaInterface bouwObjectInterface(final ObjectType objectType) {
        ...
        if (objectType.getId() == ID_PERSOON_LGM) {
            voegGettersToeVoorIndicaties(javaInterface);
        }
        return javaInterface;
    }

    /**
     * Voegt interface functies toe voor het ophalen van indicaties.
     *
     * @param javaInterface De java interface waar functies aan toegevoegd worden.
     */
    private void voegGettersToeVoorIndicaties(final JavaInterface javaInterface) {
        ...
        JavaType persoonInidcatieJavaType = naamgevingStrategie.getJavaTypeVoorElement(persoonIndicatieObjectType);
        for (Tuple tuple : soortIndicatieObjectType.getTuples()) {
            JavaFunctie getter = new JavaFunctie(JavaAccessModifier.PUBLIC,
                    persoonInidcatieJavaType, "get" + tuple.getIdentCode(), tuple.getNaam());
            getter.setJavaDoc("Geeft de indicatie " + tuple.getNaam() + " terug.");
                                                      ...
                                                      final JavaInterface javaInterface)
    {
        ...
        for (Attribuut inverseAttr : inverseAttrVoorObjectType) {
            ...
            {
                JavaType inverseType;
                JavaType javaType;

                if (isDatabaseKnipAttribuut(inverseAttr.getObjectType(), inverseAttr)) {
                    inverseType = bepaalJavaTypeVoorDatabaseKnipAttribuut(inverseAttr);
                    javaType = new JavaType(JavaType.COLLECTION, new JavaGenericParameter(inverseType, false));
                } else {
                    inverseType = naamgevingStrategie.getJavaTypeVoorElement(inverseAttr.getObjectType());
                    javaType = new JavaType(JavaType.COLLECTION, new JavaGenericParameter(inverseType, true));
                }

                final JavaAccessorFunctie getter =
                        new JavaAccessorFunctie(inverseAttr.getInverseAssociatieIdentCode(), javaType);
        ...
        for (Groep groep : groepenVoorObjectType) {
            if (behoortTotJavaLogischModel(groep)) {
                ...
                if (IDENTITEIT.equals(groep.getIdentCode())) {
                    //Er kunnen attributen in de identiteit groep zitten die in het logisch model zitten. (inLgm = 'Ja')
                    ...
                    for (Attribuut attribuut : attributen) {
                        if (behoortTotJavaLogischModel(attribuut) && behoortTotJavaOperationeelModel(attribuut)) {
                            ...
                            if (isDatabaseKnipAttribuut(objectType, attribuut)) {
                                returnJavaType = bepaalJavaTypeVoorDatabaseKnipAttribuut(attribuut);
                                functieNaam = bepaalVeldnaamVoorDatabaseKnipAttribuut(attribuut);
                            } else {
                            ...
                            final JavaAccessorFunctie getter =
                                    new JavaAccessorFunctie(functieNaam, returnJavaType);
    ...
    private List<JavaInterface> genereerLogischeGroepen() {
        ...
        final List<JavaInterface> groepInterfaces = new ArrayList<>();
    ...
    private void voegGettersToeVoorGroep(final Groep groep, final JavaInterface javaInterface) {
        ...
        for (Attribuut attribuut : attributen) {
            // Neem een attribuut op als het zowel tot het logische als het operationele model behoort.
            if (behoortTotJavaLogischModel(attribuut) && behoortTotJavaOperationeelModel(attribuut)
