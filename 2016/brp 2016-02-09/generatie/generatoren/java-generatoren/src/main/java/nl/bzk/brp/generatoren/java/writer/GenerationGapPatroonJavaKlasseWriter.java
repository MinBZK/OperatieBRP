...
public class GenerationGapPatroonJavaKlasseWriter extends AbstractGenerationGapPatroonJavaWriter<JavaKlasse> {
    ...
    protected JavaKlasse zetJavaTypeOmNaarUserType(final JavaKlasse javaKlasse) {
        ...
        // Opruimen annotatie voor user klasse
        final Iterator<JavaAnnotatie> iterator = userKlasse.getAnnotaties().iterator();
        while (iterator.hasNext()) {
            if (iterator.next().getType().equals(JavaType.GENERATED)) {
                iterator.remove();
            }
        }
        ...
        userKlasse.getInnerKlassen().clear();

        // De user klasse hoeft niet ook de BestaansPeriodeStamgegeven interface te implementeren.
        userKlasse.getSuperInterfaces().remove(JavaType.BESTAANS_PERIODE_STAMGEGEVEN);
...
//        }
    ...
    protected JavaKlasse zetJavaTypeOmNaarGeneratieType(final JavaKlasse javaKlasse) {
        ...
        for (JavaType superInterface : javaKlasse.getSuperInterfaces()) {
            JavaType javaType = superInterface;
            if (superInterface instanceof GenerationGapJavaType) {
                javaType = ((GenerationGapJavaType) javaType).getBasisType();
            }
            ...
            if (javaType.getPackagePad().equals(GeneratiePackage.BRP_MODEL_BASIS_PACKAGE.getPackage())) {
                generatieKlasse.voegSuperInterfaceToe(javaType);
            } else {
                final String superInterfaceNaam = javaType.getNaam()
                    + GenerationGapPatroonJavaInterfaceWriter.GENERATIE_INTERFACE_NAAM_SUFFIX;
                generatieKlasse.voegSuperInterfaceToe(new JavaType(superInterfaceNaam,
                    javaType.getPackagePad() + GENERATIE_TYPE_SUB_PACKAGE));
            }
        }

        // De generatie versie is abstract.
        ...
        for (JavaAnnotatie annotatie : javaKlasse.getAnnotaties()) {
            if (annotatie.getType().getNaam().equals("Table")
                    ...
                    || annotatie.getType().equals(JavaType.CACHE))
            {
        ...
        final Iterator<JavaAnnotatie> annotatiesIterator = generatieKlasse.getAnnotaties().iterator();
        while(annotatiesIterator.hasNext()) {
        	final JavaAnnotatie annotatie = annotatiesIterator.next();
        	if(annotatie.getType().equals(JavaType.ENTITY)) {
        		annotatiesIterator.remove();
        	}
        }
