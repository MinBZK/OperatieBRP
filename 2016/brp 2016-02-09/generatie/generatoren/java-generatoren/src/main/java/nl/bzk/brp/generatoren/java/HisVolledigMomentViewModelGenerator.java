...
import nl.bzk.brp.generatoren.java.naamgeving.HisVolledigMomentViewModelNaamgevingStrategie;
import nl.bzk.brp.generatoren.java.naamgeving.IdentiteitGroepNaamgevingStrategie;
...
/**
 * Deze generator genereert de model views die gebruikt zullen worden door leveringen.
 */
@Component("hisVolledigMomentViewModelGenerator")
public class HisVolledigMomentViewModelGenerator extends AbstractHisVolledigGenerator {

    private static final String ACTUELE_GEGEVENS_CHECK_METHODE_NAAM   = "heeftActueleGegevens";
    private static final String NAAM_FORMEEL_PEILMOMENT_VELD          = "formeelPeilmoment";
    private static final String NAAM_FORMEEL_PEILMOMENT_VELD_GETTER   = "getFormeelPeilmoment";
    private static final String NAAM_MATERIEEL_PEILMOMENT_VELD        = "materieelPeilmoment";
    private static final String NAAM_MATERIEEL_PEILMOMENT_VELD_GETTER = "getMaterieelPeilmoment";

    private final NaamgevingStrategie                          hisVolledigMomentViewModelNaamgevingStrategie =
        new HisVolledigMomentViewModelNaamgevingStrategie();
    ...
    private final IdentiteitGroepNaamgevingStrategie           identiteitGroepNaamgevingStrategie            = new IdentiteitGroepNaamgevingStrategie();
    ...
    private JavaKlasse genereerViewKlasseVoorObjectType(final ObjectType objectType) {
        ...
        JavaVeld hisVolledigProxyObjectVeld = new JavaVeld(
        ...
        // Voeg peilmoment velden toe, waar dat van toepassing is qua historie patroon.
        JavaVeld formeelPeilmomentVeld = new JavaVeld(JavaType.DATUMTIJD_ATTRIBUUT, NAAM_FORMEEL_PEILMOMENT_VELD, true);
        formeelPeilmomentVeld.setAccessModifier(JavaAccessModifier.PRIVATE);
        if (isFormeelPeilmomentVeldNodig || isMaterieelPeilmomentVeldNodig) {
            klasse.voegVeldToe(formeelPeilmomentVeld);
        }
        JavaVeld materieelPeilmomentVeld = new JavaVeld(JavaType.DATUM_ATTRIBUUT, NAAM_MATERIEEL_PEILMOMENT_VELD, true);
        materieelPeilmomentVeld.setAccessModifier(JavaAccessModifier.PRIVATE);
        if (isMaterieelPeilmomentVeldNodig) {
            klasse.voegVeldToe(materieelPeilmomentVeld);
        }

        /**
         * Protected getters nodig voor de peilmoment velden, omdat subtypen erbij moeten kunnen.
         */
        if (isFormeelPeilmomentVeldNodig || isMaterieelPeilmomentVeldNodig) {
            JavaAccessorFunctie formeelPeilmomentVeldGetter = new JavaAccessorFunctie(formeelPeilmomentVeld);
            formeelPeilmomentVeldGetter.setAccessModifier(JavaAccessModifier.PROTECTED);
            formeelPeilmomentVeldGetter.setJavaDoc("Retourneert formeel peilmoment voor deze view.");
            formeelPeilmomentVeldGetter.setReturnWaardeJavaDoc("Formeel peilmoment voor deze view.");
            formeelPeilmomentVeldGetter.setFinal(true);
            klasse.voegGetterToe(formeelPeilmomentVeldGetter);
        }
        if (isMaterieelPeilmomentVeldNodig) {
            JavaAccessorFunctie materieelPeilmomentVeldGetter = new JavaAccessorFunctie(materieelPeilmomentVeld);
            materieelPeilmomentVeldGetter.setAccessModifier(JavaAccessModifier.PROTECTED);
            materieelPeilmomentVeldGetter.setJavaDoc("Retourneert materieel peilmoment voor deze view.");
            materieelPeilmomentVeldGetter.setReturnWaardeJavaDoc("Materieel peilmoment voor deze view.");
            materieelPeilmomentVeldGetter.setFinal(true);
            klasse.voegGetterToe(materieelPeilmomentVeldGetter);
        }
        ...
        final JavaVeld hisVolledigVeld)
    {
        ...
        ObjectType finaalType = objectType;
        if (isSubType) {
            finaalType = objectType.getFinaalSupertype();
        }
        final boolean kentFormeleHistorieParam = bepaalOfFormeelPeilmomentVeldNodigIs(finaalType);
        final boolean kentMaterieleHistorieParam = bepaalOfMaterieelPeilmomentVeldNodigIs(finaalType);

        if (kentFormeleHistorieParam || kentMaterieleHistorieParam) {
            //Formeel peilmoment parameter.
            constructor.voegParameterToe(new JavaFunctieParameter(NAAM_FORMEEL_PEILMOMENT_VELD,
                JavaType.DATUMTIJD_ATTRIBUUT,
                "formeel peilmoment."));
        }

        if (kentMaterieleHistorieParam) {
            //Materieel peilmoment parameter.
            constructor.voegParameterToe(new JavaFunctieParameter(NAAM_MATERIEEL_PEILMOMENT_VELD,
                JavaType.DATUM_ATTRIBUUT,
                "materieel peilmoment."));
        }

        constructor.setBody(genereerConstructorBodyVoorViewKlasse(objectType, constructor, hisVolledigVeld,
            kentFormeleHistorieParam,
            kentMaterieleHistorieParam));
     ...
     * @param hisVolledigVeld       hisVolledig veld in de java klasse.
     * @param kentFormeleHistorie   indicatie of het objectype formele historie kent. (De groepen)
     * @param kentMaterieleHistorie indicatie of het objectype materiele historie kent. (De groepen)
    ...
    private String genereerConstructorBodyVoorViewKlasse(final ObjectType objectType, final Constructor constructor,
        final JavaVeld hisVolledigVeld,
        final boolean kentFormeleHistorie,
        final boolean kentMaterieleHistorie)
    {
        final boolean isSubType = objectType.getSuperType() != null;
        ...
        if (isSubType) {
            ...
            for (JavaFunctieParameter consParam : constructor.getParameters()) {
                if (NAAM_FORMEEL_PEILMOMENT_VELD.equals(consParam.getNaam())) {
                    if (kentFormeleHistorie || kentMaterieleHistorie) {
                        consBody.append(consParam.getNaam()).append(", ");
                    }
                } else if (NAAM_MATERIEEL_PEILMOMENT_VELD.equals(consParam.getNaam())) {
                    if (kentMaterieleHistorie) {
                        consBody.append(consParam.getNaam()).append(", ");
                    }
                } else {
                    consBody.append(consParam.getNaam()).append(", ");
            ...
            if (kentFormeleHistorie || kentMaterieleHistorie) {
                consBody.append("this.").append(NAAM_FORMEEL_PEILMOMENT_VELD).append(" = ")
                    .append(NAAM_FORMEEL_PEILMOMENT_VELD).append(";")
                    .append(JavaGeneratorConstante.NEWLINE_OS_ONAFHANKELIJK.getWaarde());
            }
            if (kentMaterieleHistorie) {
                consBody.append("this.").append(NAAM_MATERIEEL_PEILMOMENT_VELD).append(" = ")
                    .append(NAAM_MATERIEEL_PEILMOMENT_VELD).append(";")
                    .append(JavaGeneratorConstante.NEWLINE_OS_ONAFHANKELIJK.getWaarde());
            }
        }
    ...
    private void voegActueleGegevensCheckFunctieToe(final JavaKlasse klasse, final ObjectType objectType) {
        JavaFunctie actueleGegevensCheckFunctie =
        ...
        final JavaAccessorFunctie getter, final JavaType hisVolledigInverseType, final JavaType viewInverseType)
    {
        ...
        body.append("final Set<").append(viewInverseType.getNaam()).append("> result = new HashSet<>();")
            .append(JavaGeneratorConstante.NEWLINE_OS_ONAFHANKELIJK.getWaarde());
        body.append("for (").append(hisVolledigInverseType.getNaam()).append(" ")
        ...
        if (isSupertype(inverseAttr.getObjectType())) {
            ...
            final JavaType viewType = hisVolledigMomentViewModelNaamgevingStrategie.getJavaTypeVoorElement(
                inverseObjectType);
            body.append(String.format("final %1$s view;%2$s", viewType.getNaam(),
            ...
            for (int i = 0; i < subTypen.size(); i++) {
                ...
                boolean kentFormeleHistorieParam =
                    bepaalOfFormeelPeilmomentVeldNodigIs(subType.getFinaalSupertype());
                boolean kentMaterieleHistorieParam =
                    bepaalOfMaterieelPeilmomentVeldNodigIs(subType.getFinaalSupertype());
                ...
                if (kentFormeleHistorieParam) {
                    body.append(", ").append(NAAM_FORMEEL_PEILMOMENT_VELD_GETTER).append("()");
                }

                if (kentMaterieleHistorieParam) {
                    body.append(", ").append(NAAM_MATERIEEL_PEILMOMENT_VELD_GETTER).append("()");
                }
            ...
            if (bepaalOfFormeelPeilmomentVeldNodigIs(inverseAttr.getObjectType())) {
                body.append(", ").append(NAAM_FORMEEL_PEILMOMENT_VELD_GETTER).append("()");
            }
            if (bepaalOfMaterieelPeilmomentVeldNodigIs(inverseAttr.getObjectType())) {
                body.append(", ").append(NAAM_MATERIEEL_PEILMOMENT_VELD_GETTER).append("()");
            }
            body.append(");");
        ...
        final JavaVeld hisVolledigProxyObjectVeld)
    {
        ...
        if (kentFormeleHistorie || kentMaterieleHistorie) {
            ...
            final JavaType castType =
                kentFormeleHistorie && isIdentiteitGroep(groep) ? identiteitGroepNaamgevingStrategie.getJavaTypeVoorElement(groep)
            ...
            body.append(".get")
                ...
                .append("Historie()")
                .append(".getHistorieRecord(");
            if (kentMaterieleHistorie) {
                body.append(NAAM_MATERIEEL_PEILMOMENT_VELD_GETTER).append("(), ");
            }
            body.append(NAAM_FORMEEL_PEILMOMENT_VELD_GETTER).append("());");
        } else {
        ...
        final JavaVeld hisVolledigProxyObjectVeld)
    {
        ...
        for (Attribuut attribuut : attributen) {
                ...
                || attribuut.getNaam().equals(ID_ATTRIBUUT_NAAM))
            {
                ...
                GeneriekElement attribuutType = attribuut.getType();
                ...
                if (isIdAttribuut(attribuut)) {
                    ...
                    getterBody.append(String.format("return %1$s.get%2$s();",
                    ...
                    && BmrSoortInhoud.DYNAMISCH_OBJECT_TYPE.getCode() == attribuutType.getSoortInhoud())
                {
                    ...
                    if (isSupertype(returnObjectType)) {
                        getterBody.append(genereerBodyVoorGetterMetSupertypeAlsReturnWaarde(klasse, returnObjectType,
                            hisVolledigProxyObjectVeld));
                    } else {
                        boolean kentFormeleHistorieParam =
                            bepaalOfFormeelPeilmomentVeldNodigIs(returnObjectType);
                        boolean kentMaterieleHistorieParam =
                            bepaalOfMaterieelPeilmomentVeldNodigIs(returnObjectType);

                        getterBody.append(String.format("if (%1$s.get%2$s() != null) {%3$s",
                            hisVolledigProxyObjectVeld.getNaam(),
                            attribuut.getIdentCode(),
                            JavaGeneratorConstante.NEWLINE_OS_ONAFHANKELIJK.getWaarde()));

                        getterBody.append(String.format("return new %1$s(%2$s.get%3$s()",
                            javaTypeVoorVeld.getNaam(),
                            hisVolledigProxyObjectVeld.getNaam(),
                            attribuut.getIdentCode()));

                        if (kentFormeleHistorieParam) {
                            getterBody.append(", " + NAAM_FORMEEL_PEILMOMENT_VELD_GETTER + "()");
                        }

                        if (kentMaterieleHistorieParam) {
                            getterBody.append(", " + NAAM_MATERIEEL_PEILMOMENT_VELD_GETTER + "()");
                        }

                        getterBody.append(");");

                        getterBody.append(String.format("}%1$sreturn null;",
                            JavaGeneratorConstante.NEWLINE_OS_ONAFHANKELIJK.getWaarde()));
                    }
                } else {
        ...
        final JavaVeld hisVolledigProxyObjectVeld)
    {
        ...
        for (int i = 0; i < subTypen.size(); i++) {
            ...
            if (kentFormeleHistorieParam) {
                functieBody.append(String.format(", %1$s()", NAAM_FORMEEL_PEILMOMENT_VELD_GETTER));
            }

            if (kentMaterieleHistorieParam) {
                functieBody.append(String.format(", %1$s()", NAAM_MATERIEEL_PEILMOMENT_VELD_GETTER));
            }
