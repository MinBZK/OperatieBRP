...
public class BindingUtilGenerator extends AbstractJavaGenerator {
    ...
    private static final int           AT_NEE_ID                                 = 8944;
    private static final List<Integer> NIET_WAARDE_GEBRUIKENDE_ATTRIBUUTTYPE_IDS = Arrays.asList(AT_JA_ID, AT_NEE_ID);

    private final NaamgevingStrategie naamgevingStrategie = new AlgemeneNaamgevingStrategie();

    private final JavaKlasse bindingUtilKlasse = new JavaKlasse("BindingUtilAttribuutTypen",
     ...
     * types die attribuuttypes zijn met vaste waardes.
     *
    ...
    private void schrijfBindingUtilKlasse(final GeneratorConfiguratie generatorConfiguratie) {
        ...
        final Constructor constructor = new Constructor(JavaAccessModifier.PRIVATE, bindingUtilKlasse);
        ...
        final List<? extends AbstractJavaType> gegenereerdeKlassen =
        ...
        final JavaType javaType, final String paramNaam)
    {
        ...
        } else if (attribuutType.getBasisType().getId() == ID_BASISTYPE_DATUM_MET_ONZEKERHEID
                || attribuutType.getBasisType().getId() == ID_BASISTYPE_DATUM)
        {
            body = String.format("return BindingUtil.datumAlsGetalNaarDatumAlsString(%s);", paramNaam);
        } else if (attribuutType.getBasisType().getId() == ID_BASISTYPE_NUMERIEKE_CODE && javaType.isNumeriek()) {
            final Integer minimumLengte = attribuutType.getMinimumLengte();
            if (minimumLengte == null || minimumLengte <= 1) {
                body = String.format("return BindingUtil.getalNaarString(%s);", paramNaam);
            } else {
    ...
    {
        final String body;
        if (attribuutType.getBasisType().getId() == ID_BASISTYPE_DATUM_MET_ONZEKERHEID
                || attribuutType.getBasisType().getId() == ID_BASISTYPE_DATUM)
        {
            body = String.format("return BindingUtil.datumAlsStringNaarDatumAlsGetal(%s);", paramNaam);
        } else if (javaType.equals(JavaType.BOOLEAN) || javaType.isNumeriek()) {
