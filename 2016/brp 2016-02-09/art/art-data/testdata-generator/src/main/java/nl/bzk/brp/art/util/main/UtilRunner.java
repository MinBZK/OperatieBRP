...
import nl.bzk.brp.art.util.backup.PersonBackup;
import nl.bzk.brp.arttestdatagenerator.TestdataGenerator;
...
public final class UtilRunner {
    ...
    private void runBackup(final String url, final String userName, final String password,
            final String input, final String output) throws SQLException, ClassNotFoundException, IOException
    {
        if (input != null && output != null) {
            PersonBackup runner = new PersonBackup(url, userName, password, input, output);
            runner.run();
        } else {
    ...
    private void runConversion(final String excelDir, final String sqlFile) throws IOException {
        TestdataGenerator runner = new TestdataGenerator();
    ...
    public static void main(final String[] args) throws Exception {
        ...
        if (line.hasOption(CommandLineUtil.COMMAND_BACKUP)) {
            runner.runBackup(
                    ...
                    line.getOptionValue(CommandLineUtil.OPTION_INPUT),
                    line.getOptionValue(CommandLineUtil.OPTION_OUTPUT)
            );
