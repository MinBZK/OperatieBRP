...
import nl.bzk.brp.soapui.excel.exceptions.NietUniekDataAttribuut
 ...
 */
class DataSheets extends AbstractExcel {
     ...
     */
    DataSheets(File file, List<String> dataSheets, boolean useCache = true) {
        super(WorkbookFactory.create(file))

        dataSheets.each { String sheet ->
    ...
    private void vulDataCache(final String filePath) {
        if (this.filePath == null || !this.filePath.equals(filePath)) {
            ...
            this.dataSheets.each { String sheetName ->
     ...
     * @param sql {@link Sql} instantie om waardes op te halen
     * @param target doel om waardes in te zetten
     */
    void laadKolomVanTestGeval(String testGeval, Sql sql, DataSourceValues target) {
        Sheet sheet
        int kolomNr = -1
    ...
    private def zoekInCache(String testGeval) {
        ...
        if (index > -1) {
            log.info "Data voor [$testGeval] gevonden: $sheet - kol ${ExcelUtil.indexToColumnLetter(index)}"
    ...
    private def zoekInWorkbook(String testGeval) {
        Sheet sheet = null
        Cell cell = null
        dataSheets.any { String sh ->
            sheet = this.workbook.getSheet(sh)
            sheet.getRow(0).each {Cell c ->
                if (Cell.CELL_TYPE_STRING == c.cellType && testGeval == getCellContent(c)) {
                    cell = c
                    return
                }
            }
    ...
    private Cell[] vindAttribuutNamen(Sheet sheet) {
        List<Cell> attribuutNaamCellen = sheet.collect { Row r -> r.getCell(0, Row.RETURN_BLANK_AS_NULL)}
        ...
        attribuutNaamCellen.tail().each {Cell entry ->
            ...
            if (attrNaam) {
                if (!attrNaam.startsWith('|') && uniekeAttribuutNamen.contains(attrNaam)) {
                    def err = "[Read Excel and Write DataSource Values] Attribuutnaam: '$attrNaam', cell[A${1 + entry.rowIndex}] is niet uniek."
    ...
    private String convertCellContents(Sql sql, Cell cell) {
        ...
        switch (data.cellType) {
            ...
            case Cell.CELL_TYPE_STRING:
                String tempContents = data.stringValue
                if (tempContents.toUpperCase().trim() =~ /^SELECT\s/) {
                    log.info tempContents
                    try {
                        return sql.firstRow(data.stringValue)[0]
                    }
                    catch (Exception e) {
                        return e.message
                    }
