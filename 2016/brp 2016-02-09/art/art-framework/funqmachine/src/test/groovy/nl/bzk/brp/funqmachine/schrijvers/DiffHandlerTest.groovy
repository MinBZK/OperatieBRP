...
class DiffHandlerTest {
    ...
    void schrijfDiffZonderFilter() {
        ...
        assert out.readLines().collect({
            it.trim()
        }).join(' ').startsWith('--- test-classes/xml/1-expected.xml +++ test-classes/xml/1-actual.xml @@ -2,9 +2,9 @@')
    }
    ...
    void schrijfDiffFiltertVerschilNiet() {
        ...
        assert out.readLines().collect({
            it.trim()
        }).join(' ').startsWith('--- test-classes/xml/1-actual.xml +++ test-classes/xml/2-actual.xml @@ -2,7 +2,9 @@')
    }
