 ...
 *
 */
public class TransactieStap extends AbstractBerichtVerwerkingStap<AbstractSynchronisatieBericht,
    SynchronisatieBerichtContext, SynchronisatieResultaat>
{
     ...
     */
    private TransactionStatus maakDatabaseTransactieStatus(final AbstractSynchronisatieBericht onderwerp) {
        final DefaultTransactionDefinition def =
            maakTransactieDefinitie(TRANSACTIE_DEF_NAAM + onderwerp.getStuurgegevens().getReferentienummer()
                .getWaarde());
    ...
    @Override
    public final void voerNabewerkingStapUit(final AbstractSynchronisatieBericht bericht,
        final SynchronisatieBerichtContext context,
        final SynchronisatieResultaat resultaat)
    {
        ...
        if (voldoetAanVoorwaardenVoorCommit(resultaat, transactionStatus)) {
            ...
            LOGGER.info(
                "Transactie voor bericht met referentieNummer '{}' krijgt een rollback.",
                bericht.getStuurgegevens().getReferentienummer().getWaarde());
     ...
     */
    private boolean voldoetAanVoorwaardenVoorCommit(final BerichtVerwerkingsResultaat resultaat,
