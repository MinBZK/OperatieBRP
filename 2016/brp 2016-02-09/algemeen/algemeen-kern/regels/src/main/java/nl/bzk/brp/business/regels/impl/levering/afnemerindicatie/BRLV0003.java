...
@Named("BRLV0003")
@Regels(Regel.R1402)
public final class BRLV0003 implements Bedrijfsregel<AfnemerindicatieRegelContext> {

    private static final Logger LOGGER = LoggerFactory.getLogger();

    @Override
    public Regel getRegel() {
        return Regel.BRLV0003;
    }

    @Override
    public boolean valideer(final AfnemerindicatieRegelContext context) {
        final SoortAdministratieveHandeling soortAdministratieveHandeling = context.getSoortAdministratieveHandeling();

        if (isVanJuisteSoort(soortAdministratieveHandeling)) {
            ...
            for (final PersoonAfnemerindicatie indicatie : context.getHuidigeSituatie().getAfnemerindicaties()) {
                ...
                if (indicatie.getAfnemer().getWaarde().getID().equals(afnemer.getID())
                    ...
                    && leveringsautorisatie.getID() != null && indicatie.getLeveringsautorisatie() != null
                    && indicatie.getLeveringsautorisatie().getWaarde().getID() != null
                    && indicatie.getLeveringsautorisatie().getWaarde().getID().equals(leveringsautorisatie.getID())))
                {
                    LOGGER.info(FunctioneleMelding.LEVERING_BEDRIJFSREGEL_BRLV0003, context.geefLogmeldingFout(getRegel()));
                    return INVALIDE;
                }
            }
        }
        LOGGER.info(FunctioneleMelding.LEVERING_BEDRIJFSREGEL_BRLV0003, context.geefLogmeldingSucces(getRegel()));
