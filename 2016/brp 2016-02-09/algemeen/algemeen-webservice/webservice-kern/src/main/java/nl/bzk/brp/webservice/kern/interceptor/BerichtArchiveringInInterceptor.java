 ...
 * archiveert.
 * <p/>
 * Merk op dat deze class grotendeels is overgenomen van de {@link org.apache.cxf.interceptor.LoggingInInterceptor},
 * maar net anders is, daar de genoemde CXF class gericht is op het loggen naar een {@link java.io.PrintWriter} of een
 * {@link java.util.logging.Logger}, zonder callback mogelijkheden of toegang tot het bericht op moment van loggen,
 * terwijl dit wel noodzakelijk is. Op moment van loggen moeten immers de ids van de berichten waaronder ze gelogd zijn
 * aan de bericht context worden toegevoegd en dit lukt niet binnen de genoemde standaard interceptor van CXF.
 *
...
public class BerichtArchiveringInInterceptor extends AbstractPhaseInterceptor<Message> {
     ...
     * {@link org.apache.cxf.phase.Phase#RECEIVE}.
     */
    ...
    public void handleMessage(final Message bericht) {
        final ArchiveringBericht archiveringBericht = bouwArchiveringBericht(bericht);
        final BerichtenIds berichtenIds = archiveerBericht(archiveringBericht);
        MDC.put(MDCVeld.MDC_BERICHT_ID, berichtenIds.getIngaandBerichtId().toString());
    ...
    /**
     * Bouwt en retourneert een {@link ArchiveringBericht} instantie op, op basis van het opgegeven
    ...
    private void voegAdresToeAanArchiveringBericht(final Message bericht, final ArchiveringBericht archiveringBericht) {
        final String uri = (String) bericht.get(Message.REQUEST_URL);
        if (uri != null) {
            final StringBuilder adres = new StringBuilder(uri);
            final String query = (String) bericht.get(Message.QUERY_STRING);
            if (query != null) {
    ...
    {
        final InputStream binnenkomendBerichtStream = bericht.getContent(InputStream.class);
        if (binnenkomendBerichtStream != null) {
            final CachedOutputStream berichtUitleesStream = new CachedOutputStream();
            try {
                ...
                LOGGER.error("Fout opgetreden bij archivering van inkomend bericht: fout bij ophalen van de payload", e);
