...
public class ArchiveringBericht {
    ...
    private void vulStandaardVelden(final Message bericht) {
        ...
        final Integer berichtResponseCode = (Integer) bericht.get(Message.RESPONSE_CODE);
        if (berichtResponseCode != null) {
            responseCode = berichtResponseCode;
        }

        final String berichtEncoding = (String) bericht.get(Message.ENCODING);
        if (berichtEncoding != null) {
            encoding = berichtEncoding;
        }

        final String berichtHttpMethode = (String) bericht.get(Message.HTTP_REQUEST_METHOD);
        if (berichtHttpMethode != null) {
            httpMethode = berichtHttpMethode;
        }

        final String berichtContentType = (String) bericht.get(Message.CONTENT_TYPE);
        if (berichtContentType != null) {
            contentType = berichtContentType;
        }

        final Object berichtHeaders = bericht.get(Message.PROTOCOL_HEADERS);
     ...
     * @throws java.io.IOException indien er problemen zijn bij het ophalen/lezen van de payload van de stream.
     */
     ...
     * @param outputStream de outputstream die de payload/content van het bericht bevat.
     * @throws java.io.IOException indien er problemen zijn bij het ophalen/lezen van de payload van de stream.
     * @throws javax.xml.transform.TransformerException indien de input geen valide XML is (of andere transformatie
     * fouten oplevert).
     */
    private void vulXmlPayload(final CachedOutputStream outputStream) throws TransformerException, IOException {
        final TransformerFactory serializer = TransformerFactory.newInstance();
        // Setup indenting to "pretty print"
       // serializer.setAttribute(OutputKeys.INDENT, "yes");
        //serializer.setAttribute("{http://xml.apache.org/xslt}indent-amount", "2");

        final StringWriter swriter = new StringWriter();
        serializer.newTransformer().transform(new StreamSource(outputStream.getInputStream()), new StreamResult(swriter));
