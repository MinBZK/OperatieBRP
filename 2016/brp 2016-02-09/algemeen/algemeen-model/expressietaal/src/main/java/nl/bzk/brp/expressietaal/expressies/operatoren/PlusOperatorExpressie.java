...
import nl.bzk.brp.expressietaal.expressies.functies.FunctieDATUM;
...
/**
 * Representeert een numerieke optelexpressie.
 */
public class PlusOperatorExpressie extends AbstractRekenoperatorExpressie {
     ...
     */
    public PlusOperatorExpressie(final Expressie termLinks, final Expressie termRechts)
    {
    ...
    public final Expressie optimaliseer(final Context context) {
            ...
            && geoptimaliseerdeOperandRechts.isConstanteWaarde(ExpressieType.GETAL))
        {
            ...
            resultaat = new PlusOperatorExpressie(geoptimaliseerdeOperandLinks, geoptimaliseerdeOperandRechts);
        }
        return resultaat;
    }
    ...
    @Override
    protected final Expressie pasToeOpGetallen(final Expressie operandLinks, final Expressie operandRechts)
    {
        return new GetalLiteralExpressie(operandLinks.alsInteger() + operandRechts.alsInteger());
    }

    @Override
    protected final Expressie pasToeOpGroteGetallen(final Expressie operandLinks, final Expressie operandRechts)
    {
        return new GrootGetalLiteralExpressie(operandLinks.alsLong() + operandRechts.alsLong());
    }

    @Override
    protected final Expressie pasToeOpDatumEnGetal(final DatumLiteralExpressie operandLinks, final GetalLiteralExpressie operandRechts)
    {
        Expressie resultaat = NullValue.getInstance();
        if (operandLinks.isVolledigBekend()) {
            final DateTime dt = operandLinks.alsDateTime();
            resultaat = new DatumLiteralExpressie(dt.plusDays(operandRechts.alsInteger()));
        }
        return resultaat;
    }

    @Override
    protected final Expressie pasToeOpDatumEnPeriode(final DatumLiteralExpressie operandLinks, final PeriodeLiteralExpressie operandRechts)
    {
        Expressie resultaat = NullValue.getInstance();
        if (operandLinks.isVolledigBekend()) {
            resultaat = FunctieDATUM.maakDatum(
                operandLinks.getJaar().getWaarde() + operandRechts.getJaar(),
                operandLinks.getMaand().getWaarde() + operandRechts.getMaand(),
                operandLinks.getDag().getWaarde() + operandRechts.getDag());
        }
        return resultaat;
    }

    @Override
    protected final Expressie pasToeOpPerioden(final PeriodeLiteralExpressie operandLinks, final PeriodeLiteralExpressie operandRechts)
    {
    ...
    @Override
    protected final Expressie pasToeOpLijsten(final Expressie operandLinks, final Expressie operandRechts)
    {
        final List<Expressie> elementen = new ArrayList<Expressie>();
        for (Expressie element : operandLinks.getElementen()) {
            elementen.add(element);
        }
        for (Expressie element : operandRechts.getElementen()) {
            elementen.add(element);
        }
        return new LijstExpressie(elementen);
    }
