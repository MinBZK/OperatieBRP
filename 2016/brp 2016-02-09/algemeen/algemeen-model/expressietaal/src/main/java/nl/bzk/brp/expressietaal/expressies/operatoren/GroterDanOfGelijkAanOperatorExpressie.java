...
/**
 * Representeert een groter-dan-of-gelijk-aan-expressie.
 */
public class GroterDanOfGelijkAanOperatorExpressie extends AbstractVergelijkingsoperatorExpressie {
     ...
     */
    public GroterDanOfGelijkAanOperatorExpressie(final Expressie termLinks, final Expressie termRechts)
    {
    ...
    protected final Expressie pasToeOpBooleans(final Expressie waardeLinks, final Expressie waardeRechts) {
        return BooleanLiteralExpressie.getExpressie(waardeLinks.alsBoolean() == waardeRechts.alsBoolean()
            || waardeLinks.alsBoolean() && !waardeRechts.alsBoolean());
    }
        ...
        final DatumLiteralExpressie waardeRechts)
    {
        ...
        if (waardeLinks.isVolledigBekend() && waardeRechts.isVolledigBekend()) {
            ...
            resultaat = geefResultaatVoorGedeeltelijkOnbekendeDatums(waardeLinks, waardeRechts);
        }
    ...
    /**
     * Geeft het resultaat in geval van (gedeeltelijk) onbekende datums.
     *
     * @param waardeLinks  De waarde links.
     * @param waardeRechts De waarde rechts.
     * @return Het resultaat.
     */
    private Boolean geefResultaatVoorGedeeltelijkOnbekendeDatums(final DatumLiteralExpressie waardeLinks, final DatumLiteralExpressie waardeRechts) {
        final Boolean resultaat;
        ...
        if (jaarCompare != 0) {
            ...
            if (!(waardeLinks.isMaandDagOnbekend() || waardeRechts.isMaandDagOnbekend())) {
                resultaat = geefResultaatVoorJarenGelijkEnMaandenBekend(waardeLinks, waardeRechts);

            } else if (waardeLinks.getMaand().getWaarde() == DatumUtils.DECEMBER
                && waardeLinks.getDag().getWaarde() == DatumUtils.dagenInMaand(waardeLinks.getJaar().getWaarde(), DatumUtils.DECEMBER))
            {
                resultaat = true;
            } else {
                resultaat = null;
    ...
    /**
     * Geeft het resultaat voor de situatie waarbij de jaren gelijk zijn en maanden bekend.
     *
     * @param waardeLinks  Waarde links.
     * @param waardeRechts Waarde rechts.
     * @return Het resultaat.
     */
    private Boolean geefResultaatVoorJarenGelijkEnMaandenBekend(final DatumLiteralExpressie waardeLinks, final DatumLiteralExpressie waardeRechts) {
        final Boolean resultaat;
        // jaren gelijk, en iig maanden bekend
        final int maandCompare = waardeLinks.getMaand().compareTo(waardeRechts.getMaand());
        final int laatsteDag = DatumUtils.dagenInMaand(waardeLinks.getJaar().getWaarde(), waardeLinks.getMaand().getWaarde());

        if (maandCompare != 0 && (waardeLinks.isDagOnbekend() || waardeRechts.isDagOnbekend())) {
            resultaat = maandCompare == 1;
        } else if (waardeLinks.getDag().getWaarde() == laatsteDag) {
            resultaat = true;
        } else {
            resultaat = null;
        }
    ...
    protected final Expressie pasToeOpLijsten(final Expressie waardeLinks, final Expressie waardeRechts) {
        final LijstExpressie.Vergelijkingsresultaat v = LijstExpressie.vergelijkLijsten(waardeLinks, waardeRechts, null);
        return BooleanLiteralExpressie.getExpressie(
            v == LijstExpressie.Vergelijkingsresultaat.GROTER || v == LijstExpressie.Vergelijkingsresultaat.GELIJK);
    }
