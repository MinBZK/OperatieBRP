 ...
 */
public final class FunctiePERSOONONDERZOEKEN implements Functieberekening {
    ...
    private Expressie bepaalOnderzoeken(final Persoon persoon)
    {
        ...
        for (final PersoonOnderzoek onderzoek : persoon.getOnderzoeken()) {
    ...
    {
        final List<Expressie> values = new ArrayList<>();
        for (final PersoonOnderzoekHisVolledig onderzoek : persoon.getOnderzoeken()) {
            values.add(new BrpObjectExpressie(onderzoek, ExpressieType.ONDERZOEK));
        }
        return new LijstExpressie(values);
    }
