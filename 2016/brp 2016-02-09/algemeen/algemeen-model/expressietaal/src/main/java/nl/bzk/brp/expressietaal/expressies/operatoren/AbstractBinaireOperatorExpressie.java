 ...
 */
public abstract class AbstractBinaireOperatorExpressie extends AbstractOperatorExpressie {
     ...
     */
    AbstractBinaireOperatorExpressie(final Expressie aOperandLinks, final Expressie aOperandRechts)
    {
        super();
        this.operandLinks = veiligeExpressie(aOperandLinks);
        this.operandRechts = veiligeExpressie(aOperandRechts);
    }
    ...
    public final Expressie evalueer(final Context context) {
        ...
        if (eLinks.isFout()) {
            ...
            if (eRechts.isFout()) {
                ...
                result = pasOperatorToe(eLinks, eRechts, context);
            }
        }
        return result;
    }
    ...
    public final boolean bevatOngebondenVariabele(final String id) {
        return getOperandLinks().bevatOngebondenVariabele(id) || getOperandRechts().bevatOngebondenVariabele(id);
    }

    @Override
    protected final String stringRepresentatie() {
        final StringBuilder result = new StringBuilder();
        if (operandLinks.getPrioriteit() <= this.getPrioriteit()) {
        ...
        result.append(' ');
        if (operandRechts.getPrioriteit() <= this.getPrioriteit()) {
