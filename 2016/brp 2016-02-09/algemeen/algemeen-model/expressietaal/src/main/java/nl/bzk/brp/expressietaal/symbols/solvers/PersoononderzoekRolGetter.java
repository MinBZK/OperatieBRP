...
import nl.bzk.brp.model.hisvolledig.momentview.kern.PersoonOnderzoekView;
import nl.bzk.brp.model.logisch.kern.PersoonOnderzoek;

/**
 * Getter voor 'persoononderzoek.rol' in objecttype 'PersoonOnderzoek'.
 *
 */
@Generated(value = "nl.bzk.brp.generatoren.java.SymbolTableGenerator")
public class PersoononderzoekRolGetter implements AttributeGetter {
    ...
    public final Expressie getAttribuutWaarde(final BrpObject brpObject) {
        ...
        if (brpObject instanceof PersoonOnderzoek) {
            final Attribuut attribuut = getAttribuut(brpObject);
            if (attribuut != null) {
                resultaat = new StringLiteralExpressie(((SoortPersoonOnderzoekAttribuut) attribuut).getWaarde().getNaam());
            }
            ...
            if (attributen != null) {
                ...
                for (final Attribuut attribuut : attributen) {
                    if (attribuut != null) {
                        elementen.add(new StringLiteralExpressie(((SoortPersoonOnderzoekAttribuut) attribuut).getWaarde().getNaam()));
                    }
                }
    ...
    public final Attribuut getAttribuut(final BrpObject brpObject) {
        ...
        if (brpObject instanceof PersoonOnderzoek) {
            final PersoonOnderzoek v = (PersoonOnderzoek) brpObject;
            if (v.getStandaard() != null) {
                resultaat = v.getStandaard().getRol();
            }
        } else if (brpObject instanceof PersoonOnderzoekHisVolledig) {
            final PersoonOnderzoek x = new PersoonOnderzoekView((PersoonOnderzoekHisVolledig) brpObject, new DatumTijdAttribuut(), new DatumAttribuut());
