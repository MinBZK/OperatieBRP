...
public abstract class AbstractPersoonNationaliteitPredikaatView implements ModelMoment, PersoonNationaliteitBasis, ElementIdentificeerbaar {
     ...
     */
    public AbstractPersoonNationaliteitPredikaatView(final PersoonNationaliteitHisVolledig persoonNationaliteit, final Predicate predikaat) {
    ...
    public final PersoonPredikaatView getPersoon() {
        return new PersoonPredikaatView(persoonNationaliteit.getPersoon(), getPredikaat());
    }
    ...
    /**
     * Retourneert Standaard van Persoon \ Nationaliteit. Deze methode bepaalt met behulp van het predikaat het geldige
     ...
     *
     * @return Retourneert Standaard van Persoon \ Nationaliteit
     */
    @Override
    public final HisPersoonNationaliteitModel getStandaard() {
        final List<HisPersoonNationaliteitModel> geldigRecord =
                new ArrayList(CollectionUtils.select(persoonNationaliteit.getPersoonNationaliteitHistorie().getHistorie(), getPredikaat()));
        if (geldigRecord.size() > 1) {
            LOGGER.error(
                "Kon geen moment-view bepalen voor persoonNationaliteit met id {}. Er zijn meerdere ({}) "
