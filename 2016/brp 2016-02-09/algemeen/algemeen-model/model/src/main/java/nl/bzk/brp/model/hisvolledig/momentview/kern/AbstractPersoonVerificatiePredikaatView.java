...
public abstract class AbstractPersoonVerificatiePredikaatView implements ModelMoment, PersoonVerificatieBasis, ElementIdentificeerbaar {
     ...
     */
    public AbstractPersoonVerificatiePredikaatView(final PersoonVerificatieHisVolledig persoonVerificatie, final Predicate predikaat) {
     ...
     */
    public final PersoonPredikaatView getGeverifieerde() {
        return new PersoonPredikaatView(persoonVerificatie.getGeverifieerde(), getPredikaat());
    }
    ...
    /**
     * Retourneert Standaard van Persoon \ Verificatie. Deze methode bepaalt met behulp van het predikaat het geldige
     ...
     *
     * @return Retourneert Standaard van Persoon \ Verificatie
     */
    @Override
    public final HisPersoonVerificatieModel getStandaard() {
        final List<HisPersoonVerificatieModel> geldigRecord =
                new ArrayList(CollectionUtils.select(persoonVerificatie.getPersoonVerificatieHistorie().getHistorie(), getPredikaat()));
        if (geldigRecord.size() > 1) {
            LOGGER.error(
                "Kon geen moment-view bepalen voor persoonVerificatie met id {}. Er zijn meerdere ({}) "
