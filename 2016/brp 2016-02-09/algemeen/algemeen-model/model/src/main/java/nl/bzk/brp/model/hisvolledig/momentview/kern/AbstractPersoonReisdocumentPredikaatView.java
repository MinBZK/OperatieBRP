...
public abstract class AbstractPersoonReisdocumentPredikaatView implements ModelMoment, PersoonReisdocumentBasis, ElementIdentificeerbaar {
     ...
     */
    public AbstractPersoonReisdocumentPredikaatView(final PersoonReisdocumentHisVolledig persoonReisdocument, final Predicate predikaat) {
    ...
    public final PersoonPredikaatView getPersoon() {
        return new PersoonPredikaatView(persoonReisdocument.getPersoon(), getPredikaat());
    }
    ...
    /**
     * Retourneert Standaard van Persoon \ Reisdocument. Deze methode bepaalt met behulp van het predikaat het geldige
     ...
     *
     * @return Retourneert Standaard van Persoon \ Reisdocument
     */
    @Override
    public final HisPersoonReisdocumentModel getStandaard() {
        final List<HisPersoonReisdocumentModel> geldigRecord =
                new ArrayList(CollectionUtils.select(persoonReisdocument.getPersoonReisdocumentHistorie().getHistorie(), getPredikaat()));
        if (geldigRecord.size() > 1) {
            LOGGER.error(
                "Kon geen moment-view bepalen voor persoonReisdocument met id {}. Er zijn meerdere ({}) "
