...
public abstract class AbstractPersoonAdresPredikaatView implements ModelMoment, PersoonAdresBasis, ElementIdentificeerbaar {
     ...
     */
    public AbstractPersoonAdresPredikaatView(final PersoonAdresHisVolledig persoonAdres, final Predicate predikaat) {
    ...
    public final PersoonPredikaatView getPersoon() {
        return new PersoonPredikaatView(persoonAdres.getPersoon(), getPredikaat());
    }

    /**
     * Retourneert Standaard van Persoon \ Adres. Deze methode bepaalt met behulp van het predikaat het geldige record.
     ...
     *
     * @return Retourneert Standaard van Persoon \ Adres
     */
    @Override
    public final HisPersoonAdresModel getStandaard() {
        final List<HisPersoonAdresModel> geldigRecord =
                new ArrayList(CollectionUtils.select(persoonAdres.getPersoonAdresHistorie().getHistorie(), getPredikaat()));
        if (geldigRecord.size() > 1) {
            LOGGER.error(
                "Kon geen moment-view bepalen voor persoonAdres met id {}. Er zijn meerdere ({}) "
