...
public abstract class AbstractPersoonOnderzoekPredikaatView implements ModelMoment, PersoonOnderzoekBasis, ElementIdentificeerbaar {
     ...
     */
    public AbstractPersoonOnderzoekPredikaatView(final PersoonOnderzoekHisVolledig persoonOnderzoek, final Predicate predikaat) {
    ...
    public final PersoonPredikaatView getPersoon() {
        return new PersoonPredikaatView(persoonOnderzoek.getPersoon(), getPredikaat());
    }
    ...
    public final OnderzoekPredikaatView getOnderzoek() {
        return new OnderzoekPredikaatView(persoonOnderzoek.getOnderzoek(), getPredikaat());
    }

    /**
     * Retourneert Standaard van Persoon \ Onderzoek. Deze methode bepaalt met behulp van het predikaat het geldige
     ...
     *
     * @return Retourneert Standaard van Persoon \ Onderzoek
     */
    @Override
    public final HisPersoonOnderzoekModel getStandaard() {
        final List<HisPersoonOnderzoekModel> geldigRecord =
                new ArrayList(CollectionUtils.select(persoonOnderzoek.getPersoonOnderzoekHistorie().getHistorie(), getPredikaat()));
        if (geldigRecord.size() > 1) {
            LOGGER.error(
                "Kon geen moment-view bepalen voor persoonOnderzoek met id {}. Er zijn meerdere ({}) "
