...
/**
 * Class met utility methodes omtrend de validatie.
 */
public final class ValidatorUtil {

    /**
     * Utility classes dienen geen publieke of standaard constructor te hebben. Om deze reden heeft deze utility class een lege private constructor.
     */
     ...
     *                                exceptie die gegooid wordt door BeanUtils.getProperty
        ...
        InvocationTargetException, NoSuchMethodException
    {
        ...
        final StringTokenizer st = new StringTokenizer(veld, ".");

        Object value = object;
        while (st.hasMoreTokens()) {
            final String variabel = st.nextToken();

            final Field field = ReflectionUtils.findField(value.getClass(), variabel);
            if (field != null) {
                field.setAccessible(true);
                value = field.get(value);

                if (value instanceof DatumTijdAttribuut || value instanceof DatumBasisAttribuut) {
                    resultaat = value;
                } else if (value instanceof Attribuut) {
                    resultaat = ((Attribuut) value).getWaarde();
                } else {
                    resultaat = value;
