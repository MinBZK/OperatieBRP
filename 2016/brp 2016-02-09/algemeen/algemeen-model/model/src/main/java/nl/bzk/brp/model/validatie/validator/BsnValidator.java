 ...
 * <p/>
 * Als de cijfers van het Burgerservicenummer van links naar rechts worden aangegeven met s[0] t/m s[8] dan gelden volgende condities:
 * (9*s0)+(8*s1)+(7*s2)+...+(2*s7)+(-1*s8) is deelbaar door 11.
 *
 * @brp.bedrijfsregel BRAL0012
 */
public class BsnValidator implements ConstraintValidator<Bsn, BurgerservicenummerAttribuut> {

    private static final int BSN_LENGTE = 9;
    private static final int ELF        = 11;

    @Override
    public final void initialize(final Bsn constraintAnnotation) {
        // Niets om te initialiseren.
    }

    @Override
    public final boolean isValid(final BurgerservicenummerAttribuut bsn, final ConstraintValidatorContext context) {
        final boolean resultaat;

        if (bsn == null) {
            resultaat = true;
        } else {
            resultaat = isGeldigeBsn(bsn);
        }
     ...
     */
    private boolean isGeldigeBsn(final BurgerservicenummerAttribuut bsn) {
        boolean resultaat = false;
        final String waarde = bsn.toString();
