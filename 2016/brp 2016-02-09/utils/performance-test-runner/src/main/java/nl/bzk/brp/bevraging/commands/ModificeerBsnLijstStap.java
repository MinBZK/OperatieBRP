...
public class ModificeerBsnLijstStap implements Command {
    ...
    public boolean execute(final Context context) throws Exception {
        List<PersoonIdentificatie> persoonIdentificaties = (List<PersoonIdentificatie>) context.get(ContextParameterNames.BSNLIJST);
        ...
        if (repeat > 2) {
            for (int i = 1; i < persoonIdentificaties.size(); i++) {
                if (i % repeat == 0) {
                    PersoonIdentificatie identificatie = persoonIdentificaties.get(i - 1);
                    persoonIdentificaties.set(i, identificatie);
                }
            }
        }

        if (persoonIdentificaties.size() > count) {
            persoonIdentificaties = persoonIdentificaties.subList(0, count);
        }
        context.put(ContextParameterNames.BSNLIJST, persoonIdentificaties);
