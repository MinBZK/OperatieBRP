...
public class SheetHandlerImplSimpleInsert extends AbstractSheetHandlerImpl {
    ...
    private final int              ID_ACTIE_ADMHND_COL  = 1;
            ...
            final int rowNumber)
    {
        ...
        // ID
        long id = idcalc(Long.valueOf(getColValue(row, ID_COL, colCount)).longValue(), gemeenteCount,
                ambtenaarCount);
        ...
        // Actie heeft verwijzing naar Adm handeling en nummert mee (één op één met actie)
        if (tabelNaam.equalsIgnoreCase("kern.actie")) {
        ...
        if (tabelNaam.equalsIgnoreCase("kern.his_persadres")) {
            ...
            setColValue(clone, 12, colCount, "" + adresDTO.woonplaatsNaam);
        ...
        if (tabelNaam.equalsIgnoreCase("kern.his_persbijhouding")) {
            final boolean doDis = getColValue(row, 3, colCount) == null || getColValue(row, 3, colCount).length() == 0;

            if (doDis) {
                setColValue(clone, 7, colCount, "" + gemeente.code);
            }
        }
