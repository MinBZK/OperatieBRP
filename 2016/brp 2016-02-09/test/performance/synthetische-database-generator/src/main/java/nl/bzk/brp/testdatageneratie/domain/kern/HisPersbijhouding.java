/**
 * This file is copyright 2017 State of the Netherlands (Ministry of Interior Affairs and Kingdom Relations).
 * It is made available under the terms of the GNU Affero General Public License, version 3 as published by the Free Software Foundation.
 * The project of which this file is part, may be found at https://github.com/MinBZK/operatieBRP.
 */

package nl.bzk.brp.testdatageneratie.domain.kern;
// Generated Jul 24, 2014 5:36:58 PM by Hibernate Tools 3.2.4.GA


import java.util.Date;

/**
 * HisPersbijhouding generated by hbm2java
 */
public class HisPersbijhouding  implements HisMaterieel,IntegerAsPrimaryKey,java.io.Serializable {


     private int id;
     private int pers;
     private Integer dataanvgel;
     private Integer dateindegel;
     private Date tsreg;
     private Date tsverval;
     private Long actieinh;
     private Long actieverval;
     private String nadereaandverval;
     private Long actieaanpgel;
     private short bijhpartij;
     private short bijhaard;
     private short naderebijhaard;
     private boolean indonverwdocaanw;

    public HisPersbijhouding() {
    }

	
    public HisPersbijhouding(int id, int pers, short bijhpartij, short bijhaard, short naderebijhaard, boolean indonverwdocaanw) {
        this.id = id;
        this.pers = pers;
        this.bijhpartij = bijhpartij;
        this.bijhaard = bijhaard;
        this.naderebijhaard = naderebijhaard;
        this.indonverwdocaanw = indonverwdocaanw;
    }
    public HisPersbijhouding(int id, int pers, Integer dataanvgel, Integer dateindegel, Date tsreg, Date tsverval, Long actieinh, Long actieverval, String nadereaandverval, Long actieaanpgel, short bijhpartij, short bijhaard, short naderebijhaard, boolean indonverwdocaanw) {
       this.id = id;
       this.pers = pers;
       this.dataanvgel = dataanvgel;
       this.dateindegel = dateindegel;
       this.tsreg = tsreg;
       this.tsverval = tsverval;
       this.actieinh = actieinh;
       this.actieverval = actieverval;
       this.nadereaandverval = nadereaandverval;
       this.actieaanpgel = actieaanpgel;
       this.bijhpartij = bijhpartij;
       this.bijhaard = bijhaard;
       this.naderebijhaard = naderebijhaard;
       this.indonverwdocaanw = indonverwdocaanw;
    }
   
    public int getId() {
        return this.id;
    }
    
    public void setId(int id) {
        this.id = id;
    }
    public int getPers() {
        return this.pers;
    }
    
    public void setPers(int pers) {
        this.pers = pers;
    }
    public Integer getDataanvgel() {
        return this.dataanvgel;
    }
    
    public void setDataanvgel(Integer dataanvgel) {
        this.dataanvgel = dataanvgel;
    }
    public Integer getDateindegel() {
        return this.dateindegel;
    }
    
    public void setDateindegel(Integer dateindegel) {
        this.dateindegel = dateindegel;
    }
    public Date getTsreg() {
        return this.tsreg;
    }
    
    public void setTsreg(Date tsreg) {
        this.tsreg = tsreg;
    }
    public Date getTsverval() {
        return this.tsverval;
    }
    
    public void setTsverval(Date tsverval) {
        this.tsverval = tsverval;
    }
    public Long getActieinh() {
        return this.actieinh;
    }
    
    public void setActieinh(Long actieinh) {
        this.actieinh = actieinh;
    }
    public Long getActieverval() {
        return this.actieverval;
    }
    
    public void setActieverval(Long actieverval) {
        this.actieverval = actieverval;
    }
    public String getNadereaandverval() {
        return this.nadereaandverval;
    }
    
    public void setNadereaandverval(String nadereaandverval) {
        this.nadereaandverval = nadereaandverval;
    }
    public Long getActieaanpgel() {
        return this.actieaanpgel;
    }
    
    public void setActieaanpgel(Long actieaanpgel) {
        this.actieaanpgel = actieaanpgel;
    }
    public short getBijhpartij() {
        return this.bijhpartij;
    }
    
    public void setBijhpartij(short bijhpartij) {
        this.bijhpartij = bijhpartij;
    }
    public short getBijhaard() {
        return this.bijhaard;
    }
    
    public void setBijhaard(short bijhaard) {
        this.bijhaard = bijhaard;
    }
    public short getNaderebijhaard() {
        return this.naderebijhaard;
    }
    
    public void setNaderebijhaard(short naderebijhaard) {
        this.naderebijhaard = naderebijhaard;
    }
    public boolean isIndonverwdocaanw() {
        return this.indonverwdocaanw;
    }
    
    public void setIndonverwdocaanw(boolean indonverwdocaanw) {
        this.indonverwdocaanw = indonverwdocaanw;
    }




}


