/**
 * This file is copyright 2017 State of the Netherlands (Ministry of Interior Affairs and Kingdom Relations).
 * It is made available under the terms of the GNU Affero General Public License, version 3 as published by the Free Software Foundation.
 * The project of which this file is part, may be found at https://github.com/MinBZK/operatieBRP.
 */

package nl.bzk.brp.testdatageneratie.domain.kern;
// Generated Jul 24, 2014 5:36:58 PM by Hibernate Tools 3.2.4.GA


import java.util.Date;

/**
 * HisOnderzoek generated by hbm2java
 */
public class HisOnderzoek  implements His,IntegerAsPrimaryKey,java.io.Serializable {


     private int id;
     private int onderzoek;
     private Date tsreg;
     private Date tsverval;
     private Long actieinh;
     private Long actieverval;
     private String nadereaandverval;
     private int dataanv;
     private int verwachteafhandeldat;
     private Integer dateinde;
     private String oms;
     private short status;

    public HisOnderzoek() {
    }

	
    public HisOnderzoek(int id, int onderzoek, int dataanv, int verwachteafhandeldat, short status) {
        this.id = id;
        this.onderzoek = onderzoek;
        this.dataanv = dataanv;
        this.verwachteafhandeldat = verwachteafhandeldat;
        this.status = status;
    }
    public HisOnderzoek(int id, int onderzoek, Date tsreg, Date tsverval, Long actieinh, Long actieverval, String nadereaandverval, int dataanv, int verwachteafhandeldat, Integer dateinde, String oms, short status) {
       this.id = id;
       this.onderzoek = onderzoek;
       this.tsreg = tsreg;
       this.tsverval = tsverval;
       this.actieinh = actieinh;
       this.actieverval = actieverval;
       this.nadereaandverval = nadereaandverval;
       this.dataanv = dataanv;
       this.verwachteafhandeldat = verwachteafhandeldat;
       this.dateinde = dateinde;
       this.oms = oms;
       this.status = status;
    }
   
    public int getId() {
        return this.id;
    }
    
    public void setId(int id) {
        this.id = id;
    }
    public int getOnderzoek() {
        return this.onderzoek;
    }
    
    public void setOnderzoek(int onderzoek) {
        this.onderzoek = onderzoek;
    }
    public Date getTsreg() {
        return this.tsreg;
    }
    
    public void setTsreg(Date tsreg) {
        this.tsreg = tsreg;
    }
    public Date getTsverval() {
        return this.tsverval;
    }
    
    public void setTsverval(Date tsverval) {
        this.tsverval = tsverval;
    }
    public Long getActieinh() {
        return this.actieinh;
    }
    
    public void setActieinh(Long actieinh) {
        this.actieinh = actieinh;
    }
    public Long getActieverval() {
        return this.actieverval;
    }
    
    public void setActieverval(Long actieverval) {
        this.actieverval = actieverval;
    }
    public String getNadereaandverval() {
        return this.nadereaandverval;
    }
    
    public void setNadereaandverval(String nadereaandverval) {
        this.nadereaandverval = nadereaandverval;
    }
    public int getDataanv() {
        return this.dataanv;
    }
    
    public void setDataanv(int dataanv) {
        this.dataanv = dataanv;
    }
    public int getVerwachteafhandeldat() {
        return this.verwachteafhandeldat;
    }
    
    public void setVerwachteafhandeldat(int verwachteafhandeldat) {
        this.verwachteafhandeldat = verwachteafhandeldat;
    }
    public Integer getDateinde() {
        return this.dateinde;
    }
    
    public void setDateinde(Integer dateinde) {
        this.dateinde = dateinde;
    }
    public String getOms() {
        return this.oms;
    }
    
    public void setOms(String oms) {
        this.oms = oms;
    }
    public short getStatus() {
        return this.status;
    }
    
    public void setStatus(short status) {
        this.status = status;
    }




}


