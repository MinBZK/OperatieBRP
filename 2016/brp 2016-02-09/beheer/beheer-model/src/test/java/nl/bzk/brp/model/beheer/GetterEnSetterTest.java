...
public class GetterEnSetterTest {
    ...
    private static final String ROOT_PACKAGE = "nl.bzk.brp.model.beheer";

    private static final Map<Class<? extends Object>, Object> VALUES = new HashMap<>();
    private static final Map<String, String> ADJUST_FIELDNAME = new HashMap<>();
    ...
    static {
        ...
        VALUES.put(BigDecimal.class, new BigDecimal(1));
        VALUES.put(Timestamp.class, new Timestamp(1L));
        VALUES.put(Short.class, (short) 42);

        //ADJUST_FIELDNAME.put("nl.bzk.brp.model.beheer.autaut.HisDienst#datumTijdRegistratie", "tijdstipRegistratie");
    }
    ...
    public void testEntityGetters() throws Exception {
        final List<Class<? extends Object>> classes = getClassesInPackage(ROOT_PACKAGE);
        final List<Class<? extends Object>> entities = filterClassesOpEntities(classes);
    ...
    public void testEnumGetters() throws Exception {
        final List<Class<? extends Object>> classes = getClassesInPackage(ROOT_PACKAGE);
        final List<Class<? extends Object>> enums = filterClassesOpEnums(classes);

        for (final Class<? extends Object> clazz : enums) {
    ...
    public void testLogischModelSetters() throws Exception {
        final List<Class<? extends Object>> classes = getClassesInPackage(ROOT_PACKAGE + ".logisch");
        final List<Class<? extends Object>> modelClasses = filterClassesOpModelClasses(classes);
                        ...
                        if (isTestbareSetterMethodeVoorVeld(setterMethode, entityVeld)) {
                            ...
                            try {
                                ...
                                if (ite.getCause() instanceof NullPointerException) {
                                    // verplichte velden mogen niet op null worden gezet en dat resulteert in een NPE
                                } else {
                                    throw ite;
                                }
                            }
    ...
    private Object getStandaardWaardeVoorVeld(final Field veld) throws Exception {
        ...
        if (veld.getType().isEnum()) {
            ...
            final Constructor<? extends Object> constructor = veld.getType().getDeclaredConstructor();
    ...
    private boolean isTestbareSetterMethodeVoorVeld(final Method setterMethode, final Field veld) {
        ...
        final Class<? extends Object> paramClazz = setterMethode.getParameterTypes()[0];
    ...
    private String corrigeerVeldNaam(final Field veld) {
        String veldNaam = veld.getName();
        final String keyVeld = veld.getDeclaringClass().getName() + "#" + veld.getName();
        if (ADJUST_FIELDNAME.containsKey(keyVeld)) {
            veldNaam = ADJUST_FIELDNAME.get(keyVeld);
        }
        return veldNaam;
    }

    private String bouwGetterMethodeNaamVoorVeld(final Field veld) {
        return String.format("get%s%s", corrigeerVeldNaam(veld).substring(0, 1).toUpperCase(), veld.getName().substring(1));
    }

    private String bouwBooleanGetterMethodeNaamVoorVeld(final Field veld) {
        return String.format("is%s%s", corrigeerVeldNaam(veld).substring(0, 1).toUpperCase(), veld.getName().substring(1));
    }

    private String bouwSetterMethodeNaamVoorVeld(final Field veld) {
        return String.format("set%s%s", corrigeerVeldNaam(veld).substring(0, 1).toUpperCase(), veld.getName().substring(1));
    }
    ...
    private List<Class<? extends Object>> filterClassesOpEntities(final List<Class<? extends Object>> classes) {
        final List<Class<? extends Object>> entities = new ArrayList<>();
        for (final Class<? extends Object> clazz : classes) {
    ...
    private List<Class<? extends Object>> filterClassesOpModelClasses(final List<Class<? extends Object>> classes) {
        final List<Class<? extends Object>> modelClasses = new ArrayList<>();
        for (final Class<? extends Object> clazz : classes) {
    ...
    private List<Class<? extends Object>> filterClassesOpEnums(final List<Class<? extends Object>> classes) {
        final List<Class<? extends Object>> enums = new ArrayList<>();
        for (final Class<? extends Object> clazz : classes) {
            if (clazz.isEnum()) {
                enums.add(clazz);
            }
