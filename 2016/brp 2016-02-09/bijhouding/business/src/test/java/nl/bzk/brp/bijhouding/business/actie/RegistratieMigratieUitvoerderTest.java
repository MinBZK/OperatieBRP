...
import nl.bzk.brp.bijhouding.business.regels.impl.gegevenset.persoon.migratie.MigratieGroepVerwerker;
 ...
 *
 */
public class RegistratieMigratieUitvoerderTest {
    ...
    public void testVerzamelVerwerkingsregels() {
        ...
        RegistratieMigratieUitvoerder registratieMigratieUitvoerder = new RegistratieMigratieUitvoerder() {
            @Override
        ...
        registratieMigratieUitvoerder.verzamelVerwerkingsregels();

        final List<Verwerkingsregel> verwerkingsregels =
                (List<Verwerkingsregel>) ReflectionTestUtils
                        .getField(registratieMigratieUitvoerder, "verwerkingsregels");

        Assert.assertTrue(verwerkingsregels.size() == 1);
        Assert.assertTrue(verwerkingsregels.get(0) instanceof MigratieGroepVerwerker);
