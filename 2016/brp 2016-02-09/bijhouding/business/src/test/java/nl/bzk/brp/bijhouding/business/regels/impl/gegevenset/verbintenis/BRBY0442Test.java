...
public class BRBY0442Test {
    ...
    public void testGetRegel() {
        Assert.assertEquals(Regel.BRBY0442, brby0442.getRegel());
    }

    @Test
    public void testGeenStandaardGroep() {
        soortAH = SoortAdministratieveHandeling.VOLTREKKING_HUWELIJK_IN_BUITENLAND;
        final List<BerichtEntiteit> resultaat = brby0442.voerRegelUit(null, bouwHuwelijk(null), actie, null);
    ...
    public void testGeenLandAanvang() {
        ...
        ReflectionTestUtils.setField(huwelijk.getStandaard(), "landGebiedAanvang", null);
        final List<BerichtEntiteit> resultaat = brby0442.voerRegelUit(null, huwelijk, actie, null);
        Assert.assertEquals(0, resultaat.size());
    }

    @Test
    public void testHuwelijkNederlandLandNederland() {
        soortAH = SoortAdministratieveHandeling.VOLTREKKING_HUWELIJK_IN_NEDERLAND;
        final List<BerichtEntiteit> resultaat = brby0442.voerRegelUit(null, bouwHuwelijk(NEDERLAND), actie, null);
        Assert.assertEquals(0, resultaat.size());
    }
    ...
    public void testPartnerschapBuitenlandLandNederland() {
        soortAH = SoortAdministratieveHandeling.AANGAAN_GEREGISTREERD_PARTNERSCHAP_IN_BUITENLAND;
        final List<BerichtEntiteit> resultaat = brby0442.voerRegelUit(null, bouwHuwelijk(NEDERLAND), actie, null);
        Assert.assertEquals(1, resultaat.size());
    }
    ...
    public void testHuwelijkBuitenlandLandBuitenland() {
        soortAH = SoortAdministratieveHandeling.VOLTREKKING_HUWELIJK_IN_BUITENLAND;
        final List<BerichtEntiteit> resultaat = brby0442.voerRegelUit(null, bouwHuwelijk(BUITENLAND), actie, null);
        Assert.assertEquals(0, resultaat.size());
    }
    ...
    private HuwelijkBericht bouwHuwelijk(final LandGebied land) {
        ...
        if (land != null) {
            ...
            hgpGroep.setLandGebiedAanvang(new LandGebiedAttribuut(land));
