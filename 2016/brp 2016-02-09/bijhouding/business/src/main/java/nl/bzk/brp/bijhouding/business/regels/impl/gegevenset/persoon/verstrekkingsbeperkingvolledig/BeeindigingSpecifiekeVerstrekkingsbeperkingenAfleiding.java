...
/**
 * Beeindig alle specifieke verstrekkingsbeperkingen van een persoon, nav
 * het registreren van een volledige verstrekkingsbeperking.
 */
public class BeeindigingSpecifiekeVerstrekkingsbeperkingenAfleiding
        extends AbstractAfleidingsregel<PersoonHisVolledig>
{
     ...
     */
    public BeeindigingSpecifiekeVerstrekkingsbeperkingenAfleiding(
            final PersoonHisVolledig rootObject, final ActieModel actie)
    {
    ...
    public final Regel getRegel() {
        return Regel.VR00034a;
    }

    @Override
    public AfleidingResultaat leidAf() {
        boolean afgeleidVervallen = false;
        for (final PersoonVerstrekkingsbeperkingHisVolledig verstrekkingsbeperking
                : getModel().getVerstrekkingsbeperkingen())
        {
            // Als deze specifieke verstrekkingsbeperking nog niet vervallen is, laat deze dan vervallen
            // met als actie en tijdstip de gegevens uit de 'veroorzakende' actie.
            if (!verstrekkingsbeperking.getPersoonVerstrekkingsbeperkingHistorie().isVervallen()) {
                verstrekkingsbeperking.getPersoonVerstrekkingsbeperkingHistorie()
                        .verval(getActie(), getActie().getTijdstipRegistratie());
