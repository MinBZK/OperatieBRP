...
/**
 * DTO klasse die het resultaat van een bijhouding representeert en onder andere de tijdens de berichtuitvoering
 * opgetreden meldingen en bijgehouden/aangepaste personen bevat.
 */
public class BijhoudingResultaat extends BerichtVerwerkingsResultaatImpl implements BerichtVerwerkingsResultaat {

    private Set<Persoon> bijgehoudenPersonen;
    private Map<String, DocumentModel> bijgehoudenDocumenten;
    ...
    public BijhoudingResultaat(final List<Melding> meldingen) {
        ...
        bijgehoudenPersonen = new TreeSet<>(new Comparator<Persoon>() {
            @Override
            public int compare(final Persoon persoon1, final Persoon persoon2) {
                final int vergelijk;

                //TODO refactor naar gebruik van CompareToBuilder
                if (persoon2.getIdentificatienummers() == null || persoon2.getIdentificatienummers().getBurgerservicenummer() == null) {
                    vergelijk = -1;
                } else if (persoon1.getIdentificatienummers() == null || persoon1.getIdentificatienummers().getBurgerservicenummer() == null) {
                    vergelijk = 1;
                } else {
                    vergelijk = persoon1.getIdentificatienummers().getBurgerservicenummer().getWaarde()
                            .compareTo(persoon2.getIdentificatienummers().getBurgerservicenummer().getWaarde());
                }
                return vergelijk;
            }
        });

        bijgehoudenDocumenten = new HashMap<>();
    }
    ...
    /**
     * Retourneert de (onaanpasbare) lijst van documenten die zijn bijgehouden/aangepast tijdens de bericht verwerking.
     *
     * @return de lijst van bijgehouden/aangepaste documenten.
     */
     ...
     * Voegt een lijst van personen toe aan de huidige reeks van bijgehouden/aangepaste personen.
     *
     * @param documenten De toe te voegen documenten.
     */
    public void voegDocumentenToe(final Map<String, DocumentModel> documenten) {
        bijgehoudenDocumenten.putAll(documenten);
    }
