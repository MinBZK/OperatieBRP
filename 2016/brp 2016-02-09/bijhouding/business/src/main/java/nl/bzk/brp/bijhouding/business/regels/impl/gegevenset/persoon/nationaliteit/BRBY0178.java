...
/**
 * Persoon mag geen kinderen hebben als nationaliteit wijzigt.
 *
 * @brp.bedrijfsregel BRBY0178
 */
@Named("BRBY0178")
public class BRBY0178 implements VoorActieRegelMetMomentopname<PersoonView, PersoonBericht> {

    @Override
    public final Regel getRegel() {
        return Regel.BRBY0178;
    }
        ...
        if (huidigeSituatie != null) {
            for (Betrokkenheid betrokkenheid : huidigeSituatie.getBetrokkenheden()) {
                if (betrokkenheid.getRol().getWaarde() == SoortBetrokkenheid.OUDER
                    // Check ook expliciet of er een actieve ouderschap aanwezig is.
                    && ((Ouder) betrokkenheid).getOuderschap() != null)
                {
                    objectenDieDeRegelOvertreden.add(nieuweSituatie);
                    // Eén kind is voldoende, meerdere kinderen voegen niks toe aan het afgaan van de regel.
                    break;
                }
            }
        }
