...
/**
 * Uitvoerder voor registratie adres.
 */
public class RegistratieAdresUitvoerder extends AbstractActieUitvoerder<
        PersoonBericht, PersoonHisVolledigImpl>
{

    @Override
    protected void verzamelVerwerkingsregels() {
        // Aanname: er is op dit moment maar 1 adres toegestaan.
        final PersoonAdresBericht adresBericht = getBerichtRootObject().getAdressen().iterator().next();
        final PersoonAdresHisVolledigImpl adresModel = getModelRootObject().getAdressen().iterator().next();
