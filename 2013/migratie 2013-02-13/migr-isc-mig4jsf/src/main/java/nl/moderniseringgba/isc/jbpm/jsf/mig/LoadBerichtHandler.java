 ...
 *
 */
@TldTag(name = "loadBericht", description = "Read a berihte from the database.", attributes = {
        @TldAttribute(name = "id", description = "The ID of the bericht to load.", required = true,
                deferredType = long.class),
        @TldAttribute(name = "target",
                description = "An EL expression into which the process instance should be stored.", required = true,
                deferredType = Bericht.class) })
public final class LoadBerichtHandler extends AbstractHandler {
    ...
    protected JbpmActionListener getListener(final FaceletContext ctx) {
        return new LoadBerichtActionListener(getValueExpression(idTagAttribute, ctx, Long.class), getValueExpression(
                targetTagAttribute, ctx, Bericht.class));
    }
}
