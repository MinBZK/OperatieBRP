 ...
 */
@Component
public final class BrpIdentificatienummersMapper extends BrpMapper<PersoonIDHistorie, BrpIdentificatienummersInhoud> {

    @Override
    protected BrpIdentificatienummersInhoud mapInhoud(final PersoonIDHistorie historie) {
        // @formatter:off
        return new BrpIdentificatienummersInhoud(
                BrpMapperUtil.mapLong(historie.getAdministratienummer()), 
                BrpMapperUtil.mapLong(historie.getBurgerservicenummer())
            );        
        // @formatter:on
    }
