...
public class MaakZoekPersoonBinnenGemeenteActionTest {
    private final MaakZoekPersoonBinnenGemeenteAction subject = new MaakZoekPersoonBinnenGemeenteAction();
    ...
    public void test() {
        ...
        final ZoekPersoonVerzoekBericht verzoek =
                (ZoekPersoonVerzoekBericht) result.get("zoekPersoonBinnenGemeenteBericht");
        Assert.assertNotNull(verzoek);
        Assert.assertEquals("1234567891", verzoek.getBsn());
        Assert.assertEquals(new BrpGemeenteCode(new BigDecimal("5678")), verzoek.getBijhoudingsGemeente());
    }
