...
public class SslConnectionTest {
    ...
    public void readMoreThenSpdLengthField() {
        try {
            ...
            Assert.assertTrue(ce.getMessage().startsWith("[MELDING-8007]:"));
        } catch (final IOException e) {
            Assert.fail("Geen IOException verwacht");
        }
    }
    ...
    public void readIOException() {
        try {
            Mockito.when(socket.isClosed()).thenReturn(false);
            Mockito.when(in.read(Matchers.any(byte[].class), Matchers.anyInt(), Matchers.anyInt())).thenThrow(
                    new IOException());
            ...
            Assert.fail("Er wordt geen IOException verwacht");
        }
    }
    ...
    public void read() {
        try {
            ...
            Mockito.when(socket.isClosed()).thenReturn(false);
            Mockito.when(in.read(Matchers.any(byte[].class), Matchers.anyInt(), Matchers.anyInt())).thenAnswer(
                    new Answer<Integer>() {
            ...
            Assert.assertEquals("Lengte van de berichten niet gelijk",
                    bericht.length() - SpdConstants.TERMINATOR.length(), mesg.getLength());
        } catch (final IOException e) {
            Assert.fail("Geen IOException verwacht");
        }
    }
    ...
    public void readLengthLengthNotANumber() {
        ...
        try {
            ...
            Assert.assertEquals("Lengte van de berichten niet gelijk", -1, mesg.getLength());
        } catch (final IOException e) {
            Assert.fail("Geen IOException verwacht");
        }
    }
