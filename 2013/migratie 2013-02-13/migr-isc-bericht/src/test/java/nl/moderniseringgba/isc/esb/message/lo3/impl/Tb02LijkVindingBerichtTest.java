...
public class Tb02LijkVindingBerichtTest {
    ...
    private Tb02Bericht maakTb02Bericht(final Lo3Persoonslijst lo3PersoonsLijst) {
        final Tb02Bericht tb02Bericht = new Tb02LijkvindingBericht(BRON_GEMEENTE, AKTENUMMER, new Lo3Datum(20130115));
    ...
    public void testGettersEnSetters() throws Exception {
        final Lo3Persoonslijst pl = maakLo3Persoonslijst();
        final Tb02Bericht tb02Bericht = maakTb02Bericht(pl);
        ((Tb02LijkvindingBericht) tb02Bericht).setAktenummer(AKTENUMMER);
        ((Tb02LijkvindingBericht) tb02Bericht).setIngangsdatumGeldigheid(INGANGSDATUM);
        ((Tb02LijkvindingBericht) tb02Bericht).setRegistratieGemeente(BRON_GEMEENTE);
        ...
        assertEquals("Registratiegemeente ongelijk.", BRON_GEMEENTE,
                ((Tb02LijkvindingBericht) tb02Bericht).getRegistratieGemeente());
        assertEquals("Aktenummer ongelijk.", AKTENUMMER, ((Tb02LijkvindingBericht) tb02Bericht).getAktenummer());
        assertEquals("Aktenummer ongelijk.", INGANGSDATUM,
                ((Tb02LijkvindingBericht) tb02Bericht).getIngangsdatumGeldigheid());
    ...
    private static Lo3Persoonslijst maakLo3Persoonslijst() {
        ...
        return new Lo3PersoonslijstBuilder().persoonStapel(lo3PersoonStapel).ouder1Stapel(null).ouder2Stapel(null)
                .overlijdenStapel(lo3OverlijdenStapel).build();
    }
