...
package nl.moderniseringgba.migratie.operatie;
...
/**
 * Ondersteunt het (beperkt) herhalen van operaties, optioneel met vertraging,
 * totdat de operatie successvol is, of totdat het maximum aantal pogingen wordt
 * overschreden.
 * Success is gedefinieerd als 'geen exception'.
 * <p/>
 * Er kan gewerkt worden zowel met Callable instances als met Runnable instances.
 * Callable heeft de voorkeur, aangezien die checked exceptions mag gooien, en
 * Runnable alleen runtime exceptions kan gooien.
 */
public class Herhaal {
    ...
    public enum Strategie {
        ...
        /**
         * Gebruik een lineair toenemende vertraging. De eerste keer de basis vertraging,
         * de tweede keer 2 keer de basis vertraging, etc.
         */
        LINEAIR {
            @Override
            public long berekenWachttijd(final long basis, final int poging) {
        ...
        /**
         * Gebruik een exponentieel toenemende vertraging. De eerste keer de basis vertraging,
         * en bij elke volgende poging verdubbelt de vertraging.
         */
        EXPONENTIEEL {
            @Override
            public long berekenWachttijd(final long basis, final int poging) {
        ...
        /**
         * Bereken de wachttijd voor de vertraging strategie op basis van een basis wachttijd
         * en het aantal al ondernomen pogingen.
         *
    ...
    private static final Herhaal STANDAARD = new Herhaal(
            STANDAARD_VERTRAGING_MILLIS,
            STANDAARD_MAXIMUM_AANTAL_POGINGEN,
            STANDAARD_STRATEGIE);
    ...
    /**
     * Maak een Herhaal object aan met gegeven basis vertraging, maximum aantal pogingen, en
     * een wachtstrategie.
     *
    ...
    /**
     * Herhaal de operatie met de standaard instellinging. Dat wil zeggen probeer het maximaal
     * 10 keer met tussen elke poging een willekeurige vertraging tussen de 0 en 2 seconden.
     *
    ...
    public final <R> R herhaal(final Callable<R> operatie) throws HerhaalException {
        int pogingNummer = 1;
        final List<Exception> pogingExcepties = new ArrayList<Exception>(maximumAantalPogingen);
        while (pogingNummer <= maximumAantalPogingen) {
            boolean laatstePogingOk = true;
            try {
                return operatie.call();
                // CHECKSTYLE:OFF - Catch Exception is nodig voor correcte functionaliteit
                ...
                if (laatstePogingOk && pogingNummer > 1) {
                    LOG.warn("Operatie succesvol bij {}e poging. Excepties bij eerdere pogingen:", pogingNummer);
                    for (int i = 0; i < pogingExcepties.size(); i++) {
                        final Exception exception = pogingExcepties.get(i);
                        LOG.warn("Exceptie bij " + (i + 1) + "e poging:", exception);
                    }
                }
            }
        }
