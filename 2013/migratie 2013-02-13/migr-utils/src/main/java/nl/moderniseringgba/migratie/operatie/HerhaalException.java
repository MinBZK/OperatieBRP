...
public final class HerhaalException extends Exception {
    ...
    public HerhaalException(final List<Exception> pogingExcepties) {
        super(MessageFormat.format("Herhaalde operatie gefaald na {0} pogingen. Laatste fout: {1}",
                pogingExcepties.size(), pogingExcepties.get(pogingExcepties.size() - 1).getMessage()),
                pogingExcepties.get(pogingExcepties.size() - 1));
