...
SELECT A.id, MOD(MOD(A.id, 20000000), 720), bsn, anr, gem, wpl, postcode, nor, huisnr, huisnrtoevoeging
...
UPDATE info_ids_leveranciers A SET functionelecode_gem = (SELECT  code FROM kern.partij B WHERE A.functionelecode_gem=B.id);
