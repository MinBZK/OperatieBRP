...
public class JavaObjectTypeOperationeelBasisGenerator extends AbstractJavaGenerator<ObjectType> {
    ...
    public void genereerElement(final Writer writer, final GenerationReport report, final ObjectType objectType) {
        ...
        type.addImport(GenerationPackageNames.OBJECTTYPE_OPERATIONEEL_PACKAGE.createImportStatement(new Identifier("ActieModel")));
        ...
        type.setExtendsFrom(objectType.getSoortInhoud());
        ...
        for (Groep groep : groepen) {
            if (groep.getHistorieVastleggen() == HistorieVastleggen.GEEN.getCode()) {
                ...
                Field field = new Field(getGroepOperationeelActueel(groep).getUpperCamel(), groep.getIdentCode() + "Groep");
        ...
        String sourceCode = genereerObjectType(type, stam, inlines, groepNamen);
    ...
    private String genereerObjectType(final ObjectInterface type, final String stam, final List<Identifier> inlines, final List<Identifier> groepen) {
