...
public class SchemaValidationInInterceptor extends AbstractPhaseInterceptor<Message> {
    ...
    public void handleMessage(final Message message) {
        try {
            final Validator validator = SchemaValidationHelper.getSchema(
                    (String) message.get("SOAPAction")).newValidator();
            final SOAPMessage soapMessage = message.getContent(SOAPMessage.class);
            final SOAPBody soapBody = soapMessage.getSOAPBody();
            validator.validate(new DOMSource(soapBody.extractContentAsDocument()));
