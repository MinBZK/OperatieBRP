        ...
        extends AbstractBerichtVerwerkingsStap<AbstractBijhoudingsBericht, BerichtVerwerkingsResultaat>
{
            ...
            final BerichtVerwerkingsResultaat resultaat)
    {
        ...
        if (SoortActie.HUWELIJK == soort) {
            ...
            groep.setLandAanvang(vindLandOpCode(groep.getLandAanvangCode(),
                    groep, resultaat));
        }
        // LandEinde
        if (AttribuutTypeUtil.isNotBlank(groep.getLandEindeCode())) {
            groep.setLandEinde(vindLandOpCode(groep.getLandEindeCode(),
                    groep, resultaat));
        }
        // Gemeente aanvang
        if (AttribuutTypeUtil.isNotBlank(groep.getGemeenteAanvangCode())) {
            groep.setGemeenteAanvang(vindGemeenteOpCode(
                    groep.getGemeenteAanvangCode(), groep, resultaat));
        }
        // Gemeente einde
        if (AttribuutTypeUtil.isNotBlank(groep.getGemeenteEindeCode())) {
            groep.setGemeenteEinde(vindGemeenteOpCode(groep.getGemeenteEindeCode(),
                    groep, resultaat));
        }
        // Woonplaats aanvang
        if (AttribuutTypeUtil.isNotBlank(groep.getWoonPlaatsAanvangCode())) {
            groep.setWoonPlaatsAanvang(vindWoonplaatsOpCode(
                    groep.getWoonPlaatsAanvangCode(), groep, resultaat));
        }
        // Woonplaats einde
        if (AttribuutTypeUtil.isNotBlank(groep.getWoonPlaatsEindeCode())) {
            groep.setWoonPlaatsEinde(vindWoonplaatsOpCode(
                    groep.getWoonPlaatsEindeCode(), groep, resultaat));
        }
            ...
            final BerichtVerwerkingsResultaat resultaat)
    {
        ...
        if (AttribuutTypeUtil.isNotBlank(groep.getGemeenteGeboorteCode())) {
            groep.setGemeenteGeboorte(vindGemeenteOpCode(groep.getGemeenteGeboorteCode(), groep,
                    resultaat));
        }
        // Land geboorte
        ...
        if (AttribuutTypeUtil.isNotBlank(groep.getWoonplaatsGeboorteCode())) {
            groep.setWoonplaatsGeboorte(vindWoonplaatsOpCode(groep.getWoonplaatsGeboorteCode(), groep,
                    resultaat));
        }
    }
     ...
     * @param resultaat lijst met resultaten.
     */
    private void vulAanGeslachtsnaamComponent(
            final PersoonGeslachtsnaamcomponentBericht geslachtsnaamcomponent,
            final BerichtVerwerkingsResultaat resultaat)
    {
        ...
        if (AttribuutTypeUtil.isNotBlank(groep.getPredikaatCode())) {
            groep.setPredikaat(vindPredikaatOpCode(
                    groep.getPredikaatCode(), groep,
                    resultaat));
        }
        // Adelijke Titel geboorte
        if (AttribuutTypeUtil.isNotBlank(groep.getAdellijkeTitelCode())) {
            groep.setAdellijkeTitel(vindAdellijkeTitelOpCode(
                    groep.getAdellijkeTitelCode(), geslachtsnaamcomponent,
                    resultaat));
        }
    }
            ...
            final BerichtVerwerkingsResultaat resultaat)
    {
        if (AttribuutTypeUtil.isNotBlank(groep.getGemeenteOverlijdenCode())) {
            groep.setOverlijdenGemeente(vindGemeenteOpCode(groep.getGemeenteOverlijdenCode(),
                groep, resultaat));
        }
        if (AttribuutTypeUtil.isNotBlank(groep.getWoonplaatsOverlijdenCode())) {
            groep.setWoonplaatsOverlijden(vindWoonplaatsOpCode(groep.getWoonplaatsOverlijdenCode(),
                groep, resultaat));
        }
        if (AttribuutTypeUtil.isNotBlank(groep.getLandOverlijdenCode())) {
            groep.setLandOverlijden(vindLandOpCode(groep.getLandOverlijdenCode(),
                groep, resultaat));
        }
    }
            ...
            final BerichtVerwerkingsResultaat resultaat)
    {
        if (AttribuutTypeUtil.isNotBlank(nationaliteit.getNationaliteitcode())) {
            nationaliteit.setNationaliteit(vindNationaliteitOpCode(
            ...
            final BerichtVerwerkingsResultaat resultaat)
    {
        if (AttribuutTypeUtil.isNotBlank(groep.getRedenWijzigingAdresCode())) {
            groep.setRedenwijziging(vindRedenWijzingAdresOpCode(
                    groep.getRedenWijzigingAdresCode(), groep, resultaat));
        }
        if (AttribuutTypeUtil.isNotBlank(groep.getCode())) {
            groep.setWoonplaats(vindWoonplaatsOpCode(groep.getCode(), groep,
                    resultaat));
        }
        if (AttribuutTypeUtil.isNotBlank(groep.getGemeentecode())) {
            groep.setGemeente(vindGemeenteOpCode(groep.getGemeentecode(), groep,
                    resultaat));
        }
