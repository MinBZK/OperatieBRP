 ...
 */
public class Melding extends AbstractIdentificerendeGroep implements Comparable<Melding> {
    ...
    public void overrule() {
        if (soort == Soortmelding.OVERRULEBAAR) {
    ...
    @Override
    public int compareTo(final Melding o) {
        if (this == o) {
            return 0;
        }

        int volgorder = o.getSoort().compareTo(getSoort());

        if (volgorder == 0) {
            volgorder = getCode().name().compareTo(o.getCode().name());
        }
        if (volgorder == 0) {
            if (StringUtils.isBlank(getVerzendendId()) && StringUtils.isBlank(o.getVerzendendId())) {
                volgorder = 0;
            } else {
                if (getVerzendendId() != null) {
                    volgorder = getVerzendendId().compareTo(o.getVerzendendId());
                } else {
                    volgorder = o.getVerzendendId().compareTo(getVerzendendId());
                }
            }
        }
        if (volgorder == 0) {
            if (StringUtils.isBlank(getAttribuutNaam()) && StringUtils.isBlank(o.getAttribuutNaam())) {
                volgorder = 0;
            } else {
                if (getAttribuutNaam() != null) {
                    volgorder = getAttribuutNaam().compareTo(o.getAttribuutNaam());
                } else {
                    volgorder = o.getAttribuutNaam().compareTo(getAttribuutNaam());
                }
            }
        }
