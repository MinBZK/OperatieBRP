...
public abstract class AbstractAntwoordBericht<T extends BerichtResultaat> {
    ...
    public AbstractAntwoordBericht(final T berichtResultaat) {
        berichtStuurgegevens =
            new BerichtStuurgegevens("Ministerie BZK", "BRP", bepaalReferentieNr(),
                    bepaalCrossReferentieNummer(berichtResultaat), bepaalPrevalidatie(berichtResultaat));
    ...
    /**
     * Bepaalt of het bericht om een prevalidatie gaat.
     *
     * @param berichtResultaat Hetbericht resultaat uit de business laag.
     * @return boolean
     */
    private Boolean bepaalPrevalidatie(final T berichtResultaat) {
        if (berichtResultaat != null) {
            return berichtResultaat.getPrevalidatie();
        }
