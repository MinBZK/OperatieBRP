...
public class BRBY0106 implements BedrijfsRegel<PersistentRelatie, Relatie> {
    ...
    {
        String inputNaamKind = StringUtils.isBlank(persGeslachtsnaamcomponentKind.getNaam()) ?
                null : persGeslachtsnaamcomponentKind.getNaam();
        String inputScheidingsTekenKind = StringUtils.isBlank(persGeslachtsnaamcomponentKind.getScheidingsTeken()) ?
                null : persGeslachtsnaamcomponentKind.getScheidingsTeken();
        String inputVoorvoegelKind = StringUtils.isBlank(persGeslachtsnaamcomponentKind.getVoorvoegsel()) ?
        ...
        for (PersistentPersoonGeslachtsnaamcomponent comp : persGeslachtsnaamcomponenten) {
            isGelijk =
                (StringUtils.equals(inputNaamKind, comp.getNaam())
                 && StringUtils.equals(inputScheidingsTekenKind, comp.getScheidingsteken())
                 && StringUtils.equals(inputVoorvoegelKind, comp.getVoorvoegsel()));
        }
        return isGelijk;
    }
}
