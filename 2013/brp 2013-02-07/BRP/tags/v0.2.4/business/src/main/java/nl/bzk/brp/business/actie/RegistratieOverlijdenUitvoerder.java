    ...
    List<Melding> verwerkActie(final Actie actie, final BerichtContext berichtContext) {
        ...
        if (persoonModel == null) {
            ...
            try {
                ...
                // In de toekomst zou dit mischien generieker moeten
                if (null != persoon.getBetrokkenheden() && persoon.getBetrokkenheden().size() != 0) {
                    for (Betrokkenheid betrokkenheid : persoon.getBetrokkenheden()) {
                        if (betrokkenheid.getRelatie() != null) {
                            RelatieBericht relatieBericht = (RelatieBericht) betrokkenheid.getRelatie();
                            if (relatieBericht.getId() != null) { // we behandelen op het moment alleen relaties met bekenden id
                                RelatieModel relatieModel = relatieRepository.vindBijId(relatieBericht.getId());
                                RelatieStandaardGroepModel relatieStandaardGroepModel = new RelatieStandaardGroepModel(relatieBericht.getGegevens());
                                relatieRepository.opslaanVeranderdeRelatieStandaardGegevens(relatieModel, relatieStandaardGroepModel, persistentActie);
                            } else {
