...
public class BRBY0903 implements ActieBedrijfsRegel<Persoon> {
    ...
    public List<Melding> executeer(final Persoon huidigeSituatie, final Persoon nieuweSituatie, final Actie actie) {
        ...
        if (nieuweSituatie.getOverlijden() == null || nieuweSituatie.getOverlijden().getOverlijdenGemeente() == null) {
            ...
            Partij gemeente = nieuweSituatie.getOverlijden().getOverlijdenGemeente();
            if (!DatumUtil.isGeldigOp(gemeente.getDatumAanvang(), gemeente.getDatumEinde(),
                    nieuweSituatie.getOverlijden().getDatumOverlijden()))
            {
                meldingen =
                    Collections.singletonList(new Melding(Soortmelding.FOUT, MeldingCode.BRBY0903, MeldingCode.BRBY0903
                            .getOmschrijving(), (Identificeerbaar) nieuweSituatie.getOverlijden(), "gemeenteCode"));
            } else {
