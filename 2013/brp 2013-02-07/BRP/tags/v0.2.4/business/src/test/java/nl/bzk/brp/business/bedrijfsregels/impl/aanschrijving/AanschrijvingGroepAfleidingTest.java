...
public class AanschrijvingGroepAfleidingTest {
    ...
    public void testIndicatieE() {
        ...
        Actie actie =
            ActieBerichtBuilder.bouwNieuweActie(SoortActie.WIJZIGING_NAAMGEBRUIK).setDatumAanvang(AANVANG_DATUM)
                    .getActie();
    ...
    public void testIndicatiePPartnerNietGevonden() {
        Mockito.when(relatieRepository.haalopPartners(Matchers.anyInt(), Matchers.any(Datum.class))).thenReturn(new ArrayList<Integer>());
        PersoonBericht persoon = maakStandaardPersoon();
    ...
    public void testIndicatieV() {
        ...
        Assert.assertEquals(ACHTERNAAM_PARTNER + "-" + VOORVOEG_EIGEN + SCHEIDING_EIGEN + ACHTERNAAM_EIGEN, persoon
                .getAanschrijving().getGeslachtsnaam().getWaarde());
        Assert.assertEquals(VOORNAAM_EIGEN, persoon.getAanschrijving().getVoornamen().getWaarde());
        Assert.assertEquals(SCHEIDING_PARTNER, persoon.getAanschrijving().getScheidingsteken().getWaarde());
        Assert.assertEquals(VOORVOEG_PARTNER, persoon.getAanschrijving().getVoorvoegsel().getWaarde());
        Assert.assertEquals(hoogheid, persoon.getAanschrijving().getPredikaat());
    ...
    public void testOngeldigRootObject() {
        aanschrijvingGroepAfleiding.executeer(new RelatieBericht(), new RelatieBericht(), new ActieBericht());
    }
