...
public abstract class AbstractBijhoudingAntwoordBericht extends AbstractAntwoordBericht {
    ...
    public Set<Persoon> getBijgehoudenPersonen() {
        Set<Persoon> resultaat;
        if (bijgehoudenPersonen == null || bijgehoudenPersonen.isEmpty()) {
            resultaat = null;
        } else {
            resultaat = Collections.unmodifiableSet(bijgehoudenPersonen);
        }
        return resultaat;
    }

    public void setBijgehoudenPersonen(final Set<Persoon> bijgehoudenPersonen) {
