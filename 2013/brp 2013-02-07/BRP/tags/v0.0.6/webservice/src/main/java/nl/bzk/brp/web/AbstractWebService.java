...
public abstract class AbstractWebService<T extends BRPBericht> {
     ...
     * @param berichtResultaat het resultaat dat meegegeven dient te worden in de verwerker.
        ...
        final BerichtResultaat berichtResultaat)
    {
        ...
        try {
            ...
            if (isGeauthenticeerd(authenticatieMiddel)) {
                ...
                berichtVerwerker.verwerkBericht(bericht, context, berichtResultaat);
            } else {
                LOG.info("Authenticatie Fout: Partij niet geauthenticeerd in bericht {}",
                    berichtenIds.getIngaandBerichtId());
                berichtResultaat
                    .voegMeldingenToe(bouwBerichtResultaatMetMelding(MeldingCode.AUTH0001, null).getMeldingen());
            }
        } catch (Throwable t) {
            ...
            berichtResultaat.voegMeldingenToe(bouwBerichtResultaatMetMelding(MeldingCode.ALG0001,
                "Er is een onbekende fout opgetreden in de verwerking. Probeer later nogmaals").getMeldingen());
        }
     ...
     * toe met de opgegeven {@link MeldingCode} en omschrijving. Indien de omschrijving <code>null</code> is, zal
     * de standaard omschrijving behorende bij de MeldingCode worden gebruikt als omschrijving.
     *
        ...
        final String omschrijving)
    {
        ...
        return new BerichtResultaat(Arrays.asList(melding));
    }
