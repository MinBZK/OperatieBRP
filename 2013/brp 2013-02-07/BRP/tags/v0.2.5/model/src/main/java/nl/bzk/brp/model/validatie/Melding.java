...
public class Melding extends AbstractIdentificerendeGroep implements Comparable<Melding> {
    ...
    public int compareTo(final Melding that) {
        ...
        if (volgorder == COMPARE_EQUAL) {
            volgorder = getCode().name().compareTo(that.getCode().name());
        }
        if (volgorder == COMPARE_EQUAL) {
            if (StringUtils.isBlank(getVerzendendId()) && StringUtils.isBlank(that.getVerzendendId())) {
                volgorder = COMPARE_EQUAL;
            } else {
                if (getVerzendendId() != null) {
                    volgorder = getVerzendendId().compareTo(that.getVerzendendId());
                } else {
                    volgorder = that.getVerzendendId().compareTo(getVerzendendId());
                }
            }
        }
        if (volgorder == COMPARE_EQUAL) {
