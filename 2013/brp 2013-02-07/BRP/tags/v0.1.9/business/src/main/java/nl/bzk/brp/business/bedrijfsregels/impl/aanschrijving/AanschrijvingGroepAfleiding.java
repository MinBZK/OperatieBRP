 ...
 * <p/>
 * De voorvoegsel en scheidingsteken worden van de eerste voorkeur overgenomen.
 * De geslachtsnaam wordt overgenomen van de voorkeur persoon, en in geval van combinatie, wordt dit als volgt
 * opgebouwd:
 * geslachtsnaam van de voorkeur +<spatie> + "-" + <spatie> + (voorvoegsel + scheidingsteken + geslachtsnaam)
 * vd. andere.
 * <p/>
...
public class AanschrijvingGroepAfleiding implements BedrijfsRegel<RootObject> {
        ...
        final Datum datumAanvangGeldigheid, final Datum datumEindeGeldigheid)
    {
        ...
        if (nieuweSituatie instanceof Persoon) {
            meldingen = executeer((PersoonModel) huidigeSituatie, (PersoonBericht) nieuweSituatie,
                datumAanvangGeldigheid);
        } else {
            throw new IllegalArgumentException("Alleen rootobject van het type Persoon is hier toegestaan. Doorgegeven "
    ...
    private PersoonModel haalHuidigPartner(final PersoonModel hoofdPersoon, final Datum peilDatum) {
        ...
        if (ids.size() > 1) {
            throw new RuntimeException("Te veel partners gevonden voor persoon " + hoofdPersoon.getId()
                + " nl: " + ids);
        } else if (ids.size() == 1) {
     ...
     */
    private PersoonAanschrijvingGroepBericht leidAlgoritmischAf(
        final PersoonAanschrijvingGroepBericht aanschrijving,
        final PersoonSamengesteldeNaamGroep eigen,
        final PersoonSamengesteldeNaamGroep partner)
    {
     ...
     */
    private List<Melding> executeer(final PersoonModel huidigPersoon,
        final PersoonBericht berichtPersoon,
        final Datum peilDatum)
    {
        ...
        {
            // Nu moeten we de aanschrijving opbouwen uit de naamgebruik in combinatie met de partner.
            if (huidigPersoon.getGeslachtsnaamcomponenten().size() < 1) {
                foutMeldingen.add(new Melding(SoortMelding.FOUT_ONOVERRULEBAAR, MeldingCode.ALG0002,
                    "geslachtsnaamcomponenten is niet aanwezig.",
                    berichtPersoon.getAanschrijving(), "geslachtsnaam"));
            } else {
                PersoonSamengesteldeNaamGroep eigen = huidigPersoon.getSamengesteldeNaam();
                if (berichtPersoon.getAanschrijving().getGebruikGeslachtsnaam()
                    == WijzeGebruikGeslachtsnaam.EIGEN)
                {
                    ...
                    if (partnerPersoon == null) {
                        foutMeldingen.add(new Melding(SoortMelding.FOUT_ONOVERRULEBAAR, MeldingCode.ALG0002,
                            "Partner niet gevonden.",
                            berichtPersoon.getAanschrijving(), "geslachtsnaam"));
                    } else if (partnerPersoon.getGeslachtsnaamcomponenten().size() < 1) {
                        foutMeldingen.add(new Melding(SoortMelding.FOUT_ONOVERRULEBAAR, MeldingCode.ALG0002,
                            "Partner heeft geen geslachtsnaamcomponenten.",
                            berichtPersoon.getAanschrijving(), "geslachtsnaam"));
                    } else {
