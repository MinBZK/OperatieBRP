 ...
 * <p/>
 */
public class PersoonIndicatieMapper implements IMarshaller, IUnmarshaller {
    ...
    public void marshal(final Object obj, final IMarshallingContext marshallingContext) throws JiBXException {
        ...
        if (!(obj instanceof PersoonIndicatie)) {
            ...
            context.startTagAttributes(NAMESPACE_INDEX, bepaalIndicatieElementNaam(indicatie.getSoort()));
            context.closeStartContent();

            // Geef de content van het indicatie element
            context.startTagAttributes(NAMESPACE_INDEX, STANDAARD_WAARDE_ELEMENT_NAAM);
            context.closeStartContent();
            context.content(bepaalWaardeContent(indicatie.getWaarde()));
            context.endTag(NAMESPACE_INDEX, STANDAARD_WAARDE_ELEMENT_NAAM);

            // sluit het element
            context.endTag(NAMESPACE_INDEX, bepaalIndicatieElementNaam(indicatie.getSoort()));
        }
    }
