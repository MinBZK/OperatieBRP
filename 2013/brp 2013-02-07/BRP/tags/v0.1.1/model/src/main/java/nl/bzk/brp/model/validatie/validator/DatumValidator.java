...
public class DatumValidator implements ConstraintValidator<Datum, Integer> {
    ...
    public boolean isValid(final Integer waarde, final ConstraintValidatorContext context) {
        ...
        if (waarde == null || waarde == 0) {
            // jaar 0 is ook toegestaan.
            resultaat = true;
        } else {
            resultaat = isGeldigeDatum(waarde);
        }
        if (!resultaat) {
            LOGGER.debug("Datum is ongeldig voor [" + waarde + "]");
        }
        return resultaat;
    }
    ...
    private boolean isGeldigeDatum(final Integer datum) {
        ...
        String waarde = StringUtils.leftPad(datum.toString(), DATUM_LENGTE, '0');
        ...
        if ((datum % MOD_LAATSTE_2_CIJFERS_NUL) != 0) {
            ...
            int jaar = (datum / (MOD_LAATSTE_2_CIJFERS_NUL * MOD_LAATSTE_2_CIJFERS_NUL));
            int leftover = (datum - (jaar * MOD_LAATSTE_2_CIJFERS_NUL * MOD_LAATSTE_2_CIJFERS_NUL));
