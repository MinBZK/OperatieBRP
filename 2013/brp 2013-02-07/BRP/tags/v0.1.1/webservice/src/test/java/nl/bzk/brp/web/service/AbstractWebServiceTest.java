...
public abstract class AbstractWebServiceTest<T extends BRPBericht, Y extends BerichtResultaat> {
    ...
    public void initMocks(final Class berichtClass) {
        ...
        setBericht((BRPBericht) Mockito.mock(berichtClass));
        ...
        final Class<U> clazz, final boolean resultaat)
    {
        ...
        Mockito.when(
            getBerichtVerwerker().verwerkBericht(Matchers.any(BRPBericht.class), Matchers.any(BerichtContext.class)))
               .thenReturn(mock);
    }
    ...
    protected void initAuthenticatieService(final int aantalAuthenticatieMiddelen) {
        ...
        Mockito.when(
            getAuthenticatieService()
                .haalAuthenticatieMiddelenOp(Matchers.anyInt(), (X509Certificate) Matchers.notNull()))
    ...
    protected BRPBericht getBericht() {
        return bericht;
    }

    protected void setBericht(final BRPBericht bericht) {
