...
public abstract class AbstractGroepHistorieRepository<T extends AbstractDynamischObjectType,
    U extends AbstractGroep,
    Y extends MaterieleHistorie> implements GroepHistorieRepository<T, U, Y>
{
    private static final String EXCLUDE_FORMELE_HISTORIE = " AND hpa.historie.datumTijdVerval IS NULL ";
    ...
    public Y haalGeldigRecord(final T objectType, final Datum peildDatum) {
        ...
        final String sqlString = "SELECT hpa FROM "
                + cLaagEniteitNaam
                + " hpa "
                + "WHERE hpa.historie.datumAanvangGeldigheid = "
                + " (SELECT max(hpa2.historie.datumAanvangGeldigheid) FROM "
                +       cLaagEniteitNaam
                +       " hpa2 WHERE "
                +       " hpa2." + aLaangEntiteitNaam + "=" + ":aLaagEntiteit "
                +       " AND ( hpa2.historie.datumEindeGeldigheid IS NULL OR "
                +             " hpa2.historie.datumAanvangGeldigheid <> hpa2.historie.datumEindeGeldigheid)"
                +       " AND hpa2.historie.datumAanvangGeldigheid <=  :aanvangGeldigheid "
                +       " AND hpa2.historie.datumTijdVerval IS NULL )"
    ...
    {
        List<Y> copieVanOverlapteRecords = new ArrayList<Y>(overlapteRecords);
        ...
        for (Y y : copieVanOverlapteRecords) {
            ...
            if (y.getDatumTijdRegistratie().equals(actie.getTijdstipRegistratie())) {
