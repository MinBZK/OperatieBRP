...
public class BRAL2102 implements ActieBedrijfsRegel<Relatie> {
        ...
        final Actie actie)
    {
        ...
        if (huwelijk.getGegevens() != null && huwelijk.getGegevens().getDatumAanvang() != null) {
            if (huwelijk.isPartnershapVoltrokkenInNederland()) {
                if (!DatumUtil.isVolledigDatum(huwelijk.getGegevens().getDatumAanvang())) {
                    meldingen.add(new Melding(SoortMelding.FOUT_ONOVERRULEBAAR, MeldingCode.BRAL2102,
