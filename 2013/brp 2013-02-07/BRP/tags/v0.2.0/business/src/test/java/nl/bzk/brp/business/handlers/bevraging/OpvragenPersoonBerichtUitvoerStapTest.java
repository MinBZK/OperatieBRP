...
public class OpvragenPersoonBerichtUitvoerStapTest {
    ...
    public void testVraagOpDetailPersoonPersoonGevonden() {
        ...
        Assert.assertEquals(Long.valueOf(1235L),
            gevondenPersoon.getIdentificatienummers().getAdministratienummer().getWaarde());
    ...
    public void testVraagOpDetailPersoonMetHistorie() {
        Mockito.when(persoonRepository
            .haalPersoonOpMetBurgerservicenummer(Matchers.any(Burgerservicenummer.class)))
               .thenReturn(maakGevondenPersoon());

        OpvragenPersoonResultaat resultaat = new OpvragenPersoonResultaat(new ArrayList<Melding>());
        VraagDetailsPersoonBericht vraag = maakVraagDetailsPersoonBericht();
        opvragenPersoonBerichtUitvoerStap.voerVerwerkingsStapUitVoorBericht(vraag, null,
            resultaat);
        // methode wordt nooit aangeroepen.
        Mockito.verify(persoonRepository, Mockito.times(0))
               .vulaanAdresMetHistorie(Matchers.any(PersoonModel.class), Matchers.anyBoolean());
        ...
        Mockito.verify(persoonRepository, Mockito.times(1))
               .vulaanAdresMetHistorie(Matchers.any(PersoonModel.class), Matchers.eq(false));

        vraag.getVraag().getVraagOpties().setHistorieFormeel(Ja.Ja);
        opvragenPersoonBerichtUitvoerStap.voerVerwerkingsStapUitVoorBericht(vraag, null,
            resultaat);
        Mockito.verify(persoonRepository, Mockito.times(1))
               .vulaanAdresMetHistorie(Matchers.any(PersoonModel.class), Matchers.eq(true));
    }
    ...
    public void testHaalPersonenMetAdresOpViaBurgerservicenummerIdMetIdCodeAanduidingStap2() {
        ...
        Mockito.when(persoonRepository.haalPersonenMetWoonAdresOpViaBurgerservicenummer(Matchers.any(
            Burgerservicenummer.class))).thenReturn(personen);

        Mockito.when(persoonRepository.haalPersonenMetWoonAdresOpViaIdentificatiecodeNummeraanduiding(
            new IdentificatiecodeNummeraanduiding("abcd"))).thenReturn(personen);

        Mockito.when(persoonRepository
            .haalPersonenMetWoonAdresOpViaVolledigAdres(new NaamOpenbareRuimte("naamOpenbareRuimte"),
                new Huisnummer(12), new Huisletter("A"), null, plaats, gemeente)).thenReturn(
            Arrays.asList(new PersoonModel(new PersoonBericht()), new PersoonModel(new PersoonBericht())));
    ...
    public void testHaalPersonenMetAdresOpViaBurgerservicenummerIdMetIdCodeAanduidingStap2b() {
        ...
        Mockito.verify(persoonRepository, Mockito.times(1)).haalPersonenMetWoonAdresOpViaVolledigAdres(
            new NaamOpenbareRuimte("naamOpenbareRuimte"), new Huisnummer(12), new Huisletter("A"), null,
            plaats, gemeente);
    }
