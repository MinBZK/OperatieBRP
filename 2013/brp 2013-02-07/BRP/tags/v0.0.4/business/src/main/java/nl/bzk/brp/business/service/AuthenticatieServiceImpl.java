...
public class AuthenticatieServiceImpl implements AuthenticatieService {
     ...
     */
    @Override
    public boolean isPartijGeauthenticeerdInBRP(final Integer partijId, final X509Certificate certificaat) {
        final Partij partij = partijRepository.findOne(partijId);

        final List<PersistentAuthenticatieMiddel> authenticatieMiddelen =
            authenticatieMiddelRepository.zoekAuthMiddelenVoorPartijMetCertificaat(partij, certificaat.getSubjectDN()
                    .getName(), certificaat.getSerialNumber(), Hex.encodeHexString(certificaat.getSignature()));

        return !authenticatieMiddelen.isEmpty();
    }
}
