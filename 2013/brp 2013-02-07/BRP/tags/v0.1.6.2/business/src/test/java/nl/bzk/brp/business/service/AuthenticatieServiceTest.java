...
import nl.bzk.brp.dataaccess.repository.AuthenticatieMiddelMdlRepository;
import nl.bzk.brp.dataaccess.repository.PartijMdlRepository;
...
public class AuthenticatieServiceTest {
    ...
    private AuthenticatieMiddelMdlRepository authenticatieMiddelRepository;

    private Partij                           testPartij1;
    private Partij                           testPartij2;

    private final Integer                       partijId1 = 1200;
    private final Integer                       partijId2 = 1300;
    ...
    public void init() throws IOException, CertificateException, URISyntaxException {
        // Mock repo's
        PartijMdlRepository partijRepository = Mockito.mock(PartijMdlRepository.class);
        authenticatieMiddelRepository = Mockito.mock(AuthenticatieMiddelMdlRepository.class);

        testPartij1 = new Partij();
        ReflectionTestUtils.setField(testPartij1, "id", 1);
        testPartij2 = new Partij();
        ReflectionTestUtils.setField(testPartij2, "id", 2);
    ...
    public void testAuthenticatieMiddelAanwezig() {
        ...
        Mockito.when(
                authenticatieMiddelRepository.zoekAuthMiddelenVoorPartijMetCertificaat(testPartij1, new Subject(
                        testCertificaat.getSubjectDN().getName()), new Serial(testCertificaat.getSerialNumber()),
