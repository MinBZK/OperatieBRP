...
public class PersoonIndicatieMdlMarshallerTest {
    ...
    public void testMarshallingIndicatieMetNegatieveWaarde() throws JiBXException {
        PersoonIndicatieBericht persoonIndicatie = bouwIndicatie(SoortIndicatie.VERSTREKKINGSBEPERKING);
        persoonIndicatie.getGegevens().setWaarde(JaNee.Nee);

        mapper.marshal(new PersoonIndicatieModel(persoonIndicatie), marshallingContext);
