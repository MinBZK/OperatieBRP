...
// TODO: de nieuwe history repository moeten nog aangemaakt worden.
            ...
            final Datum datumAanvangGeldigheid, final DatumTijd tijdstipRegistratie)
    {
        ...
        if (!isBSNAlIngebruik(persoon.getIdentificatieNummers().getBurgerServiceNummer())) {
            // Kopieer nu alle gegevens naar de daadwerkelijke PersooonModel.
            PersoonModel nieuwPersoon = new PersoonModel(persoon);
            // kopieer alle niet overgezette 1-n objectTypes.
            for (PersoonAdresModel adres : persoon.getAdressen()) {
                nieuwPersoon.getAdressen().add(new PersoonAdresModel(adres, nieuwPersoon));
            }
            for (PersoonGeslachtsnaamComponentModel gnaam : persoon.getGeslachtsnaamcomponenten()) {
                nieuwPersoon.getGeslachtsnaamcomponenten().add(new PersoonGeslachtsnaamComponentModel(
                        gnaam, nieuwPersoon));
            }
            for (PersoonNationaliteitModel nationaliteit : persoon.getNationaliteiten()) {
                nieuwPersoon.getNationaliteiten().add(new PersoonNationaliteitModel(nationaliteit, nieuwPersoon));
            }
            for (PersoonVoornaamModel voornaam : persoon.getPersoonVoornaam()) {
                nieuwPersoon.getPersoonVoornaam().add(new PersoonVoornaamModel(voornaam, nieuwPersoon));
            }
            ...
            em.flush();
            werkHistorieBij(nieuwPersoon, actie, datumAanvangGeldigheid);

            em.merge(nieuwPersoon);
            return nieuwPersoon;
        } else {
            // Persoon bestaat al
