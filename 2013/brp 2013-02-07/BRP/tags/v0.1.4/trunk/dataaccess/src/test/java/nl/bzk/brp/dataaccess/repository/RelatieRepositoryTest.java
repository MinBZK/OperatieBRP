...
//
    ...
    @Test
    public void testVindSoortRelatiesMetPersonen() {
        PersistentPersoon p1 = persoonRepository.findByBurgerservicenummer("135867277");
        PersistentPersoon p2 = persoonRepository.findByBurgerservicenummer("234567891");
        ...
        Assert.assertFalse(relaties.isEmpty());

        for (PersistentRelatie relatie : relaties) {
            Assert.assertTrue(SoortRelatie.GEREGISTREERD_PARTNERSCHAP == relatie.getSoortRelatie()
                    || SoortRelatie.HUWELIJK == relatie.getSoortRelatie());
            boolean p1aanwezig = false;
            boolean p2aanwezig = false;
            for (PersistentBetrokkenheid betrokkenheid : relatie.getBetrokkenheden()) {
                Assert.assertEquals(SoortBetrokkenheid.PARTNER, betrokkenheid.getSoortBetrokkenheid());
                if (betrokkenheid.getBetrokkene().getId().equals(p1.getId())) {
                    p1aanwezig = true;
                }
                if (betrokkenheid.getBetrokkene().getId().equals(p2.getId())) {
                    p2aanwezig = true;
                }
            }
            Assert.assertTrue(p1aanwezig);
            Assert.assertTrue(p2aanwezig);
        }
    }
}
