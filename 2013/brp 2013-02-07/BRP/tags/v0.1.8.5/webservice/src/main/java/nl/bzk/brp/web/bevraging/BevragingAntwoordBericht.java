    ...
    private void bewerkViewOpPersoonVoorBinding(final PersoonModel persoon) {
        if (persoon.getBetrokkenheden() != null) {
            ...
            for (BetrokkenheidModel directeBetrokkenheid : persoon.getPartnerBetrokkenHeden()) {
                ...
                //Let op: Een relatie zonder betrokkenheden is niet meer relevant in het antwoord bericht,
                //dus hieren halen we de betrokkenheid van de gevonden persoon.
                if (relatie.getBetrokkenheden().isEmpty()) {
                    persoon.getBetrokkenheden().remove(directeBetrokkenheid);
                }
            }
        }
    }
