...
public class AanschrijvingGroepAfleiding implements ActieBedrijfsRegel<RootObject> {
    ...
    @Override
    public List<Melding>
            executeer(final RootObject huidigeSituatie, final RootObject nieuweSituatie, final Actie actie)
    {
    ...
    private PersoonModel haalHuidigPartner(final PersoonModel hoofdPersoon, final Datum peilDatum) {
        ...
        if (ids.size() > 1) {
            throw new RuntimeException("Te veel partners gevonden voor persoon " + hoofdPersoon.getId() + " nl: " + ids);
        } else if (ids.size() == 1) {
