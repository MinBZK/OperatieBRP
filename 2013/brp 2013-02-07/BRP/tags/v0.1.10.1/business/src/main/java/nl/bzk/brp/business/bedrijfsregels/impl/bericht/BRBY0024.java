 ...
 */
public class BRBY0024 implements BerichtBedrijfsRegel<BijhoudingsBericht> {
     ...
     */
    @Override
    public List<Melding> executeer(final BijhoudingsBericht bericht) {
        List<Melding> meldingen;
        if (bericht != null && bevatLijstOverlappendeActies(bericht.getBrpActies())) {
            meldingen =
                Arrays.asList(new Melding(SoortMelding.FOUT_ONOVERRULEBAAR, MeldingCode.BRBY0024, bericht, null));
        } else {
     ...
     */
    private boolean bevatLijstOverlappendeActies(final List<Actie> acties) {
