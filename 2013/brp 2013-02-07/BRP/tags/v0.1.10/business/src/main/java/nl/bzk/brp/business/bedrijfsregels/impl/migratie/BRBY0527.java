    ...
    public List<Melding> executeer(final Persoon huidigeSituatie, final Persoon nieuweSituatie, final Actie actie) {
        ...
        if (CollectionUtils.isEmpty(nieuweSituatie.getAdressen())) {
            ...
            for (PersoonAdres adres : nieuweSituatie.getAdressen()) {
                ...
                {
                    String meldingTekst =
                        String.format("%s (Gemeentecode = %s).", MeldingCode.BRBY0527.getOmschrijving(),
                                adres.getGegevens().getGemeente().getGemeentecode());
                    meldingen =
                        Collections.singletonList(new Melding(SoortMelding.FOUT_OVERRULEBAAR, MeldingCode.BRBY0527,
                                meldingTekst, (Identificeerbaar) adres, "gemeenteCode"));
                } else {
