...
public class BRBY0429 implements ActieBedrijfsRegel<Relatie> {
    ...
    public List<Melding> executeer(final Relatie huidigeSituatie, final Relatie nieuweSituatie, final Actie actie) {
        ...
        if (nieuweSituatie == null || nieuweSituatie.getGegevens() == null) {
            ...
            if (gemeenteAanvang != null && nieuweSituatie.isPartnershapVoltrokkenInNederland()) {
            ...
            if (ongeldigeGemeente) {
                String tekst = String.format(MeldingCode.BRBY0429.getOmschrijving(), gemeenteAanvang.getGemeentecode());
                meldingen =
                    Collections.singletonList(new Melding(SoortMelding.FOUT_ONOVERRULEBAAR, MeldingCode.BRBY0429,
                            tekst, (Identificeerbaar) nieuweSituatie.getGegevens(), "gemeenteAanvang"));
            } else {
                meldingen = Collections.emptyList();
            }
        }
