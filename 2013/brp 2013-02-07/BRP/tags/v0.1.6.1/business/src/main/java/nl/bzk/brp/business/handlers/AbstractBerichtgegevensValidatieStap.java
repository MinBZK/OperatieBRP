        ...
        extends AbstractBerichtVerwerkingsStap<X, Y>
{
    ...
    protected void valideer(final T t, final BerichtResultaat resultaat) {
        ...
        for (ConstraintViolation<T> v : overtredingen) {
            ...
            // rootBean is de BRPActie, leafBean is de groep, invalidValue is de waarde.
            // wat we nog missen is het veld/attribuutnaam
//            System.out.println("--- Code :" + v.getConstraintDescriptor().getAttributes().get(CONSTRAINT_CODE));
//            System.out.println("--- Soort :" + v.getConstraintDescriptor().getAttributes().get(CONSTRAINT_SOORT));
//            System.out.println("--- Rootbean :" + v.getRootBean());
//            System.out.println("--- leafBean :" + v.getLeafBean());
//            System.out.println("--- invalidValue :" + v.getInvalidValue());
//            System.out.println("--- getPropertyPath :" + v.getPropertyPath());
//            System.out.println("--- attribuutnaam :" + getAttribuutNaam(v));
//            System.out.println("--- verzendendId :" + getVerzendendId(v));
//            System.out.println("--- omschrijving :" + meldingCode.getOmschrijving());


            // TODO er wordt hier nog niet gebruik gemaakt van i18n berichten, dit zou gerefactored kunnen
            // TODO worden
            Melding melding = new Melding(soortMelding, meldingCode);
    ...
    private String getAttribuutNaam(final ConstraintViolation<T> violation) {
        ...
        while (it.hasNext()) {
            Node node = it.next();
//            System.out.println("   ----" + node.getIndex() + "," + node.isInIterable() + "," + node.getName());
            lastNode = node;
        }
        if (lastNode != null) {
            if (!lastNode.isInIterable()) {
                ...
                if (lastNode.getIndex() == null) {
                    ...
                    attribuutNaam = lastNode.getName() + "[" + lastNode.getIndex() + "]";
                }
            }
        }
    ...
    private String getVerzendendId(final ConstraintViolation<T> violation) {
        ...
        if (o instanceof IdentificerendeGroep) {
            verzendendId = ((IdentificerendeGroep) o).getVerzendendId();
        }
        return verzendendId;
    }
