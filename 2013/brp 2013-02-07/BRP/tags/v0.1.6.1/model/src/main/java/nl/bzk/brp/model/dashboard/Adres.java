...
public class Adres {
    ...
    public Adres(final PersoonAdres adres) {
        gemeente = new Gemeente(adres.getGemeente());
        if (adres.getAfgekorteNaamOpenbareRuimte()  != null) {
            straat = adres.getAfgekorteNaamOpenbareRuimte();
        } else {
            straat = adres.getNaamOpenbareRuimte();
        }
        huisnummer = adres.getHuisnummer();
        huisnummertoevoeging = adres.getHuisnummertoevoeging();
        if (adres.getWoonplaats() != null) {
            plaats = new Plaats(adres.getWoonplaats());
        }
        datumAanvangAdreshouding.setDecimalen(adres.getDatumAanvangAdreshouding());
    }
