...
public class PostcodeValidator implements ConstraintValidator<Postcode, String> {
    ...
    private boolean isGeldigePostcode(final String postcode) {
        ...
        if (postcode.length() == POSTCODE_LENGTE) {
            String nummer = postcode.substring(POSTCODE_NUMMER_VAN, POSTCODE_NUMMER_TOT);
            if (StringUtils.isNumeric(nummer) && Integer.parseInt(nummer) > LAAGSTE_POSTCODE_NUMMER) {
                String code = postcode.substring(POSTCODE_LETTER_VAN, POSTCODE_LETTER_TOT);
