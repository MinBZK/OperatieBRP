...
public abstract class AbstractVraagBerichtBindingUitTest<T> extends AbstractBerichtBindingUitTest<T> {
        ...
        final String bsn) throws ParseException
    {
        ...
        pers.getAfgeleidAdministratief()
            .setLaatstGewijzigd(new SimpleDateFormat("yyyy/MM/dd HH:mm:ss:SSS").parse(("2012/06/02 11:04:55:555")));
        // Voornamen
     ...
     */
    protected void voegIndicatieToeAanPersoon(final Persoon persoon, final SoortIndicatie soort, final boolean waarde) {
    ...
    protected OpvragenPersoonResultaat bouwOpvragenPersoonResultaatVoorCompleetPersoonMetRelaties(final Partij gemeente,
        final Land land, final Plaats plaats, final SoortRelatie relatieSoort) throws ParseException
    {
...
//        kindVanOpTeVragenPersRelatie.setDatumAanvang(Integer.valueOf(20110305));
        ...
        final String regelCode, final String meldingOmschrijving, final String verwachtAntwoordBerichtBestandsNaam)
    {
        StringBuilder verwachteWaarde = new StringBuilder()
            ...
            .append(String.format(
                "<brp:%s xmlns:brp=\"http://www.bprbzk.nl/BRP/0001\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">",
    ...
    private void bouwStuurgegevensElement(final StringBuilder stringBuilder) {
        stringBuilder.append("<brp:stuurgegevens>").append(bouwElement("organisatie", "Ministerie BZK"))
                     ...
                     .append(bouwElement("crossReferentienummer", "onbekend"))
                     .append("</brp:stuurgegevens>");
    }
...
//        stringBuilder.append(bouwElement("peilmomentMaterieel", tijdstipRegistratie));
     ...
     */
    protected Persoon bouwMinimaalPersoon() {
        Land nederland = new Land();
        nederland.setLandcode("0031");
        Partij gemeente = new Partij();
        gemeente.setGemeentecode("0010");

        Persoon minimaalPersoon = new Persoon();
        minimaalPersoon.setIdentiteit(new PersoonIdentiteit());
        minimaalPersoon.getIdentiteit().setSoort(SoortPersoon.INGESCHREVENE);
        minimaalPersoon.setIdentificatienummers(new PersoonIdentificatienummers());
        minimaalPersoon.getIdentificatienummers().setBurgerservicenummer("123456789");
        minimaalPersoon.setPersoonGeslachtsAanduiding(new PersoonGeslachtsAanduiding());
        minimaalPersoon.getPersoonGeslachtsAanduiding().setGeslachtsAanduiding(GeslachtsAanduiding.MAN);
        minimaalPersoon.setGeboorte(new PersoonGeboorte());
        minimaalPersoon.getGeboorte().setLandGeboorte(nederland);
        minimaalPersoon.getGeboorte().setDatumGeboorte(Integer.valueOf(20060325));
        minimaalPersoon.setSamengesteldenaam(new PersoonSamengesteldeNaam());
        minimaalPersoon.getSamengesteldenaam().setGeslachtsnaam("Test");
        minimaalPersoon.setSamengesteldeAanschrijving(new PersoonSamengesteldeAanschrijving());
        minimaalPersoon.getSamengesteldeAanschrijving().setGeslachtsnaam("Test");
        minimaalPersoon.setBijhoudingVerantwoordelijke(new PersoonBijhoudingsVerantwoordelijke());
        minimaalPersoon.getBijhoudingVerantwoordelijke().setVerantwoordelijke(Verantwoordelijke.COLLEGE);
        minimaalPersoon.setAfgeleidAdministratief(new PersoonAfgeleidAdministratief());
        minimaalPersoon.getAfgeleidAdministratief().setLaatstGewijzigd(new Date());
        minimaalPersoon.setInschrijving(new PersoonInschrijving());
        minimaalPersoon.getInschrijving().setDatumInschrijving(Integer.valueOf(20060325));
        minimaalPersoon.getInschrijving().setVersienummer(2L);

        PersoonAdres minimaalAdres = new PersoonAdres();
        minimaalAdres.setPersoon(minimaalPersoon);
        minimaalAdres.setLand(nederland);
        minimaalPersoon.setAdressen(new HashSet<PersoonAdres>());
        minimaalPersoon.getAdressen().add(minimaalAdres);
