...
public class InschrijvingGeboorteUitvoerderTest extends AbstractStapTest {
    ...
    public void testNormaleActie() {
        ...
        Mockito.verify(persoonRepository).werkHistorieBij(Matchers.any(PersistentPersoon.class),
                Matchers.any(Persoon.class), Matchers.any(PersistentActie.class),
        ...
        Mockito.verify(relatieRepository).werkHistorieBij(Matchers.any(PersistentRelatie.class),
                Matchers.any(PersistentActie.class), Matchers.eq(Integer.valueOf(20120325)));
        ...
        Assert.assertEquals("123456789",
                bc.getHoofdPersonen().get(0).getIdentificatienummers().getBurgerservicenummer());
        Assert.assertTrue(Arrays.asList("345678912", "234567891").
                contains(bc.getBijPersonen().get(0).getIdentificatienummers().getBurgerservicenummer()));
        Assert.assertTrue(Arrays.asList("345678912", "234567891").
                contains(bc.getBijPersonen().get(1).getIdentificatienummers().getBurgerservicenummer()));
    ...
    public void init() {
        ...
        PersistentPersoon pPersoon = new PersistentPersoon();
        pPersoon.setId(15L);
        Mockito.when(persoonRepository.opslaanNieuwPersoon(Mockito.any(Persoon.class), Mockito.anyInt(),
                Mockito.any(Date.class))).thenReturn(pPersoon);
    }
    ...
    private Betrokkenheid bouwKindBetrokkenheid(final String bsn) {
        ...
        betrokkenheid.setBetrokkene(bouwPersoon(15L, bsn));
