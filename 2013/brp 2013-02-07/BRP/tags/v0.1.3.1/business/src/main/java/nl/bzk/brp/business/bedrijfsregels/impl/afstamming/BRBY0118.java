        ...
        for (Betrokkenheid betrokkenheid : nieuweSituatie.getOuderBetrokkenheden()) {
            if (BooleanUtils.isTrue(betrokkenheid.isIndAdresGevend())) {
                try {
                    ...
                    // geen adres gevonden. technisch database inconsistent (productie of test) ==> behandel als false
                } catch (EmptyResultDataAccessException ex) {
                    // zelfde error als hierboven, alleen omgezet naar spring framework.
                    // geen adres gevonden. technisch database inconsistent (productie of test) ==> behandel als false
                }
                break;
            }
