 ...
 */
public class GeneriekeWebServiceTest extends AbstractWebServiceTest<BRPBericht, BerichtResultaat> {
    ...
    @Before
    public void setup() {
        initMocks(BRPBericht.class);
        Mockito.when(berichtVerwerker.verwerkBericht(Mockito.any(BRPBericht.class), Mockito.any(BerichtContext.class)))
    ...
    public void testBerichtMetFoutInAuthenticatieService() {
        Mockito.when(getAuthenticatieService().haalAuthenticatieMiddelenOp(Matchers.any(Integer.class),
    ...
    protected AbstractWebService getNieuweWebServiceVoorTest() {
        return new AbstractWebService<BRPBericht, BerichtResultaat>() {
            @Override
