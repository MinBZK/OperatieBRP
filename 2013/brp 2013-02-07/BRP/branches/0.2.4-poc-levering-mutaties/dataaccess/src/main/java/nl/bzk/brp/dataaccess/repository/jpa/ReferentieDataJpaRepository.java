...
class ReferentieDataJpaRepository implements ReferentieDataRepository {
    ...
    @Override
    public List<Partij> vindAllePartijen() {
        final String sql = "SELECT partij FROM Partij partij";

        return (List<Partij>) em.createQuery(sql).getResultList();
    }
    ...
    @Override
    public SoortDocument vindSoortDocumentOpNaam(final Naam naam) {
        try {
