...
import nl.bzk.brp.dataaccess.repository.BronRepository;
...
public abstract class AbstractActieUitvoerder {
    ...
    protected ActieModel persisteerActie(final Actie brpActie, final BerichtContext berichtContext) {
        // Verplaatst naar BerichtVerrijkingsStap, dit is nodig omdat de Business rules de tijdstip nodig hebben.
        ...
        // if (brpActie instanceof ActieBericht && ((ActieBericht) brpActie).getBronnen() != null) {
        // for (BronBericht bron : ((ActieBericht) brpActie).getBronnen()) {
        // BronModel bronMdel = new BronModel(bron, actie);
        // documentRepository.save(bronMdel.getDocument());
        // bronRepository.save(bronMdel);
        // }
        // }

        if (savedActie != null) {
            berichtContext.setActieId(savedActie.getId());
        }

        // Tijdelijk worden betrokken bsns aan context toegevoegd
        berichtContext.setBetrokkenBsns(verkrijgGerelateerdeBsnsVoorActie(brpActie));
    ...
    /*
     * kopie van {@link
     * nl.bzk.brp.business.dto.bijhouding.AbstractBijhoudingsBericht#voegWriteBsnLocksVoorActieToeAanResultaat}
     */
    protected List<String> verkrijgGerelateerdeBsnsVoorActie(final Actie actie) {
