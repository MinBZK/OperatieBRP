...
public abstract class AbstractPersoonIndicatieHisModel extends AbstractPersoonIndicatieStandaardGroep implements
        MaterieleHistorie
{

    @Id
    @SequenceGenerator(name = "HisPersIndicatie", sequenceName = "Kern.seq_His_PersIndicatie")
    @GeneratedValue(strategy = GenerationType.AUTO, generator = "HisPersIndicatie")
    private Long                  id;
     ...
     */
    protected AbstractPersoonIndicatieHisModel() {
        super();
    }
            ...
            final PersoonIndicatieModel persoonIndicatieModel)
    {
        ...
        persoonIndicatie = persoonIndicatieModel;
        if (persoonIndicatieStandaardGroep instanceof AbstractPersoonIndicatieHisModel) {
            historie = new MaterieleHistorieImpl(
                    ((AbstractPersoonIndicatieHisModel) persoonIndicatieStandaardGroep).getHistorie()
            );
        } else {
            historie = new MaterieleHistorieImpl();
        }
