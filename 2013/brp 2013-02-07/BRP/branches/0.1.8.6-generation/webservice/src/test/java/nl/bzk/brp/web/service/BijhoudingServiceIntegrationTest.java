...
public class BijhoudingServiceIntegrationTest extends AbstractIntegrationTest {
    ...
    public void testVerhuizing() throws Exception {
        ...
        String anr = XmlUtils.getNodeWaarde("//persoon/identificatienummers/ANummer", orgAanvraag);
    ...
    public void testInschrijvingGeboorte() throws Exception {
        ...
        String anr = XmlUtils.getNodeWaarde("//kind/persoon/identificatienummers/ANummer", orgAanvraag);
        ...
        Integer persoonId = personen.get(0).getId();
        List<GeslachtsnaamcomponentDto> geslachtscomponenten =
            GeslachtsnaamcomponentDto.getGeslachtsnaamcomponentA(simpleJdbcTemplate, persoonId);
        ...
        List<Map<String, Object>> betrokkenheden =
            simpleJdbcTemplate.queryForList("SELECT * FROM kern.betr WHERE pers = ?", persoonId);
        ...
        for (Map<String, Object> betrokkenheid : betrokkenhedenRelatie) {
            ...
            Assert.assertTrue(betrokkeneId.equals(1001) || betrokkeneId.equals(persoonId));
            if (betrokkeneId.equals(2)) {
        ...
        List<Map<String, Object>> adressen =
            simpleJdbcTemplate.queryForList("SELECT * FROM kern.persadres WHERE pers = ?", persoonId);
...
//        Assert.assertEquals("A", persoonenNaamWijzigingVrouw.get(0).get("aanschrstatushis"));
    ...
    public void testRegistreerHuwelijkEnPartnerschapMetEnkeleNietBestaandBsn() throws Exception {
        ...
        // test de rest
        Assert.assertEquals("BRBY0014", XmlUtils.getNodeWaarde("//meldingen/melding/regelCode", document));
        Assert.assertEquals("F", XmlUtils.getNodeWaarde("//meldingen/melding/soortCode", document));
        Assert.assertEquals("identificatie10", XmlUtils.getAttribuutWaarde("//meldingen/melding", "brp:cIDVerzendend",
            document));
    }
