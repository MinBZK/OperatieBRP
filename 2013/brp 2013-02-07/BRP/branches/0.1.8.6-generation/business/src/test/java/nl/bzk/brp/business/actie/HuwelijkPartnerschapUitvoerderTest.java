...
public class HuwelijkPartnerschapUitvoerderTest extends AbstractStapTest {
    ...
    public void init() {
        ...
        uitvoerder = new HuwelijkPartnerschapUitvoerder();
        ...
        ReflectionTestUtils.setField(uitvoerder, "huwelijkPartnerschapActieValidator",
                huwelijkPartnerschapActieValidator);
        ReflectionTestUtils.setField(uitvoerder, "actieRepository", actieRepository);

        Mockito.when(persoonRepository.findByBurgerservicenummer(new Burgerservicenummer("234567891"))).thenReturn(
                bouwPersoon(1, "234567891"));
        Mockito.when(persoonRepository.findByBurgerservicenummer(new Burgerservicenummer("345678912"))).thenReturn(
    ...
    public void testNormaleActie() {
        ...
        Mockito.verify(relatieRepository, Mockito.times(1)).opslaanNieuweRelatie(argument.capture(),
    ...
    private ActieBericht getHuwelijkActie() {
        ActieBericht actie = new ActieBericht();
        actie.setSoort(SoortActie.REGISTRATIE_HUWELIJK);
