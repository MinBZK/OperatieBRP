...
public class BRBY0118 implements BedrijfsRegel<Relatie> {
        ...
        final Datum datumAanvangGeldigheid, final Datum datumEindeGeldigheid)
    {
        ...
        if (meldingen.size() == 0 && adres == null) {
            Melding melding = checkAdresPersistentIsOk(nieuweSituatie, adres);
            if (melding != null) {
                meldingen.add(melding);
            }
        }
        ...
        if (meldingen.size() == 0) {
            ...
            persoonAdres.setAanduidingAdresseerbaarObject(adres.getGegevens().getAanduidingAdresseerbaarObject());
