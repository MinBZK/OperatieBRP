...
import nl.bzk.brp.dataaccess.repository.jpa.historie.GroepFormeleHistorieRepository;
...
public class PersoonJpaRepository implements PersoonRepository {
    ...
    @Inject
    private GroepHistorieRepository<PersoonModel,
    ...
    @Inject
    private GroepHistorieRepository<PersoonNationaliteitModel,
            PersoonNationaliteitHisModel> persoonNationaliteitStandaardHistorieRepository;

    @Inject
    private GroepHistorieRepository<PersoonVoornaamModel,
    ...
    public List<PersoonModel> haalPersonenMetWoonAdresOpViaVolledigAdres(final NaamOpenbareRuimte naamOpenbareRuimte,
        ...
        final LocatieOmschrijving locatieOmschrijving, final AanduidingBijHuisnummer locatietovAdres,
        final Plaats woonplaats)
    {
        ...
        query.append("WHERE (upper(adres.gegevens.naamOpenbareRuimte) = :naamOpenbareRuimte ) ")
             ...
             .append("AND (adres.gegevens.woonplaats = :woonplaats ) ").append("AND (adres.gegevens.soort = 1) ");
        ...
        final Datum datumAanvangGeldigheid)
    {
        ...
        if (!isBSNAlIngebruik(persoon.getIdentificatienummers().getBurgerservicenummer())) {
            ...
            // Persoon bestaat al
            final Integer bsn = persoon.getIdentificatienummers().getBurgerservicenummer().getWaarde();
