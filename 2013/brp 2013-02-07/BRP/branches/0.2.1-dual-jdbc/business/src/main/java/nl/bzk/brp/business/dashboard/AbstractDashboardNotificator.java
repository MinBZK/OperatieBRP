...
import nl.bzk.brp.model.dashboard.*;
        ...
        U extends AbstractBerichtRequest>
{
    ...
    private static final Logger                       LOGGER         =
            LoggerFactory.getLogger(AbstractDashboardNotificator.class);
     ...
     */
    protected abstract boolean kanVerwerken(final AbstractBijhoudingsBericht bericht);
    ...
    public void voorbereiden(final AbstractBijhoudingsBericht bericht, final BerichtContext context) {
        ...
        try {
            ...
            kenmerken.setBurgerZakenModuleNaam(bericht.getBerichtStuurgegevensGroep().getApplicatie().getWaarde());
            if (Ja.Ja == bericht.getBerichtStuurgegevensGroep().getIndPrevalidatie()) {
     ...
     */
    private void setMeldingen(final BerichtVerwerkingsResultaat resultaat, final AbstractBerichtRequest request) {
    ...
    protected Persoon laadPersoon(final List<?> personen, final int index) {
        ...
        if (personen.size() <= index) {
            ...
            List<PersoonModel> persoonModels = getPersoonRepository().haalPersonenMetWoonAdresOpViaBurgerservicenummer(burgerservicenummer);
            resultaat =
                persoonModels.size() > 0? persoonModels.get(0):null; // Als we een persoon vinden met dit bsn, geef dan de eerste terug
        }
        return resultaat;
    }
