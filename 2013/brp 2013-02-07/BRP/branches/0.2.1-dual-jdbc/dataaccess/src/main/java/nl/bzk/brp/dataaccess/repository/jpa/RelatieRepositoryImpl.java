...
import nl.bzk.brp.dataaccess.repository.RelatieRepositoryCustom;
...
/**
 * JPA specifieke implementatie van de {@link nl.bzk.brp.dataaccess.repository.RelatieRepositoryCustom} repository.
 * Deze repository biedt methodes voor het ophalen van relaties, personen in relaties en het opslaan van relaties.
 */
public class RelatieRepositoryImpl implements RelatieRepositoryCustom {

    private static final Logger LOGGER =
        LoggerFactory.getLogger(RelatieRepositoryImpl.class);
        ...
        final Datum datumAanvangGeldigheid)
    {
        ...
        // Opslaan in A-laag
        em.persist(relatie);
        LOGGER.debug("entitymanager in relatie opslaan " + em.toString());
        //LocalContainerEntityManagerFactoryBean bean = (LocalContainerEntityManagerFactoryBean) em.getEntityManagerFactory();
        //LOGGER.debug("identifier:" + bean.getPersistenceUnitName());
        LOGGER.info("Relatie opgeslagen " + relatie.getId());
    ...
    private void opslaanActie(final ActieModel actie) {
        LOGGER.info("Actie opslaan: " + actie.toString());
        actieRepository.save(actie);
    }
    ...
    private void opslaanRelatieHistorie(final RelatieModel relatie, final ActieModel actie,
        final Datum datumAanvangGeldigheid)
    {
        LOGGER.debug("Nu de historie van de relatie opslaan." + relatie.getSoort().getNaam() + "(" + relatie.getId() + ");" + relatie.toString());
        LOGGER.debug(("Actie id:" + actie.getId() + "," + actie.getSoort().getNaam()));
        opslaanActie(actie);
