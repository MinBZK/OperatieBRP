 ...
 * Deze regel gaat van uit dat de ouders van het kind van het mannelijk + vrouwelijk geslacht is.
 * En gaat van uit dat de man niet de ouder vanwaar het kind geboren is.
 * Ouders van 2x man en 2x vrouw is hiermee uitgesloten.
 *
 * Let op: deze regel moet plaats vinden
 *              NA BRPUC00112 ('moeder' is van geslacht 'V')
 *              NA BRBY0168   ('moeder' moet exact 1 zijn).
 * @brp.bedrijfsregel BRBY0170
 */
public class BRBY0170 implements ActieBedrijfsRegel<Relatie> {
        ...
        if (null != nieuweSituatie) {
            ...
            if (CollectionUtils.isNotEmpty(nietMoeders)) {
                for (Persoon ouder : nietMoeders) {
                    Persoon pVader = persoonRepository.findByBurgerservicenummer(
                            ouder.getIdentificatienummers().getBurgerservicenummer());
                    if (null != pVader) {
                        if (pVader.getGeslachtsaanduiding().getGeslachtsaanduiding() != Geslachtsaanduiding.MAN) {
                            // de niet moeder moet man zijn.
                            meldingen.add(new Melding(Soortmelding.FOUT, MeldingCode.BRBY0170,
                                    (Identificeerbaar) ouder.getGeslachtsaanduiding(), "geslachtsnaamduiding"));
                        }
                    } else {
                        LOGGER.error(String.format("Kan de persoon met bsn '%s' niet vinden."
                            + " Validatie %s niet uitgevoerd.",
                            ouder.getIdentificatienummers().getBurgerservicenummer().getWaarde(), getCode()));
                    }
                }
            }
        } else {
            LOGGER.error(String.format("De nieuwe situatie zou niet null mogen zijn, kan %s niet valideren."
                , getCode()));
        }
        return meldingen;
    }
