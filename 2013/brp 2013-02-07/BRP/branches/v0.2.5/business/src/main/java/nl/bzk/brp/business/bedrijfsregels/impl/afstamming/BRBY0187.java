 ...
 *
 * Uitzondering hierop is als de datum geboorte uit de groep Geboorte van de nieuwgeborene precies gelijk is
 * aan die van andere kinderen (meerlingsituatie).
 *
 ...
 */
public class BRBY0187 implements ActieBedrijfsRegel<Relatie> {
    ...
    private Melding controlleerKind(final Persoon kind, final PersoonModel pMoeder, final Persoon moeder) {
        ...
        Datum datumGeboorteNegenMaandenGeleden = DatumUtil.voegToeDatum(
                kind.getGeboorte().getDatumGeboorte(), Calendar.DATE, -NEGEN_MAANDEN_IN_DAGEN);
    ...
    private PersoonModel haalPersoonOp(final Persoon persoonBericht, final List<Melding> meldingen) {
        ...
        persoonModel = persoonRepository.findByBurgerservicenummer(persoonBericht.getIdentificatienummers()
