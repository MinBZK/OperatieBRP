            ...
            try {
                ...
                SOAPACTION_TO_SCHEMAMAPPING.put(
                        ...
                        fact.newSchema(new StreamSource(new File(SchemaValidationHelper.class.getResource(
                                "/xsd/BRP0100_Afstamming_Berichten.xsd").toURI()))));
                SOAPACTION_TO_SCHEMAMAPPING.put(
                        "verhuizing",
                        fact.newSchema(new StreamSource(new File(SchemaValidationHelper.class.getResource(
                                "/xsd/BRP0100_Migratie_Berichten.xsd").toURI()))));
                SOAPACTION_TO_SCHEMAMAPPING.put(
                        "correctieAdresNL",
                        fact.newSchema(new StreamSource(new File(SchemaValidationHelper.class.getResource(
                                "/xsd/BRP0100_Migratie_Berichten.xsd").toURI()))));
                /*SOAPACTION_TO_SCHEMAMAPPING.put(
                        ...
                        fact.newSchema(new StreamSource(new File(SchemaValidationHelper.class.getResource(
                                "/wsdl/BRP_HuwelijkPartnerschap_Berichten.xsd").toURI()))));*/
                SOAPACTION_TO_SCHEMAMAPPING.put(
                        "registreerOverlijden",
                        fact.newSchema(new StreamSource(new File(SchemaValidationHelper.class.getResource(
                                "/xsd/BRP0100_Overlijden_Berichten.xsd").toURI()))));
                //BEVRAGING
                SOAPACTION_TO_SCHEMAMAPPING.put(
                        "vraagDetailsPersoon",
                        fact.newSchema(new StreamSource(new File(SchemaValidationHelper.class.getResource(
                                "/xsd/BRP0100_Bevraging_Berichten.xsd").toURI()))));
                SOAPACTION_TO_SCHEMAMAPPING.put(
                ...
                if (schemaValidationSchemaErrorHandler.isWarningsOpgetreden()) {
                    LOGGER.warn("Er zijn warnings opgetreden bij het parsen van BRP xsd schema's.");
                }

                if (schemaValidationSchemaErrorHandler.isErrorsOpgetreden()) {
                    LOGGER.error("Er zijn fouten opgetreden bij het parsen van BRP xsd schema's. ");
                    //TODO Tijdelijk geen runtime exceptie opgooien totdat Gertjan de xsd fouten in bevraging
                    //TODO xsd's heeft opgelost. Zodat wij nu verder kunnen.
                    //throw new IllegalStateException("Er zijn fouten opgetreden bij het parsen van BRP xsd schema's.");
                }

            } catch (SAXException e) {
                LOGGER.error("Fout opgetreden tijdens initialisatie XSD validatie: " + e.getLocalizedMessage(), e);
                throw new RuntimeException("Fout opgetreden tijdens initialisatie XSD validatie.", e);
