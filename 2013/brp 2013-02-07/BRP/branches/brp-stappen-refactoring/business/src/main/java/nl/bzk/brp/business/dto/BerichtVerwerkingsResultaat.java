...
public class BerichtVerwerkingsResultaat extends AbstractStappenResultaat {
    ...
    public boolean bevatVerwerkingStoppendeFouten() {
        ...
        for (Melding melding : super.meldingen) {
            if (melding.getSoort() == SoortMelding.DEBLOKKEERBAAR
                || melding.getSoort() == SoortMelding.FOUT)
            {
                bevatFouten = true;
                break;
            }
    ...
    public void voegtoeOverruleMeldingen(
            final List<AdministratieveHandelingGedeblokkeerdeMeldingBericht> overrMeldingen)
    {
        if (overruleMeldingen == null) {
            overruleMeldingen = new ArrayList<AdministratieveHandelingGedeblokkeerdeMeldingBericht>(overrMeldingen);
        } else {
