...
public class PersoonModel extends AbstractPersoonModel implements Persoon {
    ...
    @Transient
    @JsonIgnore
    private final Logger LOGGER = LoggerFactory.getLogger(PersoonModel.class);
    ...
    public Boolean isOverleden() {
        return (getOpschorting() != null && RedenOpschorting.OVERLIJDEN.equals(getOpschorting().getRedenOpschorting()));
    }
    ...
    // TODO: betere manier van ophalen gegevens
    public PersoonAdresModel getWoonAdres() {
        for (PersoonAdresModel adres: getAdressen()) {
            if( adres.getGegevens()!=null && adres.getGegevens().getSoort() == FunctieAdres.WOONADRES) {
                return adres;
            } else
            {
                LOGGER.warn("Geen woonadres gevonden voor deze persoon.");
            }
        }
