/**
 * This file is copyright 2017 State of the Netherlands (Ministry of Interior Affairs and Kingdom Relations).
 * It is made available under the terms of the GNU Affero General Public License, version 3 as published by the Free Software Foundation.
 * The project of which this file is part, may be found at https://github.com/MinBZK/operatieBRP.
 */

package nl.bzk.brp.dataaccess.repository.jpa.serialization;

import java.io.IOException;

import nl.bzk.brp.model.blob.PlBlob;
import nl.bzk.brp.model.objecttype.pojo.PersoonHisModel;

/**
 */
public interface BlobSerializer {
    public byte[] serializeObject(PersoonHisModel model) throws IOException;

    public PersoonHisModel deserializeObject(PlBlob blob) throws IOException, ClassNotFoundException;
}
