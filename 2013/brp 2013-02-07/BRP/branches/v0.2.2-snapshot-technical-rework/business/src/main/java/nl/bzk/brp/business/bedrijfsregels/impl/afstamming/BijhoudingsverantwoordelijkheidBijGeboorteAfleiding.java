 ...
 */
public class BijhoudingsverantwoordelijkheidBijGeboorteAfleiding implements ActieBedrijfsRegel<Relatie> {
        ...
        final Actie actie)
    {
        ...
        if (nieuweSituatie.getKindBetrokkenheid().getBetrokkene() instanceof PersoonBericht) {
            ...
            if (kind.getBijhoudingsverantwoordelijkheid().getVerantwoordelijke() == null) {
                kind.getBijhoudingsverantwoordelijkheid().setVerantwoordelijke(Verantwoordelijke.COLLEGE);
            }
        }
