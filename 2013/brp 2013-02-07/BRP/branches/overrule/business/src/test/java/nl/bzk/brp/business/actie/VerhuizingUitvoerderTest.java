...
public class VerhuizingUitvoerderTest extends AbstractStapTest {
    ...
    @Before
    public void init() {
        Mockito.when(persoonRepository.findByBurgerservicenummer(Matchers.anyString()))
               .thenReturn(new PersistentPersoon());
    ...
    public void testMetOnbekendeReferentieExceptieUitDAL() {
        ...
        Mockito.doThrow(new OnbekendeReferentieExceptie(OnbekendeReferentieExceptie.ReferentieVeld.GEMEENTECODE, "0013",
            null)).when(persoonAdresRepository).opslaanNieuwPersoonAdres(Matchers.any(PersoonAdres.class),
            Matchers.any(Integer.class), Matchers.any(Integer.class), Matchers.any(Date.class));
    ...
    public void testJuisteDatumAanvangGeldigheidMeegegevenAanDAL() {
        ...
        Mockito.verify(persoonAdresRepository).opslaanNieuwPersoonAdres(
            Mockito.any(PersoonAdres.class), argumentCaptor.capture(), Mockito.anyInt(), Mockito.any(Date.class)
        );
