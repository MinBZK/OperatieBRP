...
public class BRBY0106Test {
    ...
    public void testOuder1ZonderIdentificatieNummers() {
        ...
        Assert.assertEquals(SoortMelding.FOUT_OVERRULEBAAR, melding.getSoort());
        Assert.assertEquals(MeldingCode.BRBY0106, melding.getCode());
        Assert.assertEquals("Er is een ouder meegegeven zonder identificatie nummers in het bericht.",
    ...
    public void testKindMet1OuderEnVerschillendeGeslachtsnamen() {
        ...
        Assert.assertEquals(SoortMelding.FOUT_OVERRULEBAAR, melding.getSoort());
        Assert.assertEquals(MeldingCode.BRBY0106, melding.getCode());
        Assert.assertEquals("Geslachtsnaam van kind komt niet overeen met die van een van de ouders of die van andere "
    ...
    public void test2OudersGeslachtsNaamKindGelijkAanOuder2EnGeenEerdereKinderen() {
        ...
        Mockito.when(persoonRepository.findByBurgerservicenummer(ouder1.getBetrokkene().getIdentificatienummers().getBurgerservicenummer())).thenReturn(pOuder1);
        Mockito.when(persoonRepository.findByBurgerservicenummer(ouder2.getBetrokkene().getIdentificatienummers().getBurgerservicenummer())).thenReturn(pOuder2);
    ...
    public void test2NietGehuwdeOudersGeslachtsNaamKindGelijkAanOuder2MaarNietAanEerderKindUitHuwelijk() {
        ...
        Mockito.when(relatieRepository.vindSoortRelatiesMetPersonen(
                Mockito.any(PersistentPersoon.class),
                Mockito.any(PersistentPersoon.class),
                Mockito.any(SoortRelatie.class))).thenReturn(gevondenFamilieRechtelijkeBetrekkingen);
    ...
    public void test2GehuwdeOudersGeslachtsNaamKindGelijkAanOuder2MaarNietAanEerderKindUitHuwelijk() {
        ...
        Mockito.when(relatieRepository.vindSoortRelatiesMetPersonen(
                Mockito.any(PersistentPersoon.class),
                Mockito.any(PersistentPersoon.class),
                Mockito.any(SoortRelatie.class), Mockito.any(SoortRelatie.class))).thenReturn(gevondenHuwelijken);
    ...
    private Betrokkenheid maakBetrokkenheid(final SoortBetrokkenheid soort, final Relatie relatie, final String bsn) {
        ...
        Persoon ouder = new Persoon();
        ouder.setIdentificatienummers(new PersoonIdentificatienummers());
        ouder.getIdentificatienummers().setBurgerservicenummer(bsn);
