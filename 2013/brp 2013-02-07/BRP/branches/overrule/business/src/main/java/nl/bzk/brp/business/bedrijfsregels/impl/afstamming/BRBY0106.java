...
public class BRBY0106 implements BedrijfsRegel<PersistentRelatie, Relatie> {
    ...
    @Override
    public Melding executeer(final PersistentRelatie huidigeSituatie,
                             final Relatie nieuweSituatie, final Integer datumAanvangGeldigheid)
    {
        ...
        if (ouder1 == null) {
            melding = new Melding(SoortMelding.FOUT_OVERRULEBAAR,
                    MeldingCode.BRBY0106,
                ...
                || (ouder2 != null && ouder2.getIdentificatienummers() == null))
        {
            ...
            pOuder1 = persoonRepository.findByBurgerservicenummer(
                    ouder1.getIdentificatienummers().getBurgerservicenummer());

            if (ouder2 != null) {
                pOuder2 = persoonRepository.findByBurgerservicenummer(
            ...
            geslachtsNaamIsVanEenVanDeOuders = isGeslachtsNaamGelijk(pOuder1.getPersoonGeslachtsnaamcomponenten(),
                    kind.getGeslachtsnaamcomponenten().get(0));

            if (!geslachtsNaamIsVanEenVanDeOuders && pOuder2 != null) {
                geslachtsNaamIsVanEenVanDeOuders = isGeslachtsNaamGelijk(pOuder2.getPersoonGeslachtsnaamcomponenten(),
                        kind.getGeslachtsnaamcomponenten().get(0));
            }
            ...
            if (!geslachtsNaamIsVanEenVanDeOuders) {
                ...
                final List<PersistentRelatie> huwelijkenEnGeregistreerdePartnerschappen =
                        relatieRepository.vindSoortRelatiesMetPersonen(
                                pOuder1,
                                pOuder2,
            ...
            final PersoonGeslachtsnaamcomponent persGeslachtsnaamcomponentKind)
    {
        ...
        for (PersistentPersoonGeslachtsnaamcomponent comp : persGeslachtsnaamcomponenten) {
            isGelijk = (StringUtils.equals(persGeslachtsnaamcomponentKind.getNaam(), comp.getNaam())
                    &&
                    StringUtils.equals(persGeslachtsnaamcomponentKind.getScheidingsTeken(), comp.getScheidingsteken())
                    && StringUtils.equals(persGeslachtsnaamcomponentKind.getVoorvoegsel(), comp.getVoorvoegsel()));
        }
        return isGelijk;
    }
}
