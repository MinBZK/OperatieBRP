...
public class RelatieJpaRepository implements RelatieRepository {
        ...
        final Date tijdstipRegistratie)
    {
        ...
        // Opslaan in C-laag
        werkHistorieBij(nieuweRelatie, tijdstipRegistratie, datumAanvang);

        return nieuweRelatie.getId();
    }

    @Override
    public List<PersistentRelatie> vindSoortRelatiesMetPersonen(final PersistentPersoon persoon1,
                                                                final PersistentPersoon persoon2,
                                                                final SoortRelatie... soorten)
    {
        final CriteriaBuilder cb = em.getCriteriaBuilder();
        final CriteriaQuery<PersistentRelatie> cq = cb.createQuery(PersistentRelatie.class);
        final Root<PersistentRelatie> root = cq.from(PersistentRelatie.class);
        cq.distinct(true);
        Join<PersistentRelatie, PersistentBetrokkenheid> join = root.join("betrokkenheden");
        Predicate predicate = cb.conjunction();
        predicate = cb.and(predicate, root.get("soortRelatie").in(soorten));
        predicate = cb.and(predicate, join.get("betrokkene").in(persoon1, persoon2));
        cq.where(predicate);
     ...
     */
    private void werkHistorieBij(final PersistentRelatie nieuweRelatie, final Date tijdstipRegistratie,
            ...
            for (PersistentBetrokkenheid betrokkenheid : nieuweRelatie.getBetrokkenheden()) {
                if (SoortBetrokkenheid.OUDER == betrokkenheid.getSoortBetrokkenheid()) {
                    betrokkenheidOuderHistorieRepository
                        .opslaanHistorie(betrokkenheid, datumAanvang, null, tijdstipRegistratie);
                }
            }
        } else {
