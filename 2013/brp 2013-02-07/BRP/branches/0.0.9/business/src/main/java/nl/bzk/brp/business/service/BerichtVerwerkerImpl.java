...
public class BerichtVerwerkerImpl implements BerichtVerwerker {
    ...
    @Transactional
    @Override
    public BerichtResultaat verwerkBericht(final BRPBericht bericht, final BerichtContext context) {
        LOGGER.debug("Berichtverwerker start verwerking bericht: {}", bericht);

        BerichtResultaat berichtResultaat = berichtResultaatFactory.creeerBerichtResultaat(bericht);
                                                     ...
                                                     final BerichtResultaat berichtResultaat)
    {
        if (bericht instanceof BijhoudingsBericht) {
            valideerBijhoudingsBerichtOpVerplichteObjecten((BijhoudingsBericht) bericht, berichtResultaat);
        }
    }
     ...
     *     op 'foutief' gezet wordt als het bericht niet valide is.
     */
    private void valideerBijhoudingsBerichtOpVerplichteObjecten(final BijhoudingsBericht bericht,
        final BerichtResultaat berichtResultaat)
    {
            ...
            for (BRPActie actie : bericht.getBrpActies()) {
                if (actie.getRootObjecten() == null || actie.getRootObjecten().isEmpty()) {
                    berichtResultaat.voegMeldingToe(new Melding(SoortMelding.FOUT_ONOVERRULEBAAR, MeldingCode.ACT0002,
                        String.format("%s: ", MeldingCode.ACT0002.getOmschrijving(),
