...
public class BRBY0531Test {
    ...
    public void testAfgelopenPeriodeActieZonderEinde() {
        ...
        Assert.assertEquals("id.actie1", meldingen.get(0).getVerzendendId());
    }
    ...
    private Actie maakActie(final String verzendendId, final Integer begin, final Integer eind) {
        ActieBericht actie =  ActieBerichtBuilder.bouwNieuweActie(SoortActie.CORRECTIE_ADRES_NL)
    ...
    private Plaats maakPlaats(final int code, final String naam, final Integer begin, final Integer eind) {
        Plaats plaats = new Plaats();
        plaats.setCode(new PlaatsCode((short) code));
        plaats.setNaam(new Naam(naam));
        if (begin != null) {
            ReflectionTestUtils.setField(plaats, "datumAanvang", new Datum(begin));
        }
        if (eind != null) {
            ReflectionTestUtils.setField(plaats, "datumEinde", new Datum(eind));
        }
        return plaats;
    }
}
