        ...
        if (null != nieuweSituatie && null != nieuweSituatie.getAdressen()) {
            for (PersoonAdres adres : nieuweSituatie.getAdressen()) {
                if (null != adres.getGegevens().getWoonplaats()) {
                    ...
                    boolean geldig = DatumUtil.isDatumsGeldigOpPeriode(
                                plaats.getDatumAanvang(), plaats.getDatumEinde(),
                                aanvang, einde);
                    if (!geldig) {
                        meldingen.add(new Melding(SoortMelding.FOUT_OVERRULEBAAR, MeldingCode.BRBY0531,
