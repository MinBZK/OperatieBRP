...
public class VerhuizingNotificatorTest {
    ...
    public void testSunnyDay() throws Exception {
        ...
        PersoonBericht persoon = PersoonBuilder.bouwPersoon("123234345", GeslachtsAanduiding.MAN, 19660417,
            StatischeObjecttypeBuilder.GEMEENTE_UTRECHT, "Ab", "de", "Vis");
        PersoonAdresBericht adres = PersoonAdresBuilder.bouwWoonadres("Langstraat", "10", "1234AA",
    ...
    public void testRudimentaireAdressen() throws Exception {
        ...
        PersoonBericht persoon = PersoonBuilder.bouwPersoon("123234345", GeslachtsAanduiding.MAN, 19660417,
            StatischeObjecttypeBuilder.GEMEENTE_UTRECHT, "Ab", "de", "Vis");
        PersoonAdresBericht adres = PersoonAdresBuilder.bouwWoonadres("Langstraat", "10", "1234AA", null,
            StatischeObjecttypeBuilder.bouwGemeente("164", "Alphen"), 20120515);
        ...
        Partij partij = StatischeObjecttypeBuilder.bouwGemeente("321", "Zeist");
        BerichtContext context = new BerichtContext(berichtenIds, authenticatieMiddelId, partij, "ref");

        BerichtResultaat resultaat = new BerichtResultaat(null);

        Mockito.when(persoonRepository.haalPersoonOpMetBurgerservicenummer(new Burgerservicenummer("123234345")))
               .thenReturn(bouwPersoonModel(persoon));
    ...
    private PersoonModel bouwPersoonModel(final PersoonBericht persoon) {
        ...
        if (persoon.getGeslachtsnaamcomponenten() != null && !persoon.getGeslachtsnaamcomponenten().isEmpty()) {
            for (PersoonGeslachtsnaamComponentBericht geslachtsnaamComponentBericht : persoon.getGeslachtsnaamcomponenten()) {
                persoonModel.getGeslachtsnaamcomponenten().add(new PersoonGeslachtsnaamComponentModel(
                    geslachtsnaamComponentBericht, persoonModel));
            }
        }
        return persoonModel;
    }
