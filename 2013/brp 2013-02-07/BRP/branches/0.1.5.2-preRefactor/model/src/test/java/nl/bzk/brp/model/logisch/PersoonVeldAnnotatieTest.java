...
/**
 * Unit test class voor de {@link nl.bzk.brp.model.logisch.Persoon} class, waarbij specifiek enkele annotatie validatie
 * tests worden uitgevoerd op velden van de {@link nl.bzk.brp.model.logisch.Persoon} class.
 */
public class PersoonVeldAnnotatieTest {
    ...
    public void testValidatieAnnotatieSoortAdresVerplicht() {
        ...
        // DatumAanvangAdreshouding verplicht
        persoonAdres.setSoort(FunctieAdres.WOONADRES);
        persoonAdres.setDatumAanvangAdreshouding(null);
        persoonAdres.setGemeente(new Partij());
        persoonAdres.setRedenWijziging(RedenWijzigingAdres.PERSOON);
        violations = beanValidator.validate(persoon, Default.class);
        Assert.assertEquals(1, violations.size());
        Assert.assertTrue(violationsBevatCode(MeldingCode.BRAL2033, violations));

        // Gemeente verplicht
        persoonAdres.setSoort(FunctieAdres.WOONADRES);
        persoonAdres.setDatumAanvangAdreshouding(20120101);
        persoonAdres.setGemeente(null);
        persoonAdres.setRedenWijziging(RedenWijzigingAdres.PERSOON);
        violations = beanValidator.validate(persoon, Default.class);
        Assert.assertEquals(1, violations.size());
        Assert.assertTrue(violationsBevatCode(MeldingCode.BRAL2033, violations));
        ...
        persoonAdres.setDatumAanvangAdreshouding(20120101);
        persoonAdres.setGemeente(new Partij());
        persoonAdres.setRedenWijziging(null);
        violations = beanValidator.validate(persoon, Default.class);
        Assert.assertEquals(1, violations.size());
        Assert.assertTrue(violationsBevatCode(MeldingCode.BRAL2033, violations));
    }
