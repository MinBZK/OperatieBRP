...
public class BijhoudingBerichtVerwerkerTest {
    ...
    @Ignore ("Moet nog goed gezet worden in het nabewerking validatie")
    @Test
    @SuppressWarnings("unchecked")
    public void testVerwerkingMeTweeOverrulesMetLeftoverStap() {
        ...
        Assert.assertEquals("id2", resultaat.getMeldingen().get(0).getReferentieID());
        Assert.assertEquals(SoortMelding.WAARSCHUWING, resultaat.getMeldingen().get(1).getSoort());
        Assert.assertEquals("id1", resultaat.getMeldingen().get(1).getReferentieID());
    ...
    private AbstractBerichtVerwerkingsStap<BerichtBericht,
        ...
        AbstractBerichtVerwerkingsStap<BerichtBericht, BerichtVerwerkingsResultaat> stap =
            new AbstractBerichtVerwerkingsStap<BerichtBericht, BerichtVerwerkingsResultaat>() {
                ...
                @Override
                public boolean voerStapUit(final BerichtBericht onderwerp, final BerichtContext context,
                                           final BerichtVerwerkingsResultaat resultaat)
                {
                    return voerVerwerkingsStapUitVoorBericht(onderwerp, (BerichtContext) context, resultaat);  //To change body of implemented methods use File | Settings | File Templates.
                }
        ...
        final SoortOverruleMelding soort)
    {
        ...
        melding.setRegel(Regel.valueOf(meldingsCode.getNaam()));
        melding.setReferentieID(verzId);
        if (soort != null) {
        ...
        AbstractBerichtVerwerkingsStap<BerichtBericht, BerichtVerwerkingsResultaat> stap =
            new AbstractBerichtVerwerkingsStap<BerichtBericht, BerichtVerwerkingsResultaat>() {
                ...
                public void voerNabewerkingStapUit(final BerichtBericht onderwerp, final BerichtContext context,
                                                   final BerichtVerwerkingsResultaat resultaat)
                {
                    naVerwerkingsStapVoorBericht(onderwerp, context, resultaat);
                }
            };
        return stap;
    }
