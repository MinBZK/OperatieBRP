...
public class BerichtContextValidatieStapTest {
    ...
    public void testGeldigeContext() {
        ...
        Mockito.when(bedrijfsRegelManager.getUitTeVoerenBerichtContextBedrijfsRegels(Matchers.any(Class.class)))
               .thenAnswer(
                   new Answer<List<BerichtContextBedrijfsRegel>>() {

                       @Override
                       public List<BerichtContextBedrijfsRegel> answer(final InvocationOnMock invocation)
                           throws Throwable
                       {
        ...
        BerichtContext context = new BerichtContext(new BerichtenIds(1L, 2L), Mockito.mock(Partij.class), "abc");
