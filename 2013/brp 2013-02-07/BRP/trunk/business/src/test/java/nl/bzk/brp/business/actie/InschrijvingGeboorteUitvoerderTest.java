...
public class InschrijvingGeboorteUitvoerderTest extends AbstractStapTest {
    ...
    @InjectMocks
    private final AbstractActieUitvoerder uitvoerder = new InschrijvingGeboorteUitvoerder();
    ...
    public void testNormaleActie() {
        ...
        ActieBericht actieBericht = getInschrijvingGeboorteActie();
        // voeg aan het kind extra informatie, omdat we nu de 'normale' dingen doen.
        RelatieBericht relatie = (RelatieBericht) actieBericht.getRootObjecten().get(0);
        for (BetrokkenheidBericht b : relatie.getBetrokkenheden()) {
            if (b instanceof KindBericht) {
                KindBericht kindBericht = (KindBericht) b;
                kindBericht.getPersoon().setVoornamen(Arrays.asList(
                        PersoonBuilder.bouwPersoonVoornaam(1, "Piet-Jan-Klaas")));

                kindBericht.getPersoon().setAdressen(Arrays.asList(
                        PersoonAdresBuilder.bouwWoonadres(
                                ...
                                20100802)));
                kindBericht.getPersoon().setGeslachtsnaamcomponenten(
                        Arrays.asList(
                        PersoonBuilder.bouwGeslachtsnaamcomponent(1, "van", "Houten")));
            }
        }

        List<Melding> meldingen = uitvoerder.voerUit(actieBericht, bc, administratieveHandelingModel);
