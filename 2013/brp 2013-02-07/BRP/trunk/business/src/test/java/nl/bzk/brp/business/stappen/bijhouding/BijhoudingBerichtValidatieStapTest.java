...
public class BijhoudingBerichtValidatieStapTest extends AbstractStapTest {
    ...
    public void testNullVoorGevondenBedrijfsRegels() {
        ...
        Mockito.when(bedrijfsRegelManager.getUitTeVoerenBijhoudingsBerichtBedrijfsRegels(Matchers.any(Class.class)))
               .thenReturn(null);
        Assert.assertTrue(bijhoudingBerichtValidatieStap
            .voerVerwerkingsStapUitVoorBericht(maakBericht(), bouwBerichtContext(), resultaat));
        Assert.assertTrue(resultaat.getMeldingen().isEmpty());
    ...
    public void testMetGevondenBedrijfsRegelMetMelding() {
        ...
        Mockito.when(bedrijfsRegelManager.getUitTeVoerenBijhoudingsBerichtBedrijfsRegels(Matchers.any(Class.class)))
               .thenAnswer(new Answer<List<BijhoudingsBerichtBedrijfsRegel>>() {
                   @Override
                   public List<BijhoudingsBerichtBedrijfsRegel> answer(final InvocationOnMock invocation) throws Throwable {
    ...
    /**
     * Retourneert een lijst met een enkele {@link nl.bzk.brp.business.bedrijfsregels.BijhoudingsBerichtBedrijfsRegel}, waarbij deze regel alleen een melding
     * retourneert indien het executeert tegen een correctie adres bericht.
     *
     * @return lijst met enkele bedrijfsregel.
     */
    private List<BijhoudingsBerichtBedrijfsRegel> bouwLijstBerichtBedrijfsRegels() {
        List<BijhoudingsBerichtBedrijfsRegel> regels = new ArrayList<BijhoudingsBerichtBedrijfsRegel>();

        regels.add(new BijhoudingsBerichtBedrijfsRegel() {
            @Override
