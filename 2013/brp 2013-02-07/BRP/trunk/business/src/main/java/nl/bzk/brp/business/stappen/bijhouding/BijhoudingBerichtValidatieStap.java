 ...
 * te controleren.
 */
public class BijhoudingBerichtValidatieStap extends
        AbstractBerichtVerwerkingsStap<AbstractBijhoudingsBericht, BerichtVerwerkingsResultaat>
{
            ...
            final BerichtVerwerkingsResultaat resultaat)
    {
        ...
        } else if (null != bedrijfsRegelManager.getUitTeVoerenBijhoudingsBerichtBedrijfsRegels(bericht.getClass())) {
            for (BijhoudingsBerichtBedrijfsRegel regel : bedrijfsRegelManager.getUitTeVoerenBijhoudingsBerichtBedrijfsRegels(
                    bericht
                            .getClass()))
            {
            ...
            final BerichtVerwerkingsResultaat resultaat)
    {
        if (bericht.getAdministratieveHandeling() != null
            && CollectionUtils.isNotEmpty(bericht.getAdministratieveHandeling().getGedeblokkeerdeMeldingen()))
        {
            for (AdministratieveHandelingGedeblokkeerdeMeldingBericht m : bericht.getAdministratieveHandeling()
                    .getGedeblokkeerdeMeldingen())
            {
                String code = null;
                if (null != m.getGedeblokkeerdeMelding() && null != m.getGedeblokkeerdeMelding().getRegel()) {
                    code = m.getGedeblokkeerdeMelding().getRegel().getCode();
                }
                if (StringUtils.isBlank(code)) {
                    resultaat.voegMeldingToe(new Melding(SoortMelding.FOUT, MeldingCode.ALG0001,
                            "Code is verplicht voor overrule."));
                } else if (!whitelistOverrulbareMeldingen.contains(code)) {
                    // test in de whitelist.
                    String msg = String.format("code %s mag niet overruled worden.", code);
                    resultaat.voegMeldingToe(new Melding(SoortMelding.FOUT, MeldingCode.ALG0001, msg, m, "regelCode"));
                    LOGGER.warn(msg);
                } else if (naverwerkingOverrulbareMeldingen != null && naverwerkingOverrulbareMeldingen.contains(code))
                {
                    // Dit om een speciale type overrule meldingen, kan pas gebeuren bij validatie NA verwerking.
                    m.getGedeblokkeerdeMelding().setSoort(SoortOverruleMelding.NABEWERKING_VALIDATIE_MELDING);
                }
            }
        }
    }
    ...
    private boolean persoonIndeLijst(final List<Persoon> lijst, final Persoon persoon) {
        for (Persoon pers : lijst) {
            if (pers.getIdentificatienummers().getBurgerservicenummer()
