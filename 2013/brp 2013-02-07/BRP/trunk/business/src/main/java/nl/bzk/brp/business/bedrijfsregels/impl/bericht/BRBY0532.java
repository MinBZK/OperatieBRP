 ...
 */
public class BRBY0532 implements BijhoudingsBerichtBedrijfsRegel<CorrectieAdresBericht> {
    ...
    public List<Melding> executeer(final CorrectieAdresBericht bericht) {
        ...
        if (bericht != null && bericht.getAdministratieveHandeling().getActies() != null && bericht.getAdministratieveHandeling().getActies().size() > 1) {
            Actie hoofdActie = bepaalHoofdActie(bericht.getAdministratieveHandeling().getActies());
        ...
        if (anderPersoonGevonden) {
            meldingen =
                Arrays.asList(new Melding(SoortMelding.FOUT, MeldingCode.BRBY0532, null, null));
        } else {
