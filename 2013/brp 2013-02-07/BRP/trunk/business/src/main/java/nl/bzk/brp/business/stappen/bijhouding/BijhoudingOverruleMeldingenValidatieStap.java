        ...
        final BerichtVerwerkingsResultaat resultaat, final boolean skipNaVerwerkingType)
    {
        ...
        if (CollectionUtils.isNotEmpty(bericht.getAdministratieveHandeling().getGedeblokkeerdeMeldingen())) {
            ...
            if (CollectionUtils.isNotEmpty(codes)) {
                ...
                resultaat.voegMeldingToe(new Melding(SoortMelding.FOUT, MeldingCode.BRBY9904,
                ...
                // verwijder het van de bericht.getAdministratieveHandeling().getGedeblokkeerdeMeldingen().
                for (String code : codes) {
                    for (AdministratieveHandelingGedeblokkeerdeMeldingBericht m :
                        bericht.getAdministratieveHandeling().getGedeblokkeerdeMeldingen())
                    {
                        if (m.getGedeblokkeerdeMelding().getRegel().getCode().equals(code)) {
                            bericht.getAdministratieveHandeling().getGedeblokkeerdeMeldingen().remove(m);
                            break;
                        }
                    }
