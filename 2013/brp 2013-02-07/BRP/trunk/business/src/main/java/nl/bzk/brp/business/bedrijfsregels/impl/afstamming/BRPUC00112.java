 ...
 */
public class BRPUC00112 implements ActieBedrijfsRegel<FamilierechtelijkeBetrekking> {
            ...
            if (null != moeder) {
                ...
                if (null != pMoeder) {
                    if (!pMoeder.getGeslachtsaanduiding().getGeslachtsaanduiding().equals(Geslachtsaanduiding.VROUW)) {
                        meldingen.add(new Melding(SoortMelding.FOUT, MeldingCode.BRPUC00112, (Identificeerbaar) moeder
                                .getGeslachtsaanduiding(), "geslachtsnaamduiding"));
                    }
                } else {
                    LOGGER.error(String.format(
                            "Kan de persoon met bsn '%s' niet vinden. Validatie %s niet uitgevoerd.", moeder
                            .getIdentificatienummers().getBurgerservicenummer().getWaarde(), getCode()));
                }
            }
        } else {
