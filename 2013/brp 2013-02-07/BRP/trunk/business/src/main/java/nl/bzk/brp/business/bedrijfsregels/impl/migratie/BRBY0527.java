    ...
    public List<Melding> executeer(final Persoon huidigeSituatie, final Persoon nieuweSituatie, final Actie actie) {
        ...
        if (CollectionUtils.isEmpty(nieuweSituatie.getAdressen())) {
            ...
            for (PersoonAdres adres : nieuweSituatie.getAdressen()) {
                Partij gemeente = adres.getStandaard().getGemeente();
                ...
                {
                    meldingen =
                        Collections.singletonList(new Melding(SoortMelding.DEBLOKKEERBAAR, MeldingCode.BRBY0527,
                            String.format(MeldingCode.BRBY0527.getOmschrijving(), adres.getStandaard().getGemeente()
                                                                                       .getCode()),
                            (Identificeerbaar) adres, "gemeenteCode"));
                } else {
