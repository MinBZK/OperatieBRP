...
public class AanschrijvingGroepAfleiding implements ActieBedrijfsRegel<RootObject> {
    ...
    {
        aanschrijving.setAdellijkeTitelAanschrijving(null);
        aanschrijving.setPredikaatAanschrijving(null);
        aanschrijving.setScheidingstekenAanschrijving(null);
        aanschrijving.setVoorvoegselAanschrijving(null);
        aanschrijving.setGeslachtsnaamAanschrijving(null);

        if (aanschrijving.getIndicatieTitelsPredikatenBijAanschrijven() == JaNee.JA) {
            aanschrijving.setAdellijkeTitelAanschrijving(samegesteldeNaam.getAdellijkeTitel());
            aanschrijving.setPredikaatAanschrijving(samegesteldeNaam.getPredikaat());
        } else {
            aanschrijving.setAdellijkeTitelAanschrijving(null);
            aanschrijving.setPredikaatAanschrijving(null);
        }
        if (AttribuutTypeUtil.isNotBlank(samegesteldeNaam.getGeslachtsnaam())) {
            aanschrijving.setGeslachtsnaamAanschrijving(samegesteldeNaam.getGeslachtsnaam());
        }
        if (AttribuutTypeUtil.isNotEmpty(samegesteldeNaam.getScheidingsteken())) {
            aanschrijving.setScheidingstekenAanschrijving(samegesteldeNaam.getScheidingsteken());
        }
        if (AttribuutTypeUtil.isNotBlank(samegesteldeNaam.getVoorvoegsel())) {
            aanschrijving.setVoorvoegselAanschrijving(samegesteldeNaam.getVoorvoegsel());
        }
    }
        ...
        final PersoonSamengesteldeNaamGroep eigen, final PersoonSamengesteldeNaamGroep partner)
    {
        if (AttribuutTypeUtil.isNotBlank(eigen.getVoornamen())) {
            aanschrijving.setVoornamenAanschrijving(eigen.getVoornamen());
        }
        if (aanschrijving.getNaamgebruik() == WijzeGebruikGeslachtsnaam.EIGEN) {
            copieerGegevens(aanschrijving, eigen);
        } else if (aanschrijving.getNaamgebruik() == WijzeGebruikGeslachtsnaam.PARTNER) {
            copieerGegevens(aanschrijving, partner);
        } else if (aanschrijving.getNaamgebruik() == WijzeGebruikGeslachtsnaam.EIGEN_PARTNER) {
            copieerGegevens(aanschrijving, eigen);
            ...
            aanschrijving.setGeslachtsnaamAanschrijving(new Geslachtsnaam(gecombineerdeNaam.toString()));
        } else if (aanschrijving.getNaamgebruik() == WijzeGebruikGeslachtsnaam.PARTNER_EIGEN) {
            copieerGegevens(aanschrijving, partner);
        ...
        final Datum peilDatum)
    {
        ...
        if (berichtPersoon.getAanschrijving() != null
            && berichtPersoon.getAanschrijving().getIndicatieAanschrijvingAlgoritmischAfgeleid() == JaNee.JA)
        {
            PersoonSamengesteldeNaamGroep eigen = huidigPersoon.getSamengesteldeNaam();
            if (berichtPersoon.getAanschrijving().getNaamgebruik() == WijzeGebruikGeslachtsnaam.EIGEN) {
