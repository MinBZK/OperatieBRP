    ...
    AbstractBerichtVerwerkingsStap<AbstractBijhoudingsBericht, BerichtVerwerkingsResultaat>
{
        ...
        final BerichtVerwerkingsResultaat resultaat)
    {
        if (StringUtils.isNotBlank(admhBericht.getPartijCode())) {
            admhBericht.setPartij(vindGemeenteOpCode(new GemeenteCode(Short.parseShort(admhBericht.getPartijCode())),
                    ...
                    if (StringUtils.isNotBlank(document.getSoortNaam())) {
                        ...
                        if (null != bron.getDocument() && null != bron.getDocument().getStandaard()) {
                            ...
                            if (StringUtils.isNotBlank(groep.getPartijCode())) {
                                groep.setPartij(vindGemeenteOpCode(new GemeenteCode(
                                        Short.parseShort(groep.getPartijCode())),
                                    groep, resultaat));
                            }
                        }
                    }
        ...
        final BerichtVerwerkingsResultaat resultaat)
    {
        ...
        if (relatie instanceof HuwelijkGeregistreerdPartnerschapBericht) {
            ...
            if (null != partnerschap.getStandaard()) {
                verrijkPartnerschapGroep(partnerschap.getStandaard(), resultaat);
            }
                ...
                if (betr instanceof KindBericht) {
                    ...
                    if (kindBericht.getPersoon() != null) {
                        ...
                        if (kind.getGeboorte() != null) {
                            if (soort == SoortActie.REGISTRATIE_GEBOORTE && null != kind.getGeboorte().getDatumGeboorte()) {
                                ...
                                // Want de (nieuwe) XSD laat het niet toe een land in te vullen en de database verplicht het.
                                if (StringUtils.isBlank(kind.getGeboorte().getLandGeboorteCode())) {
                                    kind.getGeboorte().setLandGeboorteCode(BrpConstanten.NL_LAND_CODE_STRING);
                                }
                            }
...
//        }
    ...
    /**
     * Verrijk de partnerschap groep vanuit het bericht.
     * @param groep groep uit bericht dat verrijkt wordt.
     * @param resultaat verrijkte groep.
     */
                                          ...
                                          final BerichtVerwerkingsResultaat resultaat)
    {
        ...
        if (StringUtils.isBlank(groep.getLandAanvangCode())) {
            ...
            groep.setLandAanvang(vindLandOpCode(
                    new Landcode(Short.parseShort(groep.getLandAanvangCode())), groep, resultaat));
        }
        // LandEinde
        if (StringUtils.isNotBlank(groep.getLandEindeCode())) {
            groep.setLandEinde(vindLandOpCode(
                    new Landcode(Short.parseShort(groep.getLandEindeCode())), groep, resultaat));
        }
        // Gemeente aanvang
        if (StringUtils.isNotBlank(groep.getGemeenteAanvangCode())) {
            groep.setGemeenteAanvang(
                vindGemeenteOpCode(
                        new GemeenteCode(Short.parseShort(groep.getGemeenteAanvangCode())), groep, resultaat));
        }
        // Gemeente einde
        if (StringUtils.isNotBlank(groep.getGemeenteEindeCode())) {
            groep
                .setGemeenteEinde(vindGemeenteOpCode(
                        new GemeenteCode(Short.parseShort(groep.getGemeenteEindeCode())), groep, resultaat));
        }
        // Woonplaats aanvang
        if (StringUtils.isNotBlank(groep.getWoonplaatsAanvangCode())) {
            groep.setWoonplaatsAanvang(
                vindWoonplaatsOpCode(new Woonplaatscode(
                        Short.parseShort(groep.getWoonplaatsAanvangCode())), groep, resultaat));
        }
        // Woonplaats einde
        if (StringUtils.isNotBlank(groep.getWoonplaatsEindeCode())) {
            groep.setWoonplaatsEinde(
                vindWoonplaatsOpCode(
                        new Woonplaatscode(Short.parseShort(groep.getWoonplaatsEindeCode())), groep, resultaat));
        }
    }
        ...
        final BerichtVerwerkingsResultaat resultaat)
    {
        ...
        if (StringUtils.isNotBlank(groep.getGemeenteGeboorteCode())) {
            groep.setGemeenteGeboorte(
                vindGemeenteOpCode(
                        new GemeenteCode(Short.parseShort(groep.getGemeenteGeboorteCode())), groep, resultaat));
        }
        // Land geboorte
        if (StringUtils.isNotBlank(groep.getLandGeboorteCode())) {
            groep.setLandGeboorte(vindLandOpCode(new Landcode(Short.parseShort(groep.getLandGeboorteCode())), groep,
                                                 resultaat));
        }
        // Woonplaats geboorte
        if (StringUtils.isNotBlank(groep.getWoonplaatsGeboorteCode())) {
            groep.setWoonplaatsGeboorte(
                vindWoonplaatsOpCode(new Woonplaatscode(Short.parseShort(groep.getWoonplaatsGeboorteCode())), groep,
                                     resultaat));
        }
    }
        ...
        final BerichtVerwerkingsResultaat resultaat)
    {
        if (StringUtils.isNotBlank(groep.getGemeenteOverlijdenCode())) {
            groep.setGemeenteOverlijden(
                vindGemeenteOpCode(
                        new GemeenteCode(Short.parseShort(groep.getGemeenteOverlijdenCode())), groep, resultaat));
        }
        if (StringUtils.isNotBlank(groep.getWoonplaatsOverlijdenCode())) {
            groep.setWoonplaatsOverlijden(
                vindWoonplaatsOpCode(new Woonplaatscode(Short.parseShort(groep.getWoonplaatsOverlijdenCode())), groep,
                                     resultaat));
        }
        if (StringUtils.isNotBlank(groep.getLandOverlijdenCode())) {
            groep.setLandOverlijden(vindLandOpCode(new Landcode(Short.parseShort(groep.getLandOverlijdenCode())), groep,
                                                   resultaat));
        }
    }
        ...
        final BerichtVerwerkingsResultaat resultaat)
    {
        ...
        if (StringUtils.isNotBlank(groep.getWoonplaatsCode())) {
            groep.setWoonplaats(vindWoonplaatsOpCode(
                    new Woonplaatscode(Short.parseShort(groep.getWoonplaatsCode())), groep, resultaat));
        }
        if (StringUtils.isNotBlank(groep.getGemeenteCode())) {
            groep.setGemeente(vindGemeenteOpCode(new GemeenteCode(Short.parseShort(groep.getGemeenteCode())), groep,
                                                 resultaat));
        }
        if (StringUtils.isNotBlank(groep.getLandCode())) {
            groep.setLand(vindLandOpCode(new Landcode(Short.parseShort(groep.getLandCode())), groep, resultaat));
        } else {
    ...
    private void verrijkPersoonMetBsnNummer(final PersoonBericht persoonBericht) {
        if (StringUtils.isNotBlank(persoonBericht.getTechnischeSleutel())) {
            ...
            persoonBericht.getIdentificatienummers().setBurgerservicenummer(
                new Burgerservicenummer(Integer.parseInt(persoonBericht.getTechnischeSleutel())));
        }
    }
    ...
    {
        try {
            return referentieRepository.vindRedenVerliesNLNationaliteitOpCode(
                    new RedenVerliesCode(Short.parseShort(redenVerliesCode)));
        ...
        final BerichtVerwerkingsResultaat resultaat)
    {
        try {
            return referentieRepository.vindRedenVerkregenNlNationaliteitOpCode(
                new RedenVerkrijgingCode(Short.parseShort(redenVerkrijgingCode)));
        } catch (OnbekendeReferentieExceptie ore) {
            final Melding melding =
