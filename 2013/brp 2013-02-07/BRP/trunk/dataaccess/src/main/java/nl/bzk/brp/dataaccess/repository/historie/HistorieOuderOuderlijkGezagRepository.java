/**
 * This file is copyright 2017 State of the Netherlands (Ministry of Interior Affairs and Kingdom Relations).
 * It is made available under the terms of the GNU Affero General Public License, version 3 as published by the Free Software Foundation.
 * The project of which this file is part, may be found at https://github.com/MinBZK/operatieBRP.
 */

package nl.bzk.brp.dataaccess.repository.historie;

import nl.bzk.brp.model.logisch.kern.basis.OuderOuderlijkGezagGroepBasis;
import nl.bzk.brp.model.operationeel.kern.HisOuderOuderlijkGezagModel;
import nl.bzk.brp.model.operationeel.kern.OuderModel;


/**
 * Repository voor betrokkenheid ouderlijk gezag.
 */
public interface HistorieOuderOuderlijkGezagRepository extends
        GroepMaterieleHistorieRepository<OuderModel, OuderOuderlijkGezagGroepBasis,
                        HisOuderOuderlijkGezagModel>
{

}
