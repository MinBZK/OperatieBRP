...
public abstract class AbstractWebService<T extends BerichtBericht, Y extends BerichtVerwerkingsResultaat> {
        ...
        final BerichtVerwerkingsResultaat resultaat)
    {
        ...
        archiveringService.werkIngaandBerichtInfoBij(
            ...
            ingaandBericht.getParameters(),
            ingaandBericht.getSoort());
        ...
        archiveringService.werkUitgaandBerichtInfoBij(
            ...
            antwoordBericht.getStuurgegevens(),
            antwoordBericht.getResultaat());
     ...
     */
    protected boolean isGeauthenticeerd(final Authenticatiemiddel authenticatieMiddel) {
    ...
    protected BerichtContext bouwBerichtContext(final Sleutelwaardetekst berichtReferentieNummer,
        ...
        final Authenticatiemiddel authenticatiemiddel)
    {
        return new BerichtContext(berichtenIds, authenticatiemiddel.getPartij(), berichtReferentieNummer.getWaarde());
    }

    /**
     * Bouwt een standaard {@link nl.bzk.brp.business.stappen.BerichtVerwerkingsResultaat} dat geretourneerd kan worden en
     ...
     */
    protected Authenticatiemiddel checkAuthenticatieMiddel(final T bericht, final BerichtenIds berichtenIds) {
        final X509Certificate certificaat = WebserviceUtil.haalClientCertificaatOp(wsContext);
        Short partijId = null;

        try {
            if (null == bericht.getStuurgegevens() || null == bericht.getStuurgegevens().getOrganisatie()) {
                return null;
            }
            partijId = Short.valueOf(bericht.getStuurgegevens().getOrganisatie().getWaarde());
        } catch (NumberFormatException ne) {
            return null;
        }

        Authenticatiemiddel resultaat;
