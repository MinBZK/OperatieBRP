...
public class CliParser {
     ...
     */
    public OptionSet parse(String... args) throws RuntimeException {
    ...
    private OptionParser createParser() {
        ...
        optionParser.acceptsAll(asList("threads", "T"), "Aantal Thread").withRequiredArg().ofType(
                Integer.class).defaultsTo(5).describedAs("count");

        optionParser.acceptsAll(asList("bsn", "B"), "Aantal BSNs dat wordt opgehaald").withRequiredArg().ofType(
                Integer.class).defaultsTo(1000).describedAs("amount");
        ...
        optionParser.acceptsAll(asList("message", "m"), "Commentaar in het rapport").withRequiredArg().ofType(String.class).describedAs("ARG");
        optionParser.acceptsAll(asList("file", "F"), "Naam voor het rapport bestand").withRequiredArg().ofType(String.class).describedAs("ARG");
