/**
 * This file is copyright 2017 State of the Netherlands (Ministry of Interior Affairs and Kingdom Relations).
 * It is made available under the terms of the GNU Affero General Public License, version 3 as published by the Free Software Foundation.
 * The project of which this file is part, may be found at https://github.com/MinBZK/operatieBRP.
 */

package nl.bzk.copy.model.objecttype.bericht.basis;

import nl.bzk.copy.model.basis.AbstractObjectTypeBericht;
import nl.bzk.copy.model.groep.bericht.PersoonIndicatieStandaardGroepBericht;
import nl.bzk.copy.model.objecttype.bericht.PersoonBericht;
import nl.bzk.copy.model.objecttype.logisch.basis.PersoonIndicatiesBasis;

/**
 *
 */
public abstract class AbstractPersoonIndicatieBericht extends AbstractObjectTypeBericht
        implements PersoonIndicatiesBasis
{

    private PersoonBericht persoon;
    private PersoonIndicatieStandaardGroepBericht gegevens;

    public PersoonBericht getPersoon() {
        return persoon;
    }

    public void setPersoon(final PersoonBericht persoon) {
        this.persoon = persoon;
    }

    public PersoonIndicatieStandaardGroepBericht getGegevens() {
        return gegevens;
    }

    public void setGegevens(final PersoonIndicatieStandaardGroepBericht gegevens) {
        this.gegevens = gegevens;
    }
}
