...
public class CollecteerBsnStapTest {
    ...
    public void executeReturnsTheRequiredResults() throws Exception {
        ...
        context.put(ContextParameterNames.AANTAL_PERSOONSLIJSTEN, 5);
        ...
        assertThat(result.size(), is(greaterThanOrEqualTo(5)));

        verify(jdbcTemplate).queryForList(anyString(), argThat(new MapSqlParamterSourceMatcher()), eq(
                Integer.class));
    }
    ...
    public void executeAgainstAnEmptyDatabase() throws Exception {
        // given
        when(jdbcTemplate.queryForList(anyString(), argThat(new MapSqlParamterSourceMatcher()),
                eq(Integer.class))).thenReturn(Collections.<Integer>emptyList());
