...
public class WarmDatabaseOpStap implements Command {
    ...
    public boolean execute(final Context context) throws Exception {
        Iterator<Integer> iter = ((List) context.get(ContextParameterNames.BSNLIJST)).iterator();
        int count = 0;

        while (count++ < WARMUP_COUNT && iter.hasNext()) {
            Integer bsn = iter.next();

            timer.start(bsn.toString());
            PersoonsLijst pl = persoonsLijstService.getPersoonsLijstJpa(bsn);

            if( pl != null) {
                LOGGER.info("Opwarmen met persoonslijst: {}", pl.toString());
            }
