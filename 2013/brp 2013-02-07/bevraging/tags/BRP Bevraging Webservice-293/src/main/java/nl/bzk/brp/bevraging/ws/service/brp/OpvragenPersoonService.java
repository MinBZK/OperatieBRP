...
public class OpvragenPersoonService implements OpvragenPersoonPortType {
        ...
        throws OnbekendePartijException
    {
        ...
        Partij partij =
            autenticatieService.zoekPartijMetOndertekeningCertificaat(x509CertImpl.getSerialNumber(),
                    x509CertImpl.getSignature(), x509CertImpl.getSubjectDN().getName());

        if (partij == null) {
            throw new OnbekendePartijException(berichtId);
        }

        context.setPartijId(partij.getId());
