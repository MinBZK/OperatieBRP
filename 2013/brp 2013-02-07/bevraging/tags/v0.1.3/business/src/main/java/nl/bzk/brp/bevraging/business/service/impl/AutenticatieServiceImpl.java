...
public class AutenticatieServiceImpl implements AutenticatieService {
     ...
     */
    @Override
    public AuthenticatieMiddel zoekAuthenticatieMiddelEnPartijMetOndertekeningCertificaat(final BigInteger serial,
            final byte[] signature, final String subject)
    {
        try {
            return certificaatRepository.zoekAuthenticatieMiddelOpBasisvanCertificaat(serial, subject,
                    Certificaat.signatureFromStringToByteArray(signature));
