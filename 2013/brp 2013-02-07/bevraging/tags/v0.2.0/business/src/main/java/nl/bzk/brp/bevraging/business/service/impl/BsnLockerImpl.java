...
public class BsnLockerImpl implements BsnLocker {
            ...
            final Collection<Long> writeBsnLocks)
    {
        ...
        boolean result = BerichtVerwerkingsStap.STOP_VERWERKING;
        try {
            ...
            result = BerichtVerwerkingsStap.DOORGAAN_MET_VERWERKING;

        } catch (SQLException e) {
            LOGGER.error("Kan geen lock krijgen voor bericht " + berichtId, e);
            result = BerichtVerwerkingsStap.STOP_VERWERKING;
        }
        return result;
    }
     ...
     */
    private void voegBsnsToeAanLockTabel(final Long berichtId, final Collection<Long> readBsnLocks,
            final Collection<Long> writeBsnLocks) throws SQLException
    {
        Collection<Long> alleBSNs = voegBsnCollectiesSamen(readBsnLocks, writeBsnLocks);
    ...
    private Collection<Long> voegBsnCollectiesSamen(final Collection<Long> readBsnLocks,
            final Collection<Long> writeBsnLocks)
    {
        // CHECKSTYLE:OFF
     ...
     * en gesloten en de exceptie wordt opnieuw gegooid.
     * <br/><br/>
     ...
     * worden. Het systeem dient er voor te zorgen dat als deze methode wordt aangeroepen, ook altijd nog de {@link
     * #unLock()} methode wordt aangeroepen.
     *
     * @param berichtId het id van het bericht dat verwerkt wordt.
     * @param readBsnLocks BSNs die gelezen moeten worden.
     * @param writeBsnLocks BSNs die geschreven moeten worden.
     ...
     *             van de lock(s).
     */
    private void creeerConnectieVoorTransactieEnPlaatsLock(final Long berichtId, final Collection<Long> readBsnLocks,
            final Collection<Long> writeBsnLocks) throws SQLException
    {
        Connection connectie = dataSource.getConnection();
