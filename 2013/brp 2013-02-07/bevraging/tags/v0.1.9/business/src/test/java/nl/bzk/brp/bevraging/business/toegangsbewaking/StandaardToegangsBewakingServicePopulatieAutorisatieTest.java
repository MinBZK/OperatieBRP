...
public class StandaardToegangsBewakingServicePopulatieAutorisatieTest {

    @Mock
    private PopulatieAutorisatieRepository populatieAutorisatieRepository;
    ...
    public void testFoutVoorOngeldigCriteria() throws ParserException {
        Abonnement abonnement = getAbonnementOpSoortBerichten("xxxxxxx >", null);
    ...
    public void testEnkelePersonenToegangVoorEenGeldigCriteria() throws ParserException {
        Abonnement abonnement = getAbonnementOpSoortBerichten("Leeftijd >= 2", null);
    ...
    @Test(expected = ParserException.class)
    public void testFoutVoorEenGeldigCriteriaPlusOngeldigCriteria() throws ParserException {
        Abonnement abonnement = getAbonnementOpSoortBerichten("Leeftijd >= 2", "xxxxxxx >");
    ...
    public void init() {
        ...
        ReflectionTestUtils.setField(service, "populatieAutorisatieRepository", populatieAutorisatieRepository);

        Mockito.when(
                populatieAutorisatieRepository.filterPersonenBijFilter(Matchers.anyListOf(Long.class),
        ...
        Mockito.when(
                populatieAutorisatieRepository.filterPersonenBijFilter(Matchers.anyListOf(Long.class), Matchers.eq(""),
                        Matchers.notNull(String.class))).thenReturn(Arrays.asList(1L, 2L));
    }

    /**
     * Retourneert een abonnement, inclusief doelbinding, met de twee opgegeven populatie criterai.
     *
