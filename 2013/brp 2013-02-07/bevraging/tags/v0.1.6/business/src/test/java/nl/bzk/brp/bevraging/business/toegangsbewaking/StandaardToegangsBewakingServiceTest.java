...
public class StandaardToegangsBewakingServiceTest {
    ...
    public void testCorrectFunctioneelGeautoriseerd() {
        ...
        Assert.assertTrue(service.isFunctioneelGeautoriseerd(abonnement, getOpvragenPersoonBerichtCommand()));
    }

    @Test
    public void testNietFunctioneelGeautoriseerd() {
        Abonnement abonnement = getAbonnementOpSoortBerichten(SoortBericht.ORDINAL_NUL_NIET_GEBRUIKEN);

        Assert.assertFalse(service.isFunctioneelGeautoriseerd(abonnement, getOpvragenPersoonBerichtCommand()));
    }
    ...
    public void testInvalideCallMissendAbonnement() {
        service.isFunctioneelGeautoriseerd(null, getOpvragenPersoonBerichtCommand());
    }
    ...
    private Abonnement getAbonnementOpSoortBerichten(final SoortBericht... soortBerichten) {
        ...
        for (SoortBericht soort : soortBerichten) {
            abonnement.addSoortBericht(soort);
        }
        return abonnement;
    }

    /**
     * Retourneert een standaard (leeg) {@link OpvragenPersoonBerichtCommand}.
     *
     * @return een leeg {@link OpvragenPersoonBerichtCommand}.
     */
    private OpvragenPersoonBerichtCommand getOpvragenPersoonBerichtCommand() {
        return new OpvragenPersoonBerichtCommand(new PersoonZoekCriteria(), new BrpBerichtContext());
    }
