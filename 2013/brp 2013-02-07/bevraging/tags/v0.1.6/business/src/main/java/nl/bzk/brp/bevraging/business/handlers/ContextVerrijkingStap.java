...
public class ContextVerrijkingStap extends AbstractBerichtVerwerkingsStap {
    ...
    public final boolean voerVerwerkingsStapUitVoorBericht(final BrpBerichtCommand brpBerichtCommand) {
        ...
        BrpBerichtContext context = brpBerichtCommand.getContext();
        resultaat &= verrijkPartij(context.getPartijId(), brpBerichtCommand);
        resultaat &= verrijkAbonnement(context.getAbonnementId(), context.getPartijId(), brpBerichtCommand);
            ...
            final BrpBerichtCommand brpBerichtCommand)
    {
        ...
        List<Abonnement> abonnementen = abonnementRepository.findById(abonnementId);
        if (abonnementen.size() == 0) {
            brpBerichtCommand
                    .voegFoutToe(new BerichtVerwerkingsFout(
            ...
            if (abonnement.getStatusHistorie() != StatusHistorie.A) {
                brpBerichtCommand
                        .voegFoutToe(new BerichtVerwerkingsFout(
                                ...
                                BerichtVerwerkingsFoutZwaarte.FOUT));
            } else if (!abonnement.getDoelBinding().getGeautoriseerde().getId().equals(partijId)) {
