...
public class PersoonRepositoryJPATest extends AbstractRepositoryTestCase {
    ...
    public void testZoekOpBurgerservicenummer() {
        ...
        Assert.assertEquals(18890426, persoon.getDatumGeboorte().intValue());
         ...
         * Sparse indicaties.
         */
        Assert.assertFalse(persoon.getBehandeldAlsNederlander());
        Assert.assertNull(persoon.getBelemmeringVerstrekkingReisdocument());
        Assert.assertNull(persoon.getBezitBuitenlandsReisdocument());
        Assert.assertEquals(Boolean.FALSE, persoon.getIndicatieDeelnameEUVerkiezingen());
        Assert.assertTrue(persoon.getDerdeHeeftGezag());
        Assert.assertFalse(persoon.getGepriviligeerde());
        Assert.assertTrue(persoon.getOnderCuratele());
        Assert.assertEquals(Boolean.FALSE, persoon.getIndicatieUitsluitingNLKiesrecht());
        Assert.assertFalse(persoon.getVastgesteldNietNederlander());
        ...
        Assert.assertEquals(3, bijhoudingsGemeente.getID().longValue());
        Assert.assertEquals("0034", bijhoudingsGemeente.getGemeentecode());
        ...
        List<? extends PersoonAdres> adressen = persoon.getPersoonAdresen();
