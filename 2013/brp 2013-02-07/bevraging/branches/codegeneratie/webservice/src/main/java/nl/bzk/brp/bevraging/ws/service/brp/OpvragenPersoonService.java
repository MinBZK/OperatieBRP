...
public class OpvragenPersoonService implements OpvragenPersoonPortType {
    ...
    public final OpvragenPersoonAntwoord opvragenPersoon(final OpvragenPersoonVraag vraag) {
        ...
        try {
            ...
            if (BerichtIdsThreadLocal.getBerichtenIds() != null
                && BerichtIdsThreadLocal.getBerichtenIds().getIngaandBerichtId() != null)
            {
                berichtId = BerichtIdsThreadLocal.getBerichtenIds().getIngaandBerichtId().toString();
            }
    ...
    private BerichtContext getContext(final OpvragenPersoonVraag vraag) {
        ...
        context.setIngaandBerichtId(BerichtIdsThreadLocal.getBerichtenIds().getIngaandBerichtId());
        context.setUitgaandBerichtId(BerichtIdsThreadLocal.getBerichtenIds().getUitgaandBerichtId());
        context.setAbonnementId((int) vraag.getAbonnementId());
    ...
    private AuthenticatieMiddelDTO getAuthenticatieMiddelOpBasisVanCertificaat(final MessageContext messageContext) {
        ...
        X509Certificate x509CertImpl =
            (X509Certificate) wsSecurityEngineResult.get(WSSecurityEngineResult.TAG_X509_CERTIFICATE);

        return authenticatieService.zoekAuthenticatieMiddelEnPartijMetOndertekeningCertificaat(x509CertImpl
                .getSerialNumber().longValue(), x509CertImpl.getSignature(), x509CertImpl.getSubjectDN().getName());
    }
    ...
    private Collection<Persoon> mapGevondenPersonenNaarPersoonDTOs(final PersoonZoekCriteriaAntwoord personenAntwoord) {
        ...
        for (nl.bzk.brp.domein.kern.Persoon persoon : personenAntwoord.getPersonen()) {
     ...
     * @return het opgebouwde antwoord bericht.
     */
    private OpvragenPersoonAntwoord
            bouwAntwoord(final BerichtContext context, final List<BerichtVerwerkingsFout> fouten)
    {
