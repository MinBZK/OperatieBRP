...
public class ProtocolleringStapTest {
    ...
    public void testVoerVerwerkingsStapUitVoorBericht() {
        ...
        Mockito.when(berichtRepositoryMock.findOne(ingaandBerichtID)).thenReturn(ingaandBericht);
        Mockito.when(berichtRepositoryMock.findOne(uitgaandBerichtID)).thenReturn(uitgaandBericht);

        boolean resultaat =
            protocolleringStap.voerVerwerkingsStapUitVoorBericht(getVerzoek(now, "3", "5"),
                    getContext(ingaandBerichtID, uitgaandBerichtID, 2), getAntwoord(3L, 5L));
        ...
        Mockito.verify(abonnementRepositoryMock).findOne(4);
        Mockito.verify(authenticatieMiddelRepositoryMock).findOne(5);
        Mockito.verify(berichtRepositoryMock).findOne(ingaandBerichtID);
        ...
        Mockito.verify(berichtRepositoryMock).findOne(uitgaandBerichtID);
        ...
        assertEquals(uitgaandBericht, leveringCommunicatie.getValue().getUitgaandBericht());
     ...
     */
    private BerichtContext
            getContext(final Long ingaandBerichtId, final Long uitgaandBerichtId, final Integer partijId)
    {
    ...
    public final void initMocksEnStap() {
        ...
        ReflectionTestUtils.setField(executor, "authenticatieMiddelRepository", authenticatieMiddelRepositoryMock);
        ReflectionTestUtils.setField(executor, "abonnementRepository", abonnementRepositoryMock);
        ...
        ReflectionTestUtils.setField(executor, "berichtRepository", berichtRepositoryMock);
    }
}
