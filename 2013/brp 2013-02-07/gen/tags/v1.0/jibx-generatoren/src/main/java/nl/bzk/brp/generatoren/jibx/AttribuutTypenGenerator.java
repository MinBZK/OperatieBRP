...
public class AttribuutTypenGenerator extends AbstractJibxGenerator {
    ...
    private Mapping maakMapping(final AttribuutType attribuutType) {
        ...
        mapping.setTypeName(this.naamgevingStrategie.getJavaTypeVoorElement(attribuutType).getNaam());
        mapping.set_Class(this.naamgevingStrategie.getJavaTypeVoorElement(attribuutType).getFullyQualifiedClassName());
        ...
        if (isSpecialeConversieNodigTussenXmlEnJava) {
            ...
            mapping.set_Class("nl.bzk.brp.model.basis.AbstractAttribuutType");
            mapping.setObject(new _Object());
            mapping.getObject().setCreateType(
                    this.naamgevingStrategie.getJavaTypeVoorElement(attribuutType).getFullyQualifiedClassName());
            mapping.getChoiceList().add(maakValueChoice("text", null, "waarde", javaType.getFullyQualifiedClassName(),
            ...
            mapping.getChoiceList().add(this.maakStructureMappingChoice(null, null, mapAsType, false, false));
        }
        return mapping;
    }
}
