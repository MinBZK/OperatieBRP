 ...
 *
 * Aanname: de klasse 'AbstractAttribuutType' met het veld 'waarde' is vanuit het BRP model beschikbaar.
 */
...
public class BasisTypenGenerator extends AbstractJibxGenerator {
    ...
    private Mapping maakMapping(final BasisType basisType) {
        ...
        mapping.set_Class(GeneratiePackage.BRP_MODEL_BASIS_PACKAGE.createImportClass("AbstractAttribuutType"));
        String format = null;
        String type = basisType.getNaam();

        mapping.getChoiceList().add(this.maakValueChoice("text", null, "waarde", type, null, format, null, false));
        return mapping;
    }
