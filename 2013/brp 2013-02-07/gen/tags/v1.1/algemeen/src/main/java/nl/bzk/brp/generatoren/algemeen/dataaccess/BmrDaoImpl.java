...
public class BmrDaoImpl implements BmrDao {
    ...
    private static final int ID_ATTRIBUUT_TYPE_STATUSHIS = 1000000001;
    ...
    public List<ObjectType> getObjectTypen(final BmrElementFilterObject filter) {
        String queryTemplate = "SELECT ot FROM ObjectType ot WHERE %s";
        TypedQuery<ObjectType> query = bouwQueryMetTemplateEnFilter(queryTemplate, filter, "ot", ObjectType.class);
        ...
        final boolean inclusiefGroepenUitSupertypes)
    {
        ...
        if (inclusiefGroepenUitSupertypes) {
            ...
            if (supertype != null) {
                List<Groep> supergroepen = getGroepenVoorObjectType(supertype, true);
        ...
        final BmrTargetPlatform platform)
    {
        ...
        boolean filterOpAantalBytes = filterAantalBytes != null;
        ...
        TypedQuery<BasisTypeOverrulePaar> query = em.createQuery(sql, BasisTypeOverrulePaar.class).
            setParameter("elementId", elementId).
                                                        setParameter("platformId", platform.getBmrId());
        if (filterOpAantalBytes) {
            query.setParameter("filterAantalBytes", filterAantalBytes);
        }
        List<BasisTypeOverrulePaar> resultaatParen = query.getResultList();
    ...
    /** {@inheritDoc} */
    @Override
    public List<Attribuut> getStatusHistorieAttributenVanObjectType(final ObjectType objectType) {
        // Laat de sync via de originele groep zijn object type lopen ipv direct via object type,
        // zodat de originele (logische) hierarchie in acht genomen wordt.
        final String sql = "SELECT attr FROM Attribuut attr, Groep groep "
            + "WHERE attr.elementByLaag.id = :operationeleLaag "
            + "AND attr.type.id = :statusHisType "
            + "AND attr.orgSyncid = groep.syncid "
            + "AND groep.elementByLaag.id = :logischeLaag "
            + "AND groep.objectType.id = :id";
        return em.createQuery(sql, Attribuut.class)
                 .setParameter("operationeleLaag", BmrLaag.OPERATIONEEL.getWaardeInBmr())
                 .setParameter("logischeLaag", BmrLaag.LOGISCH.getWaardeInBmr())
                 .setParameter("id", objectType.getId())
                 .setParameter("statusHisType", ID_ATTRIBUUT_TYPE_STATUSHIS)
                 .getResultList();
    }
        ...
        final int syncid, final BmrLaag laag, final Class<T> type)
    {
        ...
        Root<T> elementRoot = criteriaQuery.from(type);
    ...
    /** {@inheritDoc} */
    @Override
    public Attribuut getStatusHisAttribuutVoorGroep(final Groep groep) {
        final String sql = "SELECT statusHisAttribuut FROM Attribuut statusHisAttribuut "
            + "WHERE statusHisAttribuut.orgSyncid = :syncid "
            + "AND statusHisAttribuut.type.id = :statusHisType";
        List<Attribuut> results = em.createQuery(sql, Attribuut.class)
                                    .setParameter("syncid", groep.getSyncid())
                                    .setParameter("statusHisType", ID_ATTRIBUUT_TYPE_STATUSHIS)
                                    .getResultList();
        Attribuut statusHisAttribuut = null;
        if (results.size() == 1) {
            statusHisAttribuut = results.get(0);
        } else if (results.size() > 1) {
            throw new IllegalStateException("Meerdere status his attributen gevonden voor groep: " + groep);
        }
        return statusHisAttribuut;
    }
