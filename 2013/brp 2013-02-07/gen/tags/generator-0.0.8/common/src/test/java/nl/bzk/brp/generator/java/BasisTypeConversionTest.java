...
public class BasisTypeConversionTest {
    ...
    public void shouldConvertAllBasisTypes() {
        for (BasisType basisType : BasisType.values()) {
            logger.info("Basistype '" + basisType.getBasisType() + "' omgezet naar " + DataTypeTranslator.getImportForBasisType(basisType.getBasisType()));
        }
    ...
    public void shouldConvertToImportStatement() {
        Assert.assertEquals("ID Groot moet worden gemapped naar BigInteger maar dat is niet zo!","java.math.BigInteger", DataTypeTranslator.getImportForBasisType(BasisType.ID_GROOT.getBasisType()));
    }
