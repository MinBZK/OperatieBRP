...
public abstract class AbstractJavaGenerator<T extends GeneriekElement> {
    ...
    //TODO: Dit zou er niet moeten zijn!
    private static final int SYNC_ID_MULTI_REALITEIT_REGEL_IDENTITEIT = 2080;
    ...
    protected List<Groep> getGroepen() {
        List<Groep> resultaat = new ArrayList<Groep>();
        List<Groep> groepen = groepDao.getDynamischHistorisch();
        for (Groep groep : groepen) {
            if (!groep.getSyncid().equals(SYNC_ID_MULTI_REALITEIT_REGEL_IDENTITEIT)) {
                resultaat.add(groep);
            }
        }
        return resultaat;
    ...
    protected String formatteerGroep(final String format, final Groep groep) {
        return String.format(format, groep.getObjectType().getIdentCode(), groep.getIdentCode());
    }

    protected String getGroepLogisch(final Groep groep) {
        return formatteerGroep("%s%sGroep", groep);
    }

    protected String getGroepLogischBasis(final Groep groep) {
        return formatteerGroep("%s%sGroepBasis", groep);
    }

    protected String getGroepOperationeelActueel(final Groep groep) {
