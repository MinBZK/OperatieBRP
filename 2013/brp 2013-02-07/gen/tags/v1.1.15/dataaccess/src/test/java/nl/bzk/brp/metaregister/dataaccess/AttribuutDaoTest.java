...
public class AttribuutDaoTest extends AbstractRepositoryTestCase {
    ...
    @Test
    public void testGetStatusHisAttributen() {
        ObjectType authorisatieBesluit = objectTypeDao.getBySyncId(4852);

        // test
        Iterator<Attribuut> attributen = attribuutDao.getStatusHistorieAttributen(authorisatieBesluit).iterator();

        Assert.assertEquals("AutorisatiebesluitStatusHis", attributen.next().getIdentDb());
        Assert.assertEquals("BijhautorisatiebesluitStatus", attributen.next().getIdentDb());
        Assert.assertFalse(attributen.hasNext());
    }
