...
public class PersoonnationaliteitNationaliteitGetter implements AttributeGetter {
    ...
    /**
     * Geeft de waarde van attribuut 'nationaliteit' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'nationaliteit' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getNationaliteiten() !=
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getNationaliteit().getCode().getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'nationaliteit' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
