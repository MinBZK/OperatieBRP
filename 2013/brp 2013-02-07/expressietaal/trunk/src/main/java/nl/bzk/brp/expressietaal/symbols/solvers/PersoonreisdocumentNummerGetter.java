...
/**
 * Getter voor 'nummer' in objecttype 'PersoonReisdocument'.
 * <p/>
 ...
 */
public class PersoonreisdocumentNummerGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'nummer' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'nummer' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getReisdocumenten() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getStandaard().getNummer().getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'nummer' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
