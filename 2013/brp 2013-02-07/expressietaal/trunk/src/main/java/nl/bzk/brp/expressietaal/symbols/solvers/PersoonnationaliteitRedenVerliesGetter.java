...
/**
 * Getter voor 'reden_verlies' in objecttype 'PersoonNationaliteit'.
 * <p/>
 ...
 */
public class PersoonnationaliteitRedenVerliesGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'reden_verlies' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'reden_verlies' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getNationaliteiten() !=
                null) {
            List<PersoonNationaliteit> collectie = new ArrayList<PersoonNationaliteit>();
            collectie.addAll(((Persoon) rootObject).getNationaliteiten());
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getStandaard().getRedenVerlies().getCode()
                        .getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'reden_verlies' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
    ...
    public Integer getMaxIndex(final RootObject rootObject) {
        return ((Persoon) rootObject).getNationaliteiten().size();
    }
}
