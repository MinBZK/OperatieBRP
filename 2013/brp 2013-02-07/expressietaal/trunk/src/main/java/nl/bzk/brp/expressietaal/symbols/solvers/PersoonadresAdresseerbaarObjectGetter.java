...
/**
 * Getter voor 'adresseerbaar_object' in objecttype 'PersoonAdres'.
 * <p/>
 ...
 */
public class PersoonadresAdresseerbaarObjectGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'adresseerbaar_object' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'adresseerbaar_object' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getAdressen() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getStandaard().getAdresseerbaarObject()
                        .getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'adresseerbaar_object' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
