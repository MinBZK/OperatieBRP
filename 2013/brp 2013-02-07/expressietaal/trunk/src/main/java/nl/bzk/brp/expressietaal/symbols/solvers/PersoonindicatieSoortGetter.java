...
/**
 * Getter voor 'soort' in objecttype 'PersoonIndicatie'.
 * <p/>
 ...
 */
public class PersoonindicatieSoortGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'soort' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'soort' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getIndicaties() != null) {
            List<PersoonIndicatie> collectie = new ArrayList<PersoonIndicatie>();
            collectie.addAll(((Persoon) rootObject).getIndicaties());
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getSoort().getNaam());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'soort' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
    ...
    public Integer getMaxIndex(final RootObject rootObject) {
        return ((Persoon) rootObject).getIndicaties().size();
    }
}
