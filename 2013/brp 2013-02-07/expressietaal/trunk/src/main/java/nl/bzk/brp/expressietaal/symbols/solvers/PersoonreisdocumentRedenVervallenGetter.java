...
/**
 * Getter voor 'reden_vervallen' in objecttype 'PersoonReisdocument'.
 * <p/>
 ...
 */
public class PersoonreisdocumentRedenVervallenGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'reden_vervallen' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'reden_vervallen' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getReisdocumenten() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getStandaard().getRedenVervallen().getCode()
                        .getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'reden_vervallen' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
