...
/**
 * Getter voor 'locatie_omschrijving' in objecttype 'PersoonAdres'.
 * <p/>
 ...
 */
public class PersoonadresLocatieOmschrijvingGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'locatie_omschrijving' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'locatie_omschrijving' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getAdressen() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getStandaard().getLocatieOmschrijving()
                        .getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'locatie_omschrijving' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
