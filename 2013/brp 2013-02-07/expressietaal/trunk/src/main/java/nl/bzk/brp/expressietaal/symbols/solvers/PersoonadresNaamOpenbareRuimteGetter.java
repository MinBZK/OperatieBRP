...
/**
 * Getter voor 'naam_openbare_ruimte' in objecttype 'PersoonAdres'.
 * <p/>
 ...
 */
public class PersoonadresNaamOpenbareRuimteGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'naam_openbare_ruimte' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'naam_openbare_ruimte' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getAdressen() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getStandaard().getNaamOpenbareRuimte()
                        .getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'naam_openbare_ruimte' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
