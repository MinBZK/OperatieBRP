...
/**
 * Getter voor 'huisletter' in objecttype 'PersoonAdres'.
 * <p/>
 ...
 */
public class PersoonadresHuisletterGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'huisletter' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'huisletter' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getAdressen() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getStandaard().getHuisletter().getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'huisletter' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
