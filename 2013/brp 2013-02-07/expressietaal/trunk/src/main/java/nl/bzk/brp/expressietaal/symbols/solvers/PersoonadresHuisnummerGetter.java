...
/**
 * Getter voor 'huisnummer' in objecttype 'PersoonAdres'.
 * <p/>
 ...
 */
public class PersoonadresHuisnummerGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'huisnummer' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'huisnummer' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getAdressen() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new NumberLiteralExpressie(collectie.get(index - 1).getStandaard().getHuisnummer().getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'huisnummer' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
