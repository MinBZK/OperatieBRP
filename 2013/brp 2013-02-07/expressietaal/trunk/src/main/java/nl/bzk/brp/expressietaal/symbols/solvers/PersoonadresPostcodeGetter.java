...
/**
 * Getter voor 'postcode' in objecttype 'PersoonAdres'.
 * <p/>
 ...
 */
public class PersoonadresPostcodeGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'postcode' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'postcode' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getAdressen() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getStandaard().getPostcode().getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'postcode' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
