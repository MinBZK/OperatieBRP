...
/**
 * Getter voor 'woonplaats' in objecttype 'PersoonAdres'.
 * <p/>
 ...
 */
public class PersoonadresWoonplaatsGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'woonplaats' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'woonplaats' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getAdressen() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getStandaard().getWoonplaats().getCode()
                        .getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'woonplaats' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
