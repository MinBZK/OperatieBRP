...
/**
 * Getter voor 'volgnummer' in objecttype 'PersoonVoornaam'.
 * <p/>
 ...
 */
public class PersoonvoornaamVolgnummerGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'volgnummer' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'volgnummer' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getVoornamen() != null) {
            List<PersoonVoornaam> collectie = new ArrayList<PersoonVoornaam>();
            collectie.addAll(((Persoon) rootObject).getVoornamen());
            if (index >= 1 && index <= collectie.size()) {
                return new NumberLiteralExpressie(collectie.get(index - 1).getVolgnummer().getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'volgnummer' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
    ...
    public Integer getMaxIndex(final RootObject rootObject) {
        return ((Persoon) rootObject).getVoornamen().size();
    }
}
