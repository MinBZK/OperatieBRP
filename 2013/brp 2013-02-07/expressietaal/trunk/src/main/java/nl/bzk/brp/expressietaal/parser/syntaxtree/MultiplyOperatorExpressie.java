...
/**
 * Representeert een numerieke vermenigvuldigingsexpressie.
 */
public class MultiplyOperatorExpressie extends AbstractNumericOperatorExpressie {
     ...
     */
    public MultiplyOperatorExpressie(final Expressie termLinks, final Expressie termRechts) {
    ...
    protected final Expressie create(final Expressie operatorTermLinks, final Expressie operatorTermRechts) {
        return new MultiplyOperatorExpressie(operatorTermLinks, operatorTermRechts);
    }
    ...
    public final Expressie optimaliseer() {
        ...
        if (gereduceerdeTermLinks.isConstanteWaarde() && gereduceerdeTermLinks.getType() == ExpressieType.NUMBER) {
            ...
            } else if (((NumberLiteralExpressie) gereduceerdeTermLinks).getValue() == 1) {
                exp = gereduceerdeTermRechts;
            }
        } else if (gereduceerdeTermRechts.isConstanteWaarde() && gereduceerdeTermRechts.getType() == ExpressieType
                .NUMBER)
        {
            if (((NumberLiteralExpressie) gereduceerdeTermRechts).getValue() == 0) {
                exp = new NumberLiteralExpressie(0);
            } else if (((NumberLiteralExpressie) gereduceerdeTermRechts).getValue() == 1) {
