...
public class PersoonadresRedenWijzigingGetter implements AttributeGetter {
    ...
    /**
     * Geeft de waarde van attribuut 'reden_wijziging' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'reden_wijziging' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getAdressen() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getStandaard().getRedenWijziging().getCode()
                        .getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'reden_wijziging' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
