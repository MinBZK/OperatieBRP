...
/**
 * Getter voor 'datum_vertrek_uit_nederland' in objecttype 'PersoonAdres'.
 * <p/>
 ...
 */
public class PersoonadresDatumVertrekUitNederlandGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'datum_vertrek_uit_nederland' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'datum_vertrek_uit_nederland' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getAdressen() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new DateLiteralExpressie(collectie.get(index - 1).getStandaard().getDatumVertrekUitNederland()
                        .getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'datum_vertrek_uit_nederland' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
