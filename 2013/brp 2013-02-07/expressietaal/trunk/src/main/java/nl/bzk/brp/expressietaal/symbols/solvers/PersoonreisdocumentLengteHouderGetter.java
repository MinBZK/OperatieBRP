...
/**
 * Getter voor 'lengte_houder' in objecttype 'PersoonReisdocument'.
 * <p/>
 ...
 */
public class PersoonreisdocumentLengteHouderGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'lengte_houder' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'lengte_houder' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getReisdocumenten() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new NumberLiteralExpressie(collectie.get(index - 1).getStandaard().getLengteHouder().getWaarde
                        ());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'lengte_houder' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
