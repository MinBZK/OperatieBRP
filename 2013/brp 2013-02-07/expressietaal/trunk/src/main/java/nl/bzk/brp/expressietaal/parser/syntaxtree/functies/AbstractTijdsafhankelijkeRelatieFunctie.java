...
/**
 * Representeert een functie die een lijst van een gespecificeerde soort tijdsafhankelijke relaties van een persoon
 * teruggeeft. Voorbeeld van een dergelijke relatie is Huwelijk.
 */
public abstract class AbstractTijdsafhankelijkeRelatieFunctie extends AbstractRelatieFunctie {
     ...
     */
    protected AbstractTijdsafhankelijkeRelatieFunctie(final Keywords functie, final ExpressieType typeElementen,
    ...
    protected Expressie calculate(final List<Expressie> gereduceerdeArgumenten, final Context context) {
        if (gereduceerdeArgumenten.size() == 2) {
            EvaluatieResultaat rootObject = gereduceerdeArgumenten.get(0).evalueer(context);
            EvaluatieResultaat datum = gereduceerdeArgumenten.get(1).evalueer(context);
            if (rootObject.succes() && rootObject.getExpressie().getType() == ExpressieType.PERSOON
                    && datum.succes() && datum.getExpressie().getType() == ExpressieType.DATE)
            {
                Persoon persoon = (Persoon) ((RootObjectExpressie) rootObject.getExpressie()).getRootObject();
    ...
    /**
     * Geeft TRUE als de relatie geldig is op de gegeven datum.
     *
     * @param relatie Te beoordelen relatie.
     * @param datum   Te beoordelen datum.
     * @return TRUE als de relatie geldig is op de gegeven datum.
     */
    protected abstract boolean datumMatch(Relatie relatie, DateTime datum);
}
