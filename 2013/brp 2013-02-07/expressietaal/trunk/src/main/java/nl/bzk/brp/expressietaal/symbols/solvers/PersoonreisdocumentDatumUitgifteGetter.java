...
/**
 * Getter voor 'datum_uitgifte' in objecttype 'PersoonReisdocument'.
 * <p/>
 ...
 */
public class PersoonreisdocumentDatumUitgifteGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'datum_uitgifte' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'datum_uitgifte' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getReisdocumenten() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new DateLiteralExpressie(collectie.get(index - 1).getStandaard().getDatumUitgifte().getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'datum_uitgifte' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
