...
/**
 * Getter voor 'autoriteit_van_afgifte' in objecttype 'PersoonReisdocument'.
 * <p/>
 ...
 */
public class PersoonreisdocumentAutoriteitVanAfgifteGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'autoriteit_van_afgifte' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'autoriteit_van_afgifte' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getReisdocumenten() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getStandaard().getAutoriteitVanAfgifte()
                        .getCode().getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'autoriteit_van_afgifte' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
