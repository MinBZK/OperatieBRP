...
/**
 * Getter voor 'locatie_tov_adres' in objecttype 'PersoonAdres'.
 * <p/>
 ...
 */
public class PersoonadresLocatieTovAdresGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'locatie_tov_adres' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'locatie_tov_adres' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getAdressen() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getStandaard().getLocatietovAdres()
                        .getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'locatie_tov_adres' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
