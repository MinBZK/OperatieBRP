...
/**
 * Getter voor 'datum_aanvang_adreshouding' in objecttype 'PersoonAdres'.
 * <p/>
 ...
 */
public class PersoonadresDatumAanvangAdreshoudingGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'datum_aanvang_adreshouding' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'datum_aanvang_adreshouding' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getAdressen() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new DateLiteralExpressie(collectie.get(index - 1).getStandaard().getDatumAanvangAdreshouding()
                        .getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'datum_aanvang_adreshouding' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
