...
/**
 * Getter voor 'naam' in objecttype 'PersoonVoornaam'.
 * <p/>
 ...
 */
public class PersoonvoornaamNaamGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'naam' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'naam' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getVoornamen() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getStandaard().getNaam().getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'naam' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
