...
public class PersoonnationaliteitRedenVerkrijgingGetter implements AttributeGetter {
    ...
    /**
     * Geeft de waarde van attribuut 'reden_verkrijging' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'reden_verkrijging' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getNationaliteiten() !=
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getStandaard().getRedenVerkrijging()
                        .getCode().getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'reden_verkrijging' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
