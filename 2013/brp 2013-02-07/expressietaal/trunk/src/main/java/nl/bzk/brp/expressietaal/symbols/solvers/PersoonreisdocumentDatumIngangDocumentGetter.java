...
/**
 * Getter voor 'datum_ingang_document' in objecttype 'PersoonReisdocument'.
 * <p/>
 ...
 */
public class PersoonreisdocumentDatumIngangDocumentGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'datum_ingang_document' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'datum_ingang_document' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getReisdocumenten() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new DateLiteralExpressie(collectie.get(index - 1).getStandaard().getDatumIngangDocument()
                        .getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'datum_ingang_document' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
