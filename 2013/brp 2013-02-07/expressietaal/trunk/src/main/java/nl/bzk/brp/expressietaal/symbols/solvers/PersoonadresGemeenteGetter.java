...
public class PersoonadresGemeenteGetter implements AttributeGetter {
    ...
    /**
     * Geeft de waarde van attribuut 'gemeente' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'gemeente' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getAdressen() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getStandaard().getGemeente().getCode()
                        .getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'gemeente' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
