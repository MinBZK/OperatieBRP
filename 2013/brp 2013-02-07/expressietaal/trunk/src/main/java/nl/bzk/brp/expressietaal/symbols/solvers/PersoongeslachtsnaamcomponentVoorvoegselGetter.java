...
public class PersoongeslachtsnaamcomponentVoorvoegselGetter implements AttributeGetter {
    ...
    /**
     * Geeft de waarde van attribuut 'voorvoegsel' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'voorvoegsel' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
                ...
                && ((Persoon) rootObject).getGeslachtsnaamcomponenten() != null)
        {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getStandaard().getVoorvoegsel().getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'voorvoegsel' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
