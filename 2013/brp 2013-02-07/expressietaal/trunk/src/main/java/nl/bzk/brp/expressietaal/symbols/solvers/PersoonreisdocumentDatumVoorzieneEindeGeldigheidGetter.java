...
/**
 * Getter voor 'datum_voorziene_einde_geldigheid' in objecttype 'PersoonReisdocument'.
 * <p/>
 ...
 */
public class PersoonreisdocumentDatumVoorzieneEindeGeldigheidGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'datum_voorziene_einde_geldigheid' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'datum_voorziene_einde_geldigheid' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getReisdocumenten() != null) {
            List<PersoonReisdocument> collectie = new ArrayList<PersoonReisdocument>();
            collectie.addAll(((Persoon) rootObject).getReisdocumenten());
            if (index >= 1 && index <= collectie.size()) {
                return new DateLiteralExpressie(collectie.get(index - 1).getStandaard()
                        .getDatumVoorzieneEindeGeldigheid().getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'datum_voorziene_einde_geldigheid' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
