...
public class PersoonindicatieWaardeGetter implements AttributeGetter {
    ...
    /**
     * Geeft de waarde van attribuut 'waarde' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'waarde' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getIndicaties() != null) {
            ...
            if (index >= 1 && index <= collectie.size()) {
                return new BooleanLiteralExpressie(collectie.get(index - 1).getStandaard().getWaarde().getWaarde());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'waarde' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
