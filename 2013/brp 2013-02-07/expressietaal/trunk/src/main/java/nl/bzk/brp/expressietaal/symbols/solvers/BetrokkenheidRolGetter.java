...
/**
 * Getter voor 'rol' in objecttype 'Betrokkenheid'.
 * <p/>
 ...
 */
public class BetrokkenheidRolGetter implements AttributeGetter {

    /**
     * Geeft de waarde van attribuut 'rol' uit het gegeven (root)object.
     *
    ...
    /**
     * Geeft de waarde van attribuut 'rol' uit een bepaald
    ...
    public Expressie getAttribuutWaarde(final RootObject rootObject, final Integer index) {
        if (rootObject != null && rootObject instanceof Persoon && ((Persoon) rootObject).getBetrokkenheden() != null) {
            List<Betrokkenheid> collectie = new ArrayList<Betrokkenheid>();
            collectie.addAll(((Persoon) rootObject).getBetrokkenheden());
            if (index >= 1 && index <= collectie.size()) {
                return new StringLiteralExpressie(collectie.get(index - 1).getRol().getCode());
            }
        }
    ...
    /**
     * Geeft de hoogste indexwaarde voor 'rol' uit een bepaald
     * geïndiceerd attribuut van een (root)object.
    ...
    public Integer getMaxIndex(final RootObject rootObject) {
        return ((Persoon) rootObject).getBetrokkenheden().size();
    }
}
