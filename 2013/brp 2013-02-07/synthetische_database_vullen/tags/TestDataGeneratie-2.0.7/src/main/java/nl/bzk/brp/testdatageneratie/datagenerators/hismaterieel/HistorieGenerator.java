...
public abstract class HistorieGenerator<A, H extends HisMaterieel> {
    ...
    public List<H> generateHisMaterieels(final A actueel, final Integer dataanvHuidig) {
        ...
        for (int i = 0; i < aantalRecordsExclusiefHuidig; i++) {
            Date tsRegVroeger =
                RandomService.getPastTimestamp(tsReg.getTime() - RandomService.DAG_IN_MS, getMaxTerugInMs());
            ...
            hisVervallen.setActieByActieverval(RandomService.getActie());
    ...
    protected abstract H generateHisMaterieel(final A actueel);
