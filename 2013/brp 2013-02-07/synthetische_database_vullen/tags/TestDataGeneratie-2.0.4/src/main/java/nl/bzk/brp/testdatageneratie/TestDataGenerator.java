...
public class TestDataGenerator implements Callable<Boolean> {
    ...
    private void voegPersoonToe(final Session kernSession) {
        ...
        if (Settings.SAVE) {
            ...
            kernSession.save(new HisPersinschr(pers));
            if (pers.getVerantwoordelijke() != null) kernSession.save(new HisPersbijhverantwoordelijk(pers));
            ...
            if (pers.getVerblijfsr() != null) kernSession.save(new HisPersverblijfsr(pers));
        }
    ...
    private void voegIndicatiesToe(final Session kernSession, final Pers pers, final boolean isStatenloos) {
        ...
        for (Persindicatie indicatie : indicaties) {
            if (Settings.SAVE) {
                ...
                HisPersindicatie hisPersIndicatieActueel = new HisPersindicatie(indicatie);
                if (isFractie(3)) {
                    Date wijzigmoment = RandomService.getPastTimestamp();
                    hisPersIndicatieActueel.setDateindegel(GenUtil.naarBrpDatum(wijzigmoment));

                    HisPersindicatie hisPersIndicatieOud = new HisPersindicatie(indicatie);
                    hisPersIndicatieOud.setTsverval(wijzigmoment);
                    if (Settings.SAVE) kernSession.save(hisPersIndicatieOud);
                }
                kernSession.save(hisPersIndicatieActueel);
            }
        }
    }
    ...
    private List<Persindicatie> creeerIndicaties(final Pers pers, final boolean isStatenloos) {
        ...
        if (isFractie(21)) {
            if (isFractie(2)) {
                ...
                resultaat.add(creeerIndicatie(pers, Srtindicatie.ONDER_CURATELE));
                indicatieSet.remove(Srtindicatie.ONDER_CURATELE);
            }
        }
