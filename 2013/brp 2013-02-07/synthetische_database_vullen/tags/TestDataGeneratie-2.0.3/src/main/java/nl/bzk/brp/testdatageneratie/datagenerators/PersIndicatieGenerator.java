/**
 * This file is copyright 2017 State of the Netherlands (Ministry of Interior Affairs and Kingdom Relations).
 * It is made available under the terms of the GNU Affero General Public License, version 3 as published by the Free Software Foundation.
 * The project of which this file is part, may be found at https://github.com/MinBZK/operatieBRP.
 */

package nl.bzk.brp.testdatageneratie.datagenerators;

import nl.bzk.brp.testdatageneratie.domain.kern.Pers;
import nl.bzk.brp.testdatageneratie.domain.kern.Persindicatie;
import nl.bzk.brp.testdatageneratie.domain.kern.Srtindicatie;


public class PersIndicatieGenerator {

    public static Persindicatie creeerIndicatie(final Pers pers, final Srtindicatie soort) {
        Persindicatie persindicatie = new Persindicatie();
        persindicatie.setSrtindicatie(soort);
        persindicatie.setWaarde(true);
        persindicatie.setPers(pers);
        return persindicatie;
    }
}
