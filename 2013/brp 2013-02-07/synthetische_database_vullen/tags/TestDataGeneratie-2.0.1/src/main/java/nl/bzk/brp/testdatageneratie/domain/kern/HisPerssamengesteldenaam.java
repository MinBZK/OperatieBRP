/**
 * This file is copyright 2017 State of the Netherlands (Ministry of Interior Affairs and Kingdom Relations).
 * It is made available under the terms of the GNU Affero General Public License, version 3 as published by the Free Software Foundation.
 * The project of which this file is part, may be found at https://github.com/MinBZK/operatieBRP.
 */

package nl.bzk.brp.testdatageneratie.domain.kern;
// Generated 12-sep-2012 11:17:43 by Hibernate Tools 3.2.4.GA



/**
 * HisPerssamengesteldenaam generated by hbm2java
 */
public class HisPerssamengesteldenaam extends His {


     private Pers pers;
     private Predikaat predikaat;
     private Actie actieByActieaanpgel;
     private Adellijketitel adellijketitel;
     private String voornamen;
     private String voorvoegsel;
     private String scheidingsteken;
     private String geslnaam;
     private boolean indnreeksalsgeslnaam;
     private boolean indalgoritmischafgeleid;

    public HisPerssamengesteldenaam() {
    }

    public HisPerssamengesteldenaam(final Pers pers) {
        this.pers = pers;
        pers.setSamengesteldenaamstatushis(StatusHis.A);
        geslnaam = pers.getGeslnaam();
        indalgoritmischafgeleid = pers.getIndalgoritmischafgeleid();
        predikaat = pers.getPredikaatByPredikaat();
        adellijketitel = pers.getAdellijketitel();
        indnreeksalsgeslnaam = pers.getIndnreeksalsgeslnaam();
        voornamen = pers.getVoornamen();
        voorvoegsel = pers.getVoorvoegsel();
        scheidingsteken = pers.getScheidingsteken();
    }
   
    public Pers getPers() {
        return this.pers;
    }
    
    public void setPers(Pers pers) {
        this.pers = pers;
    }
    public Predikaat getPredikaat() {
        return this.predikaat;
    }
    
    public void setPredikaat(Predikaat predikaat) {
        this.predikaat = predikaat;
    }
    public Actie getActieByActieaanpgel() {
        return this.actieByActieaanpgel;
    }
    
    public void setActieByActieaanpgel(Actie actieByActieaanpgel) {
        this.actieByActieaanpgel = actieByActieaanpgel;
    }
    public Adellijketitel getAdellijketitel() {
        return this.adellijketitel;
    }
    
    public void setAdellijketitel(Adellijketitel adellijketitel) {
        this.adellijketitel = adellijketitel;
    }
    public String getVoornamen() {
        return this.voornamen;
    }
    
    public void setVoornamen(String voornamen) {
        this.voornamen = voornamen;
    }
    public String getVoorvoegsel() {
        return this.voorvoegsel;
    }
    
    public void setVoorvoegsel(String voorvoegsel) {
        this.voorvoegsel = voorvoegsel;
    }
    public String getScheidingsteken() {
        return this.scheidingsteken;
    }
    
    public void setScheidingsteken(String scheidingsteken) {
        this.scheidingsteken = scheidingsteken;
    }
    public String getGeslnaam() {
        return this.geslnaam;
    }
    
    public void setGeslnaam(String geslnaam) {
        this.geslnaam = geslnaam;
    }
    public boolean isIndnreeksalsgeslnaam() {
        return this.indnreeksalsgeslnaam;
    }
    
    public void setIndnreeksalsgeslnaam(boolean indnreeksalsgeslnaam) {
        this.indnreeksalsgeslnaam = indnreeksalsgeslnaam;
    }
    public boolean isIndalgoritmischafgeleid() {
        return this.indalgoritmischafgeleid;
    }
    
    public void setIndalgoritmischafgeleid(boolean indalgoritmischafgeleid) {
        this.indalgoritmischafgeleid = indalgoritmischafgeleid;
    }




}


