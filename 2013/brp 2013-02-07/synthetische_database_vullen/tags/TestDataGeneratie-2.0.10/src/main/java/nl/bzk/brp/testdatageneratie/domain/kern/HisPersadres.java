/**
 * This file is copyright 2017 State of the Netherlands (Ministry of Interior Affairs and Kingdom Relations).
 * It is made available under the terms of the GNU Affero General Public License, version 3 as published by the Free Software Foundation.
 * The project of which this file is part, may be found at https://github.com/MinBZK/operatieBRP.
 */

package nl.bzk.brp.testdatageneratie.domain.kern;
// Generated 12-sep-2012 11:17:43 by Hibernate Tools 3.2.4.GA



/**
 * HisPersadres generated by hbm2java
 */
public class HisPersadres extends HisMaterieel {


     private Land land;
     private Functieadres functieadres;
     private Rdnwijzadres rdnwijzadres;
     private Partij partij;
     private Aangadresh aangadresh;
     private Plaats plaats;
     private Persadres persadres;
     private Integer dataanvadresh;
     private String adresseerbaarobject;
     private String identcodenraand;
     private String nor;
     private String afgekortenor;
     private String gemdeel;
     private Integer huisnr;
     private String huisletter;
     private String huisnrtoevoeging;
     private String postcode;
     private String loctovadres;
     private String locoms;
     private String bladresregel1;
     private String bladresregel2;
     private String bladresregel3;
     private String bladresregel4;
     private String bladresregel5;
     private String bladresregel6;
     private Integer datvertrekuitnederland;

    public HisPersadres() {
    }

    public HisPersadres(final Persadres persadres) {
        this.persadres = persadres;
        functieadres = persadres.getFunctieadres();
        dataanvadresh = persadres.getDataanvadresh();
        land = persadres.getLand();
        aangadresh = persadres.getAangadresh();
        adresseerbaarobject = persadres.getAdresseerbaarobject();
        afgekortenor = persadres.getAfgekortenor();
        bladresregel1 = persadres.getBladresregel1();
        bladresregel2 = persadres.getBladresregel2();
        bladresregel3 = persadres.getBladresregel3();
        bladresregel4 = persadres.getBladresregel4();
        bladresregel5 = persadres.getBladresregel5();
        bladresregel6 = persadres.getBladresregel6();
        datvertrekuitnederland = persadres.getDatvertrekuitnederland();
        gemdeel = persadres.getGemdeel();
        huisletter = persadres.getHuisletter();
        huisnr = persadres.getHuisnr();
        huisnrtoevoeging = persadres.getHuisnrtoevoeging();
        identcodenraand = persadres.getIdentcodenraand();
        locoms = persadres.getLocoms();
        loctovadres = persadres.getLoctovadres();
        nor = persadres.getNor();
        partij = persadres.getPartij();
        plaats = persadres.getPlaats();
        postcode = persadres.getPostcode();
        rdnwijzadres = persadres.getRdnwijzadres();
    }

    public Land getLand() {
        return this.land;
    }

    public void setLand(final Land land) {
        this.land = land;
    }
    public Functieadres getFunctieadres() {
        return this.functieadres;
    }

    public void setFunctieadres(final Functieadres functieadres) {
        this.functieadres = functieadres;
    }
    public Rdnwijzadres getRdnwijzadres() {
        return this.rdnwijzadres;
    }

    public void setRdnwijzadres(final Rdnwijzadres rdnwijzadres) {
        this.rdnwijzadres = rdnwijzadres;
    }
    public Partij getPartij() {
        return this.partij;
    }

    public void setPartij(final Partij partij) {
        this.partij = partij;
    }
    public Aangadresh getAangadresh() {
        return this.aangadresh;
    }

    public void setAangadresh(final Aangadresh aangadresh) {
        this.aangadresh = aangadresh;
    }
    public Plaats getPlaats() {
        return this.plaats;
    }

    public void setPlaats(final Plaats plaats) {
        this.plaats = plaats;
    }
    public Persadres getPersadres() {
        return this.persadres;
    }

    public void setPersadres(final Persadres persadres) {
        this.persadres = persadres;
    }
    public Integer getDataanvadresh() {
        return this.dataanvadresh;
    }

    public void setDataanvadresh(final Integer dataanvadresh) {
        this.dataanvadresh = dataanvadresh;
    }
    public String getAdresseerbaarobject() {
        return this.adresseerbaarobject;
    }

    public void setAdresseerbaarobject(final String adresseerbaarobject) {
        this.adresseerbaarobject = adresseerbaarobject;
    }
    public String getIdentcodenraand() {
        return this.identcodenraand;
    }

    public void setIdentcodenraand(final String identcodenraand) {
        this.identcodenraand = identcodenraand;
    }
    public String getNor() {
        return this.nor;
    }

    public void setNor(final String nor) {
        this.nor = nor;
    }
    public String getAfgekortenor() {
        return this.afgekortenor;
    }

    public void setAfgekortenor(final String afgekortenor) {
        this.afgekortenor = afgekortenor;
    }
    public String getGemdeel() {
        return this.gemdeel;
    }

    public void setGemdeel(final String gemdeel) {
        this.gemdeel = gemdeel;
    }
    public Integer getHuisnr() {
        return this.huisnr;
    }

    public void setHuisnr(final Integer huisnr) {
        this.huisnr = huisnr;
    }
    public String getHuisletter() {
        return this.huisletter;
    }

    public void setHuisletter(final String huisletter) {
        this.huisletter = huisletter;
    }
    public String getHuisnrtoevoeging() {
        return this.huisnrtoevoeging;
    }

    public void setHuisnrtoevoeging(final String huisnrtoevoeging) {
        this.huisnrtoevoeging = huisnrtoevoeging;
    }
    public String getPostcode() {
        return this.postcode;
    }

    public void setPostcode(final String postcode) {
        this.postcode = postcode;
    }
    public String getLoctovadres() {
        return this.loctovadres;
    }

    public void setLoctovadres(final String loctovadres) {
        this.loctovadres = loctovadres;
    }
    public String getLocoms() {
        return this.locoms;
    }

    public void setLocoms(final String locoms) {
        this.locoms = locoms;
    }
    public String getBladresregel1() {
        return this.bladresregel1;
    }

    public void setBladresregel1(final String bladresregel1) {
        this.bladresregel1 = bladresregel1;
    }
    public String getBladresregel2() {
        return this.bladresregel2;
    }

    public void setBladresregel2(final String bladresregel2) {
        this.bladresregel2 = bladresregel2;
    }
    public String getBladresregel3() {
        return this.bladresregel3;
    }

    public void setBladresregel3(final String bladresregel3) {
        this.bladresregel3 = bladresregel3;
    }
    public String getBladresregel4() {
        return this.bladresregel4;
    }

    public void setBladresregel4(final String bladresregel4) {
        this.bladresregel4 = bladresregel4;
    }
    public String getBladresregel5() {
        return this.bladresregel5;
    }

    public void setBladresregel5(final String bladresregel5) {
        this.bladresregel5 = bladresregel5;
    }
    public String getBladresregel6() {
        return this.bladresregel6;
    }

    public void setBladresregel6(final String bladresregel6) {
        this.bladresregel6 = bladresregel6;
    }
    public Integer getDatvertrekuitnederland() {
        return this.datvertrekuitnederland;
    }

    public void setDatvertrekuitnederland(final Integer datvertrekuitnederland) {
        this.datvertrekuitnederland = datvertrekuitnederland;
    }




}


