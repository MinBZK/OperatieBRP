...
import nl.bzk.brp.testdatageneratie.datagenerators.hismaterieel.HisPersbijhaardGenerator;
...
public class PersoonGenerator implements Callable<Boolean> {
    ...
    private void voegPersoonToe(final Session kernSession) {
        ...
        if (Bijhaard.MINISTER.equals(pers.getBijhaard())) {
        ...
        List<HisPersbijhaard> hisPersbijhaardList =
                HisPersbijhaardGenerator.getInstance().generateHisMaterieels(pers, pers.getDatgeboorte());
            for (HisPersbijhaard hisPersbijhaard : hisPersbijhaardList) {
                    kernSession.save(hisPersbijhaard);
            }
        if (isFractie(His.CORRECTIE_FRACTIE)) {
            int randomHisPersbijhaard = new Random().nextInt(hisPersbijhaardList.size());
            HisPersbijhaard hisPersbijhaardCorrectie = (HisPersbijhaard) HisUtil.creeerHisCorrectie(HisPersbijhaardGenerator.getInstance().generateHisMaterieel(pers), hisPersbijhaardList.get(randomHisPersbijhaard));
            kernSession.save(hisPersbijhaardCorrectie);
        }
