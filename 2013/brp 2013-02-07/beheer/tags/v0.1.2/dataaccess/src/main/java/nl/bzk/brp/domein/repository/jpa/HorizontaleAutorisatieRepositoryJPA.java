...
import nl.bzk.brp.domein.repository.HorizontaleAutorisatieRepository;
...
public class HorizontaleAutorisatieRepositoryJPA implements HorizontaleAutorisatieRepository {
    ...
    public List<Long> filterPersonenBijFilter(final List<Long> persoonIds, final String... filters) {
        ...
        StringBuilder queryString = new StringBuilder("SELECT p.id FROM PersistentPersoon p WHERE p.id IN :persoonIds");
