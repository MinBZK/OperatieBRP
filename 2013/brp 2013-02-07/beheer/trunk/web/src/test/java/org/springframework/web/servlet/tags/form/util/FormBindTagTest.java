...
public class FormBindTagTest extends AbstractTagTest {
    ...
    public void testDoAfterBodyEersteLiActief() throws Exception {
        ...
        Assert.assertEquals("command wordt verwacht in de pageContext requestScope", "command",
                getMockPageContext().getAttribute(AbstractDataBoundFormElementTag.COMMAND_NAME_VARIABLE_NAME, PageContext.REQUEST_SCOPE));

        Assert.assertEquals("command wordt verwacht in de pageContext requestScope", "command.",
                getMockPageContext().getAttribute(NestedPathTag.NESTED_PATH_VARIABLE_NAME, PageContext.REQUEST_SCOPE));

        // TODO Hosing: zoek een manier om de controlleren of er niks geschreven is naar de pagina

        Assert.assertEquals(Tag.EVAL_BODY_INCLUDE, tagReturnValue);
    }
    ...
    public void testDoEndTag() throws Exception {
        ...
        Assert.assertEquals(Tag.EVAL_PAGE, tagReturnValue);
    }
}
