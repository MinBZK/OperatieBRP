...
public class SheetHandlerImplSimpleInsert extends AbstractSheetHandlerImpl {
    ...
    private void handleRow(final PrintWriter printWriter,
            final List<String> row, final String tableName, final int colCount,
            final int rowNumber) {
        ...
        for (int colNum = 0; colNum < colCount; colNum++) {
            ...
            if (getColValue(row, colNum, colCount) != null
                    && getColValue(row, colNum, colCount).trim().length() > 0) {
                printWriter.print(QUOTE + getColValue(row, colNum, colCount)
                        + QUOTE);
            } else {
