...
public class LogicalBackup {
    ...
    public void run() throws SQLException, ClassNotFoundException {
        ...
        List<String> dbVersion = util.getVersionNumber();
        LOGGER.debug("Oude backup data verwijdered " + dbVersion.get(0));
        updateRef.reset(dbVersion.get(0));
...
//            LOGGER.debug ("p:["+pers.getId()+","+pers.getBsn()+","+pers.getAnr()+"]");
            ...
            if (sqlInsert != null && sqlInsert.length() > 0) {
                updateRef.addSqlQuery(pers.getId(), pers.getBsn(), dbVersion.get(0), dbVersion.get(1), sqlInsert);
            }
        }
        LOGGER.debug("Einde");
