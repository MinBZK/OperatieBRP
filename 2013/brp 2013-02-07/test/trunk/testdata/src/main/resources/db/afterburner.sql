...
--                       , IndNreeks = ( Select U.IndNreeks from "kern"."his_perssamengesteldenaam" U where U.pers = pers.id AND U.dateindegel IS NULL)
...
--                       , IndNreeks = ( Select U.IndNreeks from "kern"."his_perssamengesteldenaam" U where U.pers = pers.id AND U.dateindegel IS NOT NULL)
...
insert into kern.pers (id, srt, idsstatushis, geslachtsaandstatushis, samengesteldenaamstatushis, aanschrstatushis, geboortestatushis, overlijdenstatushis, uitslnlkiesrstatushis, euverkiezingenstatushis, bijhaardstatushis, opschortingstatushis, bijhgemstatushis, verblijfstitelstatushis, pkstatushis, inschrstatushis, tslaatstewijz, immigratiestatushis, voornamen, voorvoegsel, scheidingsteken, geslnaam, IndNreeks, geslachtsaand, bvpstatushis) select id+1000000, 2, 'X', 'A', 'A', 'X', 'A', 'X', 'X', 'X', 'X', 'X', 'X', 'X', 'X','X', now(), 'X', 'Adam', 'van', ' ', 'Modernodam', false, 1, 'X' from kern.pers where geslnaam <> 'Modernodam' AND not exists (select 42 from kern.betr where rol=1 AND Pers = pers.id);
...
insert into kern.admhnd (id, srt, partij, tsontlening, toelichtingontlening) values ('0', '1', '2000', '19811002','toelichting');
...
insert into kern.Relatie(id, srt, huwelijkgeregistreerdpartner, datnaamskeuzeongeborenvrucht, naamskeuzeongeborenvruchtsta,  daterkenningongeborenvrucht, erkenningongeborenvruchtstat) select 33000000+id, 3, 'T',null,'T',null,'T' from kern.pers;
...
insert into kern.betr(id, relatie, rol, Pers, indouder, ouderschapstatushis, ouderlijkgezagstatushis)
   select 1000000+id, 33000000+id, 2, id+1000000, true, 'A', 'X' from kern.pers where geslnaam <> 'Modernodam' AND not exists (select 42 from kern.betr where rol=1 AND Pers=pers.id);
   
COMMIT;
BEGIN;

insert into kern.betr(id, relatie, rol, Pers, indouder, ouderschapstatushis, ouderlijkgezagstatushis)
   select 2000000+id, 33000000+id, 2, id+2000000, true, 'A', 'X' from kern.pers where geslnaam <> 'Modernodam' AND not exists (select 42 from kern.betr where rol=1 AND Pers=pers.id);
...
insert into kern.betr(id, relatie, rol, Pers, ouderschapstatushis, ouderlijkgezagstatushis)
	select 3000000+id, 33000000+id, 1, id, 'X', 'X' from kern.pers where geslnaam <> 'Modernodam' AND not exists (select 42 from kern.betr where rol=1 AND Pers=pers.id);
...
insert into kern.His_OuderOuderschap  (id, betr, dataanvgel, tsreg, actieinh, indouder)
	...
	where not exists (select 42 from kern.pers where geslnaam <> 'Modernodam' AND A.Pers=pers.id)
...
--SELECT SETVAL('kern.seq_His_OuderOuderschap ', MAX(id) ) FROM "kern"."His_OuderOuderschap " ;
--SELECT SETVAL('kern.seq_His_OuderOuderlijkGezag ', MAX(id) ) FROM "kern"."His_OuderOuderlijkGezag " ;
...
SELECT SETVAL('lev.seq_abonnement', (select 1+ coalesce(max(id),0)  FROM "lev"."abonnement"), true);
SELECT SETVAL('lev.seq_abonnementgegevenselement', (select 1+ coalesce(max(id),0)  FROM "lev"."abonnementgegevenselement"), true);
SELECT SETVAL('lev.seq_abonnementsrtber', (select 1+ coalesce(max(id),0)  FROM "lev"."abonnementsrtber"), true);
SELECT SETVAL('kern.seq_actie', (select 1+ coalesce(max(id),0)  FROM "kern"."actie"), true);
SELECT SETVAL('autaut.seq_authenticatiemiddel', (select 1+ coalesce(max(id),0)  FROM "autaut"."authenticatiemiddel"), true);
SELECT SETVAL('autaut.seq_autorisatiebesluit', (select 1+ coalesce(max(id),0)  FROM "autaut"."autorisatiebesluit"), true);
SELECT SETVAL('ber.seq_ber', (select 1+ coalesce(max(id),0)  FROM "ber"."ber"), true);
SELECT SETVAL('kern.seq_betr', (select 1+ coalesce(max(id),0)  FROM "kern"."betr"), true);
SELECT SETVAL('autaut.seq_bijhautorisatie', (select 1+ coalesce(max(id),0)  FROM "autaut"."bijhautorisatie"), true);
SELECT SETVAL('autaut.seq_bijhsituatie', (select 1+ coalesce(max(id),0)  FROM "autaut"."bijhsituatie"), true);
SELECT SETVAL('kern.seq_actiebron', (select 1+ coalesce(max(id),0)  FROM "kern"."actiebron"), true);
SELECT SETVAL('autaut.seq_certificaat', (select 1+ coalesce(max(id),0)  FROM "autaut"."certificaat"), true);
SELECT SETVAL('kern.seq_doc', (select 1+ coalesce(max(id),0)  FROM "kern"."doc"), true);
SELECT SETVAL('autaut.seq_doelbinding', (select 1+ coalesce(max(id),0)  FROM "autaut"."doelbinding"), true);
SELECT SETVAL('autaut.seq_doelbindinggegevenselement', (select 1+ coalesce(max(id),0)  FROM "autaut"."doelbindinggegevenselement"), true);
SELECT SETVAL('kern.seq_gegeveninonderzoek', (select 1+ coalesce(max(id),0)  FROM "kern"."gegeveninonderzoek"), true);
SELECT SETVAL('lev.seq_his_abonnement', (select 1+ coalesce(max(id),0)  FROM "lev"."his_abonnement"), true);
SELECT SETVAL('lev.seq_his_abonnementsrtber', (select 1+ coalesce(max(id),0)  FROM "lev"."his_abonnementsrtber"), true);
SELECT SETVAL('autaut.seq_his_authenticatiemiddel', (select 1+ coalesce(max(id),0)  FROM "autaut"."his_authenticatiemiddel"), true);
SELECT SETVAL('autaut.seq_his_autorisatiebesluit', (select 1+ coalesce(max(id),0)  FROM "autaut"."his_autorisatiebesluit"), true);
SELECT SETVAL('autaut.seq_his_autorisatiebesluitbijhau', (select 1+ coalesce(max(id),0)  FROM "autaut"."his_autorisatiebesluitbijhau"), true);
SELECT SETVAL('kern.seq_his_ouderouderschap', (select 1+ coalesce(max(id),0)  FROM "kern"."his_ouderouderschap"), true);
SELECT SETVAL('kern.seq_his_ouderouderlijkgezag', (select 1+ coalesce(max(id),0)  FROM "kern"."his_ouderouderlijkgezag"), true);
SELECT SETVAL('autaut.seq_his_bijhautorisatie', (select 1+ coalesce(max(id),0)  FROM "autaut"."his_bijhautorisatie"), true);
SELECT SETVAL('autaut.seq_his_bijhsituatie', (select 1+ coalesce(max(id),0)  FROM "autaut"."his_bijhsituatie"), true);
SELECT SETVAL('kern.seq_his_doc', (select 1+ coalesce(max(id),0)  FROM "kern"."his_doc"), true);
SELECT SETVAL('autaut.seq_his_doelbinding', (select 1+ coalesce(max(id),0)  FROM "autaut"."his_doelbinding"), true);
SELECT SETVAL('kern.seq_his_partijgem', (select 1+ coalesce(max(id),0)  FROM "kern"."his_partijgem"), true);
SELECT SETVAL('kern.seq_his_multirealiteitregel', (select 1+ coalesce(max(id),0)  FROM "kern"."his_multirealiteitregel"), true);
SELECT SETVAL('kern.seq_his_onderzoek', (select 1+ coalesce(max(id),0)  FROM "kern"."his_onderzoek"), true);
SELECT SETVAL('kern.seq_his_partij', (select 1+ coalesce(max(id),0)  FROM "kern"."his_partij"), true);
SELECT SETVAL('kern.seq_his_persaanschr', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persaanschr"), true);
SELECT SETVAL('kern.seq_his_persadres', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persadres"), true);
SELECT SETVAL('kern.seq_his_persbijhgem', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persbijhgem"), true);
SELECT SETVAL('kern.seq_his_persbijhaard', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persbijhaard"), true);
SELECT SETVAL('kern.seq_his_perseuverkiezingen', (select 1+ coalesce(max(id),0)  FROM "kern"."his_perseuverkiezingen"), true);
SELECT SETVAL('kern.seq_his_persgeboorte', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persgeboorte"), true);
SELECT SETVAL('kern.seq_his_persgeslachtsaand', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persgeslachtsaand"), true);
SELECT SETVAL('kern.seq_his_persgeslnaamcomp', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persgeslnaamcomp"), true);
SELECT SETVAL('kern.seq_his_persids', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persids"), true);
SELECT SETVAL('kern.seq_his_persimmigratie', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persimmigratie"), true);
SELECT SETVAL('kern.seq_his_persindicatie', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persindicatie"), true);
SELECT SETVAL('kern.seq_his_persinschr', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persinschr"), true);
SELECT SETVAL('kern.seq_his_persnation', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persnation"), true);
SELECT SETVAL('kern.seq_his_persopschorting', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persopschorting"), true);
SELECT SETVAL('kern.seq_his_persoverlijden', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persoverlijden"), true);
SELECT SETVAL('kern.seq_his_perspk', (select 1+ coalesce(max(id),0)  FROM "kern"."his_perspk"), true);
SELECT SETVAL('kern.seq_his_persreisdoc', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persreisdoc"), true);
SELECT SETVAL('kern.seq_his_perssamengesteldenaam', (select 1+ coalesce(max(id),0)  FROM "kern"."his_perssamengesteldenaam"), true);
SELECT SETVAL('kern.seq_his_persuitslnlkiesr', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persuitslnlkiesr"), true);
SELECT SETVAL('kern.seq_his_persverblijfstitel', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persverblijfstitel"), true);
SELECT SETVAL('kern.seq_his_persverificatie', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persverificatie"), true);
SELECT SETVAL('kern.seq_his_persvoornaam', (select 1+ coalesce(max(id),0)  FROM "kern"."his_persvoornaam"), true);
SELECT SETVAL('brm.seq_his_regelsituatie', (select 1+ coalesce(max(id),0)  FROM "brm"."his_regelsituatie"), true);
--SELECT SETVAL('kern.seq_his_relatie', (select 1+ coalesce(max(id),0)  FROM "kern"."his_relatie"), true);
SELECT SETVAL('autaut.seq_his_uitgeslotene', (select 1+ coalesce(max(id),0)  FROM "autaut"."his_uitgeslotene"), true);
SELECT SETVAL('lev.seq_lev', (select 1+ coalesce(max(id),0)  FROM "lev"."lev"), true);
SELECT SETVAL('lev.seq_levcommunicatie', (select 1+ coalesce(max(id),0)  FROM "lev"."levcommunicatie"), true);
SELECT SETVAL('lev.seq_levpers', (select 1+ coalesce(max(id),0)  FROM "lev"."levpers"), true);
SELECT SETVAL('kern.seq_multirealiteitregel', (select 1+ coalesce(max(id),0)  FROM "kern"."multirealiteitregel"), true);
SELECT SETVAL('kern.seq_onderzoek', (select 1+ coalesce(max(id),0)  FROM "kern"."onderzoek"), true);
SELECT SETVAL('kern.seq_partij', (select 1+ coalesce(max(id),0)  FROM "kern"."partij"), true);
SELECT SETVAL('kern.seq_partijrol', (select 1+ coalesce(max(id),0)  FROM "kern"."partijrol"), true);
SELECT SETVAL('kern.seq_pers', (select 1+ coalesce(max(id),0)  FROM "kern"."pers"), true);
SELECT SETVAL('kern.seq_persadres', (select 1+ coalesce(max(id),0)  FROM "kern"."persadres"), true);
SELECT SETVAL('kern.seq_persgeslnaamcomp', (select 1+ coalesce(max(id),0)  FROM "kern"."persgeslnaamcomp"), true);
SELECT SETVAL('kern.seq_persindicatie', (select 1+ coalesce(max(id),0)  FROM "kern"."persindicatie"), true);
SELECT SETVAL('kern.seq_persnation', (select 1+ coalesce(max(id),0)  FROM "kern"."persnation"), true);
SELECT SETVAL('kern.seq_persreisdoc', (select 1+ coalesce(max(id),0)  FROM "kern"."persreisdoc"), true);
SELECT SETVAL('kern.seq_persverificatie', (select 1+ coalesce(max(id),0)  FROM "kern"."persverificatie"), true);
SELECT SETVAL('kern.seq_persvoornaam', (select 1+ coalesce(max(id),0)  FROM "kern"."persvoornaam"), true);
SELECT SETVAL('brm.seq_regelsituatie', (select 1+ coalesce(max(id),0)  FROM "brm"."regelsituatie"), true);
SELECT SETVAL('kern.seq_relatie', (select 1+ coalesce(max(id),0)  FROM "kern"."relatie"), true);
SELECT SETVAL('kern.seq_sector', (select 1+ coalesce(max(id),0)  FROM "kern"."sector"), true);
SELECT SETVAL('autaut.seq_uitgeslotene', (select 1+ coalesce(max(id),0)  FROM "autaut"."uitgeslotene"), true);
SELECT SETVAL('kern.seq_admhnd', (select 1+ coalesce(max(id),0)  FROM "kern"."admhnd"), true);

-- Bolie vrijdag 4 januari 2013, toegevoegd om de default op false te zetten. de xsd verlangt dat dit gevuld diet te worden.
UPDATE kern.his_persbijhgem set IndOnverwDocAanw = false where IndOnverwDocAanw is null;
UPDATE kern.pers set IndOnverwDocAanw = false where IndOnverwDocAanw is null and bijhgemstatushis <> 'X';

UPDATE kern.pers set naamgebruik = 1 where naamgebruik is null and aanschrstatushis <> 'X';
UPDATE kern.his_persaanschr set naamgebruik = 1 where naamgebruik is null;


UPDATE kern.pers set indalgoritmischafgeleid = true where indalgoritmischafgeleid is null and samengesteldenaamstatushis <> 'X';
UPDATE kern.his_perssamengesteldenaam set indalgoritmischafgeleid = true where indalgoritmischafgeleid is null ;


UPDATE kern.pers set datgeboorte='19500101' where datgeboorte is null and geboortestatushis <> 'X';
UPDATE kern.pers set landgeboorte=229 where landgeboorte is null and geboortestatushis <> 'X';
