...
public class BevragingViaBsnServiceImpl implements BevragingViaBsnService {
    ...
    public Pers findPersonByBsn(final String bsn) {
        ...
        Pers persoon = null;
        query = Pers.findPersesByBsnEquals(Integer.parseInt(bsn));
        ...
        if (huidigAdres == null) {
            ...
            String sql =
                "select hpa from HisPersadres hpa where hpa.persadres.id = :persadresid"
                    + " and hpa.tsverval is null order by hpa.dataanvgel desc, hpa.dateindegel desc, hpa.id desc";
    ...
    public List<Betr> findOuders(final Pers persoon) {
        ...
        if (persoon != null) {
            List<Betr> betrokkenhedenKind = Betr.findBetrsByPers(persoon).getResultList();
    ...
    public List<Betr> findKinderen(final Pers persoon) {
        ...
        if (persoon != null) {
            List<Betr> betrokkenheden = Betr.findBetrsByPers(persoon).getResultList();
                ...
                if ("P".equals(betrokkenheid.getRol().getCode())) {
                    ...
                    for (Betr familiebetrekking : familiebetrekkingen) {
                        if ("P".equals(familiebetrekking.getRol().getCode())
                            && !familiebetrekking.getPers().getId().equals(persoon.getId()))
                        {
                            logger.debug("Partner bsn:" + familiebetrekking.getPers().getBsn());
    ...
    @Override
    public Boolean findIndicatie(final Pers persoon, final int soort) {
        if (persoon != null) {
            List<Persindicatie> indicaties = Persindicatie.findPersindicatiesByPers(persoon).getResultList();
            for (Persindicatie persindicatie : indicaties) {
                if (persindicatie.getSrt().getId() == soort) {
                    return persindicatie.getWaarde();
                }
            }
        }
        return null;
