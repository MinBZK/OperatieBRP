    ...
    <div role="main">
        ...
        <section id="person">
               ...
               <h3 class="active">Identificatienummers</h3>
				  ...
				  <div>
				   <table>                   
                    ...
                    <c:catch var="gebEx">
	                    <c:set var="jaar" value="${fn:substring(persoon.datgeboorte, 0, 4)}" />
	                    <c:set var="maand" value="${fn:substring(persoon.datgeboorte, 4, 6)}" />
	                    <c:set var="dag" value="${fn:substring(persoon.datgeboorte, 6, 8)}" />
	                </c:catch>
                    <c:choose>
                    <c:when test="${!empty gebEx}">
                         <c:out value="${persoon.datgeboorte}" />
                    </c:when>
                    <c:otherwise>
                    <c:out value="${dag}-${maand}-${jaar}"/>
                    </c:otherwise>
                    </c:choose>
                    </td>
            ...
            <section class="collapsable">
                ...
                <div>
                 <c:catch var="adresEx">
                        <c:set var="jaar" value="${fn:substring(adres.dataanvadresh, 0, 4)}" />
                        <c:set var="maand" value="${fn:substring(adres.dataanvadresh, 4, 6)}" />
                        <c:set var="dag" value="${fn:substring(adres.dataanvadresh, 6, 8)}" />
                    </c:catch>
                    <c:choose>
                    <c:when test="${!empty adresEx}">
                         <c:out value="${adres.dataanvadresh}" />
                    </c:when>
                    <c:otherwise>
                    <c:set var="vanaf" value="${dag}-${maand}-${jaar}"/>
                    </c:otherwise>
                 </c:choose>
                <c:set var="adreskopregel" value="vanaf ${vanaf}"/>
                 ...
                 <section id="adreshistorie">
                    ...
                    <c:catch var="adresEx">
                        <c:set var="jaar" value="${fn:substring(historiesadres.dataanvadresh, 0, 4)}" />
                        <c:set var="maand" value="${fn:substring(historiesadres.dataanvadresh, 4, 6)}" />
                        <c:set var="dag" value="${fn:substring(historiesadres.dataanvadresh, 6, 8)}" />
                    </c:catch>
                    <c:choose>
                    <c:when test="${!empty adresEx}">
                         <c:set var="vanaf" value="${historiesadres.dataanvadresh}" />
                    </c:when>
                    <c:otherwise>
                    <c:set var="vanaf" value="${dag}-${maand}-${jaar}"/>
                    </c:otherwise>
                 </c:choose>
                 <c:catch var="adresExTot">
                        <c:set var="jaar" value="${fn:substring(historiesadres.dateindegel, 0, 4)}" />
                        <c:set var="maand" value="${fn:substring(historiesadres.dateindegel, 4, 6)}" />
                        <c:set var="dag" value="${fn:substring(historiesadres.dateindegel, 6, 8)}" />
                    </c:catch>
                    <c:choose>
                    <c:when test="${!empty adresExTot}">
                         <c:set var="tot" value="${historiesadres.dateindegel}" />
                    </c:when>
                    <c:otherwise>
                    <c:set var="tot" value="${dag}-${maand}-${jaar}"/>
                    </c:otherwise>
                 </c:choose>
                    <c:choose>
                ...
                <div><c:if test="${not empty partner.betrokkene.bsn}">
                    <c:catch var="partnerAanvEx">
                          <c:set var="jaar" value="${fn:substring(partner.relatie.dataanv, 0, 4)}" />
                          <c:set var="maand" value="${fn:substring(partner.relatie.dataanv, 4, 6)}" />
                          <c:set var="dag" value="${fn:substring(partner.relatie.dataanv, 6, 8)}" />
                      </c:catch>
                      <c:choose>
                      <c:when test="${!empty partnerAanvEx}">
                           <c:set var="partneraanvangdatum">${partner.relatie.dataanv}</c:set>
                      </c:when>
                      <c:when test="${empty partner.relatie.dataanv}">
                         <c:set var="partneraanvangdatum" value=" "/>
                      </c:when>
                      <c:otherwise>
                            <c:set var="partneraanvangdatum" value="${dag}-${maand}-${jaar}"/>
                      </c:otherwise>
                      </c:choose>
                    <table>
                        <tr>
                            <td class="label">Datum aanvang</td><td class="data"><c:out value="${partneraanvangdatum}"/></td>
                            <td class="label">Naam</td><td class="data"><c:out value="${partner.betrokkene.voornamen}"/>&nbsp;<c:out value="${partner.betrokkene.geslnaam}"/></td>
                        </tr>
                        <tr>
                            <td class="label">BSN</td><td class="data"><a href="<c:url value='/dashboard/bevraging?bsn=${partner.betrokkene.bsn}' />"><c:out value="${partner.betrokkene.bsn}" /></td>
                            <td class="label">Datum geboorte</td><td class="data"><c:out value="${partner.betrokkene.datgeboorte}"/></td>
                        </tr>
                         <tr>
                            <td class="label">Geslacht</td><td class="data"><c:out value="${partner.betrokkene.geslachtsaand.naam}"/></td>
                            <td class="label">Plaats geboorte</td><td class="data"><c:out value="${partner.betrokkene.wplgeboorte.naam}"/></td>
                        </tr>
                        <tr>
                            <td class="label">&nbsp;</td><td class="data">&nbsp;</td>
                            <td class="label">Land geboorte</td><td class="data"><c:out value="${partner.betrokkene.landgeboorte.naam}"/></td>
                        </tr>
                    </table>    
                    </c:if>
                </div> 
                ...
                <div>
                 <section id="collapsable">
                    ...
                    <tr>
                        <td class="label">Datum aanvang</td>
                        <td class="data">
		                    <c:catch var="relatieAanvEx">
			                    <c:set var="jaar" value="${fn:substring(ouder.relatie.dataanv, 0, 4)}" />
			                    <c:set var="maand" value="${fn:substring(ouder.relatie.dataanv, 4, 6)}" />
			                    <c:set var="dag" value="${fn:substring(ouder.relatie.dataanv, 6, 8)}" />
			                </c:catch>
		                    <c:choose>
		                    <c:when test="${!empty relatieAanvEx}">
		                         <c:out value="${ouder.relatie.dataanv}" />
		                    </c:when>
		                    <c:otherwise>
		                    <c:out value="${dag}-${maand}-${jaar}"/>
		                    </c:otherwise>
                        ...
                        <td class="data"><c:out value="${ouder.betrokkene.voornamen} ${ouder.betrokkene.voorvoegsel} ${ouder.betrokkene.geslnaam}"/></td>
                    </tr> 
                     <tr>
                        <td class="label">BSN</td>
                        <td class="data">
		                    <a href="<c:url value='/dashboard/bevraging?bsn=${ouder.betrokkene.bsn}' />">
		                    	<c:out value="${ouder.betrokkene.bsn}" />
		                    </a>
                        </td>
                        <td class="label">Datum geboorte</td>
                        <td class="data">
		                    <c:catch var="ouderGebEx">
			                    <c:set var="jaar" value="${fn:substring(ouder.betrokkene.datgeboorte, 0, 4)}" />
			                    <c:set var="maand" value="${fn:substring(ouder.betrokkene.datgeboorte, 4, 6)}" />
			                    <c:set var="dag" value="${fn:substring(ouder.betrokkene.datgeboorte, 6, 8)}" />
			                </c:catch>
		                    <c:choose>
		                    <c:when test="${!empty ouderGebEx}">
		                         <c:out value="${ouder.betrokkene.datgeboorte}" />
		                    </c:when>
		                    <c:when test="${empty ouder.betrokkene.datgeboorte}">
                                 &nbsp;
                            </c:when>
		                    <c:otherwise>
		                    <c:out value="${dag}-${maand}-${jaar}"/>
                    ...
                    <tr>
                        ...
                        <td class="data">
                            <c:out value="${ouder.betrokkene.geslachtsaand.naam}"/>
                        </td>
                        <td class="label">Plaats geboorte</td>
                        <td class="data"><c:out value="${ouder.betrokkene.wplgeboorte.naam}"/></td>
                    </tr>      
                    <tr>
                        <td class="label">&nbsp;</td>
                        <td class="data">&nbsp;</td>
                        <td class="label">Land geboorte</td>
                        <td class="data"><c:out value="${ouder.betrokkene.landgeboorte.naam}"/></td>
                    </tr>   
                      </table>
            ...
            <section class="collapsable">
               ...
               <h3 class="active">Kind</h3>
                      <c:catch var="relatieAanvEx">
                          <c:set var="jaar" value="${fn:substring(kind.betrokkene.datgeboorte, 0, 4)}" />
                          <c:set var="maand" value="${fn:substring(kind.betrokkene.datgeboorte, 4, 6)}" />
                          <c:set var="dag" value="${fn:substring(kind.betrokkene.datgeboorte, 6, 8)}" />
                      </c:catch>
                      <c:choose>
                      <c:when test="${!empty relatieAanvEx}">
                           <c:set var="kindgeboortedatum">${ouder.relatie.dataanv}"</c:set>
                      </c:when>
                      <c:when test="${empty kind.betrokkene.datgeboorte}">
                         <c:set var="kindgeboortedatum" value="&nbsp;"/>
                      </c:when>
                      <c:otherwise>
                            <c:set var="kindgeboortedatum" value="${dag}-${maand}-${jaar}"/>
                      </c:otherwise>
                      </c:choose>
                  <div>
                    <table>
	                    <tr>
		                    <td class="label">Datum aanvang</td><td class="data"><c:out value="${kindgeboortedatum}"/></td>
		                    <td class="label">Naam</td><td class="data"><c:out value="${kind.betrokkene.voornamen}"/>&nbsp;<c:out value="${kind.betrokkene.geslnaam}"/></td>
		                </tr>
		                <tr>
                            <td class="label">BSN</td><td class="data"><a href="<c:url value='/dashboard/bevraging?bsn=${kind.betrokkene.bsn}' />"><c:out value="${kind.betrokkene.bsn}" /></td>
                            <td class="label">Datum geboorte</td><td class="data"><c:out value="${kindgeboortedatum}"/></td>
                        </tr>
                         <tr>
                            <td class="label">Geslacht</td><td class="data"><c:out value="${kind.betrokkene.geslachtsaand.naam}"/></td>
                            <td class="label">Plaats geboorte</td><td class="data"><c:out value="${kind.betrokkene.wplgeboorte.naam}"/></td>
                        </tr>
                        <tr>
                            <td class="label">&nbsp;</td><td class="data">&nbsp;</td>
                            <td class="label">Land geboorte</td><td class="data"><c:out value="${kind.betrokkene.landgeboorte.naam}"/></td>
                        </tr>
                    </table>    
                  </div>
                  </section>
