...
<c:set var="contextAdreshistorie" value="${baseUrl}${properties.adreshistorie_context}" />
<c:choose>
...
<head>
	...
	<link rel="stylesheet" href="${contextRoot}/resources/css/redmond/jquery-ui.css" type="text/css" media="all" />
	<script src="${contextRoot}/resources/js/libs/jquery-ui.min.js" type="text/javascript"></script>
...
<body>
	<script>
                         ...
                         success: function(data) {
                            ...
                            if(data.aantalBerichten > 0) {
                                ...
                                for( var i in data.berichten) {
                                    ...
                                    if(result.prevalidatie == true || (result.aantalMeldingen > 0 && (result.berichtDetails.indexOf("Fout") != -1 || result.berichtDetails.indexOf("Overrulebaar") != -1) ) ) {
                                        console.log('Skip prevalidatie of bericht met meldingen. Bericht van ' + result.partij + ', tekst: ' + result.bericht);
			...
			function createMessage(message) { 	
                ...
                if(message.berichtDetails.length > 0) {
                ...
                content +='Op:&nbsp;<span class="timestamp" title="' + timestamp.toJSON() + '" >' + formatDate(timestamp,"%d-%M %H:%m:%s") + '</span><br />';
	...
	<section id="search">
        ...
        <section id="person">
            <c:choose>
                ...
                <c:when test="${persoon.geslachtsaand.naam == 'Vrouw'}">
                    <c:set var="geslachtUrl" value="${contextRoot}/resources/images/vrouw.png" />
                </c:when>
                <c:otherwise>
                    <c:set var="geslachtUrl" value="" />
                </c:otherwise>
            </c:choose>
			...
			<section class="collapsable">
				  ...
				  <table>                   
                    ...
                    <td class="label">Adellijke titel</td><td class="data">
                    <c:catch var="ex">
            ...
            <section class="collapsable">
				  ...
				  <div>
				   <table>                   
                    <tr>
                    <td class="label">Naamgebruik</td><td class="data"><c:out value="${persoon.naamgebruik.oms}" /></td>
                    <td class="label">Voorvoegsel</td><td class="data"><c:out value="${persoon.voorvoegselaanschr}"/></td>
                    </tr> 
                    <tr>
                    <td class="label">Gebruik adellijke titels/predikaten</td><td class="data">
                    <c:out value="${brp:jaNee(persoon.indtitelspredikatenbijaansch)}"/>
                    </td>
                    ...
                    <td class="label">Adellijke titel</td><td class="data"><c:out value="${adellijkeTitelAanschr}"/></td>
                    </tr>
                    <tr>
            ...
            <section class="collapsable">
                ...
                <c:choose>
                    <c:when test="${empty adreshistorie}">
                        <c:set var="historyknop" value="" />
                    </c:when>
                    <c:otherwise>
                        <c:set var="historyknop"><img src="${contextRoot}/resources/images/history.png" class="historietonen" onclick="$('#adreshistorie').toggle();return false;" title="Toon historische adressen"></c:set>
                    </c:otherwise>
                 </c:choose>
                 <c:choose>
                     <c:when test="${empty adreshistorie}">
                         <c:set var="adreshistorieknop" value="" />
                     </c:when>
                     <c:when test="${empty contextAdreshistorie}">
                         <c:set var="adreshistorieknop" value="" />
                     </c:when>					 
                     <c:otherwise>
                         <c:set var="adreshistorieknop"><a href="${contextAdreshistorie}/overzicht.html?bsn=${bsn}"><img src="${contextRoot}/resources/images/adreshistorie.png" class="adreshistorie"  title="Toon historische adressen"></a></c:set>
                     </c:otherwise>
                  </c:choose>
                 <section>
                 <h3 class="active">Adres ${adreskopregel} ${adreshistorieknop} ${historyknop}</h3>
                    <div>
                 <table>                   
                    <tr>
            ...
            <section class="collapsable">
                ...
                <c:if test="${not empty partner.pers}">
                     <c:set var="partnervanaf" value="${brp:datum(partner.relatie.dataanv)}"/>
                     <c:set var="partnerkopregel" value="vanaf ${partnervanaf}"/>
                    <section class="collapsable">
                        <h3 class="active">Partner ${partnerkopregel}</h3>
