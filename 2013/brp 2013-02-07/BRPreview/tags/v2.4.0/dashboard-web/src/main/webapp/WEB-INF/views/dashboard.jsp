        ...
        $("body").fadeIn(2000);
		    ...
		    function getNewMessages() {		    	
		    	...
		    	var newMessages = $.ajax ({
					 ...
					 error : function(xhr, status) {
							console.log('Er is een fout opgetreden.' + status);
						},
			...
			function showNewMessage(messages, index) {
                ...
                if(index < messages.length)
                {
                    ...
                    if(htmlMsg == null)
                    {
						console.log("Geen html kunnen maken van bericht " + message);
			...
			function showMessage(messages, htmlMessage, index) {
                ...
                htmlMessage.slideDown(750, function() { showNewMessage(messages, index+1);});
            }
		    	...
		    	$.ajax ({
					 ...
					 success: function(data) {
						...
						if(data.aantalBerichten > 0)
                        {
                            ...
                            console.log('Aantal berichten:' + berichten.length);
							//Beperk het aantal nieuwe berichten dat we tonen tot 10, omdat we anders niet rondkomen met de verversing van het scherm
							if(berichten.length > 10) {
								try {
									var overhead = berichten.length-10;
									berichten.splice(10, overhead);
		    ...
		    function createMessage(message) { 	
                ...
                content +='<span class="timestamp" title="' + timestamp.toJSON() + '" >' + jQuery.timeago(timestamp.toISOString()) + '</span>';
