...
public class BerichtenOpvraagController {
    ...
    @RequestMapping(value = "/berichten/partij/{partij}", method = RequestMethod.GET, headers = "Accept=application/json")
    @ResponseBody
    ...
    @RequestMapping(value = "/berichten/partij/{partij}/{timestamp}", method = RequestMethod.GET, headers = "Accept=application/json")
    @ResponseBody
    public BerichtenResponse getBerichten(@PathVariable("partij") final String partij, @PathVariable("timestamp") final Long timestamp) {
    ...
    public int changeSettings(@PathVariable("berichtenPerPagina") final Long berichtenPerPagina) {
        ...
        logger.debug("Verander maximum aantal berichten per pagina naar:" + berichtenPerPagina + ", was " + settings.getMaximaalAantalBerichtenTonen());
        if (berichtenPerPagina != null) {
            ...
            logger.debug("Settings object in changesettings:" + settings.toString());
        }
