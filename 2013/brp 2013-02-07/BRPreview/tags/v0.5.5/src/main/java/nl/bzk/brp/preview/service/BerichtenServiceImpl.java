...
public class BerichtenServiceImpl implements BerichtenService {
    ...
    public void opslaan(final AbstractBerichtRequest inkomendBericht) {
        ...
        verrijkIndienBrpDbBeschikbaar(inkomendBericht);
    ...
    private void verrijkIndienBrpDbBeschikbaar(final AbstractBerichtRequest inkomendBericht) {
        long count;
        try {
            count = Srtpers.countSrtperses();
        } catch (RuntimeException e) {
            count = 0;
        }
        if (count == 0) {
            logger.warn("verrijken niet mogelijk want BRP DB is niet beschikbaar");
        } else {
            inkomendBericht.verrijk();
        }
    }
    ...
    public void setMaximumAantalBerichtenTonen(final int maximum) {
        ...
        logger.debug("settings gewijzigd voor maximaal aantal berichten in:" + settings.getMaximaalAantalBerichtenTonen());
