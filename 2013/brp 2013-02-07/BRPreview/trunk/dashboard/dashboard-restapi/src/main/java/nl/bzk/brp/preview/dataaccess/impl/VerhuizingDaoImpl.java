...
/**
 * Implementatie van de interface voor de DAO voor toegang tot verhuizing data (BRP).
 */
public class VerhuizingDaoImpl implements VerhuizingDao {
     ...
     */
    @Override
    public Adres haalOpOudAdres(final Long handelingId) {
        ...
        return brpJdbcTemplate.queryForObject(sql, namedParameters, new AdresRowMapper());
    }

    /* (non-Javadoc)
     * @see nl.bzk.brp.preview.dataaccess.AdministratieveHandelingDao#haalOpVerhuizingNieuwAdres(java.lang.Long)
     */
    @Override
    public Adres haalOpNieuwAdres(final Long handelingId) {
        ...
        final String sql = "SELECT "
                ...
                + "   FROM kern.admhnd kad\n"
                + "   JOIN kern.actie kac ON (kad.id = kac.admhnd)\n"
                + "   JOIN kern.his_persadres khp ON (khp.actieverval = kac.id)\n"
                + "   JOIN kern.persadres khpa ON (khpa.id = khp.persadres)\n"
                + "   JOIN kern.pers kpe ON (kpe.id = khpa.pers)\n"
                + "   JOIN kern.persadres kpa ON (kpa.pers = kpe.id)\n"
                + "   JOIN kern.plaats kpl ON (kpl.id = kpa.wpl)\n"
                + "   JOIN kern.partij gem ON (kpa.gem = gem.id)\n"
                + "   WHERE kad.id = :id";
