...
public class InsertTester {
	...
	DataSource dataSource = setupDataSource("jdbc:postgresql://" + InsertTest.eigenschappen.getDbHostBijhouding() + "/brp");
    ...
    private final static double interval = 5.0;
	...
	Thread thread = new Thread() {
		...
		try {
			...
			LOG.debug("Interts: " + (speed / interval) + " / sec, bercithgrootte=" + bericht.length() + " bytes, totalcount=" + totalcount);
			speed = 0;
