 ...
 */
@Component
public class PostgreSqlSchemaGenerator implements Generator {
    ...
    private void genereerPersoonsLockTabel() {
        ...
        r.regel("CREATE TABLE Kern.PersoonsLock (BSN char(9) PRIMARY KEY);");
        r.regel("INSERT INTO Kern.PersoonsLock (BSN)");
        r.regel("    SELECT 'lockrow' ");
        r.regel("    WHERE NOT EXISTS (");
        r.regel("        SELECT true");
        r.regel("        FROM Kern.PersoonsLock");
        r.regel("        WHERE BSN='lockrow'");
        r.regel("    );");
    }
