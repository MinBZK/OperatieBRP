 ...
 */
@Component
public class PostgreSqlLogischeElementenGenerator extends AbstractGenerator implements Generator {

    private ArtifactBuilder r;

    public static final int LAAG_OPERATIONEEL = 1749;
    ...
    private CharSequence genereerLogischeElementgegevens(final Domein domein) {
        ...
        for (Schema schema : domein.getSchemas()) {
            if ("Kern".equals(schema.getNaam())) {
                for (ObjectType objectType : schema.getWerkVersie().getObjectTypes(InSetOfModel.MODEL)) {
                    //Alleen voor objecttypen uit de operationele laag:
                    if (LAAG_OPERATIONEEL == objectType.getLaag()) {
                        ...
                        for (Attribuut attribuut : objectType.getAttributen()) {
                            if (attribuut.getInSetOfModel() != InSetOfModel.SET) {
