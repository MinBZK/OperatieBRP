 ...
 */
@Component
public class DomeinModelGenerator extends AbstractGenerator implements Generator {
    ...
    public void generate(final ModelElement element, final FileSystemAccess file) {
        if (!(element instanceof Domein)) {
            throw new IllegalArgumentException("Kan alleen code genereren uit een Domein element.");
        }
        JavaPackage root = new JavaPackageModelBuilder((Domein) element).buildJavaPackage(getPackagePrefix());
     ...
     */
    private void alsCommentaarRegels(final Map<SoortTekst, Tekst> teksten) {
        Tekst tekst = null;

        tekst = teksten.get(SoortTekst.DEF);
        if (tekst != null) {
    ...
    private void genereerBasisInterface(final BasisInterface type) {
        ...
        r.regel();
        r.regel("/**");
        alsCommentaarRegels(type.getObjectType().getTeksten().map());
        r.regel(" */");
    ...
    private void genereerField(final BasisPersistentClass type, final BeanProperty property) {
        ...
        if (property.isIdentifier()) {
            ...
            if (property.isManyToOne()) {
                r.regel("@ManyToOne(targetEntity = ", ((Interface) property.getType()).getImplementation().getNaam(),
                        ".class)");
                r.regel("@JoinColumn(name = \"", property.getDatabaseNaam(), "\")");
            } else if (property.isCollectie()) {
                r.regel("@OneToMany(targetEntity = ", ((Interface) property.getType()).getImplementation().getNaam(),
                        ".class, mappedBy = \"", property.getOpposite().getNaam(), "\")");
            } else {
    ...
    private void genereerPersistentObjectFactory(final PersistentObjectFactory factory) {
        ...
        r.regel();
        r.regel("import org.springframework.stereotype.Component;");
        r.regel();
        r.regel("/**");
