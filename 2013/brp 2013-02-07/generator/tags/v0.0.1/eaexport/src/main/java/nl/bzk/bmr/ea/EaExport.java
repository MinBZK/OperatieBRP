...
public class EaExport {
    ...
    private void genereerAssociatie(final Attribuut att, final Element parent) {
        ...
        associatie.setAttributeNS(XMI, "xmi:id", getId(ExportRegel.II_ASSOCIATION, naam, att.getSyncId()));
        ...
        memberEnd.setAttributeNS(XMI, "xmi:idref", getId(ExportRegel.II_ASSOCIATION_DST, naam, att.getSyncId()));
        ...
        ownedEnd.setAttributeNS(XMI, "xmi:id", getId(ExportRegel.II_ASSOCIATION_DST, naam, att.getSyncId()));
        ownedEnd.setAttribute("association", getId(ExportRegel.II_ASSOCIATION, naam, att.getSyncId()));
        ...
        type.setAttributeNS(XMI, "xmi:idref", getId(ExportRegel.II_OBJECTTYPE, dstType.getNaam(), dstType.getSyncId()));
        ...
        value.setAttributeNS(XMI, "xmi:id", getId(ExportRegel.II_ASSOCIATION_DST_CARD_LO, naam, att.getSyncId()));
        value.setAttribute("value", att.isVerplicht() ? "1" : "0");

        value = document.createElement("upperValue");
        ownedEnd.appendChild(value);
        value.setAttributeNS(XMI, "xmi:type", "uml:LiteralInteger");
        value.setAttributeNS(XMI, "xmi:id", getId(ExportRegel.II_ASSOCIATION_DST_CARD_HI, naam, att.getSyncId()));
        ...
        associatie.appendChild(memberEnd);
        memberEnd.setAttributeNS(XMI, "xmi:idref", getId(ExportRegel.II_ASSOCIATION_SRC, naam, att.getSyncId()));

        ownedEnd = document.createElement("ownedEnd");
        associatie.appendChild(ownedEnd);
        ownedEnd.setAttributeNS(XMI, "xmi:type", "uml:Property");
        ownedEnd.setAttributeNS(XMI, "xmi:id", getId(ExportRegel.II_ASSOCIATION_SRC, naam, att.getSyncId()));
        ownedEnd.setAttribute("association", getId(ExportRegel.II_ASSOCIATION, naam, att.getSyncId()));
        ownedEnd.setAttribute("visibility", "public");
        ownedEnd.setAttribute("isStatic", "false");
        ownedEnd.setAttribute("isReadOnly", "false");
        ...
        ownedEnd.appendChild(type);
        type.setAttributeNS(XMI, "xmi:idref", getId(ExportRegel.II_OBJECTTYPE, srcType.getNaam(), srcType.getSyncId()));

        value = document.createElement("lowerValue");
        ownedEnd.appendChild(value);
        value.setAttributeNS(XMI, "xmi:type", "uml:LiteralInteger");
        value.setAttributeNS(XMI, "xmi:id", getId(ExportRegel.II_ASSOCIATION_SRC_CARD_LO, naam, att.getSyncId()));
        value.setAttribute("value", "0");

        value = document.createElement("upperValue");
        ownedEnd.appendChild(value);
        value.setAttributeNS(XMI, "xmi:type", "uml:LiteralUnlimitedNatural");
        value.setAttributeNS(XMI, "xmi:id", getId(ExportRegel.II_ASSOCIATION_SRC_CARD_HI, naam, att.getSyncId()));
        value.setAttribute("value", "-1");
    ...
    private void genereerObjectType(final ObjectType type, final Element parent, final InSetOfModel inSom) {
        ...
        for (Attribuut att : type.getAttributen(inSom)) {
            ...
            if (att.getType().isAttribuutType()) {
                ...
                } else if (SoortExport.getHuidigeSoort() == SoortExport.GS) {
                    /*
                     * Bij export van het Logisch Model wordt hiervoor een associatie geëxporteerd. Hier dus niks doen.
                     */
                }
            } else {
