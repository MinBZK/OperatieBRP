...
public class StandardJavaType extends AbstractType {
    ...
    static {
        ...
        javaTypes.put("id klein", "Short");
        javaTypes.put("id normaal", "Integer");
        javaTypes.put("id groot", "Long");
    }
    ...
    public String javaType() {
        final String javaType = javaTypes.get(type.getType().getNaam().toLowerCase());
        if (javaType == null) {
            throw new IllegalStateException("Kan geen java type vinden voor: " + type.getType().getNaam());
        }
        return javaType;
    }
