...
public class Main {
            ...
            final String... generatorNames)
    {
        ...
        /*
         * Het volgende statement lijkt overbodig, maar heeft als belangrijk side-effect dat het BmrPackage bij EMF
         * geregistreerd wordt.
         */
        ...
        MetaRegister metaRegister = (MetaRegister) modelElements.get(0);
        for (String name : generatorNames) {
            ...
            if (generator != null) {
                if ("Domein".equals(generatorSpecificatie.getContainer())) {
                    modelElement = metaRegister.getDomein(modelNaam);
                } else if ("Applicatie".equals(generatorSpecificatie.getContainer())) {
                    modelElement = metaRegister.getApplicatie(modelNaam);
                }
                LOGGER.info("Generator '{}' voor {} '{}' naar directory '{}'.", new String[] { name,
                    generatorSpecificatie.getContainer(), modelElement.getNaam(), new File(target).getAbsolutePath() });
                generator.generate(modelElement, file);
            } else {
