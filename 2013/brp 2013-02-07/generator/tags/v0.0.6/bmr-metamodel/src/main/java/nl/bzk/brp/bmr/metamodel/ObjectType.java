...
public class ObjectType extends Type {
    ...
    public Iterable<Groep> getGroepen(final InSetOfModel inSom) {
        return Iterables.filter(getGroepen(), new Predicate<Groep>() {

            @Override
            public boolean apply(final Groep groep) {
                return Arrays.asList(inSom, InSetOfModel.B).contains(groep.getInSetOfModel());
            }
        });
    }
