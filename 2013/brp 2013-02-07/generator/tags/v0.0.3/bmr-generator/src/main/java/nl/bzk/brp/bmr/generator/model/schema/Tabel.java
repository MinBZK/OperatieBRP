...
public class Tabel extends SchemaElement {
    ...
    private void addDataRegels(final Kolom kolom) {
        for (Tuple tuple : objectType.getTuples()) {
            ...
            // Patch RvdP: de soort actie tabel kent een 'categorie'; idem voor de soort document tabel
            // In het BMR ontbreken echter deze kolommen. Invoegen gaat dus NIET goed.
            // Hier een patch om ze toch te vullen: in dat geval wordt veld X misbruikt.
            // HEEL LELIJK, maar wel gepatched: Kern.SrtActie
            else if ("CategorieActie".equals(kolomNaam)) {
                waarde = String.valueOf(tuple.getDatumAanvangGeldigheid());
