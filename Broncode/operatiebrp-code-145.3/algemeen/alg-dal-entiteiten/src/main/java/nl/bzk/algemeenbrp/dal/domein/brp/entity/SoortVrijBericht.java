...
@Table(name = "srtvrijber", schema = "kern")
@Cacheable
@Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
@NamedQuery(name = "SoortVrijBericht" + Constanten.ZOEK_ALLES_VOOR_CACHE, query = "from SoortVrijBericht")
public class SoortVrijBericht extends AbstractEntiteit implements Serializable, DynamischeStamtabelMetGeldigheid {
    private static final long serialVersionUID = 1L;

    @Id
    @SequenceGenerator(name = "srtvrijber_id_generator", sequenceName = "kern.seq_srtvrijber", allocationSize = 1)
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "srtvrijber_id_generator")
     ...
     * @param naam naam
     */
    public SoortVrijBericht(final String naam) {
        setNaam(naam);
    }
