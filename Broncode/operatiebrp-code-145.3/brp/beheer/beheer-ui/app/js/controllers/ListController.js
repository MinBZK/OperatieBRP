angular.module('ListController', [])
    .controller('ListController', ['$window', '$route', '$q', '$location', '$scope', 'lijst', 'item', 'resource', 'config', '$rootScope', 'instelling', 'context',
        function ($window, $route, $q, $location, $scope, lijst, item, resource, config, $rootScope, instelling, context) {
            ...
            $scope.ngFilter = config.ngFilter;
            ...
            for (var i = 0; i < config.kolommen.length; i++) {
                if (config.kolommen[i].display) {
                    ...
                    if(config.kolommen[i].type === 'selectLijst'){
                        $scope[config.kolommen[i].id + 'SelectList'] = [];
                    }
                }
            }
                        ...
                        if (config.lijstBevatGeenDetail) {
                            ...
                            $scope.resource.get({id: value.id}, function (data) {
                                ...
                                if (config.contextParam) {
                                    ...
                                    angular.forEach(config.extraContextParams, function (element) {
                                        context.params[element] = $scope.onderhandenitem.item[element];
                                    });
                                }
                            });
                        } else {
            ...
            $scope.reQuery = function () {
                ...
                if (!sort && config.sortering) {
                    sort = config.sortering;
                }
            ...
            $scope.activateItem = function (event) {
                if (event.ctrlKey === 1) {
                    $window.open($window.location.pathname + '#' + config.listUri + '/' + $scope.lijst.content[this.$index].id);
                } else {
                    tonenDetailVoorbereiden($scope.lijst.content[this.$index]);
