/**
 * This file is copyright 2017 State of the Netherlands (Ministry of Interior Affairs and Kingdom Relations).
 * It is made available under the terms of the GNU Affero General Public License, version 3 as published by the Free Software Foundation.
 * The project of which this file is part, may be found at www.github.com/MinBZK/operatieBRP.
 */

package nl.bzk.brp.domain.berichtmodel;

import nl.bzk.algemeenbrp.dal.domein.brp.entity.SoortVrijBericht;

/**
 * VrijBericht.
 */
public final class VrijBericht {

    private SoortVrijBericht soortVrijBericht;
    private String inhoud;

    /**
     * Constructor.
     *
     * @param inhoud inhoud van het vrije bericht
     * @param soortVrijBericht het soort vrije bericht.
     */
    public VrijBericht(final String inhoud, final SoortVrijBericht soortVrijBericht) {
        this.soortVrijBericht = soortVrijBericht;
        this.inhoud = inhoud;
    }

    public SoortVrijBericht getSoortVrijBericht() {
        return soortVrijBericht;
    }

    public String getInhoud() {
        return inhoud;
    }


}
