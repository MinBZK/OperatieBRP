...
/**
 * De parser voor synchroniseer persoon verzoeken.
 */
final class SynchroniseerPersoonVerzoekParser extends AbstractSynchronisatieVerzoekParser {

    @Override
    public String getPrefix() {
        return SLASH + BRP_NAMESPACE_PREFIX + SoortBericht.LVG_SYN_GEEF_SYNCHRONISATIE_PERSOON.getIdentifier();
    }

    /**
     * Vul diensspecifieke gegevens.
    ...
    public void vulDienstSpecifiekeGegevens(final SynchronisatieVerzoek verzoek, final Node node, final XPath xPath) {
        ...
        verzoek.getZoekCriteriaPersoon()
                .setCommunicatieId(getNodeTextContent(getPrefix() + "/brp:identificatiecriteria/@brp:communicatieID", xPath, node));
        verzoek.getZoekCriteriaPersoon()
                .setBsn(getNodeTextContent(getPrefix() + "/brp:identificatiecriteria/brp:burgerservicenummer", xPath, node));
    }
