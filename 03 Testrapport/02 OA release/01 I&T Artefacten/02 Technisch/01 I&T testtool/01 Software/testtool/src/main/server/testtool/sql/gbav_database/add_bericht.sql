...
INSERT INTO lo3_pl (pl_id, mutatie_dt, mutatie_activiteit_id) select case when max(pl.pl_id) is null then 1 else max(pl.pl_id)+1 end, '{mutatie_dt}', case when max(bericht.bericht_activiteit_id) is null then 1 else max(bericht.bericht_activiteit_id) end from lo3_pl pl, lo3_bericht bericht;
