...
CREATE INDEX ix_PersAdres_IdentcodeAdresseerbaarObjectU ON Kern.PersAdres (brp_unaccent(IdentcodeAdresseerbaarObject) varchar_pattern_ops) WHERE IdentcodeAdresseerbaarObject IS NOT NULL; -- Index door expliciete index in model
CREATE INDEX ix_PersAdres_IdentcodeNraandU ON Kern.PersAdres (brp_unaccent(IdentcodeNraand) varchar_pattern_ops) WHERE IdentcodeNraand IS NOT NULL; -- Index door expliciete index in model
...
CREATE INDEX ix_His_PersAdres_IdentcodeAdresseerbaarObjectU ON Kern.His_PersAdres (brp_unaccent(IdentcodeAdresseerbaarObject) varchar_pattern_ops, TsVerval) WHERE IdentcodeAdresseerbaarObject IS NOT NULL; -- Index door expliciete index in model
CREATE INDEX ix_His_PersAdres_IdentcodeNraandU ON Kern.His_PersAdres (brp_unaccent(IdentcodeNraand) varchar_pattern_ops) WHERE TsVerVal IS NULL AND IdentcodeNraand IS NOT NULL; -- Index door expliciete index in model
