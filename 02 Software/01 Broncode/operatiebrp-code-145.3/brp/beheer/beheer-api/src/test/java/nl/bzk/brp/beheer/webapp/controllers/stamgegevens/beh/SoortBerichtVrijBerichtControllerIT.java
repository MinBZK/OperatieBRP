...
public class SoortBerichtVrijBerichtControllerIT extends AbstractDatabaseTest {
     ...
     */
    @Test
    public final void testSoortBerVrijBer() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/soortbervrijber/"))
               ...
               .andExpect(MockMvcResultMatchers.jsonPath("$.content[0].naam").value("Stuur vrij bericht"))
               .andExpect(MockMvcResultMatchers.jsonPath("$.content[1].id").value(2))
               .andExpect(MockMvcResultMatchers.jsonPath("$.content[1].naam").value("Verwerk vrij bericht"))
               .andReturn();
    }
}
