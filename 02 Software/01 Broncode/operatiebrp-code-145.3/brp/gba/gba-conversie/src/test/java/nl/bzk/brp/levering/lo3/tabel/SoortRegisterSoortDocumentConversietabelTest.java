/**
 * This file is copyright 2017 State of the Netherlands (Ministry of Interior Affairs and Kingdom Relations).
 * It is made available under the terms of the GNU Affero General Public License, version 3 as published by the Free Software Foundation.
 * The project of which this file is part, may be found at https://github.com/MinBZK/operatieBRP.
 */

package nl.bzk.brp.levering.lo3.tabel;

import nl.bzk.migratiebrp.conversie.model.brp.attribuut.BrpSoortDocumentCode;
import org.junit.Assert;
import org.junit.Test;

/**
 * Test voor {@link SoortRegisterSoortDocumentConversietabel}.
 */
public class SoortRegisterSoortDocumentConversietabelTest {

    private static final String Z = "Z";

    @Test
    public void test() throws ReflectiveOperationException {
        final SoortRegisterSoortDocumentConversietabel subject = new SoortRegisterSoortDocumentConversietabel();

        Assert.assertEquals(null, subject.converteerNaarLo3(new BrpSoortDocumentCode(Z)));
    }
}
