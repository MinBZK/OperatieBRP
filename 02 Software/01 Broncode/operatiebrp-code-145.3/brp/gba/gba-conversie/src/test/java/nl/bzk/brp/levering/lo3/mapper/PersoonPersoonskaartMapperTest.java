...
public class PersoonPersoonskaartMapperTest extends AbstractMapperTestBasis {

    private final PersoonPersoonskaartMapper mapper = new PersoonPersoonskaartMapper();
    ...
    public void testSucces() {
        final BrpStapel<BrpPersoonskaartInhoud> brpInhoud =
                doMapping(mapper, MetaObjectUtil.maakIngeschrevene(b -> MetaObjectUtil.voegPersoonPersoonskaartGroepToe(b, true, "051801")));

        Assert.assertNotNull(brpInhoud);
        Assert.assertTrue(brpInhoud.size() == 1);
        Assert.assertEquals("051801", brpInhoud.get(0).getInhoud().getGemeentePKCode().getWaarde());
        Assert.assertEquals(Boolean.TRUE, brpInhoud.get(0).getInhoud().getIndicatiePKVolledigGeconverteerd().getWaarde());
    }

    @Test
    public void testLeeg() {
        final BrpStapel<BrpPersoonskaartInhoud> brpInhoud = doMapping(mapper, MetaObjectUtil.maakIngeschrevene());
    ...
    public void testGeenWaarde() {
        final BrpStapel<BrpPersoonskaartInhoud> brpInhoud =
                doMapping(mapper, MetaObjectUtil.maakIngeschrevene(b -> MetaObjectUtil.voegPersoonPersoonskaartGroepToe(b, null, null)));

        Assert.assertNotNull(brpInhoud);
        Assert.assertTrue(brpInhoud.size() == 1);
        Assert.assertNull(brpInhoud.get(0).getInhoud().getGemeentePKCode());
        Assert.assertEquals(null, brpInhoud.get(0).getInhoud().getIndicatiePKVolledigGeconverteerd());
    }
}
