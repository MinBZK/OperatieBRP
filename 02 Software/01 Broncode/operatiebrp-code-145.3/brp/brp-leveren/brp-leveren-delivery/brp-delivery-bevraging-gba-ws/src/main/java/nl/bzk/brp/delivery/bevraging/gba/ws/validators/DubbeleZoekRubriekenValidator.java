...
/**
 * Valideert de zoek criteria op dubbele rubrieken.
 * @param <T> subtype van WebserviceBericht dat gevalideerd dient te worden
 */
public class DubbeleZoekRubriekenValidator<T extends WebserviceBericht> implements Validator<T> {
    @Override
    public Optional<Antwoord> apply(final WebserviceBericht vraag) {
        Set<Integer> rubrieken = DubbeleRubriekenValidator.bepaalDubbele(vraag.getZoekRubrieken());
        return rubrieken.isEmpty()
                ? Optional.empty()
                : Optional.of(Antwoorden.foutief(
                        AntwoordBerichtResultaat.TECHNISCHE_FOUT_024,
