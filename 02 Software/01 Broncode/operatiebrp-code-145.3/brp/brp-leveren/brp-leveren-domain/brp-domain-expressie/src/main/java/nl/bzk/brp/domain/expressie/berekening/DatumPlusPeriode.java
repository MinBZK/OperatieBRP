...
/**
 * {@link DatumLiteral} plus {@link PeriodeLiteral} berekening.
 */
@Component
@BerekeningConfig(operator = OperatorType.PLUS, typeLinks = ExpressieType.DATUM, typeRechts = ExpressieType.PERIODE)
final class DatumPlusPeriode implements Berekening<DatumLiteral, PeriodeLiteral> {
    @Override
    public Expressie apply(final DatumLiteral datumLiteral, final PeriodeLiteral periodeLiteral) {
        ...
        if (datumLiteral.isVolledigBekend()) {
            final ZonedDateTime dt = datumLiteral.alsDateTime()
                    .plusYears(periodeLiteral.getJaar().isBekend() ? periodeLiteral.getJaar().getWaarde() : 0)
                    .plusMonths(periodeLiteral.getMaand().isBekend() ? periodeLiteral.getMaand().getWaarde() : 0)
                    .plusDays(periodeLiteral.getDag().isBekend() ? periodeLiteral.getDag().getWaarde() : 0);
