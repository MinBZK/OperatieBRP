...
@regels                 R2269
@usecase                BY.1.AA
@sleutelwoorden         AUAH01C70T40, Geslaagd, Logging
...
Given voer enkele update uit: update kern.his_partij set dateinde = to_number((to_char((now() at time zone 'UTC') + interval '1 day', 'YYYYMMDD')),
'99999999')
where his_partij.id = (select his_partij.partij from kern.his_partij where partij = 27017)

Given maak bijhouding caches leeg

Scenario: 2. De ondertekenaar is een geldige partij(datum voor dateinde)
          LT: AUAH01C70T40
...
When voer een bijhouding uit AUAH01C70T40.xml met ondertekenaar 'Gemeente BRP 2', en transporteur 'Gemeente BRP 1'
...
And is het antwoordbericht gelijk aan /testcases/bijhouding/AUAH/expected/AUAH01C70T40.xml voor expressie //brp:bhg_hgpRegistreerHuwelijkGeregistreerdPartnerschap_R

Then is in de database de persoon met bsn 188214161 wel als PARTNER betrokken bij een HUWELIJK
Then is in de database de persoon met bsn 894800905 wel als PARTNER betrokken bij een HUWELIJK

Then lees persoon met anummer 4291493921 uit database en vergelijk met expected AUAH01C70T40-persoon1.xml
Then lees persoon met anummer 1018496537 uit database en vergelijk met expected AUAH01C70T40-persoon2.xml

Scenario: 3. DB reset
          postconditie
Given de database is aangepast met: update kern.his_partij set dateinde = null where his_partij.id = (select his_partij.partij from kern.his_partij where partij = 27017)
