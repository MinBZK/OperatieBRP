...
Scenario:   Meerdere nationaliteiten waarbij de nationaliteit bij geboorte wordt beëindigd en kind de NL nationaliteit krijgt
            LT: ERKE04C40T20

Given alle personen zijn verwijderd
Given enkel initiele vulling uit bestand /LO3PL-AS/LO3PL-ERKE/ERKE04C40T20-001.xls
Given enkel initiele vulling uit bestand /LO3PL-AS/LO3PL-ERKE/ERKE04C40T20-002.xls

When voer een bijhouding uit ERKE04C40T20a.xml namens partij 'Gemeente BRP 1'
Then heeft het antwoordbericht verwerking Geslaagd

Then heeft $RELATIE_ID$ de waarde van de volgende query: select r.id from kern.betr b join kern.relatie r on r.id = b.relatie join kern.pers p on b.pers = p.id where p.bsn in ('443684169') and r.srt=3
Then heeft $KIND_BETROKKENHEID$ de waarde van de volgende query: select b.id from kern.betr b join kern.relatie r on r.id = b.relatie join kern.pers p on b.pers = p.id where p.bsn in ('443684169') and r.srt=3
Then heeft $NATI_ID$ de waarde van de volgende query: select n.id from kern.persnation n join kern.pers p on p.id = n.pers where p.bsn = '443684169'


When voer een bijhouding uit ERKE04C40T20b.xml namens partij 'Gemeente BRP 1'
Then heeft het antwoordbericht verwerking Geslaagd
...
Then in kern heeft select sb.naam as soortrol, sp.naam as srtpersoon, p.voornamen
                   ...
                   where b.relatie in (select relatie from kern.betr where pers in (select id from kern.pers where bsn='443684169'))
...
Then in kern heeft select sainh.naam as actieInhoud, saav.naam as actieVerval, CASE WHEN hpn.tsverval IS NULL THEN 'Leeg' ELSE 'Gevuld' END AS tsverval, hpn.dataanvgel, hpn.dateindegel,
                   saaa.naam as actieAanp, n.code, hpn.indbijhoudingbeeindigd
