...
@regels                 R2268
@usecase                BY.1.AA
@sleutelwoorden         AUAH01C60T30, Geslaagd, Logging
...
Given de database is aangepast met: update kern.partij set dateinde = to_number((to_char(now(), 'YYYYMMDD')), '99999999') where code = '507013'
And de database is aangepast met: update kern.his_partij set dateinde = to_number((to_char(now(), 'YYYYMMDD')), '99999999') where his_partij.id = (select his_partij.partij from kern.his_partij where partij = 27012)

Given maak bijhouding caches leeg

Scenario: 2. De geautoriseerde partij is geen geldige partij(datum op dateinde)
          LT: AUAH01C60T30
...
When voer een bijhouding uit AUAH01C60T30.xml namens partij 'Gemeente BRP 1'
...
And is het antwoordbericht gelijk aan /testcases/bijhouding/AUAH/expected/AUAH01C60T30.xml voor expressie //brp:bhg_hgpRegistreerHuwelijkGeregistreerdPartnerschap_R

Then is in de database de persoon met bsn 947549705 niet als PARTNER betrokken bij een HUWELIJK
Then is in de database de persoon met bsn 793373001 niet als PARTNER betrokken bij een HUWELIJK

Scenario: 3. DB reset
          postconditie
Given de database is aangepast met: update kern.partij set dateinde = null where code = '507013'
And de database is aangepast met: update kern.his_partij set dateinde = null where his_partij.id = (select his_partij.partij from kern.his_partij where partij = 27012)
