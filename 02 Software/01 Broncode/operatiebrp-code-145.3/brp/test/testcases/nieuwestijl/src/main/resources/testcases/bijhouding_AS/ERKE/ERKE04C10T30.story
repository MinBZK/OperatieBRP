...
Scenario:   De woongemeente van het kind dient een bijhoudingsvoorstel in inzake erkenning
            LT: ERKE04C10T30

Given alle personen zijn verwijderd
Given enkel initiele vulling uit bestand /LO3PL-AS/LO3PL-ERKE/ERKE04C10T30-001.xls
Given enkel initiele vulling uit bestand /LO3PL-AS/LO3PL-ERKE/ERKE04C10T30-002.xls

When voer een bijhouding uit ERKE04C10T30a.xml namens partij 'Gemeente BRP 1'
Then heeft het antwoordbericht verwerking Geslaagd

When voer een bijhouding uit ERKE04C10T30b.xml namens partij 'Gemeente BRP 2'
Then heeft het antwoordbericht verwerking Geslaagd

Then heeft $RELATIE_ID$ de waarde van de volgende query: select r.id from kern.betr b join kern.relatie r on r.id = b.relatie join kern.pers p on b.pers = p.id where p.bsn in ('537979049') and r.srt=3
Then heeft $KIND_BETROKKENHEID$ de waarde van de volgende query: select b.id from kern.betr b join kern.relatie r on r.id = b.relatie join kern.pers p on b.pers = p.id where p.bsn in ('537979049') and r.srt=3

When voer een bijhouding uit ERKE04C10T30c.xml namens partij 'Gemeente BRP 2'
Then heeft het antwoordbericht verwerking Geslaagd
Then is het antwoordbericht gelijk aan /testcases/bijhouding_AS/ERKE/expected/ERKE04C10T30.xml voor expressie /
...
Then in kern heeft select sb.naam as soortrol, sp.naam as srtpersoon, p.voornamen
                   ...
                   where b.relatie in (select relatie from kern.betr where pers in (select id from kern.pers where bsn='537979049'))
