...
Scenario:   2. Ongedaan maken van een huwelijk tussen BRP (I) en BRP (P), nav 'S', rechtsgrond, 2 bronnen, registergemeente huwelijk
            LT: ONHW04C10T20

Given alle personen zijn verwijderd
Given enkel initiele vulling uit bestand /LO3PL-ONHW/ONHW04C10T20-001.xls

When voer een bijhouding uit ONHW04C10T20a.xml namens partij 'Gemeente BRP 1'

Given pas laatste relatie van soort 1 aan tussen persoon 488001705 en persoon 485777289 met relatie id 50000001 en betrokkenheid id 50000001
Then heeft het antwoordbericht verwerking Geslaagd

Then is in de database de persoon met bsn 485777289 wel als PARTNER betrokken bij een HUWELIJK
Then is in de database de persoon met bsn 488001705 wel als PARTNER betrokken bij een HUWELIJK

When voer een bijhouding uit ONHW04C10T20b.xml namens partij 'Gemeente BRP 1'

Then heeft het antwoordbericht verwerking Geslaagd

Then is in de database de persoon met bsn 485777289 niet als PARTNER betrokken bij een HUWELIJK
Then is in de database de persoon met bsn 488001705 niet als PARTNER betrokken bij een HUWELIJK

Scenario: 3. DB inhoud
          postconditie

!-- Controleer verantwoordingsgegevens van de ActieBron
Then in kern heeft select sdoc.naam
                                      ...
                                      where sainh.naam = 'Verval huwelijk' order by sdoc.naam de volgende gegevens:
| veld   | waarde                                  |
| naam   | Ministerieel besluit                    |
----
...
Then in kern heeft select p.voornamen, id.datgeboorte, sainh.naam as actieinhoud, saav.naam as actieVerval, id.nadereaandverval, CASE WHEN tsverval IS NULL THEN 'Leeg' ELSE 'Gevuld' END AS tsverval
                   ...
                   left join kern.actie av on av.id = id.actieverval
                   left join kern.srtactie saav on av.srt = saav.id
                   where saav.naam = 'Verval huwelijk' or p.voornamen = 'Libby'
                   order by p.voornamen de volgende gegevens:
| veld              | waarde                         |
| voornamen         | Libby                          |
...
Then in kern heeft select indagsamengesteldenaam, geslnaamstam from kern.pers where geslnaamstam is NULL de volgende gegevens:
| veld                    | waarde                         |
| indagsamengesteldenaam  | false                          |
| geslnaamstam            | NULL                           |
