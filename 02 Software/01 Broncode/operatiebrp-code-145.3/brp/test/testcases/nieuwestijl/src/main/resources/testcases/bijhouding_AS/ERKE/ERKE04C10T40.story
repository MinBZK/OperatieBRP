...
Scenario:   Erkenning zorgt ervoor dat Bijzondere verblijfsrechtelijke positie wordt verkregen
            LT: ERKE04C10T40

Given alle personen zijn verwijderd
Given enkel initiele vulling uit bestand /LO3PL-AS/LO3PL-ERKE/ERKE04C10T40-001.xls
Given enkel initiele vulling uit bestand /LO3PL-AS/LO3PL-ERKE/ERKE04C10T40-002.xls

When voer een bijhouding uit ERKE04C10T40a.xml namens partij 'Gemeente BRP 1'
Then heeft het antwoordbericht verwerking Geslaagd

Then heeft $RELATIE_ID$ de waarde van de volgende query: select r.id from kern.betr b join kern.relatie r on r.id = b.relatie join kern.pers p on b.pers = p.id where p.bsn in ('972317065') and r.srt=3
Then heeft $KIND_BETROKKENHEID$ de waarde van de volgende query: select b.id from kern.betr b join kern.relatie r on r.id = b.relatie join kern.pers p on b.pers = p.id where p.bsn in ('972317065') and r.srt=3

When voer een bijhouding uit ERKE04C10T40b.xml namens partij 'Gemeente BRP 1'
Then heeft het antwoordbericht verwerking Geslaagd
...
Then in kern heeft select sb.naam as soortrol, sp.naam as srtpersoon, p.voornamen
                   ...
                   where b.relatie in (select relatie from kern.betr where pers in (select id from kern.pers where bsn='972317065'))
