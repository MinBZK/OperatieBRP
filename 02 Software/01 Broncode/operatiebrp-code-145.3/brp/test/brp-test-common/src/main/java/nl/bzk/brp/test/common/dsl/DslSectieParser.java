...
/**
 * De autautDSL bestaat uit secties (blokken) met key/value paren. Deze low-level parser maakt deze blokken en groepering op regels.
 */
public final class DslSectieParser {
     ...
     * @throws IOException
     */
    public static List<DslSectie> parse(final Resource resource) {
        final DslSectieParser parser;
        try {
            try (InputStream inputStream = resource.getInputStream()) {
                parser = new DslSectieParser(IOUtils.readLines(inputStream, StandardCharsets.UTF_8));
            }
        } catch (IOException e) {
