...
public class IT_PDT_NaderePopulatiebeperking {

    private final String expressie;

    public IT_PDT_NaderePopulatiebeperking(final String expressie) {
    ...
    public static Collection<String> data() throws Exception {
        try (final InputStream inputStream = new ClassPathResource("/pdtexpressie/dienstbundel-naderepopbep.txt").getInputStream()) {
            return IOUtils.readLines(inputStream, StandardCharsets.UTF_8).
                    stream().filter(s -> !s.contains("SELECTIE_DATUM")).collect(Collectors.toList());
        }
    }
