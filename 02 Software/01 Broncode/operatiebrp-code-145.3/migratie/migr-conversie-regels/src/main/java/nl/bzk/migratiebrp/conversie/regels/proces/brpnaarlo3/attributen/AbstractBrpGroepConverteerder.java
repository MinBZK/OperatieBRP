 ...
 */
@Component
public abstract class AbstractBrpGroepConverteerder<B extends BrpGroepInhoud, L extends Lo3CategorieInhoud> implements BrpGroepConverteerder<B, L> {
     ...
     */
    public AbstractBrpGroepConverteerder(final BrpAttribuutConverteerder attribuutConverteerder) {
        this.attribuutConverteerder = attribuutConverteerder;
    }

    /**
     * Geeft de {@link BrpAttribuutConverteerder} terug.
     * @return een instantie van de {@link BrpAttribuutConverteerder}
     */
    protected BrpAttribuutConverteerder getAttribuutConverteerder() {
        return attribuutConverteerder;
    }
    ...
    private static <T extends BrpGroepInhoud> List<BrpGroep<T>> bepaalGecreeerdeRijen(final BrpActie actie, final List<BrpGroep<T>> groepen) {
        ...
        for (final BrpGroep<T> groep : groepen) {
            boolean actieAlsActAanpGeld = groep.getActieGeldigheid() != null && actieId.equals(groep.getActieGeldigheid().getId());
            if (actieAlsActAanpGeld || actieAlsActieInhGeenActAanpGeld(actieId, groep) || actieAlsActInhEnActGeldDoorConversie(actieId, groep)) {
    ...
    private static <T extends BrpGroepInhoud> boolean actieAlsActieInhGeenActAanpGeld(Long actieId, BrpGroep<T> groep) {
        return groep.getActieInhoud() != null && actieId.equals(groep.getActieInhoud().getId())
                && groep.getActieGeldigheid() == null;
    }

    private static <T extends BrpGroepInhoud> boolean actieAlsActInhEnActGeldDoorConversie(Long actieId, BrpGroep<T> groep) {
        return groep.getActieGeldigheid() != null
    ...
    private static <T extends BrpGroepInhoud> SortedSet<BrpDatum> bepaalGeldigheidsdatums(
        ...
        for (final BrpGroep<T> rij : gecreeerdeRijen) {
            AbstractBrpGroepConverteerder.voegDatumToe(datums, rij.getHistorie().getDatumAanvangGeldigheid());
            AbstractBrpGroepConverteerder.voegDatumToe(datums, rij.getHistorie().getDatumEindeGeldigheid());
        }
    ...
    private static <T extends BrpGroepInhoud> BrpGroep<T> bepaalGeldigeRij(final Collection<BrpGroep<T>> rijen, final BrpDatum datum) {
        ...
        for (final BrpGroep<T> rij : rijen) {
            ...
            final boolean datumGelijkAanvangNaEinde = datum.compareTo(aanvang) == 0 && (einde == null || datum.compareTo(einde) > 0);
            final boolean datumVoorEindeOfEindeLeeg = einde == null || datum.compareTo(einde) < 0;
            final boolean datumGelijkAanAanvangEnEinde = einde != null && datum.compareTo(aanvang) == 0 && datum.compareTo(einde) == 0;
            if (datumGelijkAanvangNaEinde || datum.compareTo(aanvang) >= 0 && (datumVoorEindeOfEindeLeeg || datumGelijkAanAanvangEnEinde)) {
                result = rij;
                if (AbstractBrpGroepConverteerder.isPrevalerend(rij)) {
                    break;
                }
            }
        }
    ...
    private static BrpDatum maximaliseerOnbekendeDatum(final BrpDatum datum) {
        ...
        Integer datumWaarde = datum.getWaarde();
    ...
    private static boolean checkUitzonderingConversieActieAanpassingGeldigheid(final BrpActie actie, final List<? extends BrpGroep<?>> groepen) {
        ...
        for (final BrpGroep<?> groep : groepen) {
            ...
            boolean gekoppeldAlsInhoud = groep.getActieInhoud() != null && actieId.equals(groep.getActieInhoud().getId());
            boolean nietGekoppeldAlsGeldig = groep.getActieGeldigheid() == null || !actieId.equals(groep.getActieGeldigheid().getId());
            boolean gekoppeldAlsVerval = groep.getActieVerval() != null && actieId.equals(groep.getActieVerval().getId());
            if (gekoppeldAlsInhoud || nietGekoppeldAlsGeldig || gekoppeldAlsVerval) {
                return false;
            }
        }
    ...
    private static boolean checkUitzonderingAlleRijenNadereAanduidingVervalGevuld(final List<? extends BrpGroep<?>> groepen) {
        for (final BrpGroep<?> groep : groepen) {
            if (!BrpValidatie.isAttribuutGevuld(groep.getHistorie().getNadereAanduidingVerval())) {
                return false;
            }
        }
     ...
     */
    private static boolean checkActieInhoudKomtVoorInAndereRij(final List<? extends BrpGroep<?>> groepen, final BrpStapel<? extends BrpGroepInhoud> stapel) {
        for (final BrpGroep<?> groep : groepen) {
            if (groep.getActieVerval() != null && checkActieInhoudKomtVoorInAndereRijControleerStapel(stapel, groep)) {
                return true;
            }
        }
    ...
    private static boolean checkActieInhoudKomtVoorInAndereRijControleerStapel(BrpStapel<? extends BrpGroepInhoud> stapel, BrpGroep<?> groep) {
        for (final BrpGroep<?> groep2 : stapel) {
            if (!groep2.equals(groep) && groep2.getActieVerval() == null && groep.getActieInhoud().getId().equals(groep2.getActieInhoud().getId())) {
                return true;
            }
        }
        return false;
     ...
     * @param actie actie die nu verwerkt wordt
     * @param groepen brp groepen
     * @param stapel brp stapel
     * @param categorieen lo3 categorieen
     */
    @Override
    public final void converteer(
        ...
        if (AbstractBrpGroepConverteerder.checkUitzonderingConversieActieAanpassingGeldigheid(actie, groepen)) {
    ...
    private void verwerkBzmAlgoritme(final BrpActie actie, final List<BrpGroep<B>> groepen, final List<Lo3CategorieWrapper<L>> categorieen) {
        ...
        final List<BrpGroep<B>> gecreeerdeRijen = AbstractBrpGroepConverteerder.bepaalGecreeerdeRijen(actie, groepen);
        final List<BrpGroep<B>> vervallenRijen = AbstractBrpGroepConverteerder.bepaalVervallenRijen(actie, groepen);

        getLogger().debug("Gecreeerde rijen ({})", gecreeerdeRijen.size());
        getLogger().debug("Vervallen rijen ({})", vervallenRijen.size());

        final List<BrpDatum> geldigheidsdatums = new ArrayList<>(AbstractBrpGroepConverteerder.bepaalGeldigheidsdatums(gecreeerdeRijen, vervallenRijen));
        ...
        for (int datumIndex = 0; datumIndex < geldigheidsdatums.size(); datumIndex++) {
            ...
            final BrpGroep<B> gecreeerdeRij = AbstractBrpGroepConverteerder.bepaalGeldigeRij(gecreeerdeRijen, datum);
            final BrpGroep<B> vervallenRij = AbstractBrpGroepConverteerder.bepaalGeldigeRij(vervallenRijen, datum);
    ...
    private void verwerkBzmWordtGevuld(
        ...
        if (actieInhoudId.equals(actieVervalId)
                ...
                && BrpValidatie.isAttribuutGevuld(nadereAanduidingVerval)) {
     ...
     * @param groepen de rijen waarin gezocht moet worden
     ...
     * @param actie actie die nu verwerkt wordt
     * @param categorieWrapper categorieWrapper
     * @param ingang ingangdatum van huidige voorkomen
     ...
     */
    private Lo3CategorieWrapper<L> vulInhoud(final Lo3CategorieWrapper<L> wrapper, final BrpGroep<B> brp, final B brpVorige) {
    ...
    private Lo3CategorieWrapper<L> bubbelMaakOnjuist(final Lo3CategorieWrapper<L> categorieWrapper) {
        ...
        final Lo3Historie historie = new Lo3Historie(Lo3IndicatieOnjuistEnum.ONJUIST.asElement(), oud.getIngangsdatumGeldigheid(), oud.getDatumVanOpneming());
    ...
    private void verwerkUitzonderingConversieActieAanpassingGeldigheid(
        ...
        if (AbstractBrpGroepConverteerder.checkUitzonderingAlleRijenNadereAanduidingVervalGevuld(gecreeerdeRijen)) {
            getLogger().debug("uitzondering situatie 1: Alle rijen hebben een actie verval");
            if (AbstractBrpGroepConverteerder.checkUitzonderingAlleRijenAanvangGelijkAanEindeGeldigheid(gecreeerdeRijen)
    ...
    private boolean maakLo3RijAan(
        ...
        if (nieuweRij == null) {
            ...
            Lo3Documentatie documentatie = new BrpDocumentatieConverteerder(attribuutConverteerder).maakDocumentatie(actie);
            if (basis != null
            ...
            if (BrpSoortActieCode.CONVERSIE_GBA_LEEG_CATEGORIE_ONJUIST.equals(actie.getSoortActieCode())) {
                ...
                nieuweHistorie = new Lo3Historie(new Lo3IndicatieOnjuist("O", onderzoek), historie.getIngangsdatumGeldigheid(), historie.getDatumVanOpneming());
            } else {
