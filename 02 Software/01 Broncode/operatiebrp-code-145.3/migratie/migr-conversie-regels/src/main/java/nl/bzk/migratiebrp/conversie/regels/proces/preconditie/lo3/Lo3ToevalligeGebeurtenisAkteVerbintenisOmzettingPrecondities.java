...
/**
 * Preconditie controles voor een toevallige gebeurtenis: akte 3H en 5H.
 *
 * Maakt gebruik van de {@link nl.bzk.migratiebrp.conversie.regels.proces.logging.Logging#log Logging.log} methode.
 */
public final class Lo3ToevalligeGebeurtenisAkteVerbintenisOmzettingPrecondities extends AbstractLo3ToevalligeGebeurtenisPrecondities {

    /**
     * Constructor.
     * @param conversieTabelFactory {@link ConversietabelFactory}
     */
    public Lo3ToevalligeGebeurtenisAkteVerbintenisOmzettingPrecondities(
     ...
     * @param historischeSoortVerbintenis soort verbintenis (historisch)
     * @param actueleSoortVerbintenis soort verbintenis (actueel)
     */
    public void controleerToevalligeGebeurtenis(
            final Lo3ToevalligeGebeurtenis toevalligeGebeurtenis,
            final Lo3SoortVerbintenisEnum historischeSoortVerbintenis,
            final Lo3SoortVerbintenisEnum actueleSoortVerbintenis) {
        ...
        controleerVerbintenis(toevalligeGebeurtenis.getVerbintenis(), historischeSoortVerbintenis, actueleSoortVerbintenis);

        // Categorie 06: Overlijden
        controleerNietAanwezig(toevalligeGebeurtenis.getOverlijden());
    }

    private void controleerVerbintenis(
        ...
        if (!Lo3GroepUtil.isGroepAanwezig(Lo3GroepEnum.GROEP06, actueleInhoud)) {
            Foutmelding.logMeldingFout(actueleHerkomst, LogSeverity.ERROR, SoortMeldingCode.TG032, null);
        }

        // Groep 07: Ontbinding
        if (Lo3GroepUtil.isGroepAanwezig(Lo3GroepEnum.GROEP07, actueleInhoud)) {
            Foutmelding.logMeldingFout(actueleHerkomst, LogSeverity.ERROR, SoortMeldingCode.TG033, null);
        }
        ...
        if (Lo3GroepUtil.isGroepAanwezig(Lo3GroepEnum.GROEP15, actueleInhoud)
                && !actueleSoortVerbintenis.asElement().equalsWaarde(actueleInhoud.getSoortVerbintenis())) {
        ...
        if (Lo3GroepUtil.isGroepAanwezig(Lo3GroepEnum.GROEP15, historischeInhoud)
                && !historischeSoortVerbintenis.asElement().equalsWaarde(historischeInhoud.getSoortVerbintenis())) {
