...
public class PersoonsvraagAntwoordMessageListenerTest {
    ...
    @InjectMocks
    private PersoonsvraagAntwoordMessageListener subject;

    @Test
    public void testGevonden() throws Exception {
        final Persoonsantwoord antwoord = new Persoonsantwoord();
        antwoord.setInhoud("00000000Ha01A00000000000270101701100101234567890");
        Mockito.when(message.getText()).thenReturn(JsonMapper.writer().writeValueAsString(antwoord));

        subject.onMessage(message);
        ArgumentCaptor<AdHocZoekPersoonAntwoordBericht> berichtArgumentCaptor = ArgumentCaptor.forClass(AdHocZoekPersoonAntwoordBericht.class);
        Mockito.verify(iscVerzender).verstuurAdHocZoekenAntwoord(berichtArgumentCaptor.capture());

        AdHocZoekPersoonAntwoordBericht bericht = berichtArgumentCaptor.getValue();
        Assert.assertEquals("00000000Ha01A00000000000270101701100101234567890", bericht.getInhoud());
    }

    @Test
    public void testGevondenNietActueel() throws Exception {
        final Persoonsantwoord antwoord = new Persoonsantwoord();
        antwoord.setInhoud("00000000Ha01R20150101000270101701100101234567890");
        Mockito.when(message.getText()).thenReturn(JsonMapper.writer().writeValueAsString(antwoord));
        ...
        AdHocZoekPersoonAntwoordBericht bericht = berichtArgumentCaptor.getValue();
        Assert.assertEquals("00000000Ha01R20150101000270101701100101234567890", bericht.getInhoud());
    }
