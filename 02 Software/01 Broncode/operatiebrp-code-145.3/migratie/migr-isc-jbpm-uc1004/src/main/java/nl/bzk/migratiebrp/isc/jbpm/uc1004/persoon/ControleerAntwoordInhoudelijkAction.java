 ...
 */
@Component("uc1004ControleerAntwoordInhoudelijkAction")
public class ControleerAntwoordInhoudelijkAction implements SpringAction {
     ...
     */
    @Inject
    public ControleerAntwoordInhoudelijkAction(final BerichtenDao berichtenDao) {
    ...
    public final Map<String, Object> execute(final Map<String, Object> parameters) {
        ...
        final AdHocZoekPersoonAntwoordBericht antwoord = haalAntwoordOp(parameters);
    ...
    private AdHocZoekPersoonAntwoordBericht haalAntwoordOp(final Map<String, Object> parameters) {
        if (parameters.containsKey(AdHocVragenConstanten.ZOEK_PERSOON_ANTWOORD_BERICHT_SLEUTEL)) {
            final Long berichtId = (Long) parameters.get(AdHocVragenConstanten.ZOEK_PERSOON_ANTWOORD_BERICHT_SLEUTEL);
            return (AdHocZoekPersoonAntwoordBericht) berichtenDao.leesBericht(berichtId);
        } else {
            throw new IllegalStateException("Geen antwoord bericht ter controle doorgegeven in proces");
        }
    }

    private Map<String, Object> controleerAntwoord(final AdHocZoekPersoonAntwoordBericht antwoord) {
        ...
        if (antwoord.getFoutreden() != null) {
            if (AdHocZoekAntwoordFoutReden.F.value().equals(antwoord.getFoutreden().value())) {
                ...
                resultaat.put(AdHocVragenConstanten.HF01_FOUTREDEN, antwoord.getFoutreden().value());
