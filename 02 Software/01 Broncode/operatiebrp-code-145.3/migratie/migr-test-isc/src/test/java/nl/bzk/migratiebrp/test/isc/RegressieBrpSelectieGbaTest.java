...
public class RegressieBrpSelectieGbaTest extends IscRegressieConfiguration {

    @Override
    public File getInputFolder() {
        return new File("./regressie-brp-selectie-gba");
    }

    @Override
    public FilenameFilter getThemaFilter() {
        final StartsWithFilter startsWith = new StartsWithFilter("", FilterType.DIRECTORY);
        final NotEndsWithFilter notEndsWith = new NotEndsWithFilter(FilterType.DIRECTORY, "NO TEST");

        return new CompositeFilter(CompositeFilter.Type.AND, startsWith, notEndsWith);
    }
