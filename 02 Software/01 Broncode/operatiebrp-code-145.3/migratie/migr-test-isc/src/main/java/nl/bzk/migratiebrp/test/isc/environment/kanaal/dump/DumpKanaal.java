    ...
    public static final class Worker extends AbstractKanaal {
        ...
        private void dumpIscDatabaseGegevens(final StringBuilder databaseGegevens) {
            ...
            try {
                ...
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from mig_bericht") + NEW_LINE);

                // Processen
                databaseGegevens.append("Aantal records in MIG_CORRELATIE" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from mig_correlatie") + NEW_LINE);

                databaseGegevens.append("Aantal records in MIG_EXTRACTIE_PROCES" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from mig_extractie_proces") + NEW_LINE);

                databaseGegevens.append("Aantal records in MIG_FOUT" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from mig_fout") + NEW_LINE);

                databaseGegevens.append("Aantal records in MIG_LOCK" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from mig_lock") + NEW_LINE);

                databaseGegevens.append("Aantal records in MIG_LOCK_ANUMMER" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from mig_lock_anummer") + NEW_LINE);

                databaseGegevens.append("Aantal records in MIG_PROCES_GERELATEERD" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from mig_proces_gerelateerd") + NEW_LINE);

                databaseGegevens.append("Aantal records in MIG_VIRTUEEL_PROCES" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from mig_virtueel_proces") + NEW_LINE);

                databaseGegevens.append("Aantal records in MIG_VIRTUEEL_PROCES_GERELATEERD" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from mig_virtueel_proces_gerelateerd") + NEW_LINE);

                databaseGegevens.append("Aantal records in JBPM_JOB" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from jbpm_job") + NEW_LINE);

                databaseGegevens.append("Aantal records in JBPM_LOG" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from jbpm_log") + NEW_LINE);

                databaseGegevens.append("Aantal records in JBPM_PROCESSINSTANCE" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from jbpm_processinstance") + NEW_LINE);

                databaseGegevens.append("Aantal records in JBPM_TASKINSTANCE" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from jbpm_taskinstance") + NEW_LINE);

                databaseGegevens.append("Aantal records in JBPM_TOKEN" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from jbpm_token") + NEW_LINE);

                // Tellingen
                databaseGegevens.append("Aantal records in MIG_RUNTIME" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from mig_runtime") + NEW_LINE);

                databaseGegevens.append("Aantal records in MIG_TELLING_BERICHT" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from mig_telling_bericht") + NEW_LINE);

                databaseGegevens.append("Aantal records in MIG_TELLING_PROCES" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(iscDataSource, "select count(*) from mig_telling_proces") + NEW_LINE);
        ...
        private void dumpBrpDatabaseGegevens(final StringBuilder databaseGegevens) {
            ...
            try {
                ...
                databaseGegevens.append(sqlHelper.readSql(brpDataSource, "select count(*) from kern.admhnd"));

                databaseGegevens.append("Aantal records in KERN.PERS" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(brpDataSource, "select count(*) from kern.pers"));

                databaseGegevens.append("Aantal records in KERN.HIS_PERSIDS" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(brpDataSource, "select count(*) from kern.his_persids"));
            } catch (final KanaalException e) {
        ...
        private void dumpVoiscDatabaseGegevens(final StringBuilder databaseGegevens) {
            ...
            try {
                ...
                databaseGegevens.append(sqlHelper.readSql(voiscDataSource, "select count(*) from voisc.bericht"));

                databaseGegevens.append("Aantal records in VOISC.LOGBOEK" + NEW_LINE);
                databaseGegevens.append(sqlHelper.readSql(voiscDataSource, "select count(*) from voisc.logboek"));
            } catch (final KanaalException e) {
        ...
        private String bepaalAantalBerichtenOpQueue() {
            ...
            telAantalBerichtOpQueue(berichtTellingen, "levering");

            berichtTellingen.append("Queue sync.response:" + NEW_LINE);
            telAantalBerichtOpQueue(berichtTellingen, "sync.antwoord");
            berichtTellingen.append("Queue sync.request:" + NEW_LINE);
            telAantalBerichtOpQueue(berichtTellingen, "sync.verzoek");
        ...
        private void telAantalBerichten(final StringBuilder tekst, final String resource, final String type) {
            try {
                final ObjectName objectNaam =
                        new ObjectName(
                                "org.apache.activemq:type=Broker,brokerName=routeringCentrale,destinationType=" + type + ",destinationName=" + resource);
                ...
                tekst.append(
                        "De "
                                ...
                                + resource
                                + " bevat "
                                + routeringServerConnection.getAttribute(objectNaam, "QueueSize")
                                + " berichten."
                                + NEW_LINE);
            } catch (final
            JMException
