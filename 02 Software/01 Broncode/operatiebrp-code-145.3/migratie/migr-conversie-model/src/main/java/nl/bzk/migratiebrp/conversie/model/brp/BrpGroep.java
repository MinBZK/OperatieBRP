...
import nl.bzk.algemeenbrp.util.xml.annotation.Order;
 ...
 * </p>
 * @param <T> de BRP groep inhoud type
 */
@Order(elements = {BrpGroep.XML_ACTIE_INHOUD, BrpGroep.XML_ACTIE_GELDIGHEID, BrpGroep.XML_ACTIE_VERVAL, BrpGroep.XML_HISTORIE, BrpGroep.XML_INHOUD})
public final class BrpGroep<T extends BrpGroepInhoud> {
     ...
     * @param inhoud de BRP inhoud voor deze BRP groep, mag niet null zijn
     * @param historie de BRP historie voor deze BRP groep, mag niet null zijn
     * @param actieInhoud de BRP actie voor het opnemen van deze inhoud
     * @param actieVerval de BRP actie voor het vervallen van deze inhoud
     * @param actieGeldigheid de BRP actie voor het aanpassen van de geldigheid van deze inhoud
    ...
    public BrpGroep(
        ...
        if (historie == null && !(inhoud instanceof BrpDocumentInhoud)) {
        ...
        setActieGebruikt(actieInhoud, actieVerval, actieGeldigheid);
    }

    private void setActieGebruikt(final BrpActie... acties) {
        Arrays.stream(acties).forEach(actie -> {
            if (actie != null) {
    ...
    /**
     * Sorteert de lijst van {@link BrpActieBron} objecten binnen de {@link BrpActie} objecten van deze groep.
     */
    void sorteerActieBronnen() {
        if (actieInhoud != null) {
            actieInhoud.sorteer();
        }
        if (actieVerval != null) {
        ...
        if (actieGeldigheid != null) {
            actieGeldigheid.sorteer();
        }
    }
}
