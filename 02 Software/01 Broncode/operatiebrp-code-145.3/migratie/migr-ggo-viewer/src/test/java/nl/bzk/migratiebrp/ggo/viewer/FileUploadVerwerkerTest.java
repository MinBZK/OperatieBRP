...
import nl.bzk.migratiebrp.ggo.viewer.impl.FileUploadVerwerkerImpl;
...
public class FileUploadVerwerkerTest {
    ...
    public void initMocks() {
        ...
        fileUploadVerwerker = new FileUploadVerwerkerImpl(leesService, viewerService, bcmService, ggoPlGroepBuilder, logRegelConverter);
    }
    ...
    public void testVerwerkFileUploadSucces() throws Exception {
        ...
        Mockito.when(leesService.parsePersoonslijstMetSyntaxControle(Matchers.any(List.class), Matchers.any(FoutMelder.class)))
                .thenReturn(lo3Persoonslijsten.get(0));
        Mockito.when(viewerService.verwerkPrecondities(Matchers.any(Lo3Persoonslijst.class), Matchers.any(FoutMelder.class)))
                .thenReturn(lo3Persoonslijsten.get(0));
        ...
        Mockito.when(viewerService.converteerTerug(Matchers.any(BrpPersoonslijst.class), Matchers.any(FoutMelder.class)))
                .thenReturn(lo3Persoonslijsten.get(0));
        Mockito.when(bcmService.controleerDoorBCM(Matchers.any(Lo3Persoonslijst.class), Matchers.any(FoutMelder.class)))
        ...
        Mockito.when(
                ...
                .thenReturn(matches);
        Mockito.when(
                viewerService.voegLo3HerkomstToe(Matchers.any(Lo3Persoonslijst.class), Matchers.any(Lo3Persoonslijst.class), Matchers.any(FoutMelder.class)))
                .thenReturn(lo3Persoonslijsten.get(0));
    ...
    public void testVerwerkFileUploadSyntaxFail() throws Exception {
        ...
        Mockito.when(leesService.parsePersoonslijstMetSyntaxControle(lo3CatWaarde.get(0), foutMelder)).thenAnswer(invocation -> {
            Logging.log(
                    new LogRegel(
        ...
        Assert.assertTrue(persoonslijstGroepen.get(0).getGgoBrpPL().isEmpty());
    ...
    private BrpPersoonslijst createBrpPersoonslijst() {
        final BrpPartijCode brpGemeenteCode = new BrpPartijCode("059901");
