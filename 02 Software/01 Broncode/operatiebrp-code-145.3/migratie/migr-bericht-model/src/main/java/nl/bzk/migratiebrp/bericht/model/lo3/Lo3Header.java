...
public final class Lo3Header implements Serializable {
    ...
    private static final int MINIMUM_AANTAL_HEADERS = 2;
    private final Lo3HeaderVeld[] headerVelden;

    /**
     * Constructor.
     * @param headerVelden header velden
     */
    public Lo3Header(final Lo3HeaderVeld... headerVelden) {
        this.headerVelden = headerVelden;

        if (this.headerVelden.length < MINIMUM_AANTAL_HEADERS || this.headerVelden[0] != Lo3HeaderVeld.RANDOM_KEY
    ...
    public String[] parseHeaders(final String lo3Bericht) throws BerichtSyntaxException {
        ...
        for (int headerIndex = 0; headerIndex < headerVelden.length; headerIndex++) {
            ...
            final String headerValue = verwerkHeaderValue(lo3Bericht, berichtIndex, berichtLengteHeaderValue, aantalIndicator, headerVeld);
            ...
            if (!Lo3HeaderVeld.BERICHT.equals(headerVeld) && !headerVeld.isValide(headerValue)) {
                throw new BerichtSyntaxException("Header (veld=" + headerVeld.getOmschrijving() + ", waarde='" + headerValue + "') is niet valide.");
            }
    ...
    private String verwerkHeaderValue(String lo3Bericht, int berichtIndex, int berichtLengteHeaderValue, int aantalIndicator, Lo3HeaderVeld headerVeld) {
        String headerValue;
        if (Lo3HeaderVeld.BERICHT.equals(headerVeld) && berichtLengteHeaderValue == 0) {
            headerValue = "";
        } else {
            headerValue =
