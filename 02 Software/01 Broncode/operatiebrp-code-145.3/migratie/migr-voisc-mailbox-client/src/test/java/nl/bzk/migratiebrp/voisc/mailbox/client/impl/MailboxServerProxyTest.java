...
import nl.bzk.migratiebrp.voisc.spd.SpdTimeConverter;
...
public class MailboxServerProxyTest {
    ...
    private static final SpdConstants.Priority PRIORITY = SpdConstants.Priority.defaultValue();
    @Mock
    ...
    public void testLogonRequest() {
        ...
        Mockito.doAnswer(invocation -> {
            ...
            final String actual = GBACharacterSet.convertTeletexByteArrayToString((byte[]) args[0]);
            final String expected = "00018900" + MAILBOX_NR + MAILBOX_PWD + TERMINATOR;

            assertEquals(BERICHT_IS_NIET_GOED, expected, actual);
            return null;
        }).when(mbsConnection).write(Matchers.any(byte[].class));
        Mockito.when(mbsConnection.read()).thenReturn(mesg);

        // Test logon confirmation with error code
        try {
            mbsProxy.logOn(mbsConnection, mailbox);
     ...
     */
    @Test
    public void testConnectRequestWithIncorrectSSLPassword() throws IOException {
        final SslConnectionFactory factory = new SslConnectionFactory();
        final ApplicationContext applicationContext = new ClassPathXmlApplicationContext("test-mailbox.xml");
        final AutowireCapableBeanFactory beanFactory = Mockito.mock(AutowireCapableBeanFactory.class);
        ReflectionTestUtils.setField(factory, "hostname", "localhost");
        ReflectionTestUtils.setField(factory, "portNumber", 1212);
        ReflectionTestUtils.setField(factory, "keyPassword", "");
        ReflectionTestUtils.setField(factory, "keystorePassword", "changeit");
        ReflectionTestUtils.setField(factory, "truststorePassword", "changeit");
        ReflectionTestUtils.setField(factory, "applicationContext", applicationContext);
        ReflectionTestUtils.setField(mbsProxy, "sslConnectionFactory", factory);
        Mockito.when(beanFactory.getBean(Connection.class)).thenReturn(mbsConnection);

        // Test logon confirmation with error code
        try {
            mbsProxy.connect();
            Assert.fail("Er wordt een ConnectionException verwacht");
        } catch (final ConnectionException se) {
            Assert.assertTrue(se.getMessage().startsWith("[MELDING-8006]:"));
        } catch (final Exception e) {
            e.printStackTrace();
            Assert.fail(MELDING_GEEN_EXCEPTION_VERWACHT + e);
        }
    }
    ...
    public void testLogonWithConfirmationError() {
        try {
            ...
            final String logonConfirmation = "0007890910379602051100ZProefomgeving                                               " + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(logonConfirmation.getBytes());
    ...
    public void testLogonZeroBytes() {
        try {
            Mockito.when(mbsConnection.read()).thenReturn(new byte[]{});
    ...
    public void testLogonWrongLength() {
        try {
            final String logonConfirmation = "000019090000000000000000" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(logonConfirmation.getBytes());
    ...
    public void testLogonWrongOpcode() {
        try {
            final String logonConfirmation = "000079990000" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(logonConfirmation.getBytes());
    ...
    public void testLogonNFE() {
        try {
            final String logonConfirmation = "000079A90000" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(logonConfirmation.getBytes());
    ...
    public void testLogonWithIncorrectPassword() {
        try {
            ...
            final String logonConfirmation = "0007890910339602051100ZProefomgeving                                               " + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(logonConfirmation.getBytes());
    ...
    public void testLogoff() {
        final String logoffConfirmation = "000079990000" + TERMINATOR;
        try {
            Mockito.doAnswer(invocation -> {
                final Object[] args = invocation.getArguments();
                final String actual = GBACharacterSet.convertTeletexByteArrayToString((byte[]) args[0]);
                final String expected = "00003990" + TERMINATOR;

                assertEquals(BERICHT_IS_NIET_GOED, expected, actual);
                return null;
            }).when(mbsConnection).write(Matchers.any(byte[].class));
            Mockito.when(mbsConnection.read()).thenReturn(logoffConfirmation.getBytes());

            mbsProxy.logOff(mbsConnection);
            Mockito.verify(mbsConnection).read();
        } catch (final SpdProtocolException se) {
    ...
    public void testLogoffWrongOpCode() {
        final String logoffConfirmation = "000079A90000" + TERMINATOR;
        try {
            Mockito.when(mbsConnection.read()).thenReturn(logoffConfirmation.getBytes());
            mbsProxy.logOff(mbsConnection);
            Assert.fail("Er wordt een SpdProtocolException verwacht!");
        } catch (final SpdProtocolException me) {
            Assert.assertTrue(me.getMessage().startsWith("[MELDING-8012]:"));
        } catch (final Exception e) {
            Assert.fail(MELDING_GEEN_EXCEPTION_VERWACHT + e.getMessage());
        }
    }
    ...
    public void testLogoffNoBytesRead() {
        try {
            Mockito.when(mbsConnection.read()).thenReturn(new byte[0]);
            mbsProxy.logOff(mbsConnection);
            Assert.fail("Er wordt een SpdProtocolException verwacht!");
        } catch (final SpdProtocolException se) {
     ...
     */
    @Test
    public void testLogoffWithWrongOperationCode() {
        try {
            // Test logoff confirmation with incorrect operation code
            final String logoffConfirmation = "000079390000" + TERMINATOR;
    ...
    public void testChangePasswordRequest() {
        ...
        try {
            ...
            final String changePasswordConfirmation = "000079190000" + TERMINATOR;

            Mockito.when(mbsConnection.read()).thenReturn(changePasswordConfirmation.getBytes());
            Mockito.doAnswer(invocation -> {
                final Object[] args = invocation.getArguments();
                final String actual = GBACharacterSet.convertTeletexByteArrayToString((byte[]) args[0]);
                final String expected = "00019910" + MAILBOX_PWD + nwPassword + TERMINATOR;
                assertEquals(BERICHT_IS_NIET_GOED, expected, actual);
                return null;
            }).when(mbsConnection).write(Matchers.any(byte[].class));

            mbsProxy.changePassword(mbsConnection, mailbox, nwPassword);
            Mockito.verify(mbsConnection).read();
        } catch (final Exception e) {
            Assert.fail(MELDING_GEEN_EXCEPTION_VERWACHT + e.getMessage());
        }
    ...
    public void testChangePasswordRequestNewPwdUnacceptable() {
        ...
        try {
            ...
            final String changePasswordConfirmation = "000079191134" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(changePasswordConfirmation.getBytes());
    ...
    public void testChangePasswordRequestOldPwdMissing() {
        ...
        try {
            final String changePasswordConfirmation = "000079191131" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(changePasswordConfirmation.getBytes());
    ...
    public void testChangePasswordRequestOldPwdInvalid() {
        ...
        try {
            final String changePasswordConfirmation = "000079191132" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(changePasswordConfirmation.getBytes());
    ...
    public void testChangePasswordRequestNewPwdMissing() {
        ...
        try {
            final String changePasswordConfirmation = "000079191133" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(changePasswordConfirmation.getBytes());
    ...
    public void testChangePasswordRequestDefaultError() {
        ...
        try {
            final String changePasswordConfirmation = "000079193434" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(changePasswordConfirmation.getBytes());
    ...
    public void testChangePasswordRequestLengthNotMatched() {
        ...
        try {
            final String changePasswordConfirmation = "000089190000" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(changePasswordConfirmation.getBytes());
    ...
    public void testChangePasswordRequestOpcodeInvalid() {
        ...
        try {
            final String changePasswordConfirmation = "000079290000" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(changePasswordConfirmation.getBytes());
    ...
    public void testChangePasswordRequestNFE() {
        ...
        try {
            final String changePasswordConfirmation = "000079A90000" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(changePasswordConfirmation.getBytes());
    ...
    public void testPutMessage() {
        ...
        final Date expectedTime = Timestamp.from(SpdTimeConverter.convertSpdTimeStringToInstant(submissionTime));
        ...
        try {
            ...
            // MessageId[12]
            final String putMessageConfirmation = LENGTE_39 + OPERATION_CODE_190 + "0000" + dispatchSequenceNr + submissionTime + eref + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(putMessageConfirmation.getBytes());
            Mockito.doAnswer(invocation -> {
                final Object[] args = invocation.getArguments();
                final String actual = GBACharacterSet.convertTeletexByteArrayToString((byte[]) args[0]);

                final String putEnvelope = "00024" + "120" + "       " + "2" + "0" + "           " + "0";
                final String msgHeader = "00045" + "150" + eref + "            " + "       " + "001" + recipient + "0";
                final String msgBody = "00026" + "180" + berichtInhoud;
                final String expected = putEnvelope + msgHeader + msgBody + TERMINATOR;

                assertEquals(BERICHT_IS_NIET_GOED, expected, actual);
                return null;
            }).when(mbsConnection).write(Matchers.any(byte[].class));

            mbsProxy.putMessage(mbsConnection, bericht);

            assertEquals("DispatchSequenceNr is niet goed gevuld.", bericht.getDispatchSequenceNumber(), dispatchSequenceNr);
            assertEquals(
    ...
    @Test
    public void testPutMessageMetNonReceiptNotification() {
        // init
        final Integer dispatchSequenceNr = 123456789;
        final String recipient = RECIPIENT;
        final String eref = EREF;
        ...
        try {
            ...
            assertEquals("DispatchSequenceNr is niet goed gevuld.", bericht.getDispatchSequenceNumber(), dispatchSequenceNr);
            assertEquals("ExpectedTime is incorrect aan bericht.getTijdstipVerzendingOntvangst", expectedTime, bericht.getTijdstipMailbox());
        } catch (final Exception e) {
            Assert.fail(MELDING_GEEN_EXCEPTION_VERWACHT + e.getMessage());
        }
    }
    ...
    public void testPutMessageGeenBericht() {
        try {
            mbsProxy.putMessage(mbsConnection, null);
            Assert.fail("Er wordt een SpdProtocolException verwacht!");
        } catch (final SpdProtocolException se) {
    ...
    public void testPutMessageNoRecipient() {
        ...
        try {
            ...
            assertEquals("RecipientOrName should have length 7", ex.getMessage());
        } catch (final Exception e) {
            Assert.fail(MELDING_GEEN_EXCEPTION_VERWACHT + e.getMessage());
        }
    }
    ...
    public void testPutMessageNoEref() {
        ...
        try {
            ...
            assertEquals("MessageId should have length 12", ex.getMessage());
        } catch (final Exception e) {
            Assert.fail(MELDING_GEEN_EXCEPTION_VERWACHT + e.getMessage());
        }
    }
    ...
    public void testPutMessageInvalidLength() {
        ...
        bericht.setMessageId(eref);
        try {
            final String putMessageConfirmation = "00040" + OPERATION_CODE_190 + "0000" + dispatchSequenceNr + submissionTime + eref + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(putMessageConfirmation.getBytes());
    ...
    public void testPutMessageIncorrectResult() {
        ...
        try {
            ...
            final String putMessageConfirmation = LENGTE_39 + OPERATION_CODE_190 + "1051" + dispatchSequenceNr + submissionTime + eref + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(putMessageConfirmation.getBytes());
    ...
    public void testPutMessageNFE() {
        ...
        try {
            // Errorcode 1051 Geadresseerde verkeerd opgegeven.
            final String putMessageConfirmation = LENGTE_39 + "1A0" + "0000" + dispatchSequenceNr + submissionTime + eref + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(putMessageConfirmation.getBytes());
    ...
    public void testPutMessageWithConfirmationError() {
        ...
        try {
            ...
            // Length[5] + Operationcode[3] + PutResult[4] + DispatchSequenceNumber[9] + SubmissionTime[11] +
            // MessageId[12]
            final String putMessageConfirmation = LENGTE_39 + "192" + "0000" + dispatchSequenceNr + submissionTime + eref + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(putMessageConfirmation.getBytes());
    ...
    public void testPutMessageLength() {
        ...
        Mockito.doAnswer(new Answer<Object>() {
            ...
            public Object answer(final InvocationOnMock invocation) throws Throwable {
                ...
                final String vb01 = actual.substring(actual.indexOf("Vb01") - offsetVb01);
                int verzondenLength = Integer.valueOf(vb01.substring(0, 5));
                verzondenLength -= 3; // minus the length of the operationcode (180)
                assertEquals("Bericht-lengtes zijn niet gelijk", originalLength, verzondenLength);
                assertEquals("Berichten zijn niet gelijk", berichtInhoud, vb01.substring(startVb01Inhoud, vb01.length() - TERMINATOR.length()));
                return null;
            }
    ...
    public void testListMessages() {
        ...
        try {
            ...
            final String listMessagesResult = listResult + msList + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(listMessagesResult.getBytes());

            Mockito.doAnswer(invocation -> {
                final Object[] args = invocation.getArguments();
                final String actual = GBACharacterSet.convertTeletexByteArrayToString((byte[]) args[0]);
                final String expected = "00028" + "400" + "171" + "012" + "0" + "000000000" + "         " + TERMINATOR;
            ...
            // execute
            nextSequenceNr = mbsProxy.listMessages(mbsConnection, 0, seqNrs, MAX_LIST_LIMIT_NR, MSSTATUS, PRIORITY);
    ...
    public void testListMessagesWithMoreMessages() {
        ...
        listMessagesConfirmation += TERMINATOR;

        // Test listMessageResult with 170 MSSequenceNumbers
        try {
            Mockito.when(mbsConnection.read()).thenReturn(listMessagesConfirmation.getBytes());

            // execute
            final List<Integer> seqNrs = new ArrayList<>();
            final int nextSequenceNr = mbsProxy.listMessages(mbsConnection, 0, seqNrs, MAX_LIST_LIMIT_NR, MSSTATUS, PRIORITY);
            ...
            Assert.assertTrue("Het laatste item in de List is niet gelijk aan 386. Nl: " + seqNrs.get(169), seqNrs.get(169).equals(386));
        } catch (final Exception e) {
            Assert.fail(MELDING_GEEN_EXCEPTION_VERWACHT + e.getMessage());
        }
    }
    ...
    public void testListMessagesNoNewMessagesConfirmation() {
        ...
        try {
            ...
            final String listMessagesConfirmation = "00007" + "419" + "1113" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(listMessagesConfirmation.getBytes());
    ...
    public void testListMessagesWithConfirmationError() {
        ...
        try {
            // ListMessagesConfirmation: Length[5] + Operationcode[3] + ListError[4]
            final String listMessagesConfirmation = "00007" + "419" + "1114" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(listMessagesConfirmation.getBytes());
    ...
    public void testListMessagesNoBytesRead() {
        try {
            ...
            // execute
            mbsProxy.listMessages(mbsConnection, 0, new ArrayList<>(), MAX_LIST_LIMIT_NR, MSSTATUS, PRIORITY);
            Assert.fail("Er wordt een SpdProtocolException verwacht");
        } catch (final SpdProtocolException spe) {
    ...
    public void testListMessagesLessThenMinLimit() {
        try {
            ...
            Mockito.doAnswer(invocation -> {
                final Object[] args = invocation.getArguments();
                final String actual = GBACharacterSet.convertTeletexByteArrayToString((byte[]) args[0]);
                final String expected = "00028" + "400" + "040" + "012" + "0" + "000000000" + "         " + TERMINATOR;
            ...
            // execute
            mbsProxy.listMessages(mbsConnection, 0, new ArrayList<>(), ZERO_LIST_LIMIT_NR, MSSTATUS, PRIORITY);
        } catch (final Exception e) {
            Assert.fail(MELDING_GEEN_EXCEPTION_VERWACHT + e.getMessage());
        }
    }
    ...
    public void testListMessagesMoreThenMaxLimit() {
        try {
            ...
            // execute
            mbsProxy.listMessages(mbsConnection, 0, new ArrayList<>(), TOO_MUCH_LIST_LIMIT_NR, MSSTATUS, PRIORITY);
        } catch (final Exception e) {
            Assert.fail(MELDING_GEEN_EXCEPTION_VERWACHT + e.getMessage());
        }
    }
    ...
    public void testListMessagesResultNoMessagesListed() {
        try {
            final String listResult = "00003" + "410";
            final String listMessagesResult = listResult + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(listMessagesResult.getBytes());

            // execute
            int nextMSSequenceNumber = mbsProxy.listMessages(mbsConnection, 0, new ArrayList<>(), MAX_LIST_LIMIT_NR, MSSTATUS, PRIORITY);
            assertEquals(0, nextMSSequenceNumber);
        } catch (final Exception e) {
            Assert.fail(MELDING_GEEN_EXCEPTION_VERWACHT + e.getMessage());
        }
    }
    ...
    public void testListMessagesIllegalOpcode() {
        try {
            final String listResult = "00003" + "412";
            final String listMessagesResult = listResult + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(listMessagesResult.getBytes());
    ...
    public void testListMessagesOpcodeNFE() {
        try {
            final String listResult = "00003" + "41a";
            final String listMessagesResult = listResult + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(listMessagesResult.getBytes());
    ...
    public void testGetMessage() {
        ...
        final Date expectedTime = Timestamp.from(SpdTimeConverter.convertSpdTimeStringToInstant(deliveryTime));
        ...
        try {
            ...
            getMessageResult.append(String.format("%05d", inhoud.length() + 3)).append("280").append(inhoud);
            getMessageResult.append(TERMINATOR);

            Mockito.when(mbsConnection.read()).thenReturn(getMessageResult.toString().getBytes());
            Mockito.doAnswer(invocation -> {
                final Object[] args = invocation.getArguments();
                final String actual = GBACharacterSet.convertTeletexByteArrayToString((byte[]) args[0]);
                final String expected = LENGTE_12 + "200" + msSequenceNumber + TERMINATOR;

                assertEquals(BERICHT_IS_NIET_GOED, expected, actual);
                return null;
            }).when(mbsConnection).write(Matchers.any(byte[].class));

            bericht = mbsProxy.getMessage(mbsConnection, msSequenceNumber);
            Assert.assertTrue(
            ...
            assertEquals(bref.trim(), bericht.getMessageId());
            assertEquals(eref, bericht.getCorrelationId());
            // MessageEnvelope
            assertEquals(originator, bericht.getOriginator());
            assertEquals(expectedTime.toString(), bericht.getTijdstipMailbox().toString());
            assertEquals(recipient, bericht.getRecipient());
            // MessageBody
            assertEquals(inhoud, bericht.getBerichtInhoud());
    ...
    public void testGetMessageIncorrectOpcode() {
        try {
            ...
            final String nullBericht =
                    "00012210000011950000412201901900210408290918Z0408290918Z183101000042250ANTWOORD    YY000000001 19019001901900000003282" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(nullBericht.getBytes());

            mbsProxy.getMessage(mbsConnection, 123);
            Assert.fail("Er wordt een SpdProtocolException verwacht");
        } catch (final SpdProtocolException se) {
    ...
    public void testGetMessageNullBericht() {
        try {
            final String nullBericht =
                    "00012210000011950000412201901900210408290918Z0408290918Z183101000042250ANTWOORD    YY000000001 19019001901900000003280" + TERMINATOR;

            Mockito.when(mbsConnection.read()).thenReturn(nullBericht.getBytes());
            final Bericht bericht = mbsProxy.getMessage(mbsConnection, 123);

            // (MSEntry) dispatch_sequence_number
            assertEquals("DispatchSeqNr is ongelijk aan: 11950", Integer.valueOf(11950), bericht.getDispatchSequenceNumber());
            // (GetEnvelope) originator_or_recipient
            assertEquals("Originator is ongelijk aan: 1901900", "1901900", bericht.getOriginator());
            // (GetEnvelope) tijdstip_verzending_ontvangst
            final Date expectedDeliveryTime = Timestamp.from(SpdTimeConverter.convertSpdTimeStringToInstant("0408290918Z"));
            assertEquals(
            ...
            assertEquals("MessageId is ongelijk aan: ANTWOORD", bericht.getMessageId(), "ANTWOORD");
            // (MessageHeading) CrossReference
            assertEquals("CrossReference is ongelijk aan: YY000000001", bericht.getCorrelationId(), "YY000000001");
        } catch (final Exception e) {
            e.printStackTrace();
            Assert.fail(MELDING_GEEN_EXCEPTION_VERWACHT + e.getMessage());
        }
    ...
    public void testGetMessageMessageHeadingWrongLength() {
        try {
            final String nullBericht =
                    "00012210000011950000412201901900210408290918Z0408290918Z183101000041250ANTWOORD    YY000000001 19019001901900000003280" + TERMINATOR;
    ...
    public void testGetMessageMessageHeadingLengthNFE() {
        try {
            final String nullBericht =
                    "00012210000011950000412201901900210408290918Z0408290918Z18310100004a250ANTWOORD    YY000000001 19019001901900000003280" + TERMINATOR;
    ...
    public void testGetMessageMessageBodyWrongLength() {
        try {
            final String nullBericht =
                    "00012210000011950000412201901900210408290918Z0408290918Z183101000042250ANTWOORD    YY000000001 19019001901900000004280" + TERMINATOR;
    ...
    public void testGetMessageMessageBodyLengthNFE() {
        try {
            final String nullBericht =
                    "00012210000011950000412201901900210408290918Z0408290918Z183101000042250ANTWOORD    YY000000001 1901900190190000000a280" + TERMINATOR;
    ...
    public void testGetMessageDeliveryReport() {
        ...
        try {
            final String deliveryReport = "00012210000011950000482600408231236Z0812002000011234567           0001" + TERMINATOR;

            Mockito.when(mbsConnection.read()).thenReturn(deliveryReport.getBytes());

            final Bericht delRep = mbsProxy.getMessage(mbsConnection, 123);

            // dispatch_sequence_number
            assertEquals("DispatchSeqNr is ongelijk aan: 81200200", delRep.getDispatchSequenceNumber(), Integer.valueOf(81200200));
            // report_delivery_time
            final Date expectedDelRepTime = Timestamp.from(SpdTimeConverter.convertSpdTimeStringToInstant("0408231236Z"));
            assertEquals("Report delivery time is ongelijk aan: 2004-08-23 12:36", expectedDelRepTime.toString(), delRep.getTijdstipMailbox().toString());
            // originator_or_recipient
            assertEquals("Originator is ongelijk aan: 1234567", "1234567", delRep.getOriginator());
            // non_delivery_reason
            assertEquals("NonDeliveryReason is ongelijk aan: 0001", "0001", delRep.getNonDeliveryReason());
    ...
    public void testGetMessageDeliveryReportWrongLength() {
        // Test Delivery Report
        try {
            final String deliveryReport = "00012210000011950000492600408231236Z0812002000011234567           0001" + TERMINATOR;
    ...
    public void testGetMessageDeliveryReportMoreThenOneRecipient() {
        // Test Delivery Report
        try {
            final String deliveryReport = "00012210000011950000702600408231236Z0812002000021234567           00011234567           0001" + TERMINATOR;
    ...
    public void testGetMessageDeliveryReportRecipientNFE() {
        // Test Delivery Report
        try {
            final String deliveryReport = "00012210000011950000482600408231236Z08120020000a1234567           0001" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(deliveryReport.getBytes());
    ...
    public void testGetMessageStatusReport() {
        // Test Status Report
        try {
            final String statusReport = "00012210000011950000412201831010210408121045Z0408121045Z18310100002427018310101YY000000004 000000";
            Mockito.when(mbsConnection.read()).thenReturn(statusReport.getBytes());

            final Bericht statRep = mbsProxy.getMessage(mbsConnection, 123);

            // (MSEntry) dispatch_sequence_number
            assertEquals("DispatchSeqNr: ", Integer.valueOf(11950), statRep.getDispatchSequenceNumber());
            // (GetEnvelope) tijdstip_verzending_ontvangst
            final Date expectedDeliveryTime = Timestamp.from(SpdTimeConverter.convertSpdTimeStringToInstant("0408121045Z"));
            assertEquals("Tijdstip_verzending_ontvangst", expectedDeliveryTime.toString(), statRep.getTijdstipMailbox().toString());
            // (GetEnvelope) originator_or_recipient
            assertEquals("Originator", "1831010", statRep.getOriginator());
            // non_receipt_reason
            assertEquals("NonReceiptReason", "1", statRep.getNotificationType());
        } catch (final SpdProtocolException se) {
            Assert.fail("Er wordt geen SpdProtocolException verwacht! " + se.getMessage());
        } catch (final Exception e) {
            Assert.fail(MELDING_GEEN_EXCEPTION_VERWACHT + e.getMessage());
    ...
    public void testGetMessageMessageConfirmation() {
        ...
        try {
            final String getMessageConfirmation = "000072901071" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(getMessageConfirmation.getBytes());
            mbsProxy.getMessage(mbsConnection, 123);
        } catch (final SpdProtocolException se) {
    ...
    public void testGetMessageMessageConfirmationWrongLength() {
        // Test getMessage confirmation with error code (1071) Bericht niet gevonden
        try {
            final String getMessageConfirmation = "000062901071" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(getMessageConfirmation.getBytes());
            mbsProxy.getMessage(mbsConnection, 123);
        } catch (final SpdProtocolException se) {
    ...
    public void testGetMessageWrongAnswer() {
        ...
        try {
            final String logoffConfirmation = "000070090000" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(logoffConfirmation.getBytes());
            mbsProxy.getMessage(mbsConnection, 123);
        } catch (final SpdProtocolException se) {
            se.printStackTrace();
    ...
    public void testGetMessageNFE() {
        // Test met LogoffConfirmation ipv GetMessageResult
        try {
            final String logoffConfirmation = "000070A90000" + TERMINATOR;
            Mockito.when(mbsConnection.read()).thenReturn(logoffConfirmation.getBytes());
            mbsProxy.getMessage(mbsConnection, 123);
        } catch (final SpdProtocolException se) {
            Assert.assertTrue(se.getMessage().startsWith("[MELDING-8043]:"));
