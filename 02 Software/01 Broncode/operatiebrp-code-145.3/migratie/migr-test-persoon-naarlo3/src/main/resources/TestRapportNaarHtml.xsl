...
<xsl:stylesheet version="1.0"
			...
			<xsl:for-each select="key('resultaten-per-thema', @thema)">
				...
				<tr>
					...
					<xsl:choose>
						...
						<xsl:otherwise>
							<td>
								<xsl:apply-templates select="lezen" />
							</td>
							<td>
	...
	<xsl:template match="input|lezen|conversie|preconditieException">
	...
	<xsl:template name="totalen">
		...
		<xsl:variable name="totaalLezen" select="count(//resultaat/lezen)" />
		<xsl:variable name="totaalLezenOk"
			select="count(//resultaat/lezen[@status='OK'])" />
		<xsl:variable name="totaalLezenNok"
			select="count(//resultaat/lezen[@status='NOK'])" />
		<xsl:variable name="totaalLezenExceptie"
			select="count(//resultaat/lezen[@status='EXCEPTIE'])" />

		<xsl:variable name="totaalConversie" select="count(//resultaat/conversie)" />
		<xsl:variable name="totaalConversieOk"
			select="count(//resultaat/conversie[@status='OK'])" />
		<xsl:variable name="totaalConversieNok"
			select="count(//resultaat/conversie[@status='NOK'])" />
		<xsl:variable name="totaalConversieExceptie"
			select="count(//resultaat/conversie[@status='EXCEPTIE'])" />
		...
		<table id="thema_{@thema}">
			...
			<tr>
				...
				<td>
					<xsl:value-of
						select="format-number($totaalLezenOk div $totaalLezen * 100, '##')" />
					%
				</td>
				<td>
				...
				<td>
					<xsl:value-of
						select="format-number($totaalConversieOk div $totaalConversie * 100, '##')" />
					%
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<xsl:value-of
						select="format-number($totaalLezenNok div $totaalLezen * 100, '##')" />
					%
				</td>
				<td>
				...
				<td>
					<xsl:value-of
						select="format-number($totaalConversieNok div $totaalConversie * 100, '##')" />
					%
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<xsl:value-of
						select="format-number($totaalLezenExceptie div $totaalLezen * 100, '##')" />
					%
				</td>
				<td>
				...
				<td>
					<xsl:value-of
						select="format-number($totaalConversieExceptie div $totaalConversie * 100, '##')" />
					%
				</td>
			</tr>
		</table>
