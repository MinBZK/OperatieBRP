...
public class PortInitializer implements ApplicationContextInitializer<ConfigurableApplicationContext> {
    ...
    public void initialize(final ConfigurableApplicationContext applicationContext) {
        ...
        try (ServerSocket socket = new ServerSocket(0)) {
            final int port = socket.getLocalPort();
            final Logger log = LoggerFactory.getLogger();
            log.info("Configuring hornetq to port: {}", port);
            properties.setProperty("test.hornetq.port", Integer.toString(port));
        } catch (final IOException e) {
            throw new IllegalStateException("Kon geen port voor de hornetq bepalen", e);
        }
