...
if [ ! -s "/opt/kibana/config/.customSetup" ]; then
    ...
    # Wait for elastic search startup
    /dockerize --timeout 60s --wait http://elasticsearch:9200

    checkKibana=$(curl --head -i 'http://elasticsearch:9200/.kibana')
...
sed -ri "s!^(\#\s*)?(elasticsearch\.url:).*!\2 'http://$ELASTICSEARCH_HOST:$ELASTICSEARCH_PORT'!" /opt/kibana/config/kibana.yml
